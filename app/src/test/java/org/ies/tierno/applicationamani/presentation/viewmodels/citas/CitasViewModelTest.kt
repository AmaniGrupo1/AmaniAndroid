...
class CitasViewModelTest {
    ...
    private val testDisponibilidad = DisponibilidadDiaResponse(
        fecha = testFecha,
        diaCompleto = false,
    ...
    private fun createViewModel(): CitasViewModel {
        return CitasViewModel(citasRepository, profileRepository, userSessionDataStore)
    }

    // 1. Initial state - no session, empty agenda, no error
    @Test
    fun `initial state has no session, empty agenda and no error`() = runTest {
    ...
    @Test
    fun `cargarAgendaMensual failure sets errorMessage and empty agenda`() = runTest {
    ...
    @Test
    fun `cargarAgendaMensual without session does nothing`() = runTest {
    ...
    @Test
    fun `cargarDisponibilidad success populates disponibilidadDia`() = runTest {
    ...
    @Test
    fun `cargarDisponibilidad failure sets errorMessage`() = runTest {
        coEvery { citasRepository.getDisponibilidadDia(any(), any(), any()) } returns
    ...
    @Test
    fun `reservarCita with valid session returns success`() = runTest {
    ...
    @Test
    fun `reservarCita without session returns failure`() = runTest {
        // No session set - sessionFlow stays null
        viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `reservarCita without psicologoId returns failure`() = runTest {
        // Session with no psicologo and no paciente avoids cargarPsicologoAsignado call
        val sessionWithoutPsicologo = UserSession(
            idUsuario = 1L,
            nombre = "Test User",
            rol = "PACIENTE",
            idPsicologo = null,
    ...
    @Test
    fun `cancelarCita success clears errorMessage`() = runTest {
        coEvery { citasRepository.getAgendaPaciente(any(), any()) } returns
    ...
    @Test
    fun `cancelarCita failure sets errorMessage`() = runTest {
