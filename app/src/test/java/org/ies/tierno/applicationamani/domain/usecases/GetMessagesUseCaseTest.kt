...
class GetMessagesUseCaseTest {
    ...
    @Test
    fun `invoke should emit messages from repository`() =
        runTest {
    ...
    @Test
    fun `invoke should emit empty list when no messages`() =
        runTest {
    ...
    fun `invoke should propagate error when repository flow throws`() =
        runTest {
            every { repository.observeMessages(1L, 2L) } returns
                flow {
    ...
    @Test
    fun `invoke with different user ids delegates to repository`() =
        runTest {
