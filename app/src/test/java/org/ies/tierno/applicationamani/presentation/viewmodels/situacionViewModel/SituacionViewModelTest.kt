...
class SituacionViewModelTest {
    ...
    @Test
    fun `initial state with empty flow - situaciones is empty`() =
        runTest {
            every { useCase.getSituaciones() } returns flowOf(emptyList<SituacionDTO>())

            val viewModel = SituacionViewModel(useCase, userSessionDataStore)
            advanceUntilIdle()
    ...
    @Test
    fun `when useCase emits list - situaciones updates`() =
        runTest {
    ...
    @Test
    fun `when useCase emits empty list - situaciones is empty`() =
        runTest {
            every { useCase.getSituaciones() } returns flowOf(emptyList<SituacionDTO>())
    ...
    @Test
    fun `when useCase flow throws - situaciones remains empty without crash`() =
        runTest {
    ...
    @Test
    fun `useCase emits multiple values sequentially - situaciones takes last value`() =
        runTest {
