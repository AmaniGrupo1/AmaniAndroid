...
import org.ies.tierno.applicationamani.domain.usecases.generalizado.ObserveMessageReadUseCase
...
class ObserveMessageReadUseCaseTest {
    ...
    @Test
    fun `invoke should emit read status from repository`() =
        runTest {
    ...
    @Test
    fun `invoke should emit false when not read`() =
        runTest {
    ...
    fun `invoke should propagate error when repository flow throws`() =
        runTest {
            every { repository.observeMessageRead(1L, 2L) } returns
                flow {
