...
class AuthRepositoryTest {
    ...
    fun setUp() {
        ...
        // Default mock for signInWithCustomToken to avoid hanging
        val mockAuthResult = mockk<AuthResult>()
        every { firebaseAuth.signInWithCustomToken(any()) } returns Tasks.forResult(mockAuthResult)
    ...
    @Test
    fun `login should return success and save token when response is successful`() =
        runTest {
            ...
            coEvery { api.getFirebaseToken() } returns Response.success(mapOf("firebaseToken" to "fb_token"))
            ...
            coVerify { tokenHolder.setToken("abc123") }
            coVerify { userSessionDataStore.saveSession(any()) }
        }

    @Test
    fun `login should return failure with credentials message when 401`() =
        runTest {
            val request = LoginRequestDTO("user@test.com", "pass")
    ...
    @Test
    fun `login should return failure with connection message on connection exception`() =
        runTest {
            val request = LoginRequestDTO("user@test.com", "pass")
    ...
    @Test
    fun `asignarPsicologo should return success when api returns successful`() =
        runTest {
    ...
    @Test
    fun `registerPaciente should return success when api returns successful`() =
        runTest {
            val body =
                LoginResponseDTO(
                    token = "t",
    ...
    @Test
    fun `registerAdmin should return success when api returns successful`() =
        runTest {
            val body =
                LoginResponseDTO(
                    token = "t",
    ...
    @Test
    fun `registerPsicologo should return success when api returns successful`() =
        runTest {
            val body =
    ...
    @Test
    fun `getPacientesConPsicologo should emit list when api succeeds`() =
        runTest {
    ...
    @Test
    fun `getPacientesConPsicologo should emit empty list on generic exception`() =
        runTest {
    ...
    @Test
    fun `getPaciente should emit list when api succeeds`() =
        runTest {
    ...
    @Test
    fun `getPsicologos should emit list when api succeeds`() =
        runTest {
    ...
    @Test
    fun `getPacientesByPsicologo should emit list when api succeeds`() =
        runTest {
    ...
    @Test
    fun `darBajaPaciente should return success when api succeeds`() =
        runTest {
    ...
    @Test
    fun `logout should clear token and session`() =
        runTest {
            repository.logout()
    ...
    @Test
    fun `crearPacienteDesdePsicologo should return success when api succeeds`() =
        runTest {
            val body =
                LoginResponseDTO(
                    token = "t",
    ...
    @Test
    fun `getPacientesSinPsicologo should emit list when api succeeds`() =
        runTest {
