...
class ObserveMessageDeliveryUseCaseTest {
    ...
    @Test
    fun `invoke should emit delivery status from repository`() = runTest {
    ...
    @Test
    fun `invoke should emit false when not delivered`() = runTest {
    ...
    fun `invoke should propagate error when repository flow throws`() = runTest {
        every { repository.observeMessageDelivery(1L, 2L) } returns flow {
