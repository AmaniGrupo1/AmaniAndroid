...
class ObserveTypingUseCaseTest {
    ...
    @Test
    fun `invoke should emit typing status from repository`() = runTest {
    ...
    @Test
    fun `invoke should emit false when not typing`() = runTest {
    ...
    fun `invoke should propagate error when repository flow throws`() = runTest {
        every { repository.observeTyping(1L, 2L) } returns flow {
