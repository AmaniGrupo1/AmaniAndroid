...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    */
    public final void connect(int connectTimeout, int readTimeout, int writeTimeout, int pingIntervalMillis, boolean connectionRetryEnabled, Call call, EventListener eventListener) throws Throwable {
        ...
        while (true) {
            try {
                if (this.route.requiresTunnel()) {
                    ...
                    try {
                        ...
                        this.allocationLimit = 1;
                        eventListener2.connectFailed(call2, this.route.socketAddress(), this.route.proxy(), null, e3);
                    ...
                    try {
                        ...
                        this.http2Connection = null;
                        this.allocationLimit = 1;
                        eventListener2.connectFailed(call2, this.route.socketAddress(), this.route.proxy(), null, e32);
                        if (routeException == null) {
                        }
                        if (!connectionRetryEnabled) {
                        }
            ...
            try {
                ...
                this.http2Connection = null;
                this.allocationLimit = 1;
                eventListener2.connectFailed(call2, this.route.socketAddress(), this.route.proxy(), null, e322);
                if (routeException == null) {
                }
                if (!connectionRetryEnabled) {
                }
    ...
    private final Request createTunnelRequest() throws IOException {
        Request proxyConnectRequest = new Request.Builder().url(this.route.address().url()).method("CONNECT", null).header(HttpHeaders.HOST, Util.toHostHeader(this.route.address().url(), true)).header("Proxy-Connection", HttpHeaders.KEEP_ALIVE).header(HttpHeaders.USER_AGENT, Util.userAgent).build();
        Response fakeAuthChallengeResponse = new Response.Builder().request(proxyConnectRequest).protocol(Protocol.HTTP_1_1).code(407).message("Preemptive Authenticate").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1L).receivedResponseAtMillis(-1L).header(HttpHeaders.PROXY_AUTHENTICATE, "OkHttp-Preemptive").build();
    ...
    public String toString() {
        ...
        StringBuilder sbAppend = new StringBuilder().append("Connection{").append(this.route.address().url().host()).append(AbstractJsonLexerKt.COLON).append(this.route.address().url().port()).append(", proxy=").append(this.route.proxy()).append(" hostAddress=").append(this.route.socketAddress()).append(" cipherSuite=");
        Handshake handshake = this.handshake;
