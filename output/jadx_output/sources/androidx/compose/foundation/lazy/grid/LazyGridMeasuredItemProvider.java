...
public abstract class LazyGridMeasuredItemProvider extends LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo955createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo933getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m961getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* JADX INFO: renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m961getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        int crossAxisSize;
        Object key = this.itemProvider.getKey(index);
        Object contentType = this.itemProvider.getContentType(index);
        List<Placeable> list = m995getPlaceables3p2s80s(this.measureScope, index, constraints);
        if (Constraints.m7456getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m7460getMinWidthimpl(constraints);
        } else {
            boolean value$iv = Constraints.m7455getHasFixedHeightimpl(constraints);
            if (!value$iv) {
                InlineClassHelperKt.throwIllegalArgumentException("does not have fixed height");
            }
            crossAxisSize = Constraints.m7459getMinHeightimpl(constraints);
        }
        return mo955createItemO3s9Psw(index, key, contentType, crossAxisSize, mainAxisSpacing, list, constraints, lane, span);
    }
