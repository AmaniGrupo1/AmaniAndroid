    ...
    */
    public final LineConfiguration getLineConfiguration(int lineIndex) {
        ...
        if (currentLine % getBucketSize() == 0) {
            ...
            cacheThisBucket = 2 <= i2 && i2 < getBucketSize();
        }
        if (cacheThisBucket) {
            this.cachedBucketIndex = bucketIndex;
            this.cachedBucket.clear();
        }
        boolean value$iv = currentLine <= lineIndex;
        if (!value$iv) {
            InlineClassHelperKt.throwIllegalStateException("currentLine (" + currentLine + ") > lineIndex (" + lineIndex + ')');
        }
        while (currentLine < lineIndex && currentItemIndex < getTotalSize()) {
            ...
            if (currentLine % getBucketSize() == 0 && currentItemIndex < getTotalSize()) {
                ...
                boolean value$iv2 = this.buckets.size() == currentLineBucket;
                if (!value$iv2) {
        ...
        while (spansUsed2 < this.slotsPerLine && currentItemIndex < getTotalSize()) {
            ...
            currentItemIndex++;
            spans.add(GridItemSpan.m938boximpl(LazyGridSpanKt.GridItemSpan(knownCurrentItemSpan)));
    ...
    public final int spanOf(int itemIndex, int maxSpan) {
        ...
        return GridItemSpan.m942getCurrentLineSpanimpl(span);
    }
