...
public final class PrefetchHandleProvider {
    ...
    public final LazyLayoutPrefetchState.PrefetchHandle m1016schedulePremeasurem8Kt_7k(int index, long constraints, PrefetchMetrics prefetchMetrics, boolean isHighPriority, Function1<? super LazyLayoutPrefetchState.PrefetchResultScope, Unit> onItemPremeasured) {
        PrefetchScheduler prefetchScheduler = this.executor;
    ...
    public final PrefetchRequest m1015createNestedPrefetchRequestVKLhPVY(int index, long constraints, PrefetchMetrics prefetchMetrics) {
        PrefetchScheduler prefetchScheduler = this.executor;
    ...
    final class HandleAndRequestImpl implements LazyLayoutPrefetchState.PrefetchHandle, PrefetchRequest, LazyLayoutPrefetchState.PrefetchResultScope {
        ...
        public HandleAndRequestImpl(int index, PrefetchMetrics prefetchMetrics, PriorityPrefetchScheduler priorityPrefetchScheduler, Function1<? super LazyLayoutPrefetchState.PrefetchResultScope, Unit> function1) {
            ...
            this.startTime = TimeSource.Monotonic.INSTANCE.m10010markNowz9LOYto();
        }
        ...
        private HandleAndRequestImpl(PrefetchHandleProvider this$0, int index, long constraints, PrefetchMetrics prefetchMetrics, PriorityPrefetchScheduler priorityPrefetchScheduler, Function1<? super LazyLayoutPrefetchState.PrefetchResultScope, Unit> function1) {
            ...
            this.premeasureConstraints = Constraints.m7445boximpl(constraints);
        }
        ...
        public long mo1003getSizeYEO4UFw(int placeableIndex) {
            SubcomposeLayoutState.PrecomposedSlotHandle precomposedSlotHandle = this.precomposeHandle;
            return precomposedSlotHandle != null ? precomposedSlotHandle.mo6168getSizeYEO4UFw(placeableIndex) : IntSize.INSTANCE.m7681getZeroYbymL2g();
        }
        ...
        private final void updateElapsedAndAvailableTime() {
            long now = TimeSource.Monotonic.INSTANCE.m10010markNowz9LOYto();
            this.elapsedTimeNanos = Duration.m9906getInWholeNanosecondsimpl(TimeSource.Monotonic.ValueTimeMark.m10021minus6eNON_k(now, this.startTime));
        ...
        */
        private final boolean executeRequest(PrefetchRequestScope $this$executeRequest) {
            boolean isValid;
            boolean hasMoreWork;
            boolean z;
            ...
            if (!this.isCanceled) {
                ...
                int index = getIndex();
                isValid = index >= 0 && index < itemCount;
            }
            if (!isValid) {
                cleanUp();
                return false;
            }
            Object key = itemProvider.getKey(getIndex());
            if (this.keyUsedForComposition != null && !Intrinsics.areEqual(key, this.keyUsedForComposition)) {
                cleanUp();
                return false;
            }
            Object contentType = itemProvider.getContentType(getIndex());
            Averages average = this.prefetchMetrics.getAverage(contentType);
            boolean wasComposedAtStart = isComposed();
            resetAvailableTimeTo($this$executeRequest.availableTimeNanos());
            if (!isComposed()) {
                boolean z2 = ComposeFoundationFlags.isPausableCompositionInPrefetchEnabled;
                long j = this.availableTimeNanos;
                if (!z2) {
                    if (shouldExecute(j, average.getCompositionTimeNanos())) {
                ...
                } else if (shouldExecute(j, average.getResumeTimeNanos() + average.getPauseTimeNanos())) {
                    Trace.beginSection("compose:lazy:prefetch:compose");
                    try {
                        performPausableComposition($this$executeRequest, key, contentType, average);
            ...
            NestedPrefetchController $this$executeRequest_u24lambda_u244 = this.nestedPrefetchController;
            if ($this$executeRequest_u24lambda_u244 != null) {
                hasMoreWork = $this$executeRequest_u24lambda_u244.executeNestedPrefetches($this$executeRequest, average.getNestedPrefetchCount(), this.isUrgent);
            } else {
                hasMoreWork = false;
            }
            if (hasMoreWork) {
            ...
            if (nestedPrefetchController != null && nestedPrefetchController.getExecutedNestedPrefetch()) {
                updateElapsedAndAvailableTime();
                AndroidTrace_androidKt.traceValue("compose:lazy:prefetch:execute:item", getIndex());
                NestedPrefetchController nestedPrefetchController2 = this.nestedPrefetchController;
                if (nestedPrefetchController2 != null) {
                    nestedPrefetchController2.setExecutedNestedPrefetch(false);
                }
            }
            ...
            if (this.isMeasured || constraints == null) {
                ...
                if (PrefetchHandleProvider.this.getShouldPauseBetweenPrecompositionAndPremeasure() && !wasComposedAtStart) {
                    return true;
                }
                z = false;
                if (!shouldExecute(this.availableTimeNanos, average.getMeasureTimeNanos())) {
                    return true;
                }
                Trace.beginSection("compose:lazy:prefetch:measure");
                try {
                    m1017performMeasureBRTryo0(constraints.getValue());
            ...
            NestedPrefetchController controller = this.nestedPrefetchController;
            if (this.isMeasured && this.hasResolvedNestedPrefetches && controller != null) {
                int idealNestedPrefetchCount = controller.collectIdealNestedPrefetchCount();
                average.saveNestedPrefetchCount(idealNestedPrefetchCount);
                int lastNumberOfNestedPrefetchItems = controller.collectNestedPrefetchedItemsCount();
        ...
        private final void m1017performMeasureBRTryo0(long constraints) {
            ...
            if (handle != null) {
                ...
                for (int i = 0; i < placeablesCount; i++) {
                    ...
                    handle.mo6169premeasure0kLqBqw(placeableIndex, constraints);
                }
                return;
            }
        ...
        private final class NestedPrefetchController {
            ...
            /* JADX WARN: Removed duplicated region for block: B:27:0x009d A[Catch: all -> 0x00d9, TryCatch #0 {all -> 0x00d9, blocks: (B:14:0x0057, B:16:0x0061, B:18:0x0069, B:23:0x0078, B:24:0x008c, B:25:0x0095, B:27:0x009d, B:29:0x00a8, B:31:0x00ac, B:34:0x00b3, B:35:0x00b6, B:38:0x00c2, B:39:0x00c8, B:40:0x00d0), top: B:49:0x0057 }] */
            /*
                Code decompiled incorrectly, please refer to instructions dump.
            */
            public final boolean executeNestedPrefetches(PrefetchRequestScope $this$executeNestedPrefetches, int nestedPrefetchCount, boolean isUrgent) {
                ...
                try {
                    List<LazyLayoutPrefetchState> list2 = this.states;
                    int size = list2.size();
                    for (int index$iv = 0; index$iv < size; index$iv++) {
                        Object item$iv = list2.get(index$iv);
                        LazyLayoutPrefetchState it = (LazyLayoutPrefetchState) item$iv;
                        it.setRealizedNestedPrefetchCount$foundation(nestedPrefetchCount);
                    }
                    Unit unit = Unit.INSTANCE;
                    Trace.endSection();
                    ...
                    while (this.stateIndex < this.states.size()) {
                        try {
                            if (this.requestsByState[this.stateIndex] != null) {
                                list = this.requestsByState[this.stateIndex];
                                Intrinsics.checkNotNull(list);
                                while (this.requestIndex < list.size()) {
                                    PrefetchRequest $this$executeNestedPrefetches_u24lambda_u242_u240 = list.get(this.requestIndex);
                                    if (isUrgent) {
                                        HandleAndRequestImpl handleAndRequestImpl = $this$executeNestedPrefetches_u24lambda_u242_u240 instanceof HandleAndRequestImpl ? (HandleAndRequestImpl) $this$executeNestedPrefetches_u24lambda_u242_u240 : null;
                                        if (handleAndRequestImpl != null) {
                                            handleAndRequestImpl.markAsUrgent();
                                        }
                                    }
                                    this.executedNestedPrefetch = true;
                                    boolean hasMoreWork = $this$executeNestedPrefetches_u24lambda_u242_u240.execute($this$executeNestedPrefetches);
                                    if (!hasMoreWork) {
                                        this.requestIndex++;
                                    }
                                }
                                this.requestIndex = 0;
                                this.stateIndex++;
                            } else if ($this$executeNestedPrefetches.availableTimeNanos() > 0) {
                                this.requestsByState[this.stateIndex] = this.states.get(this.stateIndex).collectNestedPrefetchRequests$foundation();
                                list = this.requestsByState[this.stateIndex];
                                Intrinsics.checkNotNull(list);
                                while (this.requestIndex < list.size()) {
                                }
