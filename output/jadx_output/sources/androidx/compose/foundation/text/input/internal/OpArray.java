...
final class OpArray {
    ...
    public boolean equals(Object other) {
        return m1415equalsimpl(this.values, other);
    }
    ...
    /* JADX INFO: renamed from: constructor-impl, reason: not valid java name */
    public static int[] m1412constructorimpl(int size) {
        return m1413constructorimpl(new int[size * 3]);
    }
    ...
    public static final void m1421setimpl(int[] iArr, int index, int offset, int srcLen, int destLen) {
        iArr[index * 3] = offset;
        iArr[(index * 3) + 1] = srcLen;
        iArr[(index * 3) + 2] = destLen;
    }

    /* JADX INFO: renamed from: copyOf-pSmdads, reason: not valid java name */
    public static final int[] m1414copyOfpSmdads(int[] iArr, int newSize) {
        int[] iArrCopyOf = Arrays.copyOf(iArr, newSize * 3);
        Intrinsics.checkNotNullExpressionValue(iArrCopyOf, "copyOf(...)");
        return m1413constructorimpl(iArrCopyOf);
    }

    /* JADX INFO: renamed from: forEach-impl$default, reason: not valid java name */
    public static /* synthetic */ void m1418forEachimpl$default(int[] iArr, int max, boolean reversed, Function3 block, int i, Object obj) {
        if ((i & 2) != 0) {
            reversed = false;
        }
        if (max < 0) {
        ...
        if (reversed) {
            for (int i2 = max - 1; -1 < i2; i2--) {
                int offset = iArr[i2 * 3];
                int srcLen = iArr[(i2 * 3) + 1];
                int destLen = iArr[(i2 * 3) + 2];
                block.invoke(Integer.valueOf(offset), Integer.valueOf(srcLen), Integer.valueOf(destLen));
            }
            return;
        }
        for (int i3 = 0; i3 < max; i3++) {
            int offset2 = iArr[i3 * 3];
            int srcLen2 = iArr[(i3 * 3) + 1];
            int destLen2 = iArr[(i3 * 3) + 2];
    ...
    public static final void m1417forEachimpl(int[] iArr, int max, boolean reversed, Function3<? super Integer, ? super Integer, ? super Integer, Unit> function3) {
        if (max < 0) {
            return;
        }
        if (reversed) {
            for (int i = max - 1; -1 < i; i--) {
                int offset = iArr[i * 3];
                int srcLen = iArr[(i * 3) + 1];
                int destLen = iArr[(i * 3) + 2];
                function3.invoke(Integer.valueOf(offset), Integer.valueOf(srcLen), Integer.valueOf(destLen));
            }
            return;
        }
        for (int i2 = 0; i2 < max; i2++) {
            int offset2 = iArr[i2 * 3];
            int srcLen2 = iArr[(i2 * 3) + 1];
            int destLen2 = iArr[(i2 * 3) + 2];
