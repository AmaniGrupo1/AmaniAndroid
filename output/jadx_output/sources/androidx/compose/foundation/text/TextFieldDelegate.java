...
public final class TextFieldDelegate {
    ...
    public static final class Companion {
        ...
        public static /* synthetic */ Triple m1260layout_EkL_Y$foundation$default(Companion companion, TextDelegate textDelegate, long j, LayoutDirection layoutDirection, TextLayoutResult textLayoutResult, int i, Object obj) {
            ...
            return companion.m1263layout_EkL_Y$foundation(textDelegate, j, layoutDirection, textLayoutResult2);
        }

        @JvmStatic
        /* JADX INFO: renamed from: layout-_EkL_-Y$foundation, reason: not valid java name */
        public final Triple<Integer, Integer, TextLayoutResult> m1263layout_EkL_Y$foundation(TextDelegate textDelegate, long constraints, LayoutDirection layoutDirection, TextLayoutResult prevResultText) {
            TextLayoutResult layoutResult = textDelegate.m1250layoutNN6EwU(constraints, layoutDirection, prevResultText);
            return new Triple<>(Integer.valueOf((int) (layoutResult.getSize() >> 32)), Integer.valueOf((int) (4294967295L & layoutResult.getSize())), layoutResult);
        }

        @JvmStatic
        /* JADX INFO: renamed from: draw-Q1vqE60$foundation, reason: not valid java name */
        public final void m1262drawQ1vqE60$foundation(Canvas canvas, TextFieldValue value, long selectionPreviewHighlightRange, long deletionPreviewHighlightRange, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint highlightPaint, long selectionBackgroundColor) {
            if (!TextRange.m6922getCollapsedimpl(selectionPreviewHighlightRange)) {
                highlightPaint.mo4573setColor8_81llA(selectionBackgroundColor);
                m1259drawHighlightLepunE(canvas, selectionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m6922getCollapsedimpl(deletionPreviewHighlightRange)) {
                Color colorM4687boximpl = Color.m4687boximpl(textLayoutResult.getLayoutInput().getStyle().m6958getColor0d7_KjU());
                long it = colorM4687boximpl.m4707unboximpl();
                if (it == 16) {
                    colorM4687boximpl = null;
                }
                long textColor = colorM4687boximpl != null ? colorM4687boximpl.m4707unboximpl() : Color.INSTANCE.m4723getBlack0d7_KjU();
                highlightPaint.mo4573setColor8_81llA(Color.m4695copywmQWz5c(textColor, (14 & 1) != 0 ? Color.m4699getAlphaimpl(textColor) : Color.m4699getAlphaimpl(textColor) * 0.2f, (14 & 2) != 0 ? Color.m4703getRedimpl(textColor) : 0.0f, (14 & 4) != 0 ? Color.m4702getGreenimpl(textColor) : 0.0f, (14 & 8) != 0 ? Color.m4700getBlueimpl(textColor) : 0.0f));
                m1259drawHighlightLepunE(canvas, deletionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m6922getCollapsedimpl(value.getSelection())) {
                highlightPaint.mo4573setColor8_81llA(selectionBackgroundColor);
                m1259drawHighlightLepunE(canvas, value.getSelection(), offsetMapping, textLayoutResult, highlightPaint);
            }
            TextPainter.INSTANCE.paint(canvas, textLayoutResult);
        }

        /* JADX INFO: renamed from: drawHighlight-Le-punE, reason: not valid java name */
        private final void m1259drawHighlightLepunE(Canvas canvas, long range, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint paint) {
            int start = offsetMapping.originalToTransformed(TextRange.m6926getMinimpl(range));
            int end = offsetMapping.originalToTransformed(TextRange.m6925getMaximpl(range));
            if (start != end) {
        ...
        public final void notifyFocusedRect$foundation(TextFieldValue value, final TextDelegate textDelegate, TextLayoutResult textLayoutResult, LayoutCoordinates layoutCoordinates, TextInputSession textInputSession, boolean hasFocus, OffsetMapping offsetMapping) {
            ...
            textInputSession.notifyFocusedRect(TextFieldDelegateKt.focusedRectInRoot(textLayoutResult, layoutCoordinates, offsetMapping.originalToTransformed(TextRange.m6925getMaximpl(value.getSelection())), new Function0() { // from class: androidx.compose.foundation.text.TextFieldDelegate$Companion$$ExternalSyntheticLambda0
                @Override // kotlin.jvm.functions.Function0
                public final Object invoke() {
                    TextDelegate textDelegate2 = textDelegate;
                    return IntSize.m7668boximpl(TextFieldDelegateKt.computeSizeForDefaultText$default(textDelegate2.getStyle(), textDelegate2.getDensity(), textDelegate2.getFontFamilyResolver(), null, 0, 24, null));
                }
            }));
        }
            ...
            if (innerTextFieldCoordinates != null && innerTextFieldCoordinates.isAttached() && (decorationBoxCoordinates = textLayoutResult.getDecorationBoxCoordinates()) != null) {
                textInputSession.updateTextLayoutResult(textFieldValue, offsetMapping, textLayoutResult.getValue(), new Function1<Matrix, Unit>() { // from class: androidx.compose.foundation.text.TextFieldDelegate$Companion$updateTextLayoutResult$1$1$1
                    ...
                    public /* bridge */ /* synthetic */ Unit invoke(Matrix matrix) {
                        m1265invoke58bKbWc(matrix.m4964unboximpl());
                    ...
                    public final void m1265invoke58bKbWc(float[] fArr) {
                        if (innerTextFieldCoordinates.isAttached()) {
                            LayoutCoordinatesKt.findRootCoordinates(innerTextFieldCoordinates).mo6156transformFromEL8BTi8(innerTextFieldCoordinates, fArr);
                        }
                    }
        ...
        public final void m1264setCursorOffsetULxng0E$foundation(long position, TextLayoutResultProxy textLayoutResult, EditProcessor editProcessor, OffsetMapping offsetMapping, Function1<? super TextFieldValue, Unit> onValueChange) {
            int offset = offsetMapping.transformedToOriginal(TextLayoutResultProxy.m1290getOffsetForPosition3MmeM6k$default(textLayoutResult, position, false, 2, null));
            onValueChange.invoke(TextFieldValue.m7173copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, TextRangeKt.TextRange(offset), (TextRange) null, 5, (Object) null));
        }
        ...
        public final void onBlur$foundation(TextInputSession textInputSession, EditProcessor editProcessor, Function1<? super TextFieldValue, Unit> onValueChange) {
            onValueChange.invoke(TextFieldValue.m7173copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null));
            textInputSession.dispose();
        }

        /* JADX INFO: renamed from: applyCompositionDecoration-72CqOWE, reason: not valid java name */
        public final TransformedText m1261applyCompositionDecoration72CqOWE(long compositionRange, TransformedText transformed) {
            int startPositionTransformed = transformed.getOffsetMapping().originalToTransformed(TextRange.m6928getStartimpl(compositionRange));
            int endPositionTransformed = transformed.getOffsetMapping().originalToTransformed(TextRange.m6923getEndimpl(compositionRange));
