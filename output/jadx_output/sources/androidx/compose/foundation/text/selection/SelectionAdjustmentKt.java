...
public final class SelectionAdjustmentKt {
    ...
    public static final Selection.AnchorInfo updateSelectionBoundary(final SelectionLayout $this$updateSelectionBoundary, final SelectableInfo info, Selection.AnchorInfo previousSelectionAnchor) {
        ...
        long previousSelectionWordBoundary = info.getTextLayoutResult().m6900getWordBoundaryjx7JFs(previousSelectionOffset);
        if (!isExpanding(info, currentRawOffset, $this$updateSelectionBoundary.getIsStartHandle())) {
            return info.anchorForOffset(currentRawOffset);
        }
        if (previousSelectionOffset == TextRange.m6928getStartimpl(previousSelectionWordBoundary) || previousSelectionOffset == TextRange.m6923getEndimpl(previousSelectionWordBoundary)) {
            return updateSelectionBoundary$lambda$3(anchorSnappedToWordBoundary$delegate);
        }
        return info.anchorForOffset(currentRawOffset);
    }
    ...
    public static final Selection.AnchorInfo snapToWordBoundary(SelectableInfo $this$snapToWordBoundary, int currentLine, int currentOffset, int otherOffset, boolean isStart, boolean crossed) {
        int start;
        int end;
        long wordBoundary = $this$snapToWordBoundary.getTextLayoutResult().m6900getWordBoundaryjx7JFs(currentOffset);
        int wordStartLine = $this$snapToWordBoundary.getTextLayoutResult().getLineForOffset(TextRange.m6928getStartimpl(wordBoundary));
        if (wordStartLine == currentLine) {
            start = TextRange.m6928getStartimpl(wordBoundary);
        ...
        int wordEndLine = $this$snapToWordBoundary.getTextLayoutResult().getLineForOffset(TextRange.m6923getEndimpl(wordBoundary));
        if (wordEndLine == currentLine) {
            end = TextRange.m6923getEndimpl(wordBoundary);
        } else if (currentLine >= $this$snapToWordBoundary.getTextLayoutResult().getLineCount()) {
    ...
    private static final Selection.AnchorInfo anchorOnBoundary(SelectableInfo $this$anchorOnBoundary, boolean crossed, boolean isStart, int slot, BoundaryFunction boundaryFunction) {
        ...
        long range = boundaryFunction.mo1583getBoundaryfzxv0v0($this$anchorOnBoundary, offset);
        return $this$anchorOnBoundary.anchorForOffset(isStart ^ crossed ? TextRange.m6928getStartimpl(range) : TextRange.m6923getEndimpl(range));
    }
