...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        boolean canFitLine = lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m28getFirstimpl(ellipsisInfo.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis));
    ...
    public final WrapInfo m735getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        boolean z;
        boolean z2;
        IntIntPair ellipsis;
        boolean z3;
        ...
        if (this.overflow.getType$foundation_layout() == FlowLayoutOverflow.OverflowType.Visible) {
            ...
            z = lineIndex >= this.maxLines || IntIntPair.m29getSecondimpl(leftOver) - IntIntPair.m29getSecondimpl(nextSize.getPackedValue()) < 0;
        }
        ...
        if (nextIndexInLine == 0) {
            ...
            z2 = nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m28getFirstimpl(leftOver) - IntIntPair.m28getFirstimpl(nextSize.getPackedValue()) < 0;
        }
        ...
        if (shouldWrapItem) {
            ...
            return new WrapInfo(true, m735getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m24constructorimpl(Constraints.m7458getMaxWidthimpl(this.constraints), (IntIntPair.m29getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m21boximpl(IntIntPair.m24constructorimpl(IntIntPair.m28getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m29getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, totalContainerCrossAxisSize, 0, true, false).getIsLastItemInContainer());
        }
        int totalContainerCrossAxisSize2 = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m29getSecondimpl(nextSize.getPackedValue()));
        if (!isEllipsisWrap) {
            ellipsis = this.overflow.m743ellipsisSizeF35zmw$foundation_layout(nextItemHasNext, lineIndex, totalContainerCrossAxisSize2);
        } else {
            ellipsis = null;
        }
        if (ellipsis != null) {
            ellipsis.getPackedValue();
            z3 = nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m28getFirstimpl(leftOver) - IntIntPair.m28getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m28getFirstimpl(ellipsis.getPackedValue()) < 0;
        } else {
            z3 = false;
        }
        ...
        if (shouldWrapEllipsis) {
            if (!isEllipsisWrap) {
                WrapInfo wrapInfo = m735getWrapInfoOpUlnko(false, 0, IntIntPair.m24constructorimpl(Constraints.m7458getMaxWidthimpl(this.constraints), (IntIntPair.m29getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m29getSecondimpl(nextSize.getPackedValue()))), ellipsis, lineIndex + 1, totalContainerCrossAxisSize2, 0, true, true);
