...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo771calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long j) {
        ...
        if (this.width == IntrinsicSize.Min) {
            measuredWidth = measurable.minIntrinsicWidth(Constraints.m7457getMaxHeightimpl(j));
        } else {
            measuredWidth = measurable.maxIntrinsicWidth(Constraints.m7457getMaxHeightimpl(j));
        }
        if (measuredWidth < 0) {
            measuredWidth = 0;
        }
        return Constraints.INSTANCE.m7470fixedWidthOenEA2s(measuredWidth);
    }
