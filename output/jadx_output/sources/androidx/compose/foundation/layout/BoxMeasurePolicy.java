...
final /* data */ class BoxMeasurePolicy implements MeasurePolicy {
    ...
    public MeasureResult mo41measure3p2s80s(final MeasureScope $this$measure_u2d3p2s80s, final List<? extends Measurable> list, long j) {
        long jM7446constructorimpl;
        Placeable placeable;
        final int boxHeight;
        int boxHeight2;
        if (list.isEmpty()) {
            return MeasureScope.layout$default($this$measure_u2d3p2s80s, Constraints.m7460getMinWidthimpl(j), Constraints.m7459getMinHeightimpl(j), null, new Function1() { // from class: androidx.compose.foundation.layout.BoxMeasurePolicy$$ExternalSyntheticLambda0
        ...
        if (this.propagateMinConstraints) {
            jM7446constructorimpl = j;
        } else {
            jM7446constructorimpl = Constraints.m7446constructorimpl(ConstraintsKt.MaxDimensionsAndFocusMask & j);
        }
        long contentConstraints = jM7446constructorimpl;
        if (list.size() == 1) {
            final Measurable measurable = list.get(0);
            if (!BoxKt.getMatchesParentSize(measurable)) {
                placeable = measurable.mo6141measureBRTryo0(contentConstraints);
                int boxWidth = Math.max(Constraints.m7460getMinWidthimpl(j), placeable.getWidth());
                boxHeight = Math.max(Constraints.m7459getMinHeightimpl(j), placeable.getHeight());
                boxHeight2 = boxWidth;
            } else {
                int boxWidth2 = Constraints.m7460getMinWidthimpl(j);
                int boxHeight3 = Constraints.m7459getMinHeightimpl(j);
                placeable = measurable.mo6141measureBRTryo0(Constraints.INSTANCE.m7468fixedJhjzzOo(Constraints.m7460getMinWidthimpl(j), Constraints.m7459getMinHeightimpl(j)));
        ...
        boxWidth4.element = Constraints.m7460getMinWidthimpl(j);
        final Ref.IntRef boxHeight4 = new Ref.IntRef();
        boxHeight4.element = Constraints.m7459getMinHeightimpl(j);
        int size = list.size();
        int index$iv = 0;
        for (int index$iv2 = 0; index$iv2 < size; index$iv2++) {
            ...
            if (!BoxKt.getMatchesParentSize(measurable2)) {
                Placeable placeable3 = measurable2.mo6141measureBRTryo0(contentConstraints);
        ...
        if (index$iv != 0) {
            ...
            for (int index$iv3 = 0; index$iv3 < size2; index$iv3++) {
                ...
                if (BoxKt.getMatchesParentSize(measurable3)) {
                    placeables[index2] = measurable3.mo6141measureBRTryo0(matchParentSizeConstraints);
                }
            }
        }
