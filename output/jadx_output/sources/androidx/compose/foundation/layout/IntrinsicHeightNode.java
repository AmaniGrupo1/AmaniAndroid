...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo771calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long j) {
        int measuredHeight;
        if (this.height == IntrinsicSize.Min) {
            measuredHeight = measurable.minIntrinsicHeight(Constraints.m7458getMaxWidthimpl(j));
        } else {
            measuredHeight = measurable.maxIntrinsicHeight(Constraints.m7458getMaxWidthimpl(j));
        }
        if (measuredHeight < 0) {
            measuredHeight = 0;
        }
        return Constraints.INSTANCE.m7469fixedHeightOenEA2s(measuredHeight);
    }
