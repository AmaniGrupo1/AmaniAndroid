...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m6297constructorimpl(new int[(max * 2) + 1]);
        int[] backward = CenteredArray.m6297constructorimpl(new int[(max * 2) + 1]);
        int[] snake = Snake.m6531constructorimpl(new int[5]);
        while (stack.isNotEmpty()) {
            int newEnd = stack.pop();
            int newStart = stack.pop();
            int oldEnd = stack.pop();
            int oldStart = stack.pop();
            boolean found = m6419midPointq5eDKzI(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, snake);
            int[] snake2 = snake;
            if (found) {
                int i3 = i;
                int i4 = i2;
                if (Math.min(snake2[i2] - snake2[0], snake2[3] - snake2[i3]) > 0) {
                    Snake.m6529addDiagonalToStackimpl(snake2, diagonals);
                }
    ...
    private static final boolean m6419midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] iArr, int[] iArr2, int[] snake) {
        int i = oldStart;
        int i2 = oldEnd;
        int oldSize = i2 - i;
        ...
        int max = ((oldSize + newSize) + 1) / 2;
        int[] iArr3 = iArr;
        CenteredArray.m6303setimpl(iArr3, 1, i);
        int[] iArr4 = iArr2;
        CenteredArray.m6303setimpl(iArr4, 1, i2);
        int d = 0;
        while (d < max) {
            boolean found = m6418forward4l5_RBY(i, i2, newStart, newEnd, cb, iArr3, iArr4, d, snake);
            if (found) {
                return true;
            }
            boolean found2 = m6417backward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, iArr, iArr2, d, snake);
            if (found2) {
                return true;
            }
            d++;
    ...
    private static final boolean m6418forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] iArr, int[] iArr2, int d, int[] snake) {
        int x;
        int startX;
        boolean z;
        int i = oldEnd;
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m6300getimpl(iArr, k + 1) > CenteredArray.m6300getimpl(iArr, k - 1))) {
                int startX2 = CenteredArray.m6300getimpl(iArr, k + 1);
                x = startX2;
                startX = startX2;
            } else {
                int startX3 = CenteredArray.m6300getimpl(iArr, k - 1);
                x = startX3 + 1;
                startX = startX3;
            }
            ...
            CenteredArray.m6303setimpl(iArr, k, x);
            if (checkForSnake) {
                z = z2;
                int backwardsK = delta - k;
                if (backwardsK >= (-d) + 1 && backwardsK <= d - 1) {
                    if (CenteredArray.m6300getimpl(iArr2, backwardsK) <= x) {
    ...
    private static final boolean m6417backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] iArr, int[] iArr2, int d, int[] snake) {
        int x;
        int startX;
        boolean z;
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m6300getimpl(iArr2, k + 1) < CenteredArray.m6300getimpl(iArr2, k - 1))) {
                int startX2 = CenteredArray.m6300getimpl(iArr2, k + 1);
                x = startX2;
                startX = startX2;
            } else {
                int startX3 = CenteredArray.m6300getimpl(iArr2, k - 1);
                x = startX3 - 1;
                startX = startX3;
            }
            ...
            z = z2;
            CenteredArray.m6303setimpl(iArr2, k, x2);
            if (checkForSnake) {
                int forwardsK = delta - k;
                if (forwardsK >= (-d) && forwardsK <= d) {
                    if (CenteredArray.m6300getimpl(iArr, forwardsK) >= x2) {
