...
public final class MultiParagraphKt {
    ...
    public static final CharSequence findParagraphByIndex$lambda$2$0(ParagraphInfo it) {
        return AbstractJsonLexerKt.BEGIN_LIST + it.getStartIndex() + ", " + it.getEndIndex() + ')';
    }
    ...
    public static final void m6795findParagraphsByRangeSbBc2M(List<ParagraphInfo> list, long j, Function1<? super ParagraphInfo, Unit> function1) {
        int paragraphIndex = findParagraphByIndex(list, TextRange.m6926getMinimpl(j));
        int size = list.size();
        for (int i = paragraphIndex; i < size; i++) {
            ParagraphInfo paragraph = list.get(i);
            if (paragraph.getStartIndex() < TextRange.m6925getMaximpl(j)) {
