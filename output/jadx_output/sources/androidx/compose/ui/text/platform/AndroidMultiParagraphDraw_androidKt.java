...
public final class AndroidMultiParagraphDraw_androidKt {
    ...
    public static final void m7183drawMultiParagraph7AXcY_I(MultiParagraph $this$drawMultiParagraph_u2d7AXcY_I, Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int i) {
        Canvas canvas2;
        canvas.save();
        if ($this$drawMultiParagraph_u2d7AXcY_I.getParagraphInfoList$ui_text().size() <= 1 || (brush instanceof SolidColor)) {
            m7185drawParagraphs7AXcY_I($this$drawMultiParagraph_u2d7AXcY_I, canvas, brush, alpha, shadow, decoration, drawStyle, i);
            canvas2 = canvas;
        } else {
            ...
            Shader shader = ((ShaderBrush) brush).mo4666createShaderuvyYCjk(Size.m4512constructorimpl((v1$iv$iv << 32) | (v2$iv$iv & 4294967295L)));
            ...
            for (int index$iv2 = 0; index$iv2 < size2; index$iv2++) {
                ...
                it2.getParagraph().mo6750painthn5TExg(canvas, BrushKt.ShaderBrush(shader), alpha, shadow, decoration, drawStyle, i);
    ...
    private static final void m7185drawParagraphs7AXcY_I(MultiParagraph $this$drawParagraphs_u2d7AXcY_I, Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int i) {
        List<ParagraphInfo> paragraphInfoList$ui_text = $this$drawParagraphs_u2d7AXcY_I.getParagraphInfoList$ui_text();
        int size = paragraphInfoList$ui_text.size();
        for (int index$iv = 0; index$iv < size; index$iv++) {
            Object item$iv = paragraphInfoList$ui_text.get(index$iv);
            ParagraphInfo it = (ParagraphInfo) item$iv;
            it.getParagraph().mo6750painthn5TExg(canvas, brush, alpha, shadow, decoration, drawStyle, i);
