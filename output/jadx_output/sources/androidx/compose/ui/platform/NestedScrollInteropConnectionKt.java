...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m6668toOffsetmoWRBKg(int dx, int dy, int[] consumed, long j) {
        float overflowX;
        float overflowY;
        float offsetX;
        float offsetY;
        if (!ComposeUiFlags.isNestedScrollInteropIntegerPropagationEnabled || Math.abs(consumed[0]) == 0) {
            overflowX = 0.0f;
        } else {
            int bits$iv$iv$iv = (int) (j >> 32);
            overflowX = Float.intBitsToFloat(bits$iv$iv$iv) - reverseAxis(dx);
        }
        if (!ComposeUiFlags.isNestedScrollInteropIntegerPropagationEnabled || Math.abs(consumed[1]) == 0) {
            overflowY = 0.0f;
        } else {
        ...
        if (Float.intBitsToFloat(bits$iv$iv$iv6) >= 0.0f) {
            ...
            int bits$iv$iv$iv8 = (int) (j & 4294967295L);
    ...
    public static final int m6669toViewTypeGyEprt8(int i) {
        return NestedScrollSource.m5878equalsimpl0(i, NestedScrollSource.INSTANCE.m5890getUserInputWNlRxjI()) ? 0 : 1;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m6666getScrollAxesk4lQ0M(long j) {
        int axes = 0;
        int bits$iv$iv$iv = (int) (j >> 32);
    ...
    private static final int m6667scrollAxessFctU(long j, float minFlingVelocity) {
        int axes = 0;
        if (Math.abs(Velocity.m7743getXimpl(j)) >= minFlingVelocity) {
            axes = 0 | 1;
        }
        if (Math.abs(Velocity.m7744getYimpl(j)) >= minFlingVelocity) {
            return axes | 2;
        }
        return axes;
    }
    ...
    public static final NestedScrollConnection rememberNestedScrollInteropConnection(View hostView, Composer $composer, int $changed, int i) {
        ...
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventStart(1075877987, $changed, -1, "androidx.compose.ui.platform.rememberNestedScrollInteropConnection (NestedScrollInteropConnection.android.kt:292)");
        }
