...
final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    public void mo6602calculateMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Matrix.m4950resetimpl(matrix);
        m6605transformMatrixToWindowEL8BTi8(view, matrix);
    }

    /* JADX INFO: renamed from: transformMatrixToWindow-EL8BTi8, reason: not valid java name */
    private final void m6605transformMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Object parent = view.getParent();
        if (parent instanceof View) {
            m6605transformMatrixToWindowEL8BTi8((View) parent, matrix);
            m6604preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m6604preTranslate3XD1CNM(matrix, view.getLeft(), view.getTop());
        } else {
            int[] pos = this.tmpLocation;
            view.getLocationInWindow(pos);
            m6604preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m6604preTranslate3XD1CNM(matrix, pos[0], pos[1]);
        }
        android.graphics.Matrix viewMatrix = view.getMatrix();
        if (!viewMatrix.isIdentity()) {
            m6603preConcattUYjHk(matrix, viewMatrix);
        }
    }

    /* JADX INFO: renamed from: preConcat-tU-YjHk, reason: not valid java name */
    private final void m6603preConcattUYjHk(float[] fArr, android.graphics.Matrix other) {
        AndroidMatrixConversions_androidKt.m4565setFromtUYjHk(this.tmpMatrix, other);
        AndroidComposeView_androidKt.m6590preTransformJiSxe2E(fArr, this.tmpMatrix);
    }

    /* JADX INFO: renamed from: preTranslate-3XD1CNM, reason: not valid java name */
    private final void m6604preTranslate3XD1CNM(float[] fArr, float x, float y) {
        AndroidComposeView_androidKt.m6591preTranslatecG2Xzmc(fArr, x, y, this.tmpMatrix);
    }
}
