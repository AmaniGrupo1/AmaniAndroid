...
public final class DefaultVelocityTracker implements PlatformVelocityTracker {
    ...
    private long currentPointerPositionAccumulator = Offset.INSTANCE.m4468getZeroF1C5BW0();
    ...
    public final void m6108setCurrentPointerPositionAccumulatork4lQ0M$ui(long j) {
    ...
    public void mo6105addPositionUv8p0NA(long timeMillis, long position) {
    ...
    public long mo6106calculateVelocityAH228Gc(long maximumVelocity) {
        boolean value$iv = Velocity.m7743getXimpl(maximumVelocity) > 0.0f && Velocity.m7744getYimpl(maximumVelocity) > 0.0f;
        if (!value$iv) {
            InlineClassHelperKt.throwIllegalStateException("maximumVelocity should be a positive value. You specified=" + ((Object) Velocity.m7750toStringimpl(maximumVelocity)));
        }
        float velocityX = this.xVelocityTracker.calculateVelocity(Velocity.m7743getXimpl(maximumVelocity));
        float velocityY = this.yVelocityTracker.calculateVelocity(Velocity.m7744getYimpl(maximumVelocity));
    ...
    public void mo6104addPointerInputChangeUv8p0NA(PointerInputChange event, long offset) {
        if (VelocityTrackerKt.getVelocityTrackerAddPointsFix()) {
            m6103addPointerInputChangeWithFixUv8p0NA(event, offset);
        } else {
            m6102addPointerInputChangeLegacyUv8p0NA(event, offset);
        }
    }
    ...
    private final void m6102addPointerInputChangeLegacyUv8p0NA(PointerInputChange event, long offset) {
        ...
        while (index$iv < size) {
            ...
            long historicalDelta = Offset.m4456minusMKHz9U(it.getPosition(), previousPointerPosition);
            long previousPointerPosition2 = it.getPosition();
            this.currentPointerPositionAccumulator = Offset.m4457plusMKHz9U(this.currentPointerPositionAccumulator, historicalDelta);
            long uptimeMillis = it.getUptimeMillis();
            long previousPointerPosition3 = this.currentPointerPositionAccumulator;
            mo6105addPositionUv8p0NA(uptimeMillis, Offset.m4457plusMKHz9U(previousPointerPosition3, offset));
            index$iv++;
            previousPointerPosition = previousPointerPosition2;
        }
        long delta = Offset.m4456minusMKHz9U(event.getPosition(), previousPointerPosition);
        this.currentPointerPositionAccumulator = Offset.m4457plusMKHz9U(this.currentPointerPositionAccumulator, delta);
        mo6105addPositionUv8p0NA(event.getUptimeMillis(), Offset.m4457plusMKHz9U(this.currentPointerPositionAccumulator, offset));
    }

    /* JADX INFO: renamed from: addPointerInputChangeWithFix-Uv8p0NA, reason: not valid java name */
    private final void m6103addPointerInputChangeWithFixUv8p0NA(PointerInputChange event, long offset) {
        if (PointerEventKt.changedToDownIgnoreConsumed(event)) {
            resetTracking();
        }
        if (!PointerEventKt.changedToUpIgnoreConsumed(event)) {
            ...
            for (int index$iv = 0; index$iv < size; index$iv++) {
                Object item$iv = historical.get(index$iv);
                HistoricalChange it = (HistoricalChange) item$iv;
                mo6105addPositionUv8p0NA(it.getUptimeMillis(), Offset.m4457plusMKHz9U(it.getOriginalEventPosition(), offset));
            }
            mo6105addPositionUv8p0NA(event.getUptimeMillis(), Offset.m4457plusMKHz9U(event.getOriginalEventPosition(), offset));
        }
