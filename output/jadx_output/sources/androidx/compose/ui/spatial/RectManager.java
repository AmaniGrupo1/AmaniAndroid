...
public final class RectManager {
    ...
    public final void m6722updateOffsetsgTq6Wqs(long screenOffset, long windowOffset, float[] viewToWindowMatrix, int windowWidth, int windowHeight) {
        int analysis = RectManagerKt.m6725analyzeComponents58bKbWc(viewToWindowMatrix);
        ThrottledCallbacks throttledCallbacks = this.throttledCallbacks;
        boolean z = true;
        int $this$hasNonTranslationComponents$iv = (analysis & 2) == 0 ? 1 : 0;
        if (!throttledCallbacks.m6739updateOffsetsLDcG7Xg(screenOffset, windowOffset, $this$hasNonTranslationComponents$iv != 0 ? viewToWindowMatrix : null, windowWidth, windowHeight) && !this.isScreenOrWindowDirty) {
            z = false;
        }
    ...
    public final void scheduleDebounceCallback(boolean ensureSomethingScheduled) {
        ...
        long currentTime = Actual_androidKt.currentTimeMillis();
        long nextFrameIsh = ((long) 16) + currentTime;
    ...
    public final void onLayoutLayerPositionalPropertiesChanged(LayoutNode layoutNode) {
        if (layoutNode.isPlaced()) {
            long outerToInnerOffset = m6720outerToInnerOffsetBjo55l4(layoutNode);
            if (RectManagerKt.m6726isSetgyyYBs(outerToInnerOffset)) {
                layoutNode.m6375setOuterToInnerOffsetgyyYBs$ui(outerToInnerOffset);
                ...
                for (int i$iv$iv = 0; i$iv$iv < size$iv$iv; i$iv$iv++) {
                    ...
                    onLayoutPositionChanged$default(this, it, false, 2, null);
                }
    ...
    public final void onLayoutPositionChanged(LayoutNode layoutNode, boolean forceUpdate) {
        ...
        if (layoutNode.isPlaced()) {
            ...
            if (parent == null || parent.getHasPositionalLayerTransformationsInOffsetFromRoot()) {
                parentOuterInnerOffset = parent == null ? IntOffset.INSTANCE.m7644getZeronOccac() : IntOffset.INSTANCE.m7643getMaxnOccac();
            } else {
                if (parent.getOuterToInnerOffsetDirty()) {
                    parent.setOuterToInnerOffsetDirty$ui(false);
                    parent.m6375setOuterToInnerOffsetgyyYBs$ui(m6720outerToInnerOffsetBjo55l4(parent));
                }
                parentOuterInnerOffset = parent.getOuterToInnerOffset();
            }
            NodeCoordinator outer = layoutNode.getOuterCoordinator$ui();
            if (!RectManagerKt.m6726isSetgyyYBs(parentOuterInnerOffset) || hasPositionalLayerTransformations(outer)) {
                insertOrUpdateTransformedNodeSubhierarchy(layoutNode);
                return;
            }
            ...
            long offsetFromParent = IntOffset.m7637plusqkQi6aY(parentOuterInnerOffset, outer.getPosition());
            ...
            long size = IntSize.m7671constructorimpl((((long) width) << 32) | (((long) height) & 4294967295L));
            int semanticsId2 = layoutNode.getSemanticsId();
            if (!layoutNode.getAddedToRectList()) {
                layoutNode.setAddedToRectList$ui(true);
                boolean focusable = layoutNode.getNodes().m6421hasH91voCI$ui(NodeKind.m6460constructorimpl(1024));
                boolean gesturable = layoutNode.getNodes().m6421hasH91voCI$ui(NodeKind.m6460constructorimpl(16));
                boolean hasCallbacks = this.throttledCallbacks.getRectChangedMap().containsKey(semanticsId2);
                RectList rectList = this.rects;
                if (parent != null) {
                    rectList.insertBasedOnParentOffset(semanticsId2, parent.getSemanticsId(), IntOffset.m7633getXimpl(offsetFromParent), IntOffset.m7634getYimpl(offsetFromParent), width, height, focusable, gesturable, hasCallbacks);
                } else {
                    RectList.insert$default(rectList, semanticsId2, IntOffset.m7633getXimpl(offsetFromParent), IntOffset.m7634getYimpl(offsetFromParent), IntOffset.m7633getXimpl(offsetFromParent) + width, IntOffset.m7634getYimpl(offsetFromParent) + height, 0, focusable, gesturable, hasCallbacks, 0, 544, null);
                }
                invalidate();
            } else if (forceUpdate || !IntOffset.m7632equalsimpl0(offsetFromParent, layoutNode.getLastOffsetFromParent()) || !IntSize.m7674equalsimpl0(size, layoutNode.getLastSize())) {
                RectList rectList2 = this.rects;
                if (parent != null) {
                    semanticsId = semanticsId2;
                    rectList2.moveBasedOnParentOffset(semanticsId, parent.getSemanticsId(), IntOffset.m7633getXimpl(offsetFromParent), IntOffset.m7634getYimpl(offsetFromParent), width, height);
                } else {
                    rectList2.move(semanticsId2, IntOffset.m7633getXimpl(offsetFromParent), IntOffset.m7634getYimpl(offsetFromParent), IntOffset.m7633getXimpl(offsetFromParent) + width, IntOffset.m7634getYimpl(offsetFromParent) + height);
                    semanticsId = semanticsId2;
                }
                invalidate();
            }
            layoutNode.m6374setLastSizeozmzZPI$ui(size);
            layoutNode.m6373setLastOffsetFromParentgyyYBs$ui(offsetFromParent);
        }
    }

    /* JADX INFO: renamed from: getOffsetFromRectListFor-Bjo55l4, reason: not valid java name */
    public final long m6721getOffsetFromRectListForBjo55l4(LayoutNode layoutNode) {
        long topLeft = this.rects.getTopLeft(layoutNode.getSemanticsId());
        if (topLeft == Long.MAX_VALUE) {
            return IntOffset.INSTANCE.m7643getMaxnOccac();
        }
        int x$iv = (int) (topLeft >> 32);
        int y$iv = (int) topLeft;
        return IntOffset.m7627constructorimpl((((long) x$iv) << 32) | (((long) y$iv) & 4294967295L));
    }
    ...
    private final void resetHasPositionalLayerTransformationsForSubtreeIfNeeded(LayoutNode $this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded) {
        if ($this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded.getHasPositionalLayerTransformationsInOffsetFromRoot() && !hasPositionalLayerTransformations($this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded.getOuterCoordinator$ui())) {
            ...
            if ($this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded.getOuterToInnerOffsetDirty()) {
                long it = m6720outerToInnerOffsetBjo55l4($this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded);
                $this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded.m6375setOuterToInnerOffsetgyyYBs$ui(it);
                $this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded.setOuterToInnerOffsetDirty$ui(false);
            }
            if (IntOffset.m7632equalsimpl0($this$resetHasPositionalLayerTransformationsForSubtreeIfNeeded.getOuterToInnerOffset(), IntOffset.INSTANCE.m7643getMaxnOccac())) {
                return;
            }
    ...
    private final void insertOrUpdateTransformedNode(LayoutNode layoutNode) {
        ...
        layoutNode.m6373setLastOffsetFromParentgyyYBs$ui(IntOffset.INSTANCE.m7643getMaxnOccac());
        ...
        if (firstPlacement || !this.rects.update(id, l, t, r, b)) {
            ...
            RectList.insert$default(this.rects, id, l, t, r, b, parentId, layoutNode.getNodes().m6421hasH91voCI$ui(NodeKind.m6460constructorimpl(1024)), layoutNode.getNodes().m6421hasH91voCI$ui(NodeKind.m6460constructorimpl(16)), this.throttledCallbacks.getRectChangedMap().containsKey(id), 0, 512, null);
        }
        invalidate();
    }
    ...
    private final void boundingRectInRoot(NodeCoordinator $this$boundingRectInRoot, MutableRect rect) {
        ...
        while (coordinator2 != null) {
            ...
            if (coordinator2 != layoutNode.getOuterCoordinator$ui() || layoutNode.getHasPositionalLayerTransformationsInOffsetFromRoot()) {
                ...
                long offset = m6721getOffsetFromRectListForBjo55l4(layoutNode);
                if (IntOffset.m7632equalsimpl0(offset, IntOffset.INSTANCE.m7643getMaxnOccac())) {
                    coordinator = coordinator2;
                    j = 4294967295L;
                    c = ' ';
                } else {
                    float x$iv$iv = IntOffset.m7633getXimpl(offset);
                    float y$iv$iv = IntOffset.m7634getYimpl(offset);
                    long v1$iv$iv$iv = Float.floatToRawIntBits(x$iv$iv);
                    long v2$iv$iv$iv = Float.floatToRawIntBits(y$iv$iv);
                    rect.m4437translatek4lQ0M(Offset.m4444constructorimpl((v1$iv$iv$iv << 32) | (v2$iv$iv$iv & 4294967295L)));
                    return;
                }
            }
            OwnedLayer layer = coordinator.getLayer();
            if (layer != null) {
                float[] matrix = layer.mo6517getUnderlyingMatrixsQKQjiQ();
                if (!MatrixKt.m4966isIdentity58bKbWc(matrix)) {
                    Matrix.m4949mapimpl(matrix, rect);
                }
            }
            long jMo6389getPositionnOccac = coordinator.getPosition();
            float x$iv$iv2 = IntOffset.m7633getXimpl(jMo6389getPositionnOccac);
            float y$iv$iv2 = IntOffset.m7634getYimpl(jMo6389getPositionnOccac);
            long v1$iv$iv$iv2 = Float.floatToRawIntBits(x$iv$iv2);
            long v2$iv$iv$iv2 = Float.floatToRawIntBits(y$iv$iv2);
            rect.m4437translatek4lQ0M(Offset.m4444constructorimpl((v1$iv$iv$iv2 << c) | (v2$iv$iv$iv2 & j)));
    ...
    private final boolean hasPositionalLayerTransformations(NodeCoordinator $this$hasPositionalLayerTransformations) {
        ...
        return (layer == null || MatrixKt.m4966isIdentity58bKbWc(layer.mo6517getUnderlyingMatrixsQKQjiQ())) ? false : true;
    }

    /* JADX INFO: renamed from: outerToInnerOffset-Bjo55l4, reason: not valid java name */
    private final long m6720outerToInnerOffsetBjo55l4(LayoutNode $this$outerToInnerOffset_u2dBjo55l4) {
        NodeCoordinator terminator = $this$outerToInnerOffset_u2dBjo55l4.getOuterCoordinator$ui();
        long position = IntOffset.INSTANCE.m7644getZeronOccac();
        for (NodeCoordinator coordinator = $this$outerToInnerOffset_u2dBjo55l4.getInnerCoordinator$ui(); coordinator != null && coordinator != terminator; coordinator = coordinator.getWrappedBy()) {
            ...
            position = IntOffset.m7637plusqkQi6aY(position, coordinator.getPosition());
        }
        return position;
    }
        ...
        while (i$iv2 < items$iv.length - 2 && i$iv2 < size$iv) {
            ...
            if ((((int) (meta$iv$iv >> 61)) & 1) != 0) {
                ...
                if ($i$f$rectIntersectsRect != 0) {
                    ...
                    int count$iv$iv$iv2 = $i$f$unpackMetaValue & 33554431;
                    LayoutNode node5 = rectManager.layoutNodes.get(count$iv$iv$iv2);
                    if (node5 != null) {
                        int currentlyFocusedId3 = currentlyFocusedId2;
                        if (currentlyFocusedId3 == count$iv$iv$iv2 && currentlyFocusedId3 != -1) {
                            return null;
                        }
                        if (node5.getDepth() < bestDepth4) {
                            ...
                            if (rectManager.isDescendantOf$ui(node5, container)) {
                                ...
                                if ((this_$iv2.getAggregateChildKindSet() & iM6460constructorimpl) != 0) {
                                    ...
                                    while (true) {
                                        ...
                                        if ((it$iv$iv$iv.getKindSet() & iM6460constructorimpl) != 0) {
                                            ...
                                            while (nodePop != null) {
                                                int semanticsId2 = count$iv$iv$iv2;
                                        ...
                                        if ((semanticsId3 & iM6460constructorimpl) == 0) {
                                            break;
                                        }
                                        node$iv$iv$iv$iv = node$iv$iv$iv$iv.getChild();
                                        count$iv$iv$iv2 = semanticsId;
                                        node5 = node;
                                    }
                                } else {
                                    node = node5;
    ...
    public final boolean intersects$ui(DelegatableNode $this$intersects, int left, int top, int right, int bottom) {
        NodeCoordinator coordinator = DelegatableNodeKt.m6312requireCoordinator64DMado($this$intersects, NodeKind.m6460constructorimpl(1024));
        LayoutNode layout = coordinator.getLayoutNode();
        if (Intrinsics.areEqual(coordinator, layout.getOuterCoordinator$ui())) {
            return true;
        }
        long localTopLeft = LayoutCoordinates.m6148localPositionOfS_NoaFU$default(layout.getOuterCoordinator$ui(), coordinator, 0L, false, 6, null);
        long topLeft = layout.getOuterCoordinator$ui().mo6152localToRootMKHz9U(localTopLeft);
        long size = coordinator.mo6149getSizeYbymL2g();
