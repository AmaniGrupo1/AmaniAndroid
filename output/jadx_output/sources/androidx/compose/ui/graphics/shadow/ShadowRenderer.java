...
public abstract class ShadowRenderer {
    ...
    protected abstract void mo5409buildShadow_SMYjrA(DrawScope drawScope, long j, long j2, Path path);

    /* JADX INFO: renamed from: onDrawShadow-MLmccfk */
    protected abstract void mo5410onDrawShadowMLmccfk(DrawScope drawScope, long j, long j2, Path path, float f, ColorFilter colorFilter, Brush brush, int i);
    ...
    */
    public final void m5420drawShadowerFMhIw(DrawScope $this$drawShadow_u2derFMhIw, ColorFilter colorFilter, long j, long j2, Brush brush, float alpha, int i) {
        ColorFilter filter;
        ...
        if (colorFilter != null) {
            ...
            if (j2 != 16) {
                filter = m5419obtainTint8_81llA(j2);
            }
        } else {
            filter = null;
        }
        if ((this.generatedSize == InlineClassHelperKt.UnspecifiedPackedFloats) || !Size.m4517equalsimpl0(this.generatedSize, j) || this.generatedLayoutDirection != $this$drawShadow_u2derFMhIw.getLayoutDirection()) {
            mo5409buildShadow_SMYjrA($this$drawShadow_u2derFMhIw, j, this.cornerRadius, this.path);
            this.generatedSize = j;
            this.generatedLayoutDirection = $this$drawShadow_u2derFMhIw.getLayoutDirection();
            this.generatedDensity = $this$drawShadow_u2derFMhIw.getDensity();
        } else {
            if (!(this.generatedDensity == $this$drawShadow_u2derFMhIw.getDensity())) {
            }
        }
        mo5410onDrawShadowMLmccfk($this$drawShadow_u2derFMhIw, j, this.cornerRadius, this.path, alpha, filter, brush, i);
    }

    public void invalidateShadow() {
        this.generatedSize = Size.INSTANCE.m4529getUnspecifiedNHjbRc();
    ...
    private final ColorFilter m5419obtainTint8_81llA(long color) {
        ColorFilter colorFilter = this.shadowTint;
        if (colorFilter == null || !Color.m4698equalsimpl0(this.shadowTintColor, color)) {
            ColorFilter colorFilter2 = ColorFilter.Companion.m4738tintxETnrds$default(ColorFilter.INSTANCE, color, 0, 2, null);
    ...
    private final void updateParamsFromOutline(Outline outline) {
        if (outline instanceof Outline.Generic) {
            ...
            this.cornerRadius = CornerRadius.INSTANCE.m4424getZerokKHJgLs();
            return;
        }
        if (outline instanceof Outline.Rounded) {
            if (RoundRectKt.isSimple(((Outline.Rounded) outline).getRoundRect())) {
                this.path = null;
                this.cornerRadius = ((Outline.Rounded) outline).getRoundRect().m4502getTopLeftCornerRadiuskKHJgLs();
                return;
            } else {
