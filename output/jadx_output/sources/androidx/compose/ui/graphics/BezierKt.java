...
public final class BezierKt {
    ...
    private static final int findDerivativeRoots(PathSegment pathSegment, boolean z, float[] fArr, int i) {
        ...
        switch (WhenMappings.$EnumSwitchMapping$0[pathSegment.getType().ordinal()]) {
            ...
            case 4:
                ...
                int iFindQuadraticRoots = findQuadraticRoots(f3, f4, f5, fArr, i);
                float f6 = (f4 - f3) * 2.0f;
                return iFindQuadraticRoots + writeValidRootInUnitRange((-f6) / (((f5 - f4) * 2.0f) - f6), fArr, i + iFindQuadraticRoots);
            default:
                return 0;
        }
    }
    ...
    public static final long computeHorizontalBounds(PathSegment segment, float[] roots, int index) {
        ...
        return FloatFloatPair.m12constructorimpl(minX, maxX);
    }
    ...
    public static final long computeVerticalBounds(PathSegment segment, float[] roots, int index) {
        ...
        return FloatFloatPair.m12constructorimpl(minY, maxY);
    }
    ...
    */
    private static final int monotonicQuadraticWinding(float[] points, int offset, float x, float y, float[] tmpRoots) {
        ...
        float y02 = points[offset + 1];
        float y1 = points[offset + 3];
        float y22 = points[offset + 5];
        int rootCount = findQuadraticRoots$default((y02 - (y1 * 2.0f)) + y22, (y1 - y02) * 2.0f, y02 - y, tmpRoots, 0, 16, null);
        if (rootCount != 0) {
            xt = evaluateQuadratic(points[0], points[2], points[4], tmpRoots[0]);
        } else {
            xt = points[(1 - direction) * 2];
        }
        float $this$closeTo$iv = xt;
        if (Math.abs($this$closeTo$iv - x) < FloatEpsilon) {
            if (x == points[4]) {
            }
            return 0;
        }
    ...
    */
    private static final int monotonicCubicWinding(float[] points, int offset, float x, float y) {
        ...
        float y1 = points[offset + 3];
        float y2 = points[offset + 5];
        float y32 = points[offset + 7];
        float root = findFirstCubicRoot(y02 - y, y1 - y, y2 - y, y32 - y);
        if (Float.isNaN(root)) {
            return 0;
        }
        float xt = evaluateCubic(x0, x1, x2, x3, root);
        if (Math.abs(xt - x) < FloatEpsilon) {
            if (x == x3) {
            }
            return 0;
        }
