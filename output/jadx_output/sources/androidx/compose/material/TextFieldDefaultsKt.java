...
public final class TextFieldDefaultsKt {
    ...
    public static final State<BorderStroke> m1991animateBorderStrokeAsStateNuRrP5Q(boolean enabled, boolean isError, InteractionSource interactionSource, TextFieldColors colors, float focusedBorderThickness, float unfocusedBorderThickness, Composer $composer, int $changed) {
        ...
        State<Color> stateIndicatorColor = colors.indicatorColor(enabled, isError, interactionSource, $composer, ($changed & 14) | ($changed & 112) | ($changed & 896) | ($changed & 7168));
        float targetThickness = animateBorderStrokeAsState_NuRrP5Q$lambda$0(stateCollectIsFocusedAsState) ? focusedBorderThickness : unfocusedBorderThickness;
        if (enabled) {
            $composer.startReplaceGroup(772641254);
            ComposerKt.sourceInformation($composer, "987@43212L76");
            stateRememberUpdatedState = AnimateAsStateKt.m161animateDpAsStateAjpBEmI(targetThickness, AnimationSpecKt.tween$default(150, 0, null, 6, null), null, null, $composer, 48, 12);
            $composer.endReplaceGroup();
        } else {
            $composer.startReplaceGroup(772737540);
            ComposerKt.sourceInformation($composer, "989@43310L46");
            stateRememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Dp.m7503boximpl(unfocusedBorderThickness), $composer, ($changed >> 15) & 14);
            $composer.endReplaceGroup();
        }
        State<BorderStroke> stateRememberUpdatedState2 = SnapshotStateKt.rememberUpdatedState(new BorderStroke(stateRememberUpdatedState.getValue().m7519unboximpl(), new SolidColor(stateIndicatorColor.getValue().m4707unboximpl(), null), null), $composer, 0);
        if (ComposerKt.isTraceInProgress()) {
