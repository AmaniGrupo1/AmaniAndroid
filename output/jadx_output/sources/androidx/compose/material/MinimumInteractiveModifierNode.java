...
public final class MinimumInteractiveModifierNode extends Modifier.Node implements CompositionLocalConsumerModifierNode, LayoutModifierNode {
    ...
    public MeasureResult mo69measure3p2s80s(MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        ...
        if (enforcement) {
            width = Math.max(placeable.getWidth(), $this$measure_u2d3p2s80s.mo399roundToPx0680j_4(DpSize.m7603getWidthD9Ej5fM(size)));
        } else {
            width = placeable.getWidth();
        }
        if (enforcement) {
            height = Math.max(placeable.getHeight(), $this$measure_u2d3p2s80s.mo399roundToPx0680j_4(DpSize.m7601getHeightD9Ej5fM(size)));
        } else {
