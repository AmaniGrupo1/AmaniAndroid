...
public final class FloatSpringSpec implements FloatAnimationSpec {
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float targetValue, float initialVelocity) {
        ...
        int bits$iv$iv$iv = (int) (this.spring.m200updateValuesIJZedt4$animation_core(initialValue, initialVelocity, playTimeMillis) >> 32);
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float targetValue, float initialVelocity) {
        long playTimeMillis = playTimeNanos / 1000000;
        this.spring.setFinalPosition(targetValue);
        int bits$iv$iv$iv = (int) (4294967295L & this.spring.m200updateValuesIJZedt4$animation_core(initialValue, initialVelocity, playTimeMillis));
        return Float.intBitsToFloat(bits$iv$iv$iv);
    }
    ...
    public long getDurationNanos(float initialValue, float targetValue, float initialVelocity) {
        return SpringEstimationKt.estimateAnimationDurationMillis(this.spring.getStiffness(), this.spring.getDampingRatio(), initialVelocity / this.visibilityThreshold, (initialValue - targetValue) / this.visibilityThreshold, 1.0f) * 1000000;
    }
}
