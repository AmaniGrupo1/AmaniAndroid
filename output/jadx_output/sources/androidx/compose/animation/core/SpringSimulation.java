...
public final class SpringSimulation {
    ...
    public final long m200updateValuesIJZedt4$animation_core(float lastDisplacement, float lastVelocity, long timeElapsed) {
        double displacement;
        ...
        if (this.dampingRatio > 1.0f) {
            double s = this.naturalFreq * Math.sqrt(dampingRatioSquared - ((double) 1));
            ...
            if (z) {
                double coeffB2 = ((double) lastVelocity) + (d * ((double) adjustedDisplacement));
                ...
                double d2 = 1;
                double dampedFreq = Math.sqrt(d2 - dampingRatioSquared) * d;
                double sinCoeff = ((((double) adjustedDisplacement) * (-r)) + ((double) lastVelocity)) * (d2 / dampedFreq);
        ...
        return Motion.m191constructorimpl(v2$iv$iv);
    }
}
