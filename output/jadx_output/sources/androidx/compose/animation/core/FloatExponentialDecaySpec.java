...
public final class FloatExponentialDecaySpec implements FloatDecayAnimationSpec {
    ...
    public float getTargetValue(float initialValue, float initialVelocity) {
        ...
        double duration = (Math.log(Math.abs(getAbsVelocityThreshold() / initialVelocity)) / ((double) this.friction)) * ((double) 1000);
        return (initialValue - (initialVelocity / this.friction)) + ((initialVelocity / this.friction) * ((float) Math.exp((((double) this.friction) * duration) / ((double) 1000.0f))));
    }
}
