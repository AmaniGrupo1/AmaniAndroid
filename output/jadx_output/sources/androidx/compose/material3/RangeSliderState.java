...
public final class RangeSliderState {
    ...
    /* JADX INFO: renamed from: trackHeight$delegate, reason: from kotlin metadata */
    ...
    public final void onDrag$material3_release(boolean isStart, float offset) {
        ...
        if (isStart) {
            ...
            offsetRange = SliderKt.SliderRange(SliderKt.snapValueToTick(RangesKt.coerceIn(getRawOffsetStart$material3_release(), getMinPx(), offsetEnd), this.tickFractions, getMinPx(), getMaxPx()), offsetEnd);
        } else {
            ...
            offsetRange = SliderKt.SliderRange(offsetStart, SliderKt.snapValueToTick(RangesKt.coerceIn(getRawOffsetEnd$material3_release(), offsetStart, getMaxPx()), this.tickFractions, getMinPx(), getMaxPx()));
        }
        long scaledUserValue = m2678scaleToUserValueowVgs5E(getMinPx(), getMaxPx(), offsetRange);
        if (!SliderRange.m2800equalsimpl0(scaledUserValue, SliderKt.SliderRange(getActiveRangeStart(), getActiveRangeEnd()))) {
            if (this.onValueChange != null) {
                Function1<? super SliderRange, Unit> function1 = this.onValueChange;
                if (function1 != null) {
                    function1.invoke(SliderRange.m2797boximpl(scaledUserValue));
                    return;
                }
                return;
            }
            setActiveRangeStart(SliderRange.m2802getStartimpl(scaledUserValue));
            setActiveRangeEnd(SliderRange.m2801getEndInclusiveimpl(scaledUserValue));
        }
    }
    ...
    private final long m2678scaleToUserValueowVgs5E(float minPx, float maxPx, long offset) {
        return SliderKt.m2790scaleziovWd0(minPx, maxPx, offset, this.valueRange.getStart().floatValue(), this.valueRange.getEndInclusive().floatValue());
    }
    ...
    public final void updateMinMaxPx$material3_release() {
        float f = 2;
        float newMaxPx = Math.max(getTotalWidth$material3_release() - (getEndThumbWidth$material3_release() / f), 0.0f);
        float newMinPx = Math.min(getStartThumbWidth$material3_release() / f, newMaxPx);
