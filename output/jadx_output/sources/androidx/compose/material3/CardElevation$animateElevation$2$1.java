    ...
    public final Object invokeSuspend(Object $result) {
        ...
        switch (this.label) {
            case 0:
                ...
                if (!Dp.m7510equalsimpl0(this.$animatable.getTargetValue().m7519unboximpl(), this.$target)) {
                    boolean z = this.$enabled;
                    Animatable<Dp, AnimationVector1D> animatable = this.$animatable;
                    if (!z) {
                        this.label = 1;
                        if (animatable.snapTo(Dp.m7503boximpl(this.$target), this) == coroutine_suspended) {
                            return coroutine_suspended;
                        }
                        cardElevation$animateElevation$2$12 = this;
                    } else {
                        float fM7519unboximpl = animatable.getTargetValue().m7519unboximpl();
                        DragInteraction.Start start = null;
                        if (Dp.m7510equalsimpl0(fM7519unboximpl, this.this$0.pressedElevation)) {
                            start = new PressInteraction.Press(Offset.INSTANCE.m4468getZeroF1C5BW0(), null);
                        } else if (Dp.m7510equalsimpl0(fM7519unboximpl, this.this$0.hoveredElevation)) {
                            start = new HoverInteraction.Enter();
                        } else if (Dp.m7510equalsimpl0(fM7519unboximpl, this.this$0.focusedElevation)) {
                            start = new FocusInteraction.Focus();
                        } else if (Dp.m7510equalsimpl0(fM7519unboximpl, this.this$0.draggedElevation)) {
                            start = new DragInteraction.Start();
                        }
                        this.label = 2;
                        if (ElevationKt.m3183animateElevationrAjV9yQ(this.$animatable, this.$target, start, this.$interaction, this) == coroutine_suspended) {
                            return coroutine_suspended;
                        }
