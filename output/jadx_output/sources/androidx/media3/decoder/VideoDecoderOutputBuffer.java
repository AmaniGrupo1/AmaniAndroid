...
public class VideoDecoderOutputBuffer extends DecoderOutputBuffer {
    ...
    public boolean initForOffsetFrames(int offset, int width, int height, int yStride, int uvStride, int colorspace, int alignedHeight) {
        if (this.yuvPlanes == null) {
            this.yuvPlanes = new ByteBuffer[3];
        }
        if (this.data == null) {
        ...
        ByteBuffer data = this.data;
        int yLength = yStride * height;
        int uvLength = (height >> 1) * uvStride;
        int alignedYLength = yStride * alignedHeight;
        int alignedUvLength = (alignedHeight >> 1) * uvStride;
