...
public final class ConstantRateTimestampIterator implements TimestampIterator {
    ...
    public ConstantRateTimestampIterator(long startPositionUs, long endPositionUs, float frameRate) {
        boolean z = false;
        Preconditions.checkArgument(endPositionUs > 0);
        Preconditions.checkArgument(frameRate > 0.0f);
        if (0 <= startPositionUs && startPositionUs < endPositionUs) {
            z = true;
        }
        ...
        this.totalNumberOfFramesToAdd = Math.max(Math.round(frameRate * durationSecs), 1);
    ...
    private long getTimestampUsAfter(int numberOfFrames) {
        ...
        Preconditions.checkState(timestampUs >= 0);
        return timestampUs;
    }
}
