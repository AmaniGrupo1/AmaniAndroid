...
public final class GlProgram {
    ...
    public GlProgram(Context context, int vertexShaderResId, int fragmentShaderResId) throws IOException, GlUtil.GlException {
        this(Util.loadRawResource(context, vertexShaderResId), Util.loadRawResource(context, fragmentShaderResId));
    }
    ...
    public void setBufferAttribute(String name, float[] values, int size) {
        ((Attribute) Preconditions.checkNotNull(this.attributeByName.get(name))).setBuffer(values, size);
    }

    public void setSamplerTexIdUniform(String name, int texId, int texUnitIndex) {
        ((Uniform) Preconditions.checkNotNull(this.uniformByName.get(name))).setSamplerTexId(texId, texUnitIndex);
    }

    public void setSamplerTexIdUniform(String name, int texId, int texUnitIndex, int texMinFilter) {
        Uniform texUniform = (Uniform) Preconditions.checkNotNull(this.uniformByName.get(name));
    ...
    public void setIntUniform(String name, int value) {
        ((Uniform) Preconditions.checkNotNull(this.uniformByName.get(name))).setInt(value);
    }

    public void setIntsUniform(String name, int[] value) {
        ((Uniform) Preconditions.checkNotNull(this.uniformByName.get(name))).setInts(value);
    }

    public void setFloatUniform(String name, float value) {
        ((Uniform) Preconditions.checkNotNull(this.uniformByName.get(name))).setFloat(value);
    }

    public void setFloatsUniform(String name, float[] value) {
        ((Uniform) Preconditions.checkNotNull(this.uniformByName.get(name))).setFloats(value);
    }
    ...
    private static final class Attribute {
        ...
        public void bind() throws GlUtil.GlException {
            Buffer buffer = (Buffer) Preconditions.checkNotNull(this.buffer, "call setBuffer before bind");
    ...
    private static final class Uniform {
        ...
        public void setTexMinFilter(int texMinFilter) {
            ...
            switch (this.type) {
                ...
                case 36198:
                    ...
                    if (this.type == 35678 || !externalTexturesRequireNearestSampling) {
                        i2 = C.TEXTURE_MIN_FILTER_LINEAR;
                    } else {
                        i2 = 9728;
                    }
