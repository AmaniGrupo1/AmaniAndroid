...
public final class GlUtil {
    ...
    public static final class GlException extends Exception {
        ...
        public GlException(String message, List<Integer> errorCodes) {
            super(message);
            this.errorCodes = ImmutableList.copyOf((Collection) errorCodes);
        }
    }
    ...
    public static boolean isColorTransferSupported(int colorTransfer) throws GlException {
        if (colorTransfer == 6) {
    ...
    public static EGLContext createEglContext(EGLContext sharedContext, EGLDisplay eglDisplay, int openGlVersion, int[] configAttributes) throws GlException {
        boolean z = true;
        Preconditions.checkArgument(Arrays.equals(configAttributes, EGL_CONFIG_ATTRIBUTES_RGBA_8888) || Arrays.equals(configAttributes, EGL_CONFIG_ATTRIBUTES_RGBA_1010102));
    ...
    public static void checkGlError() throws GlException {
        ...
        while (true) {
            ...
            foundError = true;
            errorCodes.add(Integer.valueOf(error));
        }
        if (foundError) {
    ...
    public static void checkEglException(String errorMessage) throws GlException {
        ...
        if (error != 12288) {
            throw new GlException(errorMessage + ", error code: 0x" + Integer.toHexString(error), ImmutableList.of(Integer.valueOf(error)));
        }
    }
    ...
    private static void assertValidTextureSize(int width, int height) throws GlException {
        ...
        Preconditions.checkState(maxTextureSize > 0, "Create a OpenGL context first or run the GL methods on an OpenGL thread.");
        if (width < 0 || height < 0) {
    ...
    public static int createExternalTexture() throws GlException {
        ...
        bindTexture(36197, texId, C.TEXTURE_MIN_FILTER_LINEAR);
        return texId;
    }
    ...
    public static void bindTexture(int textureTarget, int texId, int sampleFilter) throws GlException {
        ...
        checkGlError();
        GLES20.glTexParameteri(textureTarget, Data.MAX_DATA_BYTES, sampleFilter);
        checkGlError();
    ...
    public static void blitFrameBuffer(int readFboId, GlRect readRect, int drawFboId, GlRect drawRect) throws GlException {
        int[] boundFramebuffer = new int[1];
        GLES20.glGetIntegerv(36006, boundFramebuffer, 0);
        checkGlError();
        GLES30.glBindFramebuffer(36008, readFboId);
        checkGlError();
        GLES30.glBindFramebuffer(36009, drawFboId);
        checkGlError();
        GLES30.glBlitFramebuffer(readRect.left, readRect.bottom, readRect.right, readRect.top, drawRect.left, drawRect.bottom, drawRect.right, drawRect.top, 16384, C.TEXTURE_MIN_FILTER_LINEAR);
        checkGlError();
        GLES20.glBindFramebuffer(36160, boundFramebuffer[0]);
        checkGlError();
    }
    ...
    public static int createPixelBufferObject(int size) throws GlException {
        ...
        checkGlError();
        GLES30.glBindBuffer(35051, ids[0]);
        checkGlError();
        GLES30.glBufferData(35051, size, null, 35049);
        checkGlError();
    ...
    public static void schedulePixelBufferRead(int readFboId, int width, int height, int bufferId) throws GlException {
        focusFramebufferUsingCurrentContext(readFboId, width, height);
        GLES30.glBindBuffer(35051, bufferId);
        checkGlError();
    ...
    public static ByteBuffer mapPixelBufferObject(int bufferId, int size) throws GlException {
        GLES20.glBindBuffer(35051, bufferId);
        checkGlError();
        ByteBuffer mappedPixelBuffer = (ByteBuffer) GLES30.glMapBufferRange(35051, 0, size, 1);
        checkGlError();
        GLES30.glBindBuffer(35051, 0);
        checkGlError();
    ...
    private static void focusRenderTarget(EGLDisplay eglDisplay, EGLContext eglContext, EGLSurface eglSurface, int framebuffer, int width, int height) throws GlException {
        ...
        if (!eglSurface.equals(EGL14.EGL_NO_SURFACE) && framebuffer == 0 && getContextMajorVersion() >= 3) {
