...
public final class TimestampAdjuster {
        ...
        if (canInitialize) {
            ...
            while (!isInitialized()) {
                if (timeoutMs == 0) {
                    ...
                    Preconditions.checkState(remainingTimeoutMs > 0);
    ...
    public synchronized long adjustSampleTimestamp(long timeUs) {
        ...
        if (!isInitialized()) {
            if (this.firstSampleTimestampUs == MODE_SHARED) {
                desiredSampleTimestampUs = ((Long) Preconditions.checkNotNull(this.nextSampleTimestampUs.get())).longValue();
            } else {
    ...
    public static long ptsToUs(long pts) {
        return Util.scaleLargeTimestamp(pts, 1000000L, 90000L);
    }
    ...
    public static long usToNonWrappedPts(long us) {
        return Util.scaleLargeTimestamp(us, 90000L, 1000000L);
    }
}
