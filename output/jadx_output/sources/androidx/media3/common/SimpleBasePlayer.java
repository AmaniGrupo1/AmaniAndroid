...
public abstract class SimpleBasePlayer extends BasePlayer {
    ...
    public static final class State {
        ...
        public static final class Builder {
            ...
            public Builder setVolume(float volume) {
                ...
                this.unmuteVolume = volume != 0.0f ? volume : this.volume;
                this.volume = volume;
                return this;
            }

            public Builder setUnmuteVolume(float unmuteVolume) {
                Preconditions.checkArgument(unmuteVolume >= 0.0f && unmuteVolume <= 1.0f);
            ...
            public Builder setDeviceVolume(int deviceVolume) {
                Preconditions.checkArgument(deviceVolume >= 0);
            ...
            public Builder setPlaylist(List<MediaItemData> playlist) {
                ...
                for (int i = 0; i < playlist.size(); i++) {
                    Preconditions.checkArgument(uids.add(playlist.get(i).uid), "Duplicate MediaItemData UID in playlist");
                }
            ...
            public Builder setPlaylist(Timeline timeline, Tracks currentTracks, MediaMetadata currentMetadata) {
                this.playlist = null;
            ...
            public Builder setCurrentAd(int adGroupIndex, int adIndexInAdGroup) {
                Preconditions.checkArgument((adGroupIndex == -1) == (adIndexInAdGroup == -1));
        ...
        private State(Builder builder) {
            ...
            if (!builder.timeline.isEmpty()) {
                ...
                if (builder.currentAdGroupIndex != -1) {
                    ...
                    if (adCountInGroup != -1) {
                        Preconditions.checkArgument(builder.currentAdIndexInAdGroup < adCountInGroup, "Ad group has less ads than adIndexInGroupIndex");
                    }
                }
                if (builder.playlist != null) {
                    MediaItemData mediaItemData = (MediaItemData) builder.playlist.get(mediaItemIndex);
                    currentTracks = mediaItemData.tracks;
                    currentMetadata = mediaItemData.mediaMetadata;
                }
                if (currentMetadata == null) {
                    currentMetadata = SimpleBasePlayer.getCombinedMediaMetadata(builder.timeline.getWindow(mediaItemIndex, new Timeline.Window()).mediaItem, (Tracks) Preconditions.checkNotNull(currentTracks));
                ...
                currentTracks = currentTracks == null ? Tracks.EMPTY : currentTracks;
                if (currentMetadata == null) {
            ...
            contentPositionMsSupplier = builder.contentPositionMs != null ? (builder.currentAdGroupIndex == -1 && builder.playWhenReady && builder.playbackState == 3 && builder.playbackSuppressionReason == 0 && builder.contentPositionMs.longValue() != C.TIME_UNSET) ? PositionSupplier.getExtrapolating(builder.contentPositionMs.longValue(), builder.playbackParameters.speed) : PositionSupplier.getConstant(builder.contentPositionMs.longValue()) : contentPositionMsSupplier;
            PositionSupplier adPositionMsSupplier = builder.adPositionMsSupplier;
            adPositionMsSupplier = builder.adPositionMs != null ? (builder.currentAdGroupIndex != -1 && builder.playWhenReady && builder.playbackState == 3 && builder.playbackSuppressionReason == 0) ? PositionSupplier.getExtrapolating(builder.adPositionMs.longValue(), 1.0f) : PositionSupplier.getConstant(builder.adPositionMs.longValue()) : adPositionMsSupplier;
            ...
            this.currentTracks = (Tracks) Preconditions.checkNotNull(currentTracks);
            this.currentMetadata = currentMetadata;
        ...
        public ImmutableList<MediaItemData> getPlaylist() {
            if (!(this.timeline instanceof PlaylistTimeline)) {
                Timeline.Window window = new Timeline.Window();
                Timeline.Period period = new Timeline.Period();
                ImmutableList.Builder<MediaItemData> items = ImmutableList.builderWithExpectedSize(this.timeline.getWindowCount());
                for (int i = 0; i < this.timeline.getWindowCount(); i++) {
                    items.add(MediaItemData.buildFromState(this, i, period, window));
                }
                return items.build();
            }
            return ((PlaylistTimeline) this.timeline).playlist;
        }
    ...
    private static final class PlaylistTimeline extends Timeline {
        ...
        public Timeline.Period getPeriodByUid(Object periodUid, Timeline.Period period) {
            int periodIndex = ((Integer) Preconditions.checkNotNull(this.periodIndexByUid.get(periodUid))).intValue();
    ...
    public static final class MediaItemData {
        ...
        public static final class Builder {
            ...
            public Builder setDefaultPositionUs(long defaultPositionUs) {
                Preconditions.checkArgument(defaultPositionUs >= 0);
                this.defaultPositionUs = defaultPositionUs;
                return this;
            }

            public Builder setDurationUs(long durationUs) {
                Preconditions.checkArgument(durationUs == C.TIME_UNSET || durationUs >= 0);
                this.durationUs = durationUs;
                return this;
            }

            public Builder setPositionInFirstPeriodUs(long positionInFirstPeriodUs) {
                Preconditions.checkArgument(positionInFirstPeriodUs >= 0);
            ...
            public Builder setPeriods(List<PeriodData> periods) {
                ...
                for (int i = 0; i < periodCount - 1; i++) {
                    Preconditions.checkArgument(periods.get(i).durationUs != C.TIME_UNSET, "Periods other than last need a duration");
                    for (int j = i + 1; j < periodCount; j++) {
                        Preconditions.checkArgument(!periods.get(i).uid.equals(periods.get(j).uid), "Duplicate PeriodData UIDs in period list");
                    }
                }
        ...
        public static MediaItemData buildFromState(State state, int mediaItemIndex, Timeline.Period period, Timeline.Window window) {
            boolean isCurrentItem = SimpleBasePlayer.getCurrentMediaItemIndexInternal(state) == mediaItemIndex;
            ...
            for (int i = window.firstPeriodIndex; i <= window.lastPeriodIndex; i++) {
                state.timeline.getPeriod(i, period, true);
                periods.add(new PeriodData.Builder(Preconditions.checkNotNull(period.uid)).setAdPlaybackState(period.adPlaybackState).setDurationUs(period.durationUs).setIsPlaceholder(period.isPlaceholder).build());
            }
            return new Builder(window.uid).setDefaultPositionUs(window.defaultPositionUs).setDurationUs(window.durationUs).setElapsedRealtimeEpochOffsetMs(window.elapsedRealtimeEpochOffsetMs).setIsDynamic(window.isDynamic).setIsPlaceholder(window.isPlaceholder).setIsSeekable(window.isSeekable).setLiveConfiguration(window.liveConfiguration).setManifest(window.manifest).setMediaItem(window.mediaItem).setMediaMetadata(isCurrentItem ? state.currentMetadata : null).setPeriods(periods.build()).setPositionInFirstPeriodUs(window.positionInFirstPeriodUs).setPresentationStartTimeMs(window.presentationStartTimeMs).setTracks(isCurrentItem ? state.currentTracks : Tracks.EMPTY).setWindowStartTimeMs(window.windowStartTimeMs).build();
        }
        ...
        public Timeline.Period getPeriod(int windowIndex, int periodIndexInMediaItem, Timeline.Period period) {
            if (this.periods.isEmpty()) {
                period.set(this.uid, this.uid, windowIndex, this.positionInFirstPeriodUs + this.durationUs, -this.positionInFirstPeriodUs, AdPlaybackState.NONE, this.isPlaceholder);
            } else {
    ...
    public interface PositionSupplier {
        ...
        static PositionSupplier getConstant(final long positionMs) {
            return new PositionSupplier() { // from class: androidx.media3.common.SimpleBasePlayer$PositionSupplier$$ExternalSyntheticLambda1
                ...
                public final long get() {
                    return SimpleBasePlayer.PositionSupplier.lambda$getConstant$0(positionMs);
                }
            };
        }

        static /* synthetic */ long lambda$getConstant$0(long positionMs) {
            return positionMs;
        }

        static PositionSupplier getExtrapolating(final long currentPositionMs, final float playbackSpeed) {
            ...
            return new PositionSupplier() { // from class: androidx.media3.common.SimpleBasePlayer$PositionSupplier$$ExternalSyntheticLambda0
                @Override // androidx.media3.common.SimpleBasePlayer.PositionSupplier
                public final long get() {
                    return SimpleBasePlayer.PositionSupplier.lambda$getExtrapolating$1(currentPositionMs, startTimeMs, playbackSpeed);
                }
            };
        }

        static /* synthetic */ long lambda$getExtrapolating$1(long currentPositionMs, long startTimeMs, float playbackSpeed) {
    ...
    public static final class LivePositionSupplier implements PositionSupplier {
        ...
        @Override // androidx.media3.common.SimpleBasePlayer.PositionSupplier
        public long get() {
            return this.finalValue != C.TIME_UNSET ? this.finalValue : this.livePosition.get();
        }
    }
    ...
    protected SimpleBasePlayer(Looper applicationLooper, Clock clock) {
        ...
        ListenerSet<Player.Listener> listenerSet = new ListenerSet<>(applicationLooper, clock, new ListenerSet.IterationFinishedEvent() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda46
            @Override // androidx.media3.common.util.ListenerSet.IterationFinishedEvent
            public final void invoke(Object obj, FlagSet flagSet) {
                this.f$0.m7823lambda$new$0$androidxmedia3commonSimpleBasePlayer((Player.Listener) obj, flagSet);
            }
        });
    ...
    /* synthetic */ void m7823lambda$new$0$androidxmedia3commonSimpleBasePlayer(Player.Listener listener, FlagSet flags) {
    ...
    public final void setPlayWhenReady(final boolean playWhenReady) {
        ...
        updateStateForPendingOperation(handleSetPlayWhenReady(playWhenReady), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda1
    ...
    private void setMediaItemsInternal(final List<MediaItem> mediaItems, final int startIndex, final long startPositionMs) {
        Preconditions.checkArgument(startIndex == -1 || startIndex >= 0);
        final State state = this.state;
        if (!shouldHandleCommand(20) && (mediaItems.size() != 1 || !shouldHandleCommand(31))) {
            return;
        }
        updateStateForPendingOperation(handleSetMediaItems(mediaItems, startIndex, startPositionMs), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda37
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return this.f$0.m7827x396b5ff4(mediaItems, state, startIndex, startPositionMs);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$setMediaItemsInternal$2$androidx-media3-common-SimpleBasePlayer, reason: not valid java name */
    /* synthetic */ State m7827x396b5ff4(List mediaItems, State state, int startIndex, long startPositionMs) {
        ...
        return getStateWithNewPlaylistAndPosition(state, placeholderPlaylist, startIndex, startPositionMs, this.window);
    }
    ...
    public final void addMediaItems(int index, final List<MediaItem> mediaItems) {
        ...
        updateStateForPendingOperation(handleAddMediaItems(correctedIndex, mediaItems), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda42
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return this.f$0.m7821lambda$addMediaItems$3$androidxmedia3commonSimpleBasePlayer(state, mediaItems, correctedIndex);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$addMediaItems$3$androidx-media3-common-SimpleBasePlayer, reason: not valid java name */
    /* synthetic */ State m7821lambda$addMediaItems$3$androidxmedia3commonSimpleBasePlayer(State state, List mediaItems, int correctedIndex) {
        List<MediaItemData> placeholderPlaylist = buildMutablePlaylistFromState(state, this.period, this.window);
        for (int i = 0; i < mediaItems.size(); i++) {
            placeholderPlaylist.add(i + correctedIndex, getPlaceholderMediaItemData((MediaItem) mediaItems.get(i)));
        }
        if (!state.timeline.isEmpty()) {
            return getStateWithNewPlaylist(state, placeholderPlaylist, this.period, this.window);
        }
        return getStateWithNewPlaylistAndPosition(state, placeholderPlaylist, state.currentMediaItemIndex, state.contentPositionMsSupplier.get(), this.window);
    }

    @Override // androidx.media3.common.Player
    public final void moveMediaItems(final int fromIndex, int toIndex, int newIndex) {
        verifyApplicationThreadAndInitState();
        Preconditions.checkArgument(fromIndex >= 0 && toIndex >= fromIndex && newIndex >= 0);
        final State state = this.state;
        int playlistSize = state.timeline.getWindowCount();
        ...
        final int correctedNewIndex = Math.min(newIndex, playlistSize - (correctedToIndex - fromIndex));
        if (fromIndex != correctedToIndex && correctedNewIndex != fromIndex) {
            updateStateForPendingOperation(handleMoveMediaItems(fromIndex, correctedToIndex, correctedNewIndex), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda55
                @Override // com.google.common.base.Supplier
                public final Object get() {
                    return this.f$0.m7822lambda$moveMediaItems$4$androidxmedia3commonSimpleBasePlayer(state, fromIndex, correctedToIndex, correctedNewIndex);
                }
            });
        }
    }
    ...
    /* synthetic */ State m7822lambda$moveMediaItems$4$androidxmedia3commonSimpleBasePlayer(State state, int fromIndex, int correctedToIndex, int correctedNewIndex) {
        List<MediaItemData> placeholderPlaylist = buildMutablePlaylistFromState(state, this.period, this.window);
    ...
    public final void replaceMediaItems(final int fromIndex, int toIndex, final List<MediaItem> mediaItems) {
        ...
        if (shouldHandleCommand(20) && fromIndex <= playlistSize) {
            final int correctedToIndex = Math.min(toIndex, playlistSize);
            updateStateForPendingOperation(handleReplaceMediaItems(fromIndex, correctedToIndex, mediaItems), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda35
                @Override // com.google.common.base.Supplier
                public final Object get() {
                    return this.f$0.m7825x7bc5132c(state, mediaItems, correctedToIndex, fromIndex);
                }
            });
        }
    }
    ...
    /* synthetic */ State m7825x7bc5132c(State state, List mediaItems, int correctedToIndex, int fromIndex) {
        State updatedState;
        List<MediaItemData> placeholderPlaylist = buildMutablePlaylistFromState(state, this.period, this.window);
        for (int i = 0; i < mediaItems.size(); i++) {
        ...
        if (!state.timeline.isEmpty()) {
            updatedState = getStateWithNewPlaylist(state, placeholderPlaylist, this.period, this.window);
        } else {
            updatedState = getStateWithNewPlaylistAndPosition(state, placeholderPlaylist, state.currentMediaItemIndex, state.contentPositionMsSupplier.get(), this.window);
        }
        if (fromIndex < correctedToIndex) {
            Util.removeRange(placeholderPlaylist, fromIndex, correctedToIndex);
            return getStateWithNewPlaylist(updatedState, placeholderPlaylist, this.period, this.window);
        }
        return updatedState;
    }
    ...
    public final void removeMediaItems(final int fromIndex, int toIndex) {
        ...
        updateStateForPendingOperation(handleRemoveMediaItems(fromIndex, correctedToIndex), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda59
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return this.f$0.m7824x3b22ba57(state, fromIndex, correctedToIndex);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$removeMediaItems$6$androidx-media3-common-SimpleBasePlayer, reason: not valid java name */
    /* synthetic */ State m7824x3b22ba57(State state, int fromIndex, int correctedToIndex) {
        List<MediaItemData> placeholderPlaylist = buildMutablePlaylistFromState(state, this.period, this.window);
        Util.removeRange(placeholderPlaylist, fromIndex, correctedToIndex);
        return getStateWithNewPlaylist(state, placeholderPlaylist, this.period, this.window);
    }
    ...
    public final void prepare() {
        ...
        updateStateForPendingOperation(handlePrepare(), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda62
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setRepeatMode(final int repeatMode) {
        ...
        updateStateForPendingOperation(handleSetRepeatMode(repeatMode), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda48
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setShuffleModeEnabled(final boolean shuffleModeEnabled) {
        ...
        updateStateForPendingOperation(handleSetShuffleModeEnabled(shuffleModeEnabled), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda43
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    protected final void seekTo(final int mediaItemIndex, final long positionMs, int seekCommand, boolean isRepeatingCurrentItem) {
        verifyApplicationThreadAndInitState();
        Preconditions.checkArgument(mediaItemIndex == -1 || mediaItemIndex >= 0);
        final State state = this.state;
        if (!shouldHandleCommand(seekCommand)) {
            return;
        }
        final boolean ignoreSeekForPlaceholderState = mediaItemIndex == -1 || isPlayingAd() || (!state.timeline.isEmpty() && mediaItemIndex >= state.timeline.getWindowCount());
        updateStateForPendingOperation(handleSeek(mediaItemIndex, positionMs, seekCommand), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda58
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return this.f$0.m7826lambda$seekTo$10$androidxmedia3commonSimpleBasePlayer(ignoreSeekForPlaceholderState, state, mediaItemIndex, positionMs);
            }
    ...
    /* synthetic */ State m7826lambda$seekTo$10$androidxmedia3commonSimpleBasePlayer(boolean ignoreSeekForPlaceholderState, State state, int mediaItemIndex, long positionMs) {
        if (ignoreSeekForPlaceholderState) {
            return state;
        }
        return getStateWithNewPlaylistAndPosition(state, null, mediaItemIndex, positionMs, this.window);
    }
    ...
    public final void setPlaybackParameters(final PlaybackParameters playbackParameters) {
        ...
        updateStateForPendingOperation(handleSetPlaybackParameters(playbackParameters), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda64
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void stop() {
        ...
        updateStateForPendingOperation(handleStop(), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda61
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return this.f$0.m7828lambda$stop$12$androidxmedia3commonSimpleBasePlayer(state);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$stop$12$androidx-media3-common-SimpleBasePlayer, reason: not valid java name */
    /* synthetic */ State m7828lambda$stop$12$androidxmedia3commonSimpleBasePlayer(State state) {
    ...
    public final void release() {
        ...
        updateStateForPendingOperation(handleRelease(), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda51
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setTrackSelectionParameters(final TrackSelectionParameters parameters) {
        ...
        updateStateForPendingOperation(handleSetTrackSelectionParameters(parameters), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda66
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setPlaylistMetadata(final MediaMetadata mediaMetadata) {
        ...
        updateStateForPendingOperation(handleSetPlaylistMetadata(mediaMetadata), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda47
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setVolume(final float volume) {
        ...
        updateStateForPendingOperation(handleSetVolume(volume, 0), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda49
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void mute() {
        ...
        if (shouldHandleCommand(24) && this.state.volume != 0.0f) {
            updateStateForPendingOperation(handleSetVolume(0.0f, 1), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda36
                @Override // com.google.common.base.Supplier
                public final Object get() {
    ...
    public final void setVideoSurface(Surface surface) {
        ...
        if (surface == null) {
            ...
            updateStateForPendingOperation(handleSetVideoOutput(surface), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda38
                @Override // com.google.common.base.Supplier
                public final Object get() {
    ...
    public final void setVideoSurfaceHolder(final SurfaceHolder surfaceHolder) {
        ...
        if (surfaceHolder == null) {
            ...
            updateStateForPendingOperation(handleSetVideoOutput(surfaceHolder), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda50
                @Override // com.google.common.base.Supplier
                public final Object get() {
    ...
    public final void setVideoSurfaceView(final SurfaceView surfaceView) {
        ...
        if (surfaceView == null) {
            ...
            updateStateForPendingOperation(handleSetVideoOutput(surfaceView), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda52
                @Override // com.google.common.base.Supplier
                public final Object get() {
    ...
    public final void setVideoTextureView(TextureView textureView) {
        ...
        updateStateForPendingOperation(handleSetVideoOutput(textureView), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda3
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    private void clearVideoOutput(Object videoOutput) {
        ...
        updateStateForPendingOperation(handleClearVideoOutput(videoOutput), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda54
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setDeviceVolume(final int volume) {
        ...
        updateStateForPendingOperation(handleSetDeviceVolume(volume, 1), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda2
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setDeviceVolume(final int volume, int flags) {
        ...
        updateStateForPendingOperation(handleSetDeviceVolume(volume, flags), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda44
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return state.buildUpon().setDeviceVolume(volume).build();
            }
    ...
    public final void increaseDeviceVolume() {
        ...
        updateStateForPendingOperation(handleIncreaseDeviceVolume(1), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda56
            @Override // com.google.common.base.Supplier
            public final Object get() {
                SimpleBasePlayer.State state2 = state;
    ...
    public final void increaseDeviceVolume(int flags) {
        ...
        updateStateForPendingOperation(handleIncreaseDeviceVolume(flags), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda41
            @Override // com.google.common.base.Supplier
            public final Object get() {
                SimpleBasePlayer.State state2 = state;
                return state2.buildUpon().setDeviceVolume(state2.deviceVolume + 1).build();
    ...
    public final void decreaseDeviceVolume() {
        ...
        updateStateForPendingOperation(handleDecreaseDeviceVolume(1), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda45
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void decreaseDeviceVolume(int flags) {
        ...
        updateStateForPendingOperation(handleDecreaseDeviceVolume(flags), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda63
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return state.buildUpon().setDeviceVolume(Math.max(0, r0.deviceVolume - 1)).build();
            }
    ...
    public final void setDeviceMuted(final boolean muted) {
        ...
        updateStateForPendingOperation(handleSetDeviceMuted(muted, 1), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda53
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    public final void setDeviceMuted(final boolean muted, int flags) {
        ...
        updateStateForPendingOperation(handleSetDeviceMuted(muted, flags), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda65
            @Override // com.google.common.base.Supplier
            public final Object get() {
                return state.buildUpon().setIsDeviceMuted(muted).build();
            }
    ...
    public final void setAudioAttributes(final AudioAttributes audioAttributes, boolean handleAudioFocus) {
        ...
        updateStateForPendingOperation(handleSetAudioAttributes(audioAttributes, handleAudioFocus), new Supplier() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda57
            @Override // com.google.common.base.Supplier
            public final Object get() {
    ...
    protected ListenableFuture<?> handleReplaceMediaItems(int fromIndex, int toIndex, List<MediaItem> mediaItems) {
        ...
        return Util.transformFutureAsync(addFuture, new AsyncFunction() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda60
            @Override // com.google.common.util.concurrent.AsyncFunction
            public final ListenableFuture apply(Object obj) {
                return SimpleBasePlayer.lambda$handleReplaceMediaItems$33(removeFuture, obj);
            }
        });
    }

    static /* synthetic */ ListenableFuture lambda$handleReplaceMediaItems$33(ListenableFuture removeFuture, Object unused) throws Exception {
        return removeFuture;
    }
    ...
    private void updateStateAndInformListeners(final State newState, boolean forceSeekDiscontinuity, boolean isRepeatingCurrentItem) {
        ...
        if (timelineChanged) {
            final int timelineChangeReason = getTimelineChangeReason(previousState.timeline, newState.timeline, this.window);
        ...
        if (positionDiscontinuityReason != -1) {
            final Player.PositionInfo previousPositionInfo = getPositionInfo(previousState, false, this.window, this.period);
            ...
            this.listeners.queueEvent(11, new ListenerSet.Event() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda15
                @Override // androidx.media3.common.util.ListenerSet.Event
                public final void invoke(Object obj) {
                    SimpleBasePlayer.lambda$updateStateAndInformListeners$35(positionDiscontinuityReason, previousPositionInfo, positionInfo, (Player.Listener) obj);
                }
            });
        }
        if (mediaItemTransitionReason != -1) {
            if (!newState.timeline.isEmpty()) {
                mediaItem = newState.timeline.getWindow(getCurrentMediaItemIndexInternal(newState), this.window).mediaItem;
            } else {
                mediaItem = null;
            }
        ...
        if (!previousState.currentTracks.equals(newState.currentTracks)) {
            this.listeners.queueEvent(2, new ListenerSet.Event() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda31
                @Override // androidx.media3.common.util.ListenerSet.Event
                public final void invoke(Object obj) {
                    ((Player.Listener) obj).onTracksChanged(newState.currentTracks);
                }
            });
        }
        if (!previousState.currentMetadata.equals(newState.currentMetadata)) {
            this.listeners.queueEvent(14, new ListenerSet.Event() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda32
                @Override // androidx.media3.common.util.ListenerSet.Event
                public final void invoke(Object obj) {
                    ((Player.Listener) obj).onMediaMetadataChanged(newState.currentMetadata);
                }
            });
        }
        if (previousState.isLoading != newState.isLoading) {
            this.listeners.queueEvent(3, new ListenerSet.Event() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda33
                @Override // androidx.media3.common.util.ListenerSet.Event
                public final void invoke(Object obj) {
                    SimpleBasePlayer.lambda$updateStateAndInformListeners$42(newState, (Player.Listener) obj);
                }
            });
        }
        ...
        if (newState.newlyRenderedFirstFrame) {
            this.listeners.queueEvent(26, new ListenerSet.Event() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda20
                @Override // androidx.media3.common.util.ListenerSet.Event
                public final void invoke(Object obj) {
        ...
        if (!previousState.currentCues.equals(newState.currentCues)) {
            this.listeners.queueEvent(27, new ListenerSet.Event() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda24
                @Override // androidx.media3.common.util.ListenerSet.Event
                public final void invoke(Object obj) {
                    SimpleBasePlayer.lambda$updateStateAndInformListeners$61(newState, (Player.Listener) obj);
                }
            });
        }
    ...
    static /* synthetic */ void lambda$updateStateAndInformListeners$35(int positionDiscontinuityReason, Player.PositionInfo previousPositionInfo, Player.PositionInfo positionInfo, Player.Listener listener) {
        listener.onPositionDiscontinuity(positionDiscontinuityReason);
        listener.onPositionDiscontinuity(previousPositionInfo, positionInfo, positionDiscontinuityReason);
    }

    static /* synthetic */ void lambda$updateStateAndInformListeners$42(State newState, Player.Listener listener) {
        listener.onLoadingChanged(newState.isLoading);
        listener.onIsLoadingChanged(newState.isLoading);
    }

    static /* synthetic */ void lambda$updateStateAndInformListeners$61(State newState, Player.Listener listener) {
    ...
    private void updateStateForPendingOperation(final ListenableFuture<?> pendingOperation, Supplier<State> placeholderStateSupplier, boolean forceSeekDiscontinuity, boolean isRepeatingCurrentItem) {
        ...
        pendingOperation.addListener(new Runnable() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda39
            @Override // java.lang.Runnable
            public final void run() {
                this.f$0.m7829xb533777c(pendingOperation);
            }
        }, new Executor() { // from class: androidx.media3.common.SimpleBasePlayer$$ExternalSyntheticLambda40
    ...
    /* synthetic */ void m7829xb533777c(ListenableFuture pendingOperation) {
    ...
    private static long getPositionOrDefaultInMediaItem(long positionMs, State state, Timeline.Window window) {
    ...
    private static int getCurrentPeriodIndexInternal(State state, Timeline.Window window, Timeline.Period period) {
        ...
        return getPeriodIndexFromWindowPosition(state.timeline, currentMediaItemIndex, getContentPositionMsInternal(state, window), window, period);
    }
    ...
    private static int getTimelineChangeReason(Timeline previousTimeline, Timeline newTimeline, Timeline.Window window) {
        if (previousTimeline.getWindowCount() != newTimeline.getWindowCount()) {
            return 0;
        }
        int i = 0;
        while (true) {
            if (i >= previousTimeline.getWindowCount()) {
                return 1;
            }
            Object previousUid = previousTimeline.getWindow(i, window).uid;
            Object newUid = newTimeline.getWindow(i, window).uid;
    ...
    private static int getPositionDiscontinuityReason(State previousState, State newState, boolean forceSeekDiscontinuity, Timeline.Window window, Timeline.Period period) {
        ...
        if (!newPeriodUid.equals(previousPeriodUid) || previousState.currentAdGroupIndex != newState.currentAdGroupIndex || previousState.currentAdIndexInAdGroup != newState.currentAdIndexInAdGroup) {
            ...
            long previousPositionMs = getCurrentPeriodOrAdPositionMs(previousState, previousPeriodUid, period, window);
        ...
        long previousPositionMs2 = getCurrentPeriodOrAdPositionMs(previousState, previousPeriodUid, period, window);
    ...
    private static long getCurrentPeriodOrAdPositionMs(State state, Object currentPeriodUid, Timeline.Period period, Timeline.Window window) {
        if (state.currentAdGroupIndex != -1) {
            return state.adPositionMsSupplier.get();
        }
        return getContentPositionMsInternal(state, window) - state.timeline.getPeriodByUid(currentPeriodUid, period).getPositionInWindowMs();
    }
    ...
    private static Player.PositionInfo getPositionInfo(State state, boolean useDiscontinuityPosition, Timeline.Window window, Timeline.Period period) {
        ...
        if (state.timeline.isEmpty()) {
            ...
            Object periodUid2 = state.timeline.getPeriod(periodIndex, period, true).uid;
        ...
        if (useDiscontinuityPosition) {
            ...
            long contentPositionMs3 = getContentPositionMsInternal(state, window);
            if (state.currentAdGroupIndex != -1) {
    ...
    private static int getMediaItemTransitionReason(State previousState, State newState, int positionDiscontinuityReason, boolean isRepeatingCurrentItem, Timeline.Window window) {
        ...
        if (positionDiscontinuityReason != 0 || getContentPositionMsInternal(previousState, window) <= getContentPositionMsInternal(newState, window)) {
    ...
    private static int getMediaItemIndexInNewPlaylist(Timeline oldTimeline, Timeline newTimeline, int oldMediaItemIndex, Timeline.Period period, Timeline.Window window) {
        if (oldTimeline.isEmpty()) {
            if (oldMediaItemIndex < newTimeline.getWindowCount()) {
                return oldMediaItemIndex;
            }
            return -1;
        }
        int oldFirstPeriodIndex = oldTimeline.getWindow(oldMediaItemIndex, window).firstPeriodIndex;
        Object oldFirstPeriodUid = Preconditions.checkNotNull(oldTimeline.getPeriod(oldFirstPeriodIndex, period, true).uid);
        if (newTimeline.getIndexOfPeriod(oldFirstPeriodUid) == -1) {
            return -1;
        }
        return newTimeline.getPeriodByUid(oldFirstPeriodUid, period).windowIndex;
    }

    private static State getStateWithNewPlaylist(State oldState, List<MediaItemData> newPlaylist, Timeline.Period period, Timeline.Window window) {
        ...
        int newIndex = getMediaItemIndexInNewPlaylist(oldTimeline, newTimeline, oldIndex, period, window);
        long newPositionMs = newIndex == -1 ? C.TIME_UNSET : oldPositionMs;
        for (int i = oldIndex + 1; newIndex == -1 && i < oldTimeline.getWindowCount(); i++) {
            newIndex = getMediaItemIndexInNewPlaylist(oldTimeline, newTimeline, i, period, window);
        }
        int i2 = oldState.playbackState;
        if (i2 != 1 && newIndex == -1) {
            stateBuilder.setPlaybackState(4).setIsLoading(false);
        }
        return buildStateForNewPosition(stateBuilder, oldState, oldPositionMs, newTimeline, newIndex, newPositionMs, true, window);
    }

    private static State getStateWithNewPlaylistAndPosition(State oldState, List<MediaItemData> newPlaylist, int newIndex, long newPositionMs, Timeline.Window window) {
        State.Builder stateBuilder = oldState.buildUpon();
        Timeline newTimeline = newPlaylist == null ? oldState.timeline : new PlaylistTimeline(newPlaylist);
        if (oldState.playbackState != 1) {
            if (newTimeline.isEmpty() || (newIndex != -1 && newIndex >= newTimeline.getWindowCount())) {
                stateBuilder.setPlaybackState(4).setIsLoading(false);
            } else {
        ...
        long oldPositionMs = oldState.contentPositionMsSupplier.get();
        return buildStateForNewPosition(stateBuilder, oldState, oldPositionMs, newTimeline, newIndex, newPositionMs, false, window);
    }

    /* JADX WARN: Multi-variable type inference failed */
    private static State buildStateForNewPosition(State.Builder stateBuilder, State oldState, long oldPositionMs, Timeline newTimeline, int newIndex, long newPositionMs, boolean keepAds, Timeline.Window window) {
        long newPositionMs2;
        int newIndex2 = newIndex;
        long oldPositionMs2 = getPositionOrDefaultInMediaItem(oldPositionMs, oldState, window);
        if (!newTimeline.isEmpty() && (newIndex2 == -1 || newIndex2 >= newTimeline.getWindowCount())) {
            newIndex2 = 0;
            newPositionMs2 = C.TIME_UNSET;
        } else {
            newPositionMs2 = newPositionMs;
        }
        if (!newTimeline.isEmpty() && newPositionMs2 == C.TIME_UNSET) {
            newPositionMs2 = newTimeline.getWindow(newIndex2, window).getDefaultPositionMs();
        }
        boolean keepTracksAndMetadata = false;
        boolean oldOrNewPlaylistEmpty = oldState.timeline.isEmpty() || newTimeline.isEmpty();
        boolean mediaItemChanged = (oldOrNewPlaylistEmpty || oldState.timeline.getWindow(getCurrentMediaItemIndexInternal(oldState), window).uid.equals(newTimeline.getWindow(newIndex2, window).uid)) ? false : true;
        if (newTimeline.isEmpty()) {
            stateBuilder.setPlaylist(newTimeline, Tracks.EMPTY, null);
        } else if (newTimeline instanceof PlaylistTimeline) {
            MediaItemData mediaItemData = (MediaItemData) ((PlaylistTimeline) newTimeline).playlist.get(newIndex2);
            stateBuilder.setPlaylist(newTimeline, mediaItemData.tracks, mediaItemData.mediaMetadata);
        } else {
            if (!oldOrNewPlaylistEmpty && !mediaItemChanged) {
                keepTracksAndMetadata = true;
            }
            stateBuilder.setPlaylist(newTimeline, keepTracksAndMetadata ? oldState.currentTracks : Tracks.EMPTY, keepTracksAndMetadata ? oldState.currentMetadata : null);
        }
        if (oldOrNewPlaylistEmpty || mediaItemChanged || newPositionMs2 < oldPositionMs2) {
            long newPositionMs3 = newPositionMs2;
            stateBuilder.setCurrentMediaItemIndex(newIndex2).setCurrentAd(-1, -1).setContentPositionMs(newPositionMs3).setContentBufferedPositionMs(PositionSupplier.getConstant(newPositionMs3)).setTotalBufferedDurationMs(PositionSupplier.ZERO);
            return stateBuilder.build();
        }
        if (newPositionMs2 == oldPositionMs2) {
            ...
            if (oldState.currentAdGroupIndex != -1 && keepAds) {
                ...
                stateBuilder.setCurrentAd(-1, -1).setTotalBufferedDurationMs(PositionSupplier.getConstant(getContentBufferedPositionMsInternal(oldState, window) - oldPositionMs2));
            }
        } else {
            long contentBufferedDurationMs = Math.max(getContentBufferedPositionMsInternal(oldState, window), newPositionMs2);
            long totalBufferedDurationMs = Math.max(0L, oldState.totalBufferedDurationMsSupplier.get() - (newPositionMs2 - oldPositionMs2));
            stateBuilder.setCurrentMediaItemIndex(newIndex2).setCurrentAd(-1, -1).setContentPositionMs(newPositionMs2).setContentBufferedPositionMs(PositionSupplier.getConstant(contentBufferedDurationMs)).setTotalBufferedDurationMs(PositionSupplier.getConstant(totalBufferedDurationMs));
        }
        return stateBuilder.build();
    }
    ...
    private static List<MediaItemData> buildMutablePlaylistFromState(State state, Timeline.Period period, Timeline.Window window) {
        ...
        for (int i = 0; i < state.timeline.getWindowCount(); i++) {
            items.add(MediaItemData.buildFromState(state, i, period, window));
        }
        return items;
    }
