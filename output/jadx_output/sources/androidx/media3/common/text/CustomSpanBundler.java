...
final class CustomSpanBundler {
    ...
    public static ArrayList<Bundle> bundleCustomSpans(Spanned text) {
        ...
        for (VoiceSpan span3 : (VoiceSpan[]) text.getSpans(0, text.length(), VoiceSpan.class)) {
            Bundle bundle4 = spanToBundle(text, span3, 4, span3.toBundle());
            bundledCustomSpans.add(bundle4);
        }
    ...
    public static void unbundleAndApplyCustomSpan(Bundle customSpanBundle, Spannable text) {
        ...
        switch (customSpanType) {
            case 1:
                text.setSpan(RubySpan.fromBundle((Bundle) Preconditions.checkNotNull(span)), start, end, flags);
                break;
            case 2:
                text.setSpan(TextEmphasisSpan.fromBundle((Bundle) Preconditions.checkNotNull(span)), start, end, flags);
                break;
            case 3:
                text.setSpan(new HorizontalTextInVerticalContextSpan(), start, end, flags);
                break;
            case 4:
                text.setSpan(VoiceSpan.fromBundle((Bundle) Preconditions.checkNotNull(span)), start, end, flags);
                break;
        }
    }
