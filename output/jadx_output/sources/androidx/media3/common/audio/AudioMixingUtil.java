...
public final class AudioMixingUtil {
        ...
        for (int i = 0; i < framesToMix; i++) {
            ...
            for (int outputChannel2 = 0; outputChannel2 < outputChannels; outputChannel2++) {
                ...
                if (int16Output) {
                    ...
                    if (clipFloatOutput) {
                        fConstrainValue = Util.constrainValue(outputFrame[outputChannel2], -1.0f, 1.0f);
                    } else {
    ...
    private static float getPcmSample(ByteBuffer buffer, boolean int16Buffer, boolean int16Output) {
        return int16Output ? int16Buffer ? buffer.getShort() : floatSampleToInt16Pcm(buffer.getFloat()) : int16Buffer ? int16SampleToFloatPcm(buffer.getShort()) : buffer.getFloat();
    }
