...
final class MediaPeriodHolder {
    ...
    public MediaPeriodHolder(RendererCapabilities[] rendererCapabilities, long rendererPositionOffsetUs, TrackSelector trackSelector, Allocator allocator, MediaSourceList mediaSourceList, MediaPeriodInfo info, TrackSelectorResult emptyTrackSelectorResult, long targetPreloadBufferDurationUs) {
        ...
        this.mediaPeriod = createMediaPeriod(info.id, mediaSourceList, allocator, info.startPositionUs, info.endPositionUs, info.isPrecededByTransitionFromSameStream);
    }
    ...
    public void handlePrepared(float playbackSpeed, Timeline timeline, boolean playWhenReady) throws ExoPlaybackException {
        this.prepared = true;
        ...
        this.info = this.info.copyWithStartPositionUs(newStartPositionUs, this.info.liveStreamStartPositionProjectionUs);
    }
    ...
    public TrackSelectorResult selectTracks(float f, Timeline timeline, boolean z) throws ExoPlaybackException {
        ...
        while (true) {
            ...
            if (trackSelectorResultSelectTracks.isRendererEnabled(i2)) {
                Preconditions.checkState(trackSelectorResultSelectTracks.selections[i2] != null || this.rendererCapabilities[i2].getTrackType() == -2);
            } else {
                Preconditions.checkState(trackSelectorResultSelectTracks.selections[i2] == null);
            }
            i2++;
        }
    ...
    public long applyTrackSelection(TrackSelectorResult newTrackSelectorResult, long positionUs, boolean forceRecreateStreams, boolean[] streamResetFlags) {
        ...
        for (int i2 = 0; i2 < this.sampleStreams.length; i2++) {
            if (this.sampleStreams[i2] != null) {
                Preconditions.checkState(newTrackSelectorResult.isRendererEnabled(i2));
                if (this.rendererCapabilities[i2].getTrackType() != -2) {
                    this.hasEnabledTracks = true;
                }
            } else {
                Preconditions.checkState(newTrackSelectorResult.selections[i2] == null);
            }
        }
        return positionUs2;
    }
    ...
    private static MediaPeriod createMediaPeriod(MediaSource.MediaPeriodId id, MediaSourceList mediaSourceList, Allocator allocator, long startPositionUs, long endPositionUs, boolean isPrecededByTransitionFromSameStream) {
        MediaPeriod mediaPeriod = mediaSourceList.createPeriod(id, allocator, startPositionUs);
        if (endPositionUs != C.TIME_UNSET) {
            return new ClippingMediaPeriod(mediaPeriod, !isPrecededByTransitionFromSameStream, 0L, endPositionUs);
        }
        return mediaPeriod;
    }
    ...
    public void prepare(MediaPeriod.Callback callback, long startPositionUs) {
        ...
        this.mediaPeriod.prepare(callback, startPositionUs);
    }
}
