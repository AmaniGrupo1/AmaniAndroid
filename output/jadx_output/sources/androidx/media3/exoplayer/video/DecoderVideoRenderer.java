...
public abstract class DecoderVideoRenderer extends BaseRenderer {
    ...
    protected void onInputFormatChanged(FormatHolder formatHolder) throws ExoPlaybackException {
        ...
        if (this.sourceDrmSession != this.decoderDrmSession) {
            evaluation = new DecoderReuseEvaluation(this.decoder.getName(), (Format) Preconditions.checkNotNull(oldFormat), newFormat, 0, 128);
        } else {
            evaluation = canReuseDecoder(this.decoder.getName(), (Format) Preconditions.checkNotNull(oldFormat), newFormat);
        }
        ...
        this.eventDispatcher.inputFormatChanged((Format) Preconditions.checkNotNull(this.inputFormat), evaluation);
    }
    ...
    protected void renderOutputBuffer(VideoDecoderOutputBuffer outputBuffer, long presentationTimeUs, Format outputFormat) throws DecoderException {
        ...
        if (renderYuv) {
            ((VideoDecoderOutputBufferRenderer) Preconditions.checkNotNull(this.outputBufferRenderer)).setOutputBuffer(outputBuffer);
        } else {
            renderOutputBufferToSurface(outputBuffer, (Surface) Preconditions.checkNotNull(this.outputSurface));
        }
        this.consecutiveDroppedFrameCount = 0;
    ...
    private void maybeInitDecoder() throws ExoPlaybackException {
        ...
        try {
            ...
            this.decoder = createDecoder((Format) Preconditions.checkNotNull(this.inputFormat), cryptoConfig);
            this.decoder.setOutputStartTimeUs(getLastResetPositionUs());
            setDecoderOutputMode(this.outputMode);
            long decoderInitializedTimestamp = SystemClock.elapsedRealtime();
            this.eventDispatcher.decoderInitialized(((Decoder) Preconditions.checkNotNull(this.decoder)).getName(), decoderInitializedTimestamp, decoderInitializedTimestamp - decoderInitializingTimestamp);
    ...
    private boolean feedInputBuffer() throws ExoPlaybackException, DecoderException {
        ...
        DecoderInputBuffer inputBuffer = (DecoderInputBuffer) Preconditions.checkNotNull(this.inputBuffer);
        if (this.decoderReinitializationState == 1) {
            inputBuffer.setFlags(4);
            ((Decoder) Preconditions.checkNotNull(this.decoder)).queueInputBuffer(inputBuffer);
            this.inputBuffer = null;
        ...
        switch (readSource(formatHolder, inputBuffer, 0)) {
            ...
            case -4:
                ...
                if (this.waitingForFirstSampleInFormat) {
                    this.formatQueue.add(inputBuffer.timeUs, (Format) Preconditions.checkNotNull(this.inputFormat));
    ...
    private boolean drainOutputBuffer(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException, DecoderException {
        if (this.outputBuffer == null) {
            this.outputBuffer = (VideoDecoderOutputBuffer) ((Decoder) Preconditions.checkNotNull(this.decoder)).dequeueOutputBuffer();
            if (this.outputBuffer == null) {
                return false;
            }
    ...
    private boolean processOutputBuffer(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException, DecoderException {
        ...
        VideoDecoderOutputBuffer outputBuffer = (VideoDecoderOutputBuffer) Preconditions.checkNotNull(this.outputBuffer);
        ...
        if (shouldForceRender(earlyUs)) {
            renderOutputBuffer(outputBuffer, presentationTimeUs, (Format) Preconditions.checkNotNull(this.outputFormat));
            return true;
        }
