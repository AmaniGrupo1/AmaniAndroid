...
public class SampleQueue implements TrackOutput {
    ...
    public static SampleQueue createWithDrm(Allocator allocator, DrmSessionManager drmSessionManager, DrmSessionEventListener.EventDispatcher drmEventDispatcher) {
        return new SampleQueue(allocator, (DrmSessionManager) Preconditions.checkNotNull(drmSessionManager), (DrmSessionEventListener.EventDispatcher) Preconditions.checkNotNull(drmEventDispatcher));
    }
    ...
    public final synchronized void setReadEndTimeUs(long readEndTimeUs) throws Throwable {
        Throwable th;
        ...
        try {
            try {
                ...
                int i = -1;
                if (readEndTimeUs == Long.MIN_VALUE) {
                    try {
                        this.readEndTimeAbsoluteIndex = -1;
                        return;
                    } catch (Throwable th2) {
                    ...
                    if (readEndTimeUs > this.largestQueuedTimestampUs) {
                        sampleQueue = this;
                        readEndTimeUs2 = readEndTimeUs;
                    } else {
                        sampleQueue = this;
                        readEndTimeUs2 = readEndTimeUs;
                        readEndTimeRelativeIndex = sampleQueue.findSampleAfter(this.relativeFirstIndex, this.length, readEndTimeUs2, false);
                    }
                    if (readEndTimeRelativeIndex != -1) {
                        i = sampleQueue.absoluteFirstIndex + readEndTimeRelativeIndex;
                    }
                    sampleQueue.readEndTimeAbsoluteIndex = i;
                    sampleQueue.readEndTimeUs = readEndTimeUs2;
                    return;
                }
            } catch (Throwable th3) {
                th = th3;
    ...
    public final void discardUpstreamFrom(long timeUs) {
        ...
        Preconditions.checkArgument(timeUs > getLargestReadTimestampUs());
    ...
    public synchronized boolean isReady(boolean loadingFinished) {
        ...
        boolean z = true;
        if (this.readEndTimeAbsoluteIndex != -1 && readIndex >= this.readEndTimeAbsoluteIndex) {
            return true;
        }
        if (!hasNextSample()) {
        ...
        if (this.sharedSampleMetadata.get(readIndex).format != this.downstreamFormat) {
            return true;
        }
    ...
    public final synchronized boolean seekTo(int sampleIndex) {
        ...
        if (sampleIndex >= this.absoluteFirstIndex && sampleIndex <= this.absoluteFirstIndex + this.length) {
            if (this.readEndTimeAbsoluteIndex != -1 && sampleIndex >= this.readEndTimeAbsoluteIndex) {
                return false;
            }
            this.startTimeUs = Long.MIN_VALUE;
    ...
    /* JADX WARN: Removed duplicated region for block: B:37:0x0067 A[DONT_GENERATE] */
    /* JADX WARN: Removed duplicated region for block: B:39:0x0069 A[Catch: all -> 0x007f, TRY_ENTER, TRY_LEAVE, TryCatch #4 {all -> 0x007f, blocks: (B:39:0x0069, B:51:0x007d, B:34:0x005f), top: B:63:0x0001 }] */
    /* JADX WARN: Removed duplicated region for block: B:45:0x0076  */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public final synchronized boolean seekTo(long timeUs, boolean allowTimeBeyondBuffer) throws Throwable {
        Throwable th;
        int relativeReadIndex;
        long j;
        long jMin;
        ...
        try {
            try {
                rewind();
                relativeReadIndex = getRelativeIndex(this.readPosition);
                j = this.readEndTimeUs;
                jMin = this.largestQueuedTimestampUs;
            } catch (Throwable th2) {
                th = th2;
            }
        } catch (Throwable th3) {
        ...
        if (j == Long.MIN_VALUE) {
            long maxPossibleSeekTimeUs = jMin;
            if (!hasNextSample()) {
            }
            return false;
        }
        try {
            jMin = Math.min(jMin, this.readEndTimeUs);
            long maxPossibleSeekTimeUs2 = jMin;
            if (!hasNextSample() && timeUs >= this.timesUs[relativeReadIndex]) {
                if (timeUs <= maxPossibleSeekTimeUs2 || allowTimeBeyondBuffer) {
                    boolean z = this.discardAllSamplesToStartTime;
                    int i = this.length;
                    if (z) {
                        try {
                            timeUs2 = timeUs;
                            try {
                                ...
                                sampleQueue.startTimeUs = timeUs2;
                                sampleQueue.readPosition += offset;
                                return true;
                            } catch (Throwable th4) {
                                th = th4;
                            }
                        ...
                        try {
                            ...
                            sampleQueue = this;
                            offset = sampleQueue.findSampleBefore(relativeReadIndex, i2, timeUs2, true);
                            if (offset != -1) {
                            }
                        } catch (Throwable th6) {
                            th = th6;
    ...
    */
    public final synchronized void skip(int count) {
        ...
        if (count >= 0) {
            try {
                z = this.readPosition + count <= this.length;
            } catch (Throwable th) {
                throw th;
            }
        }
    ...
    */
    ...
    private synchronized int peekSampleMetadata(FormatHolder formatHolder, DecoderInputBuffer buffer, boolean formatRequired, boolean loadingFinished, SampleExtrasHolder extrasHolder) {
        boolean readEndTimeReached = false;
        buffer.waitingForKeys = false;
        int readIndex = getReadIndex();
        if (this.readEndTimeAbsoluteIndex != -1 && readIndex >= this.readEndTimeAbsoluteIndex) {
            readEndTimeReached = true;
        }
        ...
        if (!loadingFinished && !this.isLastSampleQueued && !readEndTimeReached) {
            if (this.upstreamFormat == null || (!formatRequired && this.upstreamFormat == this.downstreamFormat)) {
                return -3;
            }
            onFormatResult((Format) Preconditions.checkNotNull(this.upstreamFormat), formatHolder);
            return -5;
        }
        buffer.setFlags(4);
    ...
    /* JADX WARN: Removed duplicated region for block: B:16:0x0026 A[Catch: all -> 0x0042, TRY_ENTER, TryCatch #1 {all -> 0x0042, blocks: (B:3:0x0001, B:5:0x0007, B:17:0x0028, B:16:0x0026), top: B:39:0x0001 }] */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    private synchronized long discardSampleMetadataTo(long timeUs, boolean toKeyframe, boolean stopAtReadPosition) throws Throwable {
        Throwable th;
        int i;
        try {
            try {
                ...
                if (stopAtReadPosition) {
                    try {
                        i = this.readPosition != this.length ? this.readPosition + 1 : this.length;
                    } catch (Throwable th2) {
                        th = th2;
                    }
                }
    ...
    private synchronized void commitSample(long timeUs, int sampleFlags, long offset, int size, TrackOutput.CryptoData cryptoData) {
        ...
        if (this.length > 0) {
            ...
            Preconditions.checkArgument(this.offsets[previousSampleRelativeIndex] + ((long) this.sizes[previousSampleRelativeIndex]) <= offset);
        }
        ...
        if (this.readEndTimeUs != Long.MIN_VALUE && this.readEndTimeAbsoluteIndex == -1 && timeUs >= this.readEndTimeUs) {
        ...
        if (this.sharedSampleMetadata.isEmpty() || !this.sharedSampleMetadata.getEndValue().format.equals(this.upstreamFormat)) {
            Format upstreamFormat = (Format) Preconditions.checkNotNull(this.upstreamFormat);
    ...
    private long discardUpstreamSampleMetadata(int discardFromIndex) {
        ...
        boolean z = false;
        Preconditions.checkArgument(discardCount >= 0 && discardCount <= this.length - this.readPosition);
        ...
        if (this.readEndTimeAbsoluteIndex != -1 && discardFromIndex < this.readEndTimeAbsoluteIndex) {
            this.readEndTimeAbsoluteIndex = -1;
        }
    ...
    private static boolean canDiscardAllSamplesToStartTime(String mimeType, String codec) {
        int trackType = MimeTypes.getTrackType(mimeType);
        return trackType == 1 && MimeTypes.allSamplesAreSyncSamples(mimeType, codec);
    }
