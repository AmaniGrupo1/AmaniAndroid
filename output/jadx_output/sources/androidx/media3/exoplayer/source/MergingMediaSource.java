...
public final class MergingMediaSource extends CompositeMediaSource<Integer> {
    ...
    public MergingMediaSource(boolean adjustPeriodTimeOffsets, boolean clipDurations, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, MediaSource... mediaSources) {
        ...
        int i2 = mediaSources.length;
        this.timelines = new Timeline[i2];
    ...
    public MediaPeriod createPeriod(MediaSource.MediaPeriodId id, Allocator allocator, long startPositionUs) {
        ...
        for (int i = 0; i < periods.length; i++) {
            MediaSource.MediaPeriodId mediaPeriodId = id.copyWithPeriodUid(this.timelines[i].getUidOfPeriod(periodIndex));
            periods[i] = this.mediaSources[i].createPeriod(mediaPeriodId, allocator, startPositionUs - this.periodTimeOffsetsUs[periodIndex][i]);
            this.mediaPeriods.get(i).add(new MediaPeriodAndId(mediaPeriodId, periods[i]));
        }
        ...
        MediaPeriod mediaPeriod2 = new ClippingMediaPeriod(mediaPeriod, false, 0L, ((Long) Preconditions.checkNotNull(this.clippedDurationsUs.get(id.periodUid))).longValue(), this.enableClippingInMediaPeriod);
    ...
    public void releasePeriod(MediaPeriod mediaPeriod) {
        if (this.clipDurations) {
            ...
            while (true) {
                ...
                Map.Entry<Object, ClippingMediaPeriod> entry = it.next();
        ...
        for (int i = 0; i < this.mediaSources.length; i++) {
            List<MediaPeriodAndId> mediaPeriodsForSource = this.mediaPeriods.get(i);
            MediaPeriod childPeriod = mergingPeriod.getChildPeriod(i);
            int j = 0;
            while (true) {
                if (j >= mediaPeriodsForSource.size()) {
                    break;
                }
    ...
    public MediaSource.MediaPeriodId getMediaPeriodIdForChildMediaPeriodId(Integer childSourceId, MediaSource.MediaPeriodId mediaPeriodId) {
        List<MediaPeriodAndId> childMediaPeriodIds = this.mediaPeriods.get(childSourceId.intValue());
    ...
    private static final class ClippedTimeline extends ForwardingTimeline {
        ...
        public ClippedTimeline(Timeline timeline, Map<Object, Long> clippedDurationsUs) {
            ...
            while (i2 < periodCount) {
                ...
                long clippedDurationUs = ((Long) Preconditions.checkNotNull(clippedDurationsUs.get(period.uid))).longValue();
