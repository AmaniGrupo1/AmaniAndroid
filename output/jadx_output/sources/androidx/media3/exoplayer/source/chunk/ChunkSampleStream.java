...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader, Loader.Callback<Chunk>, Loader.ReleaseCallback {
    ...
    public ChunkSampleStream(int primaryTrackType, int[] embeddedTrackTypes, Format[] embeddedTrackFormats, T chunkSource, SequenceableLoader.Callback<ChunkSampleStream<T>> callback, Allocator allocator, long positionUs, DrmSessionManager drmSessionManager, DrmSessionEventListener.EventDispatcher drmEventDispatcher, LoadErrorHandlingPolicy loadErrorHandlingPolicy, MediaSourceEventListener.EventDispatcher mediaSourceEventDispatcher, boolean handleInitialDiscontinuity, long firstChunkStartTimeUs, ReleasableExecutor downloadExecutor) {
        ...
        this.loader = downloadExecutor != null ? new Loader(downloadExecutor) : new Loader(TAG);
    ...
    public ChunkSampleStream<T>.EmbeddedSampleStream selectEmbeddedTrack(long positionUs, int trackType) throws Throwable {
        for (int i = 0; i < this.embeddedSampleQueues.length; i++) {
            if (this.embeddedTrackTypes[i] == trackType) {
                Preconditions.checkState(!this.embeddedTracksSelected[i]);
    ...
    public void seekToUs(long positionUs) throws Throwable {
        ...
        while (true) {
            ...
            BaseMediaChunk mediaChunk = this.mediaChunks.get(i2);
        ...
        if (seekToMediaChunk != null) {
            ...
            if (nextLoadPositionUs != Long.MIN_VALUE && positionUs >= nextLoadPositionUs) {
                allowTimeBeyondBuffer = false;
            } else {
                allowTimeBeyondBuffer = true;
            }
            seekInsideBuffer = this.primarySampleQueue.seekTo(positionUs, allowTimeBeyondBuffer);
        }
        if (seekInsideBuffer) {
        ...
        if (this.loader.isLoading()) {
            ...
            while (i < length2) {
                SampleQueue embeddedSampleQueue2 = sampleQueueArr2[i];
    ...
    public void setEndPositionUs(long endPositionUs) throws Throwable {
        this.primarySampleQueue.setReadEndTimeUs(endPositionUs);
    ...
    public void onLoadStarted(Chunk loadable, long elapsedRealtimeMs, long loadDurationMs, int retryCount) {
        LoadEventInfo loadEventInfo;
        if (retryCount == 0) {
            loadEventInfo = new LoadEventInfo(loadable.loadTaskId, loadable.dataSpec, elapsedRealtimeMs);
        } else {
            loadEventInfo = new LoadEventInfo(loadable.loadTaskId, loadable.dataSpec, loadable.getUri(), loadable.getResponseHeaders(), elapsedRealtimeMs, loadDurationMs, loadable.bytesLoaded());
        }
        LoadEventInfo loadEventInfo2 = loadEventInfo;
        this.mediaSourceEventDispatcher.loadStarted(loadEventInfo2, loadable.type, this.primaryTrackType, loadable.trackFormat, loadable.trackSelectionReason, loadable.trackSelectionData, loadable.startTimeUs, loadable.endTimeUs, retryCount);
    }
        ...
        if (this.chunkSource.onChunkLoadError(loadable, cancelable, loadErrorInfo, this.loadErrorHandlingPolicy)) {
            if (cancelable) {
                ...
                if (isMediaChunk) {
                    ...
                    Preconditions.checkState(removed == loadable);
                    if (this.mediaChunks.isEmpty()) {
                        this.pendingResetPositionUs = this.lastSeekPositionUs;
                    }
                }
    ...
    public boolean continueLoading(LoadingInfo loadingInfo) {
        ...
        this.loader.startLoading(loadable, this, this.loadErrorHandlingPolicy.getMinimumLoadableRetryCount(loadable.type));
        return z;
    }
    ...
    public void reevaluateBuffer(long positionUs) {
        ...
        if (this.loader.isLoading()) {
            Chunk loadingChunk = (Chunk) Preconditions.checkNotNull(this.loadingChunk);
    ...
    public void discardUpstreamSamplesForClippedDuration(long clippedDurationUs) {
        ...
        if (isPendingReset() || clippedDurationUs == C.TIME_UNSET || this.mediaChunks.isEmpty()) {
            return;
        }
        BaseMediaChunk lastMediaChunk = getLastMediaChunk();
        if (lastMediaChunk.clippedEndTimeUs != C.TIME_UNSET) {
            j = lastMediaChunk.clippedEndTimeUs;
        } else {
            j = lastMediaChunk.endTimeUs;
        }
        long lastMediaChunkEndTimeUs = j;
        ...
        long j2 = 1;
        long minDiscardPositionUs = Math.max(clippedDurationUs, this.primarySampleQueue.getLargestReadTimestampUs() + 1);
        this.primarySampleQueue.discardUpstreamFrom(minDiscardPositionUs);
        SampleQueue[] sampleQueueArr = this.embeddedSampleQueues;
        int length = sampleQueueArr.length;
        int i = 0;
        while (i < length) {
            SampleQueue embeddedSampleQueue = sampleQueueArr[i];
            long j3 = j2;
            long minDiscardPositionUs2 = Math.max(clippedDurationUs, embeddedSampleQueue.getLargestReadTimestampUs() + j3);
            embeddedSampleQueue.discardUpstreamFrom(minDiscardPositionUs2);
            i++;
            j2 = j3;
        }
        this.mediaSourceEventDispatcher.upstreamDiscarded(this.primaryTrackType, clippedDurationUs, largestQueuedTimestampUs);
    }
