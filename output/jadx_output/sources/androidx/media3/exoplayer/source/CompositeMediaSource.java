...
public abstract class CompositeMediaSource<T> extends BaseMediaSource {
    ...
    public abstract void m7952x28f9175(T t, MediaSource mediaSource, Timeline timeline);
    ...
    protected final void prepareChildSource(final T id, MediaSource mediaSource) {
        Preconditions.checkArgument(!this.childSources.containsKey(id));
        MediaSource.MediaSourceCaller caller = new MediaSource.MediaSourceCaller() { // from class: androidx.media3.exoplayer.source.CompositeMediaSource$$ExternalSyntheticLambda0
            ...
            public final void onSourceInfoRefreshed(MediaSource mediaSource2, Timeline timeline) {
                this.f$0.m7952x28f9175(id, mediaSource2, timeline);
            }
        };
        ...
        mediaSource.addEventListener((Handler) Preconditions.checkNotNull(this.eventHandler), eventListener);
        mediaSource.addDrmEventListener((Handler) Preconditions.checkNotNull(this.eventHandler), eventListener);
    ...
    protected final void enableChildSource(T id) {
        MediaSourceAndListener<T> enabledChild = (MediaSourceAndListener) Preconditions.checkNotNull(this.childSources.get(id));
        enabledChild.mediaSource.enable(enabledChild.caller);
    }

    protected final void disableChildSource(T id) {
        MediaSourceAndListener<T> disabledChild = (MediaSourceAndListener) Preconditions.checkNotNull(this.childSources.get(id));
        disabledChild.mediaSource.disable(disabledChild.caller);
    }

    protected final void releaseChildSource(T id) {
        MediaSourceAndListener<T> removedChild = (MediaSourceAndListener) Preconditions.checkNotNull(this.childSources.remove(id));
    ...
    private final class ForwardingEventListener implements MediaSourceEventListener, DrmSessionEventListener {
        ...
        public void onLoadStarted(int windowIndex, MediaSource.MediaPeriodId mediaPeriodId, LoadEventInfo loadEventData, MediaLoadData mediaLoadData, int retryCount) {
            if (maybeUpdateEventDispatcher(windowIndex, mediaPeriodId)) {
                this.mediaSourceEventDispatcher.loadStarted(loadEventData, maybeUpdateMediaLoadData(mediaLoadData, mediaPeriodId), retryCount);
            }
        }
        ...
        private boolean maybeUpdateEventDispatcher(int childWindowIndex, MediaSource.MediaPeriodId childMediaPeriodId) {
            ...
            if (this.mediaSourceEventDispatcher.windowIndex != windowIndex || !Objects.equals(this.mediaSourceEventDispatcher.mediaPeriodId, mediaPeriodId)) {
                this.mediaSourceEventDispatcher = CompositeMediaSource.this.createEventDispatcher(windowIndex, mediaPeriodId);
            }
            if (this.drmEventDispatcher.windowIndex != windowIndex || !Objects.equals(this.drmEventDispatcher.mediaPeriodId, mediaPeriodId)) {
