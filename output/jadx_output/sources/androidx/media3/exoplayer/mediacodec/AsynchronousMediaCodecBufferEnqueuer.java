...
class AsynchronousMediaCodecBufferEnqueuer implements MediaCodecBufferEnqueuer {
    ...
    public void doHandleMessage(Message msg) throws Throwable {
        ...
        switch (msg.what) {
            ...
            case 4:
                ...
                break;
            default:
                MutatorMutex$$ExternalSyntheticBackportWithForwarding0.m(this.pendingRuntimeException, null, new IllegalStateException(String.valueOf(msg.what)));
                break;
        }
        if (params != null) {
    ...
    private static void copy(CryptoInfo cryptoInfo, MediaCodec.CryptoInfo frameworkCryptoInfo) {
        ...
        frameworkCryptoInfo.key = (byte[]) Preconditions.checkNotNull(copy(cryptoInfo.key, frameworkCryptoInfo.key));
        frameworkCryptoInfo.iv = (byte[]) Preconditions.checkNotNull(copy(cryptoInfo.iv, frameworkCryptoInfo.iv));
