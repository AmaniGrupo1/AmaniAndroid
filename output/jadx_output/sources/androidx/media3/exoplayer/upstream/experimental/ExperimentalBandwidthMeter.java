...
public final class ExperimentalBandwidthMeter implements BandwidthMeter, TransferListener {
    ...
    public static final class Builder {
        ...
        private final Map<Integer, Long> initialBitrateEstimates = new HashMap(8);

        public Builder(Context context) {
            this.context = context.getApplicationContext();
            this.initialBitrateEstimates.put(0, 1000000L);
            Map<Integer, Long> map = this.initialBitrateEstimates;
            Long lValueOf = Long.valueOf(C.TIME_UNSET);
            map.put(2, lValueOf);
            this.initialBitrateEstimates.put(3, lValueOf);
            this.initialBitrateEstimates.put(4, lValueOf);
            this.initialBitrateEstimates.put(5, lValueOf);
            this.initialBitrateEstimates.put(10, lValueOf);
            this.initialBitrateEstimates.put(9, lValueOf);
            this.initialBitrateEstimates.put(7, lValueOf);
        }
        ...
        public Builder setInitialBitrateEstimate(String countryCode) {
            ...
            for (Integer networkType : this.initialBitrateEstimates.keySet()) {
                setInitialBitrateEstimate(networkType.intValue(), ExperimentalBandwidthMeter.getInitialBitrateEstimatesForCountry(countryCode2, networkType.intValue()));
            }
            return this;
        }
    ...
    private ExperimentalBandwidthMeter(Context context, Map<Integer, Long> initialBitrateEstimates, TimeToFirstByteEstimator timeToFirstByteEstimator, BandwidthEstimator bandwidthEstimator, boolean resetOnNetworkTypeChange) {
        ...
        this.initialBitrateEstimate = getInitialBitrateEstimateForNetworkType(this.networkType);
    ...
    public synchronized long getBitrateEstimate() {
        long j;
        long bandwidthEstimate = this.bandwidthEstimator.getBandwidthEstimate();
        if (bandwidthEstimate != Long.MIN_VALUE) {
            j = bandwidthEstimate;
        } else {
            j = this.initialBitrateEstimate;
        }
        return j;
    }
    ...
    @Override // androidx.media3.exoplayer.upstream.BandwidthMeter
    public synchronized void addEventListener(Handler eventHandler, BandwidthMeter.EventListener eventListener) {
        Preconditions.checkNotNull(eventHandler);
        Preconditions.checkNotNull(eventListener);
        this.bandwidthEstimator.addEventListener(eventHandler, eventListener);
    }
    ...
    public synchronized void onTransferInitializing(DataSource source, DataSpec dataSpec, boolean isNetwork) {
        if (isTransferAtFullNetworkSpeed(dataSpec, isNetwork)) {
            ...
            this.bandwidthEstimator.onTransferInitializing(source);
        }
    }
    ...
    public synchronized void onTransferStart(DataSource source, DataSpec dataSpec, boolean isNetwork) {
        if (isTransferAtFullNetworkSpeed(dataSpec, isNetwork)) {
            ...
            this.bandwidthEstimator.onTransferStart(source);
        }
    }
    ...
    public synchronized void onBytesTransferred(DataSource source, DataSpec dataSpec, boolean isNetwork, int bytesTransferred) {
        if (isTransferAtFullNetworkSpeed(dataSpec, isNetwork)) {
            this.bandwidthEstimator.onBytesTransferred(source, bytesTransferred);
        }
    }
    ...
    public synchronized void onTransferEnd(DataSource source, DataSpec dataSpec, boolean isNetwork) {
        if (isTransferAtFullNetworkSpeed(dataSpec, isNetwork)) {
            this.bandwidthEstimator.onTransferEnd(source);
        }
    }
        ...
        if (this.networkType == 0 || this.resetOnNetworkTypeChange) {
            ...
            if (this.networkType != networkType || this.countryCode == null) {
                ...
                if (networkType != 1 && networkType != 0 && networkType != 8) {
                    ...
                    this.bandwidthEstimator.onNetworkTypeChange(this.initialBitrateEstimate);
