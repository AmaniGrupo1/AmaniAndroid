...
public final class DefaultAllocator implements Allocator {
    ...
    public DefaultAllocator(boolean trimOnReset, int individualAllocationSize, int initialAllocationCount) {
        Preconditions.checkArgument(individualAllocationSize > 0);
        Preconditions.checkArgument(initialAllocationCount >= 0);
    ...
    public synchronized Allocation allocate() {
        ...
        if (this.availableCount > 0) {
            ...
            allocation = (Allocation) Preconditions.checkNotNull(allocationArr[i]);
    ...
    public synchronized void trim() {
        ...
        if (this.initialAllocationBlock != null) {
            ...
            while (lowIndex <= highIndex) {
                Allocation lowAllocation = (Allocation) Preconditions.checkNotNull(this.availableAllocations[lowIndex]);
                if (lowAllocation.data == this.initialAllocationBlock) {
                    lowIndex++;
                } else {
                    Allocation highAllocation = (Allocation) Preconditions.checkNotNull(this.availableAllocations[highIndex]);
