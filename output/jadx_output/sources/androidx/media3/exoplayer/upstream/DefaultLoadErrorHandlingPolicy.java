...
public class DefaultLoadErrorHandlingPolicy implements LoadErrorHandlingPolicy {
    ...
    public long getRetryDelayMsFor(LoadErrorHandlingPolicy.LoadErrorInfo loadErrorInfo) {
        if (isAnyCauseNonRetriable(loadErrorInfo.exception)) {
            return C.TIME_UNSET;
        }
    ...
    private boolean isAnyCauseNonRetriable(Throwable exception) {
    ...
    private boolean isNonRetriableException(Throwable exception) {
        return (exception instanceof ParserException) || (exception instanceof FileNotFoundException) || (exception instanceof HttpDataSource.CleartextNotPermittedException) || (exception instanceof Loader.UnexpectedLoaderException) || ((exception instanceof DataSourceException) && ((DataSourceException) exception).reason == 2008);
    }
}
