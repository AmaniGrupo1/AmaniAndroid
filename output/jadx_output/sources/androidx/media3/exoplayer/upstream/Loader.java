...
public final class Loader implements LoaderErrorThrower {
    ...
    public interface Callback<T extends Loadable> {
        ...
        default void onLoadStarted(T loadable, long elapsedRealtimeMs, long loadDurationMs, int retryCount) {
        }
    }
    ...
    public Loader(String threadNameSuffix) {
        this(ReleasableExecutor.from(Util.newSingleThreadExecutor(THREAD_NAME_PREFIX + threadNameSuffix), new Consumer() { // from class: androidx.media3.exoplayer.upstream.Loader$$ExternalSyntheticLambda0
    ...
    public <T extends Loadable> long startLoading(T loadable, Callback<T> callback, int defaultMinRetryCount) {
        Looper looper = (Looper) Preconditions.checkNotNull(Looper.myLooper());
        this.fatalError = null;
    ...
    public void release(ReleaseCallback callback) {
        ...
        if (callback != null) {
            this.downloadExecutor.execute(new ReleaseTask(callback));
        }
    ...
    private final class LoadTask<T extends Loadable> extends Handler implements Runnable {
        ...
        public void cancel(boolean released) {
            ...
            if (released) {
                ...
                ((Callback) Preconditions.checkNotNull(this.callback)).onLoadCanceled(this.loadable, nowMs, nowMs - this.startTimeMs, true);
                this.callback = null;
            }
        }
        ...
        public void handleMessage(Message msg) {
            ...
            Callback<T> callback = (Callback) Preconditions.checkNotNull(this.callback);
            if (this.canceled) {
        ...
        private void execute() {
            long nowMs = SystemClock.elapsedRealtime();
            long durationMs = nowMs - this.startTimeMs;
            ((Callback) Preconditions.checkNotNull(this.callback)).onLoadStarted(this.loadable, nowMs, durationMs, this.errorCount);
            this.currentError = null;
