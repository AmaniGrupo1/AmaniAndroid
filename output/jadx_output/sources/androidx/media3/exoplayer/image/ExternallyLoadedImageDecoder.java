...
public final class ExternallyLoadedImageDecoder implements ImageDecoder {
    ...
    public static final class Factory implements ImageDecoder.Factory {
        ...
        public int supportsFormat(Format format) {
            int i;
            boolean isExternallyLoadedImage = Objects.equals(format.sampleMimeType, MimeTypes.APPLICATION_EXTERNALLY_LOADED_IMAGE);
            if (isExternallyLoadedImage) {
                i = 4;
            } else if (MimeTypes.isImage(format.sampleMimeType)) {
                i = 1;
            } else {
                i = 0;
            }
    ...
    public void queueInputBuffer(DecoderInputBuffer inputBuffer) {
        ...
        Uri imageUri = Uri.parse(new String(inputData.array(), inputData.arrayOffset(), inputData.remaining(), StandardCharsets.UTF_8));
        this.pendingDecode = this.bitmapResolver.resolve(new ExternalImageRequest(imageUri));
        this.pendingDecodeTimeUs = inputBuffer.timeUs;
        inputBuffer.clear();
    }
    ...
    private void resetState() {
        if (this.pendingDecode != null) {
            this.pendingDecode.cancel(false);
            this.pendingDecode = null;
        }
        this.pendingEndOfStream = false;
