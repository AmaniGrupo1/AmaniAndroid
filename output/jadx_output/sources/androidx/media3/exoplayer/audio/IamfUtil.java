...
public final class IamfUtil {
    ...
    public static int getChannelMaskForOutputLayout(int outputLayout) {
        switch (outputLayout) {
            case 0:
                return 12;
            case 1:
    ...
    public static int getOutputLayoutForCurrentConfiguration(AudioCapabilities audioCapabilities, boolean useIntegratedBinauralRenderer) {
        if (!audioCapabilities.getSpatializerChannelMasks().isEmpty()) {
            if (useIntegratedBinauralRenderer) {
                return 14;
            }
            int spatializerChannelMask = getFirstCompatibleChannelMask(audioCapabilities.getSpatializerChannelMasks());
            if (spatializerChannelMask != 0) {
        ...
        int speakerLayoutChannelMask = getFirstCompatibleChannelMask(audioCapabilities.getSpeakerLayoutChannelMasks());
        if (speakerLayoutChannelMask != 0) {
            return getOutputLayoutForChannelMask(speakerLayoutChannelMask);
        }
        return 0;
    }
    ...
    public static int getOutputChannelMaskForCurrentConfiguration(AudioCapabilities audioCapabilities) {
        ...
        int speakerLayoutChannelMask = getFirstCompatibleChannelMask(audioCapabilities.getSpeakerLayoutChannelMasks());
        if (speakerLayoutChannelMask != 0) {
            return speakerLayoutChannelMask;
        }
        return 12;
    }

    private static int getFirstCompatibleChannelMask(List<Integer> channelMasks) {
        Iterator<Integer> it = channelMasks.iterator();
        while (it.hasNext()) {
            int channelMask = it.next().intValue();
            if (IAMF_SUPPORTED_CHANNEL_MASKS.contains(Integer.valueOf(channelMask))) {
                return channelMask;
            }
        }
        return 0;
