...
public final class SilenceSkippingAudioProcessor extends BaseAudioProcessor {
    ...
    private void shortenSilenceSilenceUntilNoise(ByteBuffer inputBuffer) {
        ...
        Preconditions.checkState(this.maybeSilenceBufferContentsSize <= this.maybeSilenceBuffer.length);
        ...
        if (this.maybeSilenceBufferContentsSize == this.maybeSilenceBuffer.length || shouldTransitionToNoisyState) {
            if (this.outputSilenceFramesSinceNoise == 0) {
                if (shouldTransitionToNoisyState) {
                    ...
                    Preconditions.checkState(volumeChangeType >= this.maybeSilenceBuffer.length / 2);
            ...
            Preconditions.checkState(bytesConsumed % this.bytesPerFrame == 0, "bytesConsumed is not aligned to frame size: %s", bytesConsumed);
            Preconditions.checkState(sizeBeforeOutput >= bytesToOutput);
    ...
    private int calculateShortenedSilenceLength(int silenceToShortenBytes) {
        ...
        Preconditions.checkState(bytesNeededToReachMax >= 0);
    ...
    private void outputRange(byte[] data, int size, int rampType) {
        Preconditions.checkArgument(size % this.bytesPerFrame == 0, "byteOutput size is not aligned to frame size %s", size);
    ...
    private void outputSilence(int sizeToOutput, int rampType) {
        ...
        Preconditions.checkArgument(this.maybeSilenceBufferContentsSize >= sizeToOutput);
        ...
        Preconditions.checkArgument(sizeToOutput % this.bytesPerFrame == 0, "sizeToOutput is not aligned to frame size: %s", sizeToOutput);
        Preconditions.checkState(this.maybeSilenceBufferStartIndex < this.maybeSilenceBuffer.length);
