...
public final class PlaybackStatsListener implements AnalyticsListener, PlaybackSessionManager.Listener {
    ...
    public void onSessionActive(AnalyticsListener.EventTime eventTime, String sessionId) {
        ((PlaybackStatsTracker) Preconditions.checkNotNull(this.playbackStatsTrackers.get(sessionId))).onForeground();
    }

    @Override // androidx.media3.exoplayer.analytics.PlaybackSessionManager.Listener
    public void onAdPlaybackStarted(AnalyticsListener.EventTime eventTime, String contentSessionId, String adSessionId) {
        ((PlaybackStatsTracker) Preconditions.checkNotNull(this.playbackStatsTrackers.get(contentSessionId))).onInterruptedByAd();
    }
    ...
    public void onSessionFinished(AnalyticsListener.EventTime eventTime, String sessionId, boolean automaticTransitionToNextPlayback) {
        ...
        PlaybackStatsTracker tracker = (PlaybackStatsTracker) Preconditions.checkNotNull(this.playbackStatsTrackers.remove(sessionId));
        AnalyticsListener.EventTime startEventTime = (AnalyticsListener.EventTime) Preconditions.checkNotNull(this.sessionStartEventTimes.remove(sessionId));
    ...
    public void onEvents(Player player, AnalyticsListener.Events events) {
        ...
        for (String session : this.playbackStatsTrackers.keySet()) {
            ...
            tracker.onEvents(player, eventTime, z, j2, hasDiscontinuityToPlayback, i, hasAudioUnderrun, startedLoading, playerError, exc, j, j3, hasFormatData ? this.videoFormat : null, hasFormatData ? this.audioFormat : null, hasVideoSize ? this.videoSize : null);
        }
    ...
    private static final class PlaybackStatsTracker {
        ...
        private void maybeUpdateVideoFormat(AnalyticsListener.EventTime eventTime, Format newFormat) {
            if (Objects.equals(this.currentVideoFormat, newFormat)) {
                return;
            }
            maybeRecordVideoFormatTime(eventTime.realtimeMs);
            if (newFormat != null) {
        ...
        private void maybeUpdateAudioFormat(AnalyticsListener.EventTime eventTime, Format newFormat) {
            if (Objects.equals(this.currentAudioFormat, newFormat)) {
                return;
            }
            maybeRecordAudioFormatTime(eventTime.realtimeMs);
