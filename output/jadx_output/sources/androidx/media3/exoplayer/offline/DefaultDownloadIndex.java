...
public final class DefaultDownloadIndex implements WritableDownloadIndex {
    ...
    static String encodeStreamKeys(List<StreamKey> streamKeys) {
        ...
        for (int i = 0; i < streamKeys.size(); i++) {
            ...
            stringBuilder.append(streamKey.periodIndex).append('.').append(streamKey.groupIndex).append('.').append(streamKey.streamIndex).append(AbstractJsonLexerKt.COMMA);
        }
    ...
    private static String getStateQuery(int... states) {
        ...
        for (int i = 0; i < states.length; i++) {
            if (i > 0) {
                selectionBuilder.append(AbstractJsonLexerKt.COMMA);
            }
    ...
    private static List<StreamKey> decodeStreamKeys(String encodedStreamKeys) {
        ...
        for (String streamKeysString : streamKeysStrings) {
            ...
            Preconditions.checkState(indices.length == 3);
