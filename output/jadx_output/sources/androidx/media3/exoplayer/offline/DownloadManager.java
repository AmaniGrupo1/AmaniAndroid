...
public final class DownloadManager {
    ...
    public void setMaxParallelDownloads(int maxParallelDownloads) {
        Preconditions.checkArgument(maxParallelDownloads > 0);
    ...
    public void setMinRetryCount(int minRetryCount) {
        Preconditions.checkArgument(minRetryCount >= 0);
    ...
    static final class InternalHandler extends Handler {
        ...
        private void removeAllDownloads() {
            ...
            for (int i = 0; i < this.downloads.size(); i++) {
                this.downloads.set(i, copyDownloadWithState(this.downloads.get(i), 5, 0));
            }
            int i2 = 0;
            while (true) {
                ...
                arrayList.add(copyDownloadWithState(terminalDownloads.get(i2), 5, 0));
                i2++;
            }
        ...
        private void syncStoppedDownload(Task activeTask) {
            ...
            Preconditions.checkState(!activeTask.isRemove);
        ...
        private void onContentLengthChanged(Task task, long contentLength) {
            ...
            Download download = (Download) Preconditions.checkNotNull(getDownload(downloadId, false));
        ...
        private void onTaskStopped(Task task) {
            ...
            switch (download.state) {
                case 2:
                    Preconditions.checkState(!isRemove);
        ...
        private Download putDownloadWithState(Download download, int state, int stopReason) {
            Preconditions.checkState((state == 3 || state == 4) ? false : true);
