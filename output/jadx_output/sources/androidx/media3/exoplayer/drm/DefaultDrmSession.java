...
class DefaultDrmSession implements DrmSession {
    ...
    public DefaultDrmSession(UUID uuid, ExoMediaDrm mediaDrm, ProvisioningManager provisioningManager, ReferenceCountListener referenceCountListener, List<DrmInitData.SchemeData> schemeDatas, int mode, boolean playClearSamplesWithoutKeys, boolean isPlaceholderSession, byte[] offlineLicenseKeySetId, HashMap<String, String> keyRequestParameters, MediaDrmCallback callback, Looper playbackLooper, LoadErrorHandlingPolicy loadErrorHandlingPolicy, PlayerId playerId) {
        ...
        if (offlineLicenseKeySetId != null) {
            ...
            this.schemeDatas = Collections.unmodifiableList((List) Preconditions.checkNotNull(schemeDatas));
        }
    ...
    public boolean requiresSecureDecoder(String mimeType) {
        verifyPlaybackThread();
        return this.mediaDrm.requiresSecureDecoder((byte[]) Preconditions.checkNotNull(this.sessionId), mimeType);
    }
    ...
    */
    ...
    public void onProvisionResponse(Object request, Object response) {
        if (request == this.currentProvisionRequest) {
            ...
            try {
                this.mediaDrm.provideProvisionResponse(((MediaDrmCallback.Response) response).data);
    ...
    private long getLicenseDurationRemainingSec() {
        ...
        Pair<Long, Long> pair = (Pair) Preconditions.checkNotNull(WidevineUtil.getLicenseDurationRemainingSec(this));
    ...
    private void postKeyRequest(byte[] scope, int type, boolean allowRetry) {
        try {
            synchronized (this.keyRequestInfoLock) {
                this.currentKeyRequestInfo = new KeyRequestInfo.Builder();
                if (this.schemeDatas != null) {
                    this.currentKeyRequestInfo.setSchemeDatas(this.schemeDatas);
                }
            }
            this.currentKeyRequest = this.mediaDrm.getKeyRequest(scope, this.schemeDatas, type, this.keyRequestParameters);
            ((RequestHandler) Util.castNonNull(this.requestHandler)).post(2, Preconditions.checkNotNull(this.currentKeyRequest), allowRetry);
        } catch (Exception | NoSuchMethodError e) {
            onKeysError(e, true);
        }
    }
    ...
    public void onKeyResponse(Object request, Object response) {
        final KeyRequestInfo keyRequestInfo;
        if (request != this.currentKeyRequest || !isOpen()) {
            return;
        }
        this.currentKeyRequest = null;
        synchronized (this.keyRequestInfoLock) {
            keyRequestInfo = ((KeyRequestInfo.Builder) Preconditions.checkNotNull(this.currentKeyRequestInfo)).build();
            this.currentKeyRequestInfo = null;
        }
        ...
        try {
            ...
            dispatchEvent(new Consumer() { // from class: androidx.media3.exoplayer.drm.DefaultDrmSession$$ExternalSyntheticLambda2
                @Override // androidx.media3.common.util.Consumer
                public final void accept(Object obj) {
                    ((DrmSessionEventListener.EventDispatcher) obj).drmKeysLoaded(keyRequestInfo);
                }
            });
        } catch (Exception | NoSuchMethodError e) {
            onKeysError(e, true);
            ...
            try {
                switch (msg.what) {
                    case 1:
                        ...
                        break;
                    case 2:
                        MediaDrmCallback.Response keyResponse = DefaultDrmSession.this.callback.executeKeyRequest(DefaultDrmSession.this.uuid, (ExoMediaDrm.KeyRequest) requestTask.request);
                        response = keyResponse;
                        synchronized (DefaultDrmSession.this.keyRequestInfoLock) {
                            if (DefaultDrmSession.this.currentKeyRequestInfo != null && keyResponse.loadEventInfo != null) {
                                DefaultDrmSession.this.currentKeyRequestInfo.addLoadInfo(keyResponse.loadEventInfo.copyWithTaskIdAndDurationMs(requestTask.taskId, SystemClock.elapsedRealtime() - requestTask.startTimeMs));
                            }
                            break;
                        }
                        break;
