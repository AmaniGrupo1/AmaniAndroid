...
public final class FrameworkMediaDrm implements ExoMediaDrm {
        ...
        if (listener == null) {
            ...
            onEventListener = new MediaDrm.OnEventListener() { // from class: androidx.media3.exoplayer.drm.FrameworkMediaDrm$$ExternalSyntheticLambda2
                ...
                public final void onEvent(MediaDrm mediaDrm2, byte[] bArr, int i, int i2, byte[] bArr2) {
                    this.f$0.m7929x5e84e274(listener, mediaDrm2, bArr, i, i2, bArr2);
                }
            };
        }
    ...
    /* synthetic */ void m7929x5e84e274(ExoMediaDrm.OnEventListener listener, MediaDrm mediaDrm, byte[] sessionId, int event, int extra, byte[] data) {
        ...
        if (listener == null) {
            ...
            onKeyStatusChangeListener = new MediaDrm.OnKeyStatusChangeListener() { // from class: androidx.media3.exoplayer.drm.FrameworkMediaDrm$$ExternalSyntheticLambda3
                ...
                public final void onKeyStatusChange(MediaDrm mediaDrm2, byte[] bArr, List list, boolean z) {
                    this.f$0.m7931xc78bb65c(listener, mediaDrm2, bArr, list, z);
                }
            };
        }
    ...
    /* synthetic */ void m7931xc78bb65c(ExoMediaDrm.OnKeyStatusChangeListener listener, MediaDrm mediaDrm, byte[] sessionId, List keyInfo, boolean hasNewUsableKey) {
        ...
        if (listener == null) {
            ...
            onExpirationUpdateListener = new MediaDrm.OnExpirationUpdateListener() { // from class: androidx.media3.exoplayer.drm.FrameworkMediaDrm$$ExternalSyntheticLambda1
                ...
                public final void onExpirationUpdate(MediaDrm mediaDrm2, byte[] bArr, long j) {
                    this.f$0.m7930x3bcdcffc(listener, mediaDrm2, bArr, j);
                }
            };
        }
    ...
    /* synthetic */ void m7930x3bcdcffc(ExoMediaDrm.OnExpirationUpdateListener listener, MediaDrm mediaDrm, byte[] sessionId, long expirationTimeMs) {
    ...
    public ExoMediaDrm.KeyRequest getKeyRequest(byte[] scope, List<DrmInitData.SchemeData> schemeDatas, int keyType, HashMap<String, String> optionalParameters) throws NotProvisionedException {
        ...
        if (schemeDatas == null) {
            ...
            byte[] initData2 = adjustRequestInitData(this.uuid, (byte[]) Preconditions.checkNotNull(schemeData.data));
    ...
    private String adjustLicenseServerUrl(String licenseServerUrl) {
        ...
        if (Build.VERSION.SDK_INT >= 33 && "https://default.url".equals(licenseServerUrl)) {
    ...
    /* JADX WARN: Type inference failed for: r0v6, types: [android.media.MediaCrypto] */
    /* JADX WARN: Type inference failed for: r0v7 */
    @Override // androidx.media3.exoplayer.drm.ExoMediaDrm
    public boolean requiresSecureDecoder(byte[] bArr, String str) {
        if (Build.VERSION.SDK_INT >= 31 && isMediaDrmRequiresSecureDecoderImplemented()) {
            return Api31.requiresSecureDecoder(this.mediaDrm, str, this.mediaDrm.getSecurityLevel(bArr));
        }
        MediaCrypto mediaCrypto = 0;
        mediaCrypto = 0;
        try {
            try {
                ...
                mediaCrypto.release();
                mediaCrypto = zRequiresSecureDecoderComponent;
    ...
    public FrameworkCryptoConfig createCryptoConfig(byte[] sessionId) throws MediaCryptoException {
        return new FrameworkCryptoConfig(adjustUuid(this.uuid), sessionId);
    }
    ...
    private static DrmInitData.SchemeData getSchemeData(UUID uuid, List<DrmInitData.SchemeData> schemeDatas) {
        ...
        if (Build.VERSION.SDK_INT >= 28 && schemeDatas.size() > 1) {
            ...
            for (int i = 0; i < schemeDatas.size(); i++) {
                ...
                byte[] schemeDataData = (byte[]) Preconditions.checkNotNull(schemeData.data);
                if (Objects.equals(schemeData.mimeType, firstSchemeData.mimeType) && Objects.equals(schemeData.licenseServerUrl, firstSchemeData.licenseServerUrl) && PsshAtomUtil.isPsshAtom(schemeDataData)) {
            ...
            if (canConcatenateData) {
                ...
                for (int i2 = 0; i2 < schemeDatas.size(); i2++) {
                    byte[] schemeDataData2 = (byte[]) Preconditions.checkNotNull(schemeDatas.get(i2).data);
        ...
        for (int i3 = 0; i3 < schemeDatas.size(); i3++) {
            ...
            int version = PsshAtomUtil.parseVersion((byte[]) Preconditions.checkNotNull(schemeData2.data));
            if (version == 1) {
                return schemeData2;
            }
        }
    ...
    private static byte[] adjustRequestInitData(UUID uuid, byte[] initData) {
        ...
        if (cdmRequiresCommonPsshUuid(uuid) && (psshAtom = PsshAtomUtil.parsePsshAtom(initData)) != null) {
            initData = PsshAtomUtil.buildPsshAtom(C.COMMON_PSSH_UUID, psshAtom.keyIds, psshAtom.schemeData);
        }
        if (C.PLAYREADY_UUID.equals(uuid) && "Amazon".equals(Build.MANUFACTURER) && (("AFTB".equals(Build.MODEL) || "AFTS".equals(Build.MODEL) || "AFTM".equals(Build.MODEL) || "AFTT".equals(Build.MODEL)) && (psshData = PsshAtomUtil.parseSchemeSpecificData(initData, uuid)) != null)) {
            return psshData;
        }
        return initData;
    }
    ...
    private static String adjustRequestMimeType(UUID uuid, String mimeType) {
        if (Build.VERSION.SDK_INT < 26 && C.CLEARKEY_UUID.equals(uuid) && (MimeTypes.VIDEO_MP4.equals(mimeType) || MimeTypes.AUDIO_MP4.equals(mimeType))) {
            return "cenc";
        }
        return mimeType;
    }
    ...
    private static class Api31 {
        ...
        public static boolean requiresSecureDecoder(MediaDrm mediaDrm, String mimeType, int securityLevel) {
        ...
        public static void setLogSessionIdOnMediaDrmSession(MediaDrm mediaDrm, byte[] drmSessionId, PlayerId playerId) {
            ...
            if (!logSessionId.equals(LogSessionId.LOG_SESSION_ID_NONE)) {
                MediaDrm.PlaybackComponent playbackComponent = (MediaDrm.PlaybackComponent) Preconditions.checkNotNull(mediaDrm.getPlaybackComponent(drmSessionId));
