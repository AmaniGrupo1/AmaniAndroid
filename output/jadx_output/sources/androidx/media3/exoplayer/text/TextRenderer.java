...
public final class TextRenderer extends BaseRenderer implements Handler.Callback {
    ...
    public TextRenderer(TextOutput output, Looper outputLooper, SubtitleDecoderFactory subtitleDecoderFactory) {
        super(3);
        this.output = (TextOutput) Preconditions.checkNotNull(output);
        ...
        switch (readResult) {
            case -4:
                if (this.cueDecoderInputBuffer.isEndOfStream()) {
                    ...
                    ByteBuffer cueData = (ByteBuffer) Preconditions.checkNotNull(this.cueDecoderInputBuffer.data);
    ...
    private void renderFromSubtitles(long positionUs) {
        ...
        if (this.nextSubtitle == null) {
            ((SubtitleDecoder) Preconditions.checkNotNull(this.subtitleDecoder)).setPositionUs(positionUs);
            try {
        ...
        while (!this.inputStreamEnded) {
            try {
                ...
                if (nextInputBuffer == null) {
                    nextInputBuffer = ((SubtitleDecoder) Preconditions.checkNotNull(this.subtitleDecoder)).dequeueInputBuffer();
                    if (nextInputBuffer == null) {
                        return;
                    } else {
                ...
                if (this.decoderReplacementState == 1) {
                    ...
                    ((SubtitleDecoder) Preconditions.checkNotNull(this.subtitleDecoder)).queueInputBuffer(nextInputBuffer);
    ...
    public boolean isReady() {
        ...
        if (((CuesResolver) Preconditions.checkNotNull(this.cuesResolver)).getNextCueChangeTimeUs(this.lastRendererPositionUs) != Long.MIN_VALUE) {
            return true;
        }
        try {
    ...
    private static boolean hasEventsAfter(Subtitle subtitle, long timeUs) {
        return subtitle != null && subtitle.getEventTimeCount() > 0 && subtitle.getEventTime(subtitle.getEventTimeCount() - 1) > timeUs;
    }
    ...
    private void initSubtitleDecoder() {
        ...
        this.subtitleDecoder = this.subtitleDecoderFactory.createDecoder((Format) Preconditions.checkNotNull(this.streamFormat));
    ...
    private long getPresentationTimeUs(long positionUs) {
        Preconditions.checkState(positionUs != C.TIME_UNSET);
