...
public final class CacheDataSource implements DataSource {
    ...
    public int read(byte[] buffer, int offset, int length) throws Throwable {
        ...
        DataSpec requestDataSpec = (DataSpec) Preconditions.checkNotNull(this.requestDataSpec);
        DataSpec currentDataSpec = (DataSpec) Preconditions.checkNotNull(this.currentDataSpec);
        try {
            if (this.readPosition >= this.checkCachePosition) {
                openNextSource(requestDataSpec, true);
            }
            try {
                int bytesRead = ((DataSource) Preconditions.checkNotNull(this.currentDataSource)).read(buffer, offset, length);
                if (bytesRead != -1) {
                    ...
                    if (isReadingFromUpstream()) {
                        if (currentDataSpec.length != -1) {
                            j = 0;
                            if (this.currentDataSourceBytesRead < currentDataSpec.length) {
                            }
                        }
