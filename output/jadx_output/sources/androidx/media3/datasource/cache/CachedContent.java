...
final class CachedContent {
    ...
    public SimpleCacheSpan setLastTouchTimestamp(SimpleCacheSpan cacheSpan, long lastTouchTimestamp, boolean updateFile) {
        ...
        Preconditions.checkState(this.cachedSpans.remove(cacheSpan));
        File file = (File) Preconditions.checkNotNull(cacheSpan.file);
        if (!updateFile) {
            lastTouchTimestamp2 = lastTouchTimestamp;
        } else {
            File directory = (File) Preconditions.checkNotNull(file.getParentFile());
    ...
    private static final class Range {
        ...
        public boolean intersects(long otherPosition, long otherLength) {
            return this.position <= otherPosition ? this.length == -1 || this.position + this.length > otherPosition : otherLength == -1 || otherPosition + otherLength > this.position;
        }
    }
}
