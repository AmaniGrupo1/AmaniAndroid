...
final class SimpleCacheSpan extends CacheSpan {
    ...
    public static SimpleCacheSpan createCacheEntry(File file, long length, long lastTouchTimestamp, CachedContentIndex index) {
        ...
        int id = Integer.parseInt((String) Preconditions.checkNotNull(matcher.group(1)));
        ...
        long position = Long.parseLong((String) Preconditions.checkNotNull(matcher.group(2)));
        if (lastTouchTimestamp != C.TIME_UNSET) {
            lastTouchTimestamp2 = lastTouchTimestamp;
        } else {
            lastTouchTimestamp2 = Long.parseLong((String) Preconditions.checkNotNull(matcher.group(3)));
        }
    ...
    private static File upgradeFile(File file, CachedContentIndex index) {
        ...
        if (matcher.matches()) {
            key = Util.unescapeFileName((String) Preconditions.checkNotNull(matcher.group(1)));
        } else {
            matcher = CACHE_FILE_PATTERN_V1.matcher(filename);
            if (matcher.matches()) {
                key = (String) Preconditions.checkNotNull(matcher.group(1));
            }
        }
        if (key == null) {
            return null;
        }
        File newCacheFile = getCacheFile((File) Preconditions.checkNotNull(file.getParentFile()), index.assignIdForKey(key), Long.parseLong((String) Preconditions.checkNotNull(matcher.group(2))), Long.parseLong((String) Preconditions.checkNotNull(matcher.group(3))));
