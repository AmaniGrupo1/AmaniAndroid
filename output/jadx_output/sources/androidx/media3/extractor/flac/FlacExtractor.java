...
public final class FlacExtractor implements Extractor {
    ...
    private void readMetadataBlocks(ExtractorInput input) throws IOException {
        ...
        ((TrackOutput) Util.castNonNull(this.trackOutput)).format(format.buildUpon().setContainerMimeType(MimeTypes.AUDIO_FLAC).build());
    ...
    private int readFrames(ExtractorInput input, PositionHolder seekPosition) throws IOException {
        ...
        int remainingBufferCapacity = this.buffer.getData().length - this.buffer.limit();
        if (this.buffer.bytesLeft() < 16 && remainingBufferCapacity < 16) {
