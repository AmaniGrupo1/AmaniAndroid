...
public final class DefaultExtractorInput implements ExtractorInput {
    ...
    public int peek(byte[] target, int offset, int length) throws IOException {
        ...
        if (peekBufferRemainingBytes == 0) {
            ...
            bytesPeeked = defaultExtractorInput.readFromUpstream(this.peekBuffer, this.peekBufferPosition, length, 0, true);
            if (bytesPeeked == -1) {
                return -1;
            }
            defaultExtractorInput.peekBufferLength += bytesPeeked;
        } else {
            defaultExtractorInput = this;
            bytesPeeked = Math.min(length, peekBufferRemainingBytes);
        }
        System.arraycopy(defaultExtractorInput.peekBuffer, defaultExtractorInput.peekBufferPosition, target, offset, bytesPeeked);
        defaultExtractorInput.peekBufferPosition += bytesPeeked;
        return bytesPeeked;
    }
