...
public final class MpegAudioReader implements ElementaryStreamReader {
    ...
    public MpegAudioReader(String language, int roleFlags, String containerMimeType) {
        this.state = 0;
    ...
    private void readHeaderRemainder(ParsableByteArray source) {
        ...
        if (!this.hasOutputFormat) {
            ...
            Format format = new Format.Builder().setId(this.formatId).setContainerMimeType(this.containerMimeType).setSampleMimeType(this.header.mimeType).setMaxInputSize(4096).setChannelCount(this.header.channels).setSampleRate(this.header.sampleRate).setLanguage(this.language).setRoleFlags(this.roleFlags).build();
