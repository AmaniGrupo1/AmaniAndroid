...
public final class AdtsReader implements ElementaryStreamReader {
    ...
    public AdtsReader(boolean exposeId3, String containerMimeType) {
        this(exposeId3, null, 0, containerMimeType);
    }

    public AdtsReader(boolean exposeId3, String language, int roleFlags, String containerMimeType) {
    ...
    public void createTracks(ExtractorOutput extractorOutput, TsPayloadReader.TrackIdGenerator idGenerator) {
        ...
        if (this.exposeId3) {
            ...
            this.id3Output.format(new Format.Builder().setId(idGenerator.getFormatId()).setContainerMimeType(this.containerMimeType).setSampleMimeType(MimeTypes.APPLICATION_ID3).build());
            return;
        }
    ...
    private void parseAdtsHeader() throws ParserException {
        ...
        if (!z) {
            ...
            Format format = new Format.Builder().setId(this.formatId).setContainerMimeType(this.containerMimeType).setSampleMimeType(MimeTypes.AUDIO_AAC).setCodecs(aacConfig.codecs).setChannelCount(aacConfig.channelCount).setSampleRate(aacConfig.sampleRateHz).setInitializationData(Collections.singletonList(audioSpecificConfig)).setLanguage(this.language).setRoleFlags(this.roleFlags).build();
