...
public final class PsExtractor implements Extractor {
    ...
    */
    public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException {
        ...
        int systemHeaderLength2 = nextStartCode & (-256);
        if ((systemHeaderLength2 >> 8) != 1) {
            input.skipFully(1);
            return 0;
        }
        int streamId = nextStartCode & 255;
        PesReader payloadReader2 = this.psPayloadReaders.get(streamId);
        if (!this.foundAllTracks) {
            if (payloadReader2 != null) {
                ...
                if (elementaryStreamReader != null) {
                    ...
                    PesReader payloadReader3 = new PesReader(elementaryStreamReader, this.timestampAdjuster);
                    this.psPayloadReaders.put(streamId, payloadReader3);
                    payloadReader2 = payloadReader3;
                }
            ...
            payloadReader2 = payloadReader;
            if (!this.foundAudioTrack) {
                maxSearchPosition = MAX_SEARCH_LENGTH;
                if (input.getPosition() > maxSearchPosition) {
                }
            }
        ...
        payloadReader2.consume(this.psPacketBuffer);
