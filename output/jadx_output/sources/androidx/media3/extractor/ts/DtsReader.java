...
public final class DtsReader implements ElementaryStreamReader {
    ...
    public DtsReader(String language, int roleFlags, int maxHeaderSize, String containerMimeType) {
    ...
    private void parseCoreHeader() {
        ...
        if (this.format == null) {
            this.format = DtsUtil.parseDtsFormat(frameData, this.formatId, this.language, this.roleFlags, this.containerMimeType, null);
    ...
    private void updateFormatWithDtsHeaderInfo(DtsUtil.DtsHeader dtsHeader) {
        ...
        if (this.format == null || dtsHeader.channelCount != this.format.channelCount || dtsHeader.sampleRate != this.format.sampleRate || !Objects.equals(dtsHeader.mimeType, this.format.sampleMimeType)) {
            Format.Builder formatBuilder = this.format == null ? new Format.Builder() : this.format.buildUpon();
            this.format = formatBuilder.setId(this.formatId).setContainerMimeType(this.containerMimeType).setSampleMimeType(dtsHeader.mimeType).setChannelCount(dtsHeader.channelCount).setSampleRate(dtsHeader.sampleRate).setLanguage(this.language).setRoleFlags(this.roleFlags).build();
            this.output.format(this.format);
        }
    }
}
