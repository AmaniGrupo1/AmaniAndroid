...
public final class H263Reader implements ElementaryStreamReader {
    ...
    H263Reader(UserDataReader userDataReader, String containerMimeType) {
        ...
        while (true) {
            ...
            if (!this.hasOutputFormat) {
                ...
                if (this.csdBuffer.onStartCode(startCodeValue, lengthToStartCode < 0 ? -lengthToStartCode : 0)) {
                    this.output.format(parseCsdBuffer(this.csdBuffer, this.csdBuffer.volStartPosition, (String) Preconditions.checkNotNull(this.formatId), this.containerMimeType));
    ...
    private static Format parseCsdBuffer(CsdBuffer csdBuffer, int volStartPosition, String formatId, String containerMimeType) {
        ...
        buffer.skipBit();
        buffer.skipBit();
        return new Format.Builder().setId(formatId).setContainerMimeType(containerMimeType).setSampleMimeType(MimeTypes.VIDEO_MP4V).setWidth(videoObjectLayerWidth).setHeight(videoObjectLayerHeight).setPixelWidthHeightRatio(pixelWidthHeightRatio).setInitializationData(Collections.singletonList(csdData)).build();
    }
