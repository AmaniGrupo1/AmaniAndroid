...
public final class MpeghReader implements ElementaryStreamReader {
    ...
    private void parseConfig(ParsableBitArray bitArray) throws ParserException {
        ...
        if (this.mainStreamLabel != this.header.packetLabel) {
            ...
            Format format = new Format.Builder().setId(this.formatId).setContainerMimeType(this.containerMimeType).setSampleMimeType(MimeTypes.AUDIO_MPEGH_MHM1).setSampleRate(this.samplingRate).setCodecs(codecs).setInitializationData(initializationData).build();
