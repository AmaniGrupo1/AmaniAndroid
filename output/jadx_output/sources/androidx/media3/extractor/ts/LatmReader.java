...
public final class LatmReader implements ElementaryStreamReader {
    ...
    public LatmReader(String language, int roleFlags, String containerMimeType) {
    ...
    private void parseStreamMuxConfig(ParsableBitArray data) throws ParserException {
        ...
        if (this.audioMuxVersionA == 0) {
            ...
            if (audioMuxVersion == 0) {
                ...
                Format format = new Format.Builder().setId(this.formatId).setContainerMimeType(this.containerMimeType).setSampleMimeType(MimeTypes.AUDIO_AAC).setCodecs(this.codecs).setChannelCount(this.channelCount).setSampleRate(this.sampleRateHz).setInitializationData(Collections.singletonList(initData)).setLanguage(this.language).setRoleFlags(this.roleFlags).build();
