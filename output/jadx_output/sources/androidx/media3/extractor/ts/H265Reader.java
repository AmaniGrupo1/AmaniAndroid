...
public final class H265Reader implements ElementaryStreamReader {
    ...
    public void consume(ParsableByteArray data) {
        ...
        while (data.bytesLeft() > 0) {
            ...
            int offset2 = offset;
            while (offset2 < limit) {
                int nalUnitOffset2 = NalUnitUtil.findNalUnit(dataArray, offset2, limit, this.prefixFlags);
                if (nalUnitOffset2 == limit) {
                    nalUnitData(dataArray, offset2, limit);
                    return;
                }
                int nalUnitType = NalUnitUtil.getH265NalUnitType(dataArray, nalUnitOffset2);
                if (nalUnitOffset2 > 0 && dataArray[nalUnitOffset2 - 1] == 0) {
                    nalUnitOffset = nalUnitOffset2 - 1;
                    prefixSize = 4;
                } else {
                    nalUnitOffset = nalUnitOffset2;
                    prefixSize = 3;
                }
                ...
                offset2 = nalUnitOffset + prefixSize;
            }
        }
    }
    ...
    private void endNalUnit(long position, int offset, int discardPadding, long pesTimeUs) {
        ...
        if (!this.hasOutputFormat) {
            ...
            if (this.vps.isCompleted() && this.sps.isCompleted() && this.pps.isCompleted()) {
                Format format = parseMediaFormat(this.formatId, this.vps, this.sps, this.pps, this.containerMimeType);
                this.output.format(format);
                Preconditions.checkState(format.maxNumReorderSamples != -1);
    ...
    private static Format parseMediaFormat(String formatId, NalUnitTargetBuffer vps, NalUnitTargetBuffer sps, NalUnitTargetBuffer pps, String containerMimeType) {
        ...
        NalUnitUtil.H265SpsData spsData = NalUnitUtil.parseH265SpsNalUnit(sps.nalData, 3, sps.nalLength, null);
        String codecs = null;
        ...
        return new Format.Builder().setId(formatId).setContainerMimeType(containerMimeType).setSampleMimeType(MimeTypes.VIDEO_H265).setCodecs(codecs).setWidth(spsData.width).setHeight(spsData.height).setDecodedWidth(spsData.decodedWidth).setDecodedHeight(spsData.decodedHeight).setColorInfo(new ColorInfo.Builder().setColorSpace(spsData.colorSpace).setColorRange(spsData.colorRange).setColorTransfer(spsData.colorTransfer).setLumaBitdepth(spsData.bitDepthLumaMinus8 + 8).setChromaBitdepth(spsData.bitDepthChromaMinus8 + 8).build()).setPixelWidthHeightRatio(spsData.pixelWidthHeightRatio).setMaxNumReorderSamples(spsData.maxNumReorderPics).setMaxSubLayers(spsData.maxSubLayersMinus1 + 1).setInitializationData(Collections.singletonList(csdData)).build();
    }
    ...
    private static final class SampleReader {
        ...
        private void outputSample(int i) {
            if (this.sampleTimeUs != C.TIME_UNSET && this.nalUnitPosition != this.samplePosition) {
