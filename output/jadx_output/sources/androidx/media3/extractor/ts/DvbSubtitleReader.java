...
public final class DvbSubtitleReader implements ElementaryStreamReader {
    ...
    public DvbSubtitleReader(List<TsPayloadReader.DvbSubtitleInfo> subtitleInfos, String containerMimeType) {
    ...
    public void createTracks(ExtractorOutput extractorOutput, TsPayloadReader.TrackIdGenerator idGenerator) {
        for (int i = 0; i < this.outputs.length; i++) {
            ...
            output.format(new Format.Builder().setId(idGenerator.getFormatId()).setContainerMimeType(this.containerMimeType).setSampleMimeType(MimeTypes.APPLICATION_DVBSUBS).setInitializationData(Collections.singletonList(subtitleInfo.initializationData)).setLanguage(subtitleInfo.language).build());
