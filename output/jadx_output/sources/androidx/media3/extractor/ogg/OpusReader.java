...
final class OpusReader extends StreamReader {
    ...
    protected boolean readHeaders(ParsableByteArray packet, long position, StreamReader.SetupData setupData) throws ParserException {
        if (peekPacketStartsWith(packet, OPUS_ID_HEADER_SIGNATURE)) {
            ...
            setupData.format = new Format.Builder().setContainerMimeType(MimeTypes.AUDIO_OGG).setSampleMimeType(MimeTypes.AUDIO_OPUS).setChannelCount(channelCount).setSampleRate(OpusUtil.SAMPLE_RATE).setInitializationData(initializationData).build();
            return true;
        }
