...
final class VorbisReader extends StreamReader {
    ...
    protected long preparePayload(ParsableByteArray packet) {
        ...
        int packetBlockSize = decodeBlockSize(packet.getData()[0], (VorbisSetup) Preconditions.checkNotNull(this.vorbisSetup));
    ...
    protected boolean readHeaders(ParsableByteArray packet, long position, StreamReader.SetupData setupData) throws IOException {
        ...
        setupData.format = new Format.Builder().setContainerMimeType(MimeTypes.AUDIO_OGG).setSampleMimeType(MimeTypes.AUDIO_VORBIS).setAverageBitrate(idHeader.bitrateNominal).setPeakBitrate(idHeader.bitrateMaximum).setChannelCount(idHeader.channels).setSampleRate(idHeader.sampleRate).setInitializationData(codecInitializationData).setMetadata(metadata).build();
        return true;
    }
