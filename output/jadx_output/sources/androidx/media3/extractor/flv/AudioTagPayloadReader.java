...
final class AudioTagPayloadReader extends TagPayloadReader {
    ...
    protected boolean parseHeader(ParsableByteArray data) throws TagPayloadReader.UnsupportedFormatException {
        if (!this.hasParsedAudioDataHeader) {
            ...
            if (this.audioFormat == 2) {
                ...
                Format format = new Format.Builder().setContainerMimeType(MimeTypes.VIDEO_FLV).setSampleMimeType(MimeTypes.AUDIO_MPEG).setChannelCount(1).setSampleRate(sampleRate).build();
                ...
                Format format2 = new Format.Builder().setContainerMimeType(MimeTypes.VIDEO_FLV).setSampleMimeType(mimeType).setChannelCount(1).setSampleRate(8000).build();
    ...
    protected boolean parsePayload(ParsableByteArray data, long timeUs) throws ParserException {
        ...
        if (packetType == 0 && !this.hasOutputFormat) {
            ...
            Format format = new Format.Builder().setContainerMimeType(MimeTypes.VIDEO_FLV).setSampleMimeType(MimeTypes.AUDIO_AAC).setCodecs(aacConfig.codecs).setChannelCount(aacConfig.channelCount).setSampleRate(aacConfig.sampleRateHz).setInitializationData(Collections.singletonList(audioSpecificConfig)).build();
            this.output.format(format);
            this.hasOutputFormat = true;
            return false;
        }
