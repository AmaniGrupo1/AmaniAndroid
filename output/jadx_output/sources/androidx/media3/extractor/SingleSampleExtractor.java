...
public final class SingleSampleExtractor implements Extractor {
    ...
    private void readSegment(ExtractorInput input) throws IOException {
        int result = ((TrackOutput) Preconditions.checkNotNull(this.trackOutput)).sampleData((DataReader) input, 1024, true);
        if (result == -1) {
            this.state = 2;
    ...
    private void outputImageTrackAndSeekMap(String sampleMimeType) {
        ...
        this.trackOutput.format(new Format.Builder().setContainerMimeType(sampleMimeType).setSampleMimeType(sampleMimeType).build());
