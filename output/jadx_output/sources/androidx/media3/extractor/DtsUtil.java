...
public final class DtsUtil {
    ...
    private static final int[] TWICE_BITRATE_KBPS_BY_RATE = {64, 112, 128, PsExtractor.AUDIO_STREAM, 224, 256, RendererCapabilities.DECODER_SUPPORT_MASK, 448, 512, 640, ValidationPath.MAX_PATH_LENGTH_BYTES, 896, 1024, 1152, 1280, 1536, 1920, 2048, 2304, 2560, 2688, 2816, 2823, 2944, 3072, 3840, 4096, 6144, 7680};
    ...
    public static Format parseDtsFormat(byte[] frame, String trackId, String language, int roleFlags, String containerMimeType, DrmInitData drmInitData) {
        int bitrate;
        ...
        return new Format.Builder().setId(trackId).setContainerMimeType(containerMimeType).setSampleMimeType(MimeTypes.AUDIO_DTS).setAverageBitrate(bitrate).setChannelCount(channelCount + (frameBits.readBits(2) > 0 ? 1 : 0)).setSampleRate(sampleRate).setDrmInitData(drmInitData).setLanguage(language).setRoleFlags(roleFlags).build();
    }
    ...
    public static DtsHeader parseDtsUhdHeader(byte[] header, AtomicInteger uhdAudioChunkId) throws ParserException {
        ...
        int ftocPayloadInBytes2 = parseUnsignedVarInt(headerBits, UHD_FTOC_PAYLOAD_LENGTH_TABLE, true) + 1;
        long frameDurationUs = C.TIME_UNSET;
        if (!syncFrameFlag) {
            ftocPayloadInBytes = ftocPayloadInBytes2;
            ...
            checkCrc(header, ftocPayloadInBytes2);
            ...
            switch (baseDurationIndex) {
                case 0:
                    ...
                    break;
                case 1:
                    baseDuration2 = WindowSizeClass.HEIGHT_DP_MEDIUM_LOWER_BOUND;
                    break;
                case 2:
    ...
    public static boolean isSampleDtsHd(ExtractorInput input, int sampleSize) throws IOException {
        ...
        if (!input.peekFully(sampleData.getData(), 0, sampleSize, true)) {
            return false;
        }
        input.resetPeekPosition();
        int word = sampleData.peekInt();
        if (getFrameType(word) != 1 || sampleData.bytesLeft() < 10) {
            return false;
        }
        byte[] header = new byte[10];
        sampleData.readBytes(header, 0, 10);
        ...
        int word2 = sampleData.readInt();
        return getFrameType(word2) == 2;
    }
