...
public final class WavExtractor implements Extractor {
    ...
    private void readFileType(ExtractorInput input) throws IOException {
        Preconditions.checkState(input.getPosition() == 0);
    ...
    private int readSampleData(ExtractorInput input) throws IOException {
        ...
        return ((OutputWriter) Preconditions.checkNotNull(this.outputWriter)).sampleData(input, bytesLeft) ? -1 : 0;
    }
    ...
    private static final class PassthroughOutputWriter implements OutputWriter {
        ...
        public PassthroughOutputWriter(ExtractorOutput extractorOutput, TrackOutput trackOutput, WavFormat wavFormat, String mimeType, int pcmEncoding) throws ParserException {
            ...
            this.format = new Format.Builder().setContainerMimeType(MimeTypes.AUDIO_WAV).setSampleMimeType(mimeType).setAverageBitrate(constantBitrate).setPeakBitrate(constantBitrate).setMaxInputSize(this.targetSampleSizeBytes).setChannelCount(wavFormat.numChannels).setSampleRate(wavFormat.frameRateHz).setPcmEncoding(pcmEncoding).build();
        }
        ...
        public void init(int dataStartPosition, long dataEndPosition) {
            WavSeekMap wavSeekMap = new WavSeekMap(this.wavFormat, 1, dataStartPosition, dataEndPosition);
    ...
    private static final class ImaAdPcmOutputWriter implements OutputWriter {
        ...
        @Override // androidx.media3.extractor.wav.WavExtractor.OutputWriter
        public void init(int dataStartPosition, long dataEndPosition) {
            WavSeekMap wavSeekMap = new WavSeekMap(this.wavFormat, this.framesPerBlock, dataStartPosition, dataEndPosition);
            this.extractorOutput.seekMap(wavSeekMap);
            this.trackOutput.format(this.format);
            this.trackOutput.durationUs(wavSeekMap.getDurationUs());
        }
