...
final class SsaStyle {
    ...
    public static SsaStyle fromStyleLine(String styleLine, Format format) {
        ...
        boolean z2;
        int borderStyle;
        Preconditions.checkArgument(styleLine.startsWith("Style:"));
        ...
        try {
            ...
            boolean z3 = true;
            boolean z4 = false;
            if (format.boldIndex == -1 || !parseBooleanValue(styleValues[format.boldIndex].trim())) {
                z = true;
                z3 = false;
            } else {
                z = true;
            }
            if (format.italicIndex == -1 || !parseBooleanValue(styleValues[format.italicIndex].trim())) {
                z2 = false;
            } else {
                z2 = false;
                z4 = z;
            }
            if (format.underlineIndex != -1 && parseBooleanValue(styleValues[format.underlineIndex].trim())) {
                z2 = true;
            }
            boolean z5 = format.strikeoutIndex != -1 && parseBooleanValue(styleValues[format.strikeoutIndex].trim());
            if (format.borderStyleIndex != -1) {
                borderStyle = parseBorderStyle(styleValues[format.borderStyleIndex].trim());
            } else {
                borderStyle = -1;
            }
            return new SsaStyle(strTrim, alignment, color, color2, fontSize, z3, z4, z2, z5, borderStyle);
    ...
    static final class Overrides {
        ...
        public static Overrides parseFromDialogue(String text) {
            ...
            while (matcher.find()) {
                String braceContents = (String) Preconditions.checkNotNull(matcher.group(1));
                try {
        ...
        private static PointF parsePosition(String styleOverride) {
            ...
            return new PointF(Float.parseFloat(((String) Preconditions.checkNotNull(x)).trim()), Float.parseFloat(((String) Preconditions.checkNotNull(y)).trim()));
        }
