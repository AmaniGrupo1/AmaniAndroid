...
public final class Cea708Decoder extends CeaDecoder {
    ...
    protected void decode(SubtitleInputBuffer inputBuffer) {
        ...
        while (this.ccData.bytesLeft() >= 3) {
            ...
            if (ccType == 2 || ccType == 3) {
                if (ccValid) {
                    if (ccType != 3) {
                        Preconditions.checkArgument(ccType == 2);
                        ...
                        int packetSize = ccData1 & Utf8.REPLACEMENT_BYTE;
                        if (packetSize == 0) {
                            packetSize = 64;
                        }
    ...
    private void handleC1Command(int command) {
        switch (command) {
            ...
            case 134:
            case 135:
                int window = command + WorkInfo.STOP_REASON_FOREGROUND_SERVICE_TIMEOUT;
    ...
    private static final class CueInfoBuilder {
        ...
        public static int getArgbColorFromCeaColor(int red, int green, int blue, int opacity) {
            int alpha;
            Preconditions.checkElementIndex(red, 4);
            Preconditions.checkElementIndex(green, 4);
            Preconditions.checkElementIndex(blue, 4);
            Preconditions.checkElementIndex(opacity, 4);
            switch (opacity) {
                case 0:
                case 1:
                    alpha = 255;
            ...
            return Color.argb(alpha, red > 1 ? 255 : 0, green > 1 ? 255 : 0, blue <= 1 ? 0 : 255);
        }
    }
