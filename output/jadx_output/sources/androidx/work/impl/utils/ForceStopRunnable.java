...
public class ForceStopRunnable implements Runnable {
            ...
            while (true) {
                try {
                    ...
                    try {
                        forceStopRunnable();
                        break;
                    } catch (SQLiteAccessPermException | SQLiteCantOpenDatabaseException | SQLiteConstraintException | SQLiteDatabaseCorruptException | SQLiteDatabaseLockedException | SQLiteDiskIOException | SQLiteFullException | SQLiteTableLockedException exception) {
                        this.mRetryCount++;
                        if (this.mRetryCount >= 3) {
                            ...
                            long duration = ((long) this.mRetryCount) * BACKOFF_DURATION_MS;
                            Logger.get().debug(TAG, "Retrying after " + duration, exception);
                            sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
                        }
                    }
                    long duration2 = ((long) this.mRetryCount) * BACKOFF_DURATION_MS;
    ...
    public boolean isForceStopped() {
        int flags = C.BUFFER_FLAG_LAST_SAMPLE;
        try {
