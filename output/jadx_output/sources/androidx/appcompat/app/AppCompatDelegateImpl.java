...
class AppCompatDelegateImpl extends AppCompatDelegate implements MenuBuilder.Callback, LayoutInflater.Factory2 {
    ...
    */
    private void openPanel(PanelFeatureState st, KeyEvent event) {
        ...
        if (st.decorView == null || st.refreshDecorView) {
            ...
            if (st.createdPanelView != null && (lp = st.createdPanelView.getLayoutParams()) != null && lp.width == -1) {
                width = -1;
            }
            st.isHandled = false;
        ...
        width = -2;
        st.isHandled = false;
        WindowManager.LayoutParams lp32 = new WindowManager.LayoutParams(width, -2, st.x, st.y, 1002, 8519680, -3);
        lp32.gravity = st.gravity;
        lp32.windowAnimations = st.windowAnimations;
        wm.addView(st.decorView, lp32);
        st.isOpen = true;
        if (st.featureId != 0) {
        }
    }
    ...
    */
    final int updateStatusGuard(WindowInsetsCompat insets, Rect rectInsets) {
        ...
        int systemWindowInsetTop3 = 0;
        if (insets != null) {
            systemWindowInsetTop3 = insets.getSystemWindowInsetTop();
        } else if (rectInsets != null) {
            systemWindowInsetTop3 = rectInsets.top;
        }
        boolean showStatusGuard = false;
        if (this.mActionModeView == null || !(this.mActionModeView.getLayoutParams() instanceof ViewGroup.MarginLayoutParams)) {
            int systemWindowInsetTop4 = systemWindowInsetTop3;
            i = 0;
        ...
        if (this.mActionModeView.isShown()) {
            ...
            if (newTopMargin > 0 && this.mStatusGuard == null) {
                ...
                this.mSubDecor.addView(this.mStatusGuard, -1, lp);
            } else {
                systemWindowInsetTop2 = systemWindowInsetTop3;
                if (this.mStatusGuard != null) {
                    ViewGroup.MarginLayoutParams lp2 = (ViewGroup.MarginLayoutParams) this.mStatusGuard.getLayoutParams();
            ...
            int systemWindowInsetTop5 = systemWindowInsetTop3;
            if (mlp.topMargin == 0) {
                i = 0;
    ...
    private static Configuration generateConfigDelta(Configuration base, Configuration change) {
        ...
        if ((base.screenLayout & PsExtractor.AUDIO_STREAM) != (change.screenLayout & PsExtractor.AUDIO_STREAM)) {
            delta.screenLayout |= change.screenLayout & PsExtractor.AUDIO_STREAM;
        }
        if ((base.screenLayout & 48) != (change.screenLayout & 48)) {
            delta.screenLayout |= change.screenLayout & 48;
        }
        if ((base.screenLayout & ValidationPath.MAX_PATH_LENGTH_BYTES) != (change.screenLayout & ValidationPath.MAX_PATH_LENGTH_BYTES)) {
            delta.screenLayout |= change.screenLayout & ValidationPath.MAX_PATH_LENGTH_BYTES;
        }
        if (Build.VERSION.SDK_INT >= 26) {
