...
final class Utf8 {
                    ...
                    if (j <= limit - 4) {
                        if (i + 1 != in.length()) {
                            ...
                            if (Character.isSurrogatePair(c2, low)) {
                                int codePoint = Character.toCodePoint(c2, low);
                                int j5 = j + 1;
                                out[j] = (byte) ((codePoint >>> 18) | PsExtractor.VIDEO_STREAM_MASK);
                                int j6 = j5 + 1;
    ...
    static final class UnsafeProcessor extends Processor {
        ...
        String decodeUtf8(byte[] bytes, int index, int size) throws InvalidProtocolBufferException {
            ...
            if (s.indexOf(65533) < 0 || Arrays.equals(s.getBytes(Internal.UTF_8), Arrays.copyOfRange(bytes, index, index + size))) {
                return s;
            }
            throw InvalidProtocolBufferException.invalidUtf8();
        }
                            ...
                            if (outIx <= outLimit - 4) {
                                if (inIx + 1 != inLimit) {
                                    ...
                                    if (Character.isSurrogatePair(c4, low)) {
                                        ...
                                        UnsafeUtil.putByte(out, outIx, (byte) ((codePoint >>> 18) | PsExtractor.VIDEO_STREAM_MASK));
        ...
        void encodeUtf8Direct(String in, ByteBuffer out) {
            ...
            if (inIx == inLimit) {
                Java8Compatibility.position(out, (int) (outIx - address2));
                return;
            }
            while (inIx < inLimit) {
                char c2 = in.charAt(inIx);
                            ...
                            if (outIx <= outLimit - 4) {
                                if (inIx + 1 != inLimit) {
                                    ...
                                    if (Character.isSurrogatePair(c2, low)) {
                                        ...
                                        long outIx2 = outIx + j2;
                                        UnsafeUtil.putByte(outIx, (byte) ((codePoint2 >>> 18) | PsExtractor.VIDEO_STREAM_MASK));
