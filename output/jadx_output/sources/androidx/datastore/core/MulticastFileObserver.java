    ...
    public static final class Companion {
        ...
        public final DisposableHandle observe(File parent, final Function1<? super String, Unit> observer) {
            ...
            synchronized (MulticastFileObserver.LOCK) {
                ...
                MulticastFileObserver multicastFileObserver2 = fileObservers$datastore_core.get(key);
                if (multicastFileObserver2 == null) {
                    Intrinsics.checkNotNull(key);
                    multicastFileObserver = new MulticastFileObserver(key, null);
                    fileObservers$datastore_core.put(key, multicastFileObserver);
                } else {
            ...
            return new DisposableHandle() { // from class: androidx.datastore.core.MulticastFileObserver$Companion$$ExternalSyntheticLambda0
                ...
                public final void dispose() {
                    MulticastFileObserver.Companion.observe$lambda$1(key, observer);
                }
            };
        }

        static final void observe$lambda$1(String $key, Function1 $observer) {
            synchronized (MulticastFileObserver.LOCK) {
                MulticastFileObserver filesystemObserver = MulticastFileObserver.INSTANCE.getFileObservers$datastore_core().get($key);
                if (filesystemObserver != null) {
                    filesystemObserver.delegates.remove($observer);
                    if (filesystemObserver.delegates.isEmpty()) {
                        MulticastFileObserver.INSTANCE.getFileObservers$datastore_core().remove($key);
        ...
        public final void removeAllObservers$datastore_core() {
            synchronized (MulticastFileObserver.LOCK) {
                Iterable $this$forEach$iv = MulticastFileObserver.INSTANCE.getFileObservers$datastore_core().values();
                for (Object element$iv : $this$forEach$iv) {
