...
public final class RememberNavigationEventStateKt {
    public static final <T extends NavigationEventInfo> NavigationEventState<T> rememberNavigationEventState(final T t, final List<? extends T> list, final List<? extends T> list2, Composer $composer, int $changed, int i) {
        ...
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventStart(116613162, $changed, -1, "androidx.navigationevent.compose.rememberNavigationEventState (RememberNavigationEventState.kt:44)");
        }
        ...
        if (it$iv == Composer.INSTANCE.getEmpty()) {
            Object value$iv = new NavigationEventState(t, list, list2);
            $composer.updateRememberedValue(value$iv);
            it$iv = value$iv;
        }
        final NavigationEventState<T> navigationEventState = (NavigationEventState) it$iv;
        ComposerKt.sourceInformationMarkerEnd($composer);
        ComposerKt.sourceInformationMarkerStart($composer, 396108899, "CC(remember):RememberNavigationEventState.kt#9igjgp");
        boolean invalid$iv = (((($changed & 14) ^ 6) > 4 && $composer.changedInstance(t)) || ($changed & 6) == 4) | $composer.changedInstance(list) | $composer.changedInstance(list2);
        Object it$iv2 = $composer.rememberedValue();
        if (invalid$iv || it$iv2 == Composer.INSTANCE.getEmpty()) {
            Object value$iv2 = new Function0() { // from class: androidx.navigationevent.compose.RememberNavigationEventStateKt$$ExternalSyntheticLambda0
    ...
    static final Unit rememberNavigationEventState$lambda$2$lambda$1(NavigationEventState $state, NavigationEventInfo $currentInfo, List $backInfo, List $forwardInfo) {
        $state.setCurrentInfo$navigationevent_compose($currentInfo);
        $state.setBackInfo$navigationevent_compose($backInfo);
        $state.setForwardInfo$navigationevent_compose($forwardInfo);
