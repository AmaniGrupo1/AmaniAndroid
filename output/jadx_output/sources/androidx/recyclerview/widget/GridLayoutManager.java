    ...
    */
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        if (layingOutInPrimaryDirection) {
            ...
            int itemSpanIndex = gridLayoutManager.getSpanIndex(recycler, state, layoutState.mCurrentPosition);
            int itemSpanSize2 = gridLayoutManager.getSpanSize(recycler, state, layoutState.mCurrentPosition);
            remainingSpan = itemSpanIndex + itemSpanSize2;
            itemSpanSize = 0;
        }
        while (itemSpanSize < count && layoutState.hasMore(state) && remainingSpan > 0) {
            int pos = layoutState.mCurrentPosition;
            ...
            gridLayoutManager.mSet[itemSpanSize] = view;
            itemSpanSize++;
        }
        if (itemSpanSize == 0) {
        ...
        gridLayoutManager.assignSpans(recycler, state, itemSpanSize, layingOutInPrimaryDirection);
        int i2 = 0;
        ...
        if (!flexibleInOtherDir) {
            ...
            for (int i3 = 0; i3 < itemSpanSize; i3++) {
                ...
                gridLayoutManager.measureChild(view3, 1073741824, true);
