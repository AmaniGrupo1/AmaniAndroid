...
public final class AssetUriFetcher implements Fetcher {
    ...
    public AssetUriFetcher(Uri data, Options options) {
        this.data = data;
    ...
    public Object fetch(Continuation<? super FetchResult> continuation) {
        String path = CollectionsKt.joinToString$default(CollectionsKt.drop(this.data.getPathSegments(), 1), "/", null, null, 0, null, null, 62, null);
        return new SourceResult(ImageSources.create(Okio.buffer(Okio.source(this.options.getContext().getAssets().open(path))), this.options.getContext(), new AssetMetadata(path)), Utils.getMimeTypeFromUrl(MimeTypeMap.getSingleton(), path), DataSource.DISK);
    }
