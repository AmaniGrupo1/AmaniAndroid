...
public final class DrawableUtils {
    ...
    public final Bitmap convertToBitmap(Drawable drawable, Bitmap.Config config, Size size, Scale scale, boolean allowInexactSize) {
        ...
        double multiplier = DecodeUtils.computeSizeMultiplier(it, srcHeight, Sizes.isOriginal(size) ? it : Utils.toPx(size.getWidth(), scale), Sizes.isOriginal(size) ? srcHeight : Utils.toPx(size.getHeight(), scale), scale);
        int bitmapWidth = MathKt.roundToInt(((double) it) * multiplier);
        int bitmapHeight = MathKt.roundToInt(((double) srcHeight) * multiplier);
