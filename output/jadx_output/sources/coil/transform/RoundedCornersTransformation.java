...
public final class RoundedCornersTransformation implements Transformation {
    ...
    public RoundedCornersTransformation(float topLeft, float topRight, float bottomLeft, float bottomRight) {
        ...
        this.cacheKey = getClass().getName() + '-' + this.topLeft + AbstractJsonLexerKt.COMMA + this.topRight + AbstractJsonLexerKt.COMMA + this.bottomLeft + AbstractJsonLexerKt.COMMA + this.bottomRight;
    }
    ...
    public Object transform(Bitmap input, Size size, Continuation<? super Bitmap> continuation) {
        ...
        Pair<Integer, Integer> pairCalculateOutputSize = calculateOutputSize(input, size);
        int outputWidth = pairCalculateOutputSize.component1().intValue();
        int outputHeight = pairCalculateOutputSize.component2().intValue();
    ...
    private final Pair<Integer, Integer> calculateOutputSize(Bitmap input, Size size) {
        ...
        if ((dstWidth instanceof Dimension.Pixels) && (dstHeight instanceof Dimension.Pixels)) {
            return TuplesKt.to(Integer.valueOf(((Dimension.Pixels) dstWidth).px), Integer.valueOf(((Dimension.Pixels) dstHeight).px));
        }
        ...
        int outputWidth = MathKt.roundToInt(((double) input.getWidth()) * multiplier);
        int outputHeight = MathKt.roundToInt(((double) input.getHeight()) * multiplier);
        return TuplesKt.to(Integer.valueOf(outputWidth), Integer.valueOf(outputHeight));
    }
