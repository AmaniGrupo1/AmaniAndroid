...
public final class CoreResolverKt {
    public static final Set<Scope> flatten(List<Scope> scopes) {
        ...
        ArrayDeque stack = new ArrayDeque(CollectionsKt.asReversed(scopes));
        while (!stack.isEmpty()) {
            Scope current = (Scope) stack.removeLast();
            if (flatten.add(current)) {
                Iterator<Scope> it = current.getLinkedScopes$koin_core().iterator();
