...
public final class ZoneRules implements Serializable {
    ...
    void writeExternalTimeZone(DataOutput out) {
    ...
    private ZoneOffsetTransition[] findTransitionArray(int year) {
        ...
        if (this.timeZone != null) {
            ...
            long max = 31968000 + lower;
            ZoneOffsetTransition[] transArray2 = NO_TRANSITIONS;
            while (lower < max) {
                long upper = 7776000 + lower;
                long j2 = j;
                if (curOffsetMillis != this.timeZone.getOffset(upper * j2)) {
                    while (upper - lower > 1) {
                        long lower2 = lower;
                        long middle = Math.floorDiv(upper + lower, 2L);
                        if (this.timeZone.getOffset(middle * j2) == curOffsetMillis) {
                            lower = middle;
                        } else {
                            upper = middle;
                            lower = lower2;
                    ...
                    long lower3 = lower;
                    if (this.timeZone.getOffset(lower3 * j2) != curOffsetMillis) {
                        upper = lower3;
                    }
                    ZoneOffset old = offsetFromMillis(curOffsetMillis);
                    int curOffsetMillis2 = this.timeZone.getOffset(upper * j2);
    ...
    public ZoneOffsetTransition previousTransition(Instant instant) {
        ...
        if (this.timeZone == null) {
            ...
            if (this.lastRules.length > 0 && epochSec > lastHistoric) {
                ...
                if (year2 > lastHistoricYear) {
                    return findTransitionArray(year2)[r5.length - 1];
                }
            }
            int index = Arrays.binarySearch(this.savingsInstantTransitions, epochSec);
            if (index < 0) {
        ...
        ZoneOffsetTransition[] transArray2 = findTransitionArray(year3);
        for (int i2 = transArray2.length - 1; i2 >= 0; i2--) {
            if (epochSec2 > transArray2[i2].toEpochSecond()) {
                return transArray2[i2];
            }
        }
        if (year3 > 1800) {
            ZoneOffsetTransition[] transArray3 = findTransitionArray(year3 - 1);
            for (int i3 = transArray3.length - 1; i3 >= 0; i3--) {
                if (epochSec2 > transArray3[i3].toEpochSecond()) {
                    return transArray3[i3];
                }
            }
            long probeSec = Math.min(epochSec2 - 31104000, (Clock.systemUTC().millis() / 1000) + 31968000);
            ...
            while (min <= probeSec) {
                ...
                if (curOffsetMillis != probeOffsetMillis) {
                    ...
                    ZoneOffsetTransition[] transArray4 = findTransitionArray(year4 + 1);
                    for (int i4 = transArray4.length - 1; i4 >= 0; i4--) {
                        if (epochSec2 > transArray4[i4].toEpochSecond()) {
                            return transArray4[i4];
                        }
                    }
                    return findTransitionArray(year4)[r2.length - 1];
                }
                probeSec -= 7776000;
                z = z2;
            }
    ...
    public List<ZoneOffsetTransitionRule> getTransitionRules() {
        return Duration.DurationUnits.AnonymousClass0.m(this.lastRules);
    }
