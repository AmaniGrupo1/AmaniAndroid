...
public final class ZoneOffsetTransitionRule implements Serializable {
    ...
    static ZoneOffsetTransitionRule readExternal(DataInput in) throws IOException {
        ...
        ZoneOffset std = ZoneOffset.ofTotalSeconds(stdByte == 255 ? in.readInt() : (stdByte + WorkInfo.STOP_REASON_FOREGROUND_SERVICE_TIMEOUT) * 900);
    ...
    public ZoneOffsetTransition createTransition(int year) {
        ...
        if (b < 0) {
            date = LocalDate.of(year, month, this.month.length(IsoChronology.INSTANCE.isLeapYear(year)) + 1 + this.dom);
            if (this.dow != null) {
                date = date.with(TemporalAdjusters.previousOrSame(this.dow));
            }
        } else {
            date = LocalDate.of(year, month, this.dom);
            if (this.dow != null) {
    ...
    public String toString() {
        ...
        buf.append(" at ").append(this.timeEndOfDay ? "24:00" : this.time.toString()).append(" ").append(this.timeDefinition).append(", standard offset ").append(this.standardOffset).append(AbstractJsonLexerKt.END_LIST);
        return buf.toString();
    }
