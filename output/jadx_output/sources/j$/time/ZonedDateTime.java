...
public final class ZonedDateTime implements Temporal, ChronoZonedDateTime<LocalDate>, Serializable {
    ...
    public static ZonedDateTime parse(CharSequence text, DateTimeFormatter formatter) {
        ...
        return (ZonedDateTime) formatter.parse(text, new TemporalQuery() { // from class: j$.time.ZonedDateTime.0
    ...
    public ZonedDateTime plus(long amountToAdd, TemporalUnit unit) {
        if (unit instanceof ChronoUnit) {
            boolean zIsDateBased = unit.isDateBased();
            LocalDateTime localDateTime = this.dateTime;
            if (zIsDateBased) {
                return resolveLocal(localDateTime.plus(amountToAdd, unit));
            }
            return resolveInstant(localDateTime.plus(amountToAdd, unit));
        }
    ...
    public long until(Temporal endExclusive, TemporalUnit unit) {
        ZonedDateTime end = from((TemporalAccessor) endExclusive);
        if (unit instanceof ChronoUnit) {
