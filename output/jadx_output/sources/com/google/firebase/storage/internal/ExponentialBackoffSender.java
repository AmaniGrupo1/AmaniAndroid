...
public class ExponentialBackoffSender {
    ...
    public ExponentialBackoffSender(Context context, InternalAuthProvider authProvider, InteropAppCheckTokenProvider appCheckProvider, long retryTime) {
    ...
    public void sendWithExponentialBackoff(NetworkRequest request, boolean closeRequest) {
        ...
        InternalAuthProvider internalAuthProvider = this.authProvider;
        if (closeRequest) {
            request.performRequest(Util.getCurrentAuthToken(internalAuthProvider), Util.getCurrentAppCheckToken(this.appCheckProvider), this.context);
        } else {
            request.performRequestStart(Util.getCurrentAuthToken(internalAuthProvider), Util.getCurrentAppCheckToken(this.appCheckProvider));
        }
        int currentSleepTime = 1000;
        while (clock.elapsedRealtime() + ((long) currentSleepTime) <= deadLine && !request.isResultSuccess() && isRetryableError(request.getResultCode())) {
            try {
                ...
                request.reset();
                InternalAuthProvider internalAuthProvider2 = this.authProvider;
                if (closeRequest) {
                    request.performRequest(Util.getCurrentAuthToken(internalAuthProvider2), Util.getCurrentAppCheckToken(this.appCheckProvider), this.context);
                } else {
                    request.performRequestStart(Util.getCurrentAuthToken(internalAuthProvider2), Util.getCurrentAppCheckToken(this.appCheckProvider));
                }
