...
class GetDownloadUrlTask implements Runnable {
    ...
    private Uri extractDownloadUrl(JSONObject response) {
        ...
        if (!TextUtils.isEmpty(downloadTokens)) {
            ...
            Uri.Builder uriBuilder = this.storageRef.getStorageReferenceUri().getHttpUri().buildUpon();
    ...
    public void run() {
        NetworkRequest request = new GetMetadataNetworkRequest(this.storageRef.getStorageReferenceUri(), this.storageRef.getApp());
