...
public class StreamDownloadTask extends StorageTask<TaskSnapshot> {
    ...
    public InputStream createDownloadStream() throws Exception {
        ...
        this.request = new GetNetworkRequest(this.storageRef.getStorageReferenceUri(), this.storageRef.getApp(), this.bytesDownloaded);
        ...
        if (success) {
            ...
            if (!TextUtils.isEmpty(newEtag) && this.eTagVerification != null && !this.eTagVerification.equals(newEtag)) {
                this.resultCode = 409;
    ...
    static class StreamProgressWrapper extends InputStream {
        ...
        public int read(byte[] buffer, int byteOffset, int byteCount) throws IOException {
            int bytesRead = 0;
            while (ensureStream()) {
                while (byteCount > StreamDownloadTask.PREFERRED_CHUNK_SIZE) {
                    try {
        ...
        public long skip(long byteCount) throws IOException {
            ...
            while (ensureStream()) {
                while (byteCount > StreamDownloadTask.PREFERRED_CHUNK_SIZE) {
                    try {
                        long deltaBytesSkipped = this.mWrappedStream.skip(StreamDownloadTask.PREFERRED_CHUNK_SIZE);
