...
public class UploadTask extends StorageTask<TaskSnapshot> {
    ...
    UploadTask(StorageReference targetRef, StorageMetadata metadata, byte[] bytes) {
        ...
        this.mSender = new ExponentialBackoffSender(storage.getApp().getApplicationContext(), this.mAuthProvider, this.mAppCheckProvider, storage.getMaxUploadRetryTimeMillis());
    }

    UploadTask(StorageReference targetRef, StorageMetadata metadata, Uri file, Uri existingUploadUri) throws FileNotFoundException {
        this.mBytesUploaded = new AtomicLong(0L);
        this.mCurrentChunkSize = 262144;
        this.mUploadUri = null;
        this.mException = null;
    ...
    private void beginResumableUpload() {
        ...
        NetworkRequest startRequest = new ResumableUploadStartRequest(this.mStorageRef.getStorageReferenceUri(), this.mStorageRef.getApp(), this.mMetadata != null ? this.mMetadata.createJSONObject() : null, mimeType);
    ...
    private boolean recoverStatus(boolean withRetry) {
        ...
        NetworkRequest queryRequest = new ResumableUploadQueryRequest(this.mStorageRef.getStorageReferenceUri(), this.mStorageRef.getApp(), this.mUploadUri);
        if (RESUMABLE_FINAL_STATUS.equals(this.mServerStatus)) {
            return false;
        }
        if (withRetry) {
    ...
    private void uploadChunk() {
        try {
            ...
            NetworkRequest uploadRequest = new ResumableUploadByteRequest(this.mStorageRef.getStorageReferenceUri(), this.mStorageRef.getApp(), this.mUploadUri, this.mStreamBuffer.get(), this.mBytesUploaded.get(), bytesToUpload, this.mStreamBuffer.isFinished());
