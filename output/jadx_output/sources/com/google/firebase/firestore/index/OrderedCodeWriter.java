...
public class OrderedCodeWriter {
    ...
    public void writeUtf8Ascending(CharSequence sequence) {
        ...
        while (i < utf16Length) {
            ...
            if (c < 128) {
                ...
                writeByteAscending((byte) ((c >>> '\f') | WindowSizeClass.HEIGHT_DP_MEDIUM_LOWER_BOUND));
                writeByteAscending((byte) (((c >>> 6) & 63) | 128));
                writeByteAscending((byte) (128 | (c & '?')));
            } else {
                int codePoint = Character.codePointAt(sequence, i);
                i++;
                writeByteAscending((byte) ((codePoint >>> 18) | PsExtractor.VIDEO_STREAM_MASK));
    ...
    public void writeUtf8Descending(CharSequence sequence) {
        ...
        while (i < utf16Length) {
            ...
            if (c < 128) {
                ...
                writeByteDescending((byte) ((c >>> '\f') | WindowSizeClass.HEIGHT_DP_MEDIUM_LOWER_BOUND));
                writeByteDescending((byte) (((c >>> 6) & 63) | 128));
                writeByteDescending((byte) (128 | (c & '?')));
            } else {
                int codePoint = Character.codePointAt(sequence, i);
                i++;
                writeByteDescending((byte) ((codePoint >>> 18) | PsExtractor.VIDEO_STREAM_MASK));
    ...
    public void writeUnsignedLongAscending(long value) {
        ...
        while (true) {
            i2--;
            if (i2 >= this.position) {
                this.buffer[i2] = (byte) (255 & value);
                value >>>= 8;
            } else {
                int i3 = this.position;
                this.position = i3 + len;
                return;
            }
        }
    }
    ...
    public void writeSignedLongAscending(long value) {
        ...
        if (val < 64) {
            ...
            int i = this.position;
            this.position = i + 1;
            bArr[i] = (byte) (((long) LENGTH_TO_HEADER_BITS[1][0]) ^ value);
            return;
        }
        int len = signedNumLength(val);
        ensureAvailable(len);
        if (len < 2) {
            throw new AssertionError(String.format("Invalid length (%d) returned by signedNumLength", Integer.valueOf(len)));
        }
        byte signByte = value < 0 ? (byte) -1 : (byte) 0;
    ...
    public void writeDoubleAscending(double val) {
        ...
        writeUnsignedLongAscending(v ^ (v < 0 ? -1L : Long.MIN_VALUE));
    }

    public void writeDoubleDescending(double val) {
        long v = Double.doubleToLongBits(val);
        writeUnsignedLongDescending(v ^ (v < 0 ? -1L : Long.MIN_VALUE));
    }
    ...
    private int signedNumLength(long n) {
        ...
        return IntMath.divide(numBits + 1, 7, RoundingMode.UP);
    }
