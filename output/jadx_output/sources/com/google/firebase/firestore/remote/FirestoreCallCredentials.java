...
final class FirestoreCallCredentials extends CallCredentials {
    ...
    public void applyRequestMetadata(CallCredentials.RequestInfo requestInfo, Executor executor, final CallCredentials.MetadataApplier metadataApplier) {
        final Task<String> authTask = this.authProvider.getToken();
        final Task<String> appCheckTask = this.appCheckProvider.getToken();
        Tasks.whenAll((Task<?>[]) new Task[]{authTask, appCheckTask}).addOnCompleteListener(Executors.DIRECT_EXECUTOR, new OnCompleteListener() { // from class: com.google.firebase.firestore.remote.FirestoreCallCredentials$$ExternalSyntheticLambda0
            @Override // com.google.android.gms.tasks.OnCompleteListener
            public final void onComplete(Task task) {
                FirestoreCallCredentials.lambda$applyRequestMetadata$0(authTask, metadataApplier, appCheckTask, task);
            }
        });
    }
