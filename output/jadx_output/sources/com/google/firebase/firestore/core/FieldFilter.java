...
public class FieldFilter extends Filter {
    ...
    public boolean matches(Document doc) {
        ...
        return this.operator == Operator.NOT_EQUAL ? (other == null || other.hasNullValue() || !matchesComparison(Values.compare(other, this.value))) ? false : true : other != null && Values.typeOrder(other) == Values.typeOrder(this.value) && matchesComparison(Values.compare(other, this.value));
    }
    ...
    BooleanExpression toPipelineExpr() {
        Field x = new Field(this.field);
        BooleanExpression exists = x.exists();
        switch (this.operator) {
            case LESS_THAN:
                return Expression.and(exists, x.lessThan(this.value));
            case LESS_THAN_OR_EQUAL:
                return Expression.and(exists, x.lessThanOrEqual(this.value));
            case EQUAL:
                return Expression.and(exists, x.equal(this.value));
            case NOT_EQUAL:
                return x.notEqual(this.value);
            case GREATER_THAN:
                return Expression.and(exists, x.greaterThan(this.value));
            case GREATER_THAN_OR_EQUAL:
                return Expression.and(exists, x.greaterThanOrEqual(this.value));
            case ARRAY_CONTAINS:
                return Expression.and(exists, x.arrayContains(this.value));
            case ARRAY_CONTAINS_ANY:
                return Expression.and(exists, x.arrayContainsAny(this.value.getArrayValue().getValuesList()));
            case IN:
                return Expression.and(exists, x.equalAny(this.value.getArrayValue().getValuesList()));
            case NOT_IN:
                List<Value> list = this.value.getArrayValue().getValuesList();
