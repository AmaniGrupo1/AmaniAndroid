...
public final class BackgroundQueue {
    ...
    private static final int maxParallelism = RangesKt.coerceAtLeast(Runtime.getRuntime().availableProcessors(), 2);
    private static final Executor executor = ExecutorsKt.asExecutor(Dispatchers.getIO().limitedParallelism(maxParallelism, "firestore.BackgroundQueue"));

    public final void submit(final Runnable runnable) {
        Intrinsics.checkNotNullParameter(runnable, "runnable");
        final State submittingState = this.state;
        ...
        executor.execute(new Runnable() { // from class: com.google.firebase.firestore.util.BackgroundQueue$$ExternalSyntheticLambda0
            ...
            public final void run() {
                BackgroundQueue.submit$lambda$1(runnable, submittingState);
            }
        });
    }

    static final void submit$lambda$1(Runnable $runnable, State $submittingState) {
        try {
            $runnable.run();
        } finally {
