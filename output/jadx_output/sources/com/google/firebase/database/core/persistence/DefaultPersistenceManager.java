...
public class DefaultPersistenceManager implements PersistenceManager {
    ...
    public CacheNode serverCache(QuerySpec query) {
        ...
        boolean zIsQueryComplete = this.trackedQueryManager.isQueryComplete(query);
        TrackedQueryManager trackedQueryManager = this.trackedQueryManager;
        if (zIsQueryComplete) {
            complete = true;
            TrackedQuery trackedQuery = trackedQueryManager.findTrackedQuery(query);
    ...
    public void updateServerCache(QuerySpec query, Node node) {
        boolean zLoadsAllData = query.loadsAllData();
        PersistenceStorageEngine persistenceStorageEngine = this.storageLayer;
        if (zLoadsAllData) {
            persistenceStorageEngine.overwriteServerCache(query.getPath(), node);
        } else {
            persistenceStorageEngine.mergeIntoServerCache(query.getPath(), node);
        }
    ...
    public void setQueryComplete(QuerySpec query) {
        ...
        TrackedQueryManager trackedQueryManager = this.trackedQueryManager;
        if (zLoadsAllData) {
            trackedQueryManager.setQueriesComplete(query.getPath());
        } else {
            trackedQueryManager.setQueryCompleteIfExists(query);
        }
    }
    ...
    public void setTrackedQueryKeys(QuerySpec query, Set<ChildKey> keys) {
        ...
        Utilities.hardAssert(trackedQuery != null && trackedQuery.active, "We only expect tracked keys for currently-active queries.");
