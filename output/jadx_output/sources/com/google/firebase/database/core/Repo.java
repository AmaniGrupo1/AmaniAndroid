...
public class Repo implements PersistentConnection.Delegate {
    ...
    private void restoreWrites(PersistenceManager persistenceManager) {
        ...
        for (final UserWriteRecord write : writes) {
            ...
            boolean zIsOverwrite = write.isOverwrite();
    ...
    public void onRangeMergeUpdate(List<String> pathSegments, List<RangeMerge> merges, Long tagNumber) {
        ...
        if (tagNumber != null) {
            events = syncTree.applyTaggedRangeMerges(path, parsedMerges, new Tag(tagNumber.longValue()));
        } else {
            events = syncTree.applyServerRangeMerges(path, parsedMerges);
        }
        if (events.size() > 0) {
            rerunTransactions(path);
        }
    ...
    class AnonymousClass9 implements Runnable {
            ...
            if (serverValue == null) {
                ...
                task.addOnCompleteListener(executorService, new OnCompleteListener() { // from class: com.google.firebase.database.core.Repo$9$$ExternalSyntheticLambda1
                    ...
                    public final void onComplete(Task task2) {
                        this.f$0.m8318lambda$run$1$comgooglefirebasedatabasecoreRepo$9(taskCompletionSource2, persisted, query, repo2, task2);
                    }
                });
                return;
            }
        ...
        /* synthetic */ void m8318lambda$run$1$comgooglefirebasedatabasecoreRepo$9(TaskCompletionSource source, DataSnapshot persisted, Query query, Repo repo, Task task) {
            List<? extends Event> events;
            ...
            boolean zLoadsAllData = spec.loadsAllData();
            Repo repo2 = Repo.this;
            if (zLoadsAllData) {
                events = repo2.serverSyncTree.applyServerOverwrite(spec.getPath(), serverNode);
            } else {
                events = repo2.serverSyncTree.applyTaggedQueryOverwrite(spec.getPath(), serverNode, Repo.this.getServerSyncTree().tagForQuery(spec));
            }
                    ...
                    public void onRequestResult(String optErrorCode, String optErrorMessage) {
                        ...
                        if (error == null) {
                            ...
                            for (final TransactionData txn2 : queue) {
                                ...
                                Repo.this.removeEventCallback(new ValueEventRegistration(Repo.this, txn2.outstandingListener, QuerySpec.defaultQueryAtPath(txn2.path)));
                            }
                        ...
                        List<TransactionData> list = queue;
                        if (code == -1) {
