...
public class SyncTree {
    ...
    private class ListenContainer implements ListenHashProvider, CompletionListener {
        ...
        public List<? extends Event> onListenComplete(DatabaseError error) {
            ...
            if (tag != null) {
                return syncTree.applyTaggedListenComplete(this.tag);
            }
            return syncTree.applyListenComplete(query.getPath());
        }
    }
    ...
    /* synthetic */ Node m8319x4ba5d756(QuerySpec query) throws Exception {
        ...
        if (syncPoint == null) {
            ...
            if (serverCacheNode2 != null) {
                completeServerCache = serverCacheNode2;
            } else {
                completeServerCache = syncPoint.getCompleteServerCache(Path.getEmptyPath());
            }
            serverCacheNode2 = completeServerCache;
        }
        CacheNode serverCache = new CacheNode(IndexedNode.from(serverCacheNode2 != null ? serverCacheNode2 : EmptyNode.Empty(), query.getIndex()), serverCacheNode2 != null, false);
