...
public final class CompoundWrite implements Iterable<Map.Entry<Path, Node>> {
    ...
    public List<NamedNode> getCompleteChildren() {
        ...
        Node value = this.writeTree.getValue();
        ImmutableTree<Node> immutableTree = this.writeTree;
        if (value != null) {
            for (NamedNode entry : immutableTree.getValue()) {
                children.add(new NamedNode(entry.getName(), entry.getNode()));
            }
        } else {
            for (Map.Entry<ChildKey, ImmutableTree<Node>> entry2 : immutableTree.getChildren()) {
