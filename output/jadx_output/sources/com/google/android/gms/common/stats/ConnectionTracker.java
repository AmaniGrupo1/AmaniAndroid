...
public class ConnectionTracker {
    ...
    public boolean bindService(Context context, Intent intent, ServiceConnection conn, int flags) {
        return zzb(context, context.getClass().getName(), intent, conn, flags, true, null);
    }

    public void unbindService(Context context, ServiceConnection conn) {
        if (zzc(conn)) {
            ConcurrentHashMap concurrentHashMap = this.zza;
            if (concurrentHashMap.containsKey(conn)) {
                try {
                    zzd(context, (ServiceConnection) concurrentHashMap.get(conn));
                    return;
                } finally {
