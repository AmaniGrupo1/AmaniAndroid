    ...
    public static final class ScaleAndIndexes {
        ...
        public Map<Integer, Double> computeInPlace(double... dataset) {
            ...
            for (int i = 0; i < scaleAndIndexes.indexes.length; i++) {
                long numerator = ((long) scaleAndIndexes.indexes[i]) * ((long) (dataset.length - 1));
                int quotient = (int) LongMath.divide(numerator, scaleAndIndexes.scale, RoundingMode.DOWN);
            ...
            Quantiles.selectAllInPlace(requiredSelections2, 0, requiredSelectionsCount - 1, dataset, 0, dataset.length - 1);
            ...
            while (i2 < scaleAndIndexes.indexes.length) {
                ...
                int[] iArr = scaleAndIndexes.indexes;
                if (remainder2 == 0) {
                    ret.put(Integer.valueOf(iArr[i2]), Double.valueOf(dataset[quotient2]));
                    requiredSelections = requiredSelections2;
                } else {
                    requiredSelections = requiredSelections2;
                    ret.put(Integer.valueOf(iArr[i2]), Double.valueOf(Quantiles.interpolate(dataset[quotient2], dataset[quotient2 + 1], remainder2, scaleAndIndexes.scale)));
                }
                i2++;
