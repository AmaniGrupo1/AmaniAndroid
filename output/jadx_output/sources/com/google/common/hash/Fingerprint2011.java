...
final class Fingerprint2011 extends AbstractNonStreamingHashFunction {
    ...
    private static long fullFingerprint(byte[] bytes, int offset, int length) {
        ...
        long z = K0 ^ LittleEndianByteArray.load64(bytes, (offset + length) - 56);
        long[] v = new long[2];
        long[] w = new long[2];
        weakHashLength32WithSeeds(bytes, (offset + length) - 64, length, y, v);
        byte[] bArr = bytes;
        weakHashLength32WithSeeds(bArr, (offset + length) - 32, ((long) length) * K1, K0, w);
        ...
        int offset2 = offset;
        while (true) {
            long x4 = Long.rotateRight(x3 + y2 + v[0] + LittleEndianByteArray.load64(bArr, offset2 + 16), 37) * K1;
            long x5 = v[1];
            long y3 = Long.rotateRight(x5 + y2 + LittleEndianByteArray.load64(bArr, offset2 + 48), 42) * K1;
            long x6 = x4 ^ w[1];
            long y4 = y3 ^ v[0];
            long z3 = Long.rotateRight(w[0] ^ z2, 33);
            weakHashLength32WithSeeds(bArr, offset2, v[1] * K1, w[0] + x6, v);
            int offset3 = offset2;
            long[] w2 = w;
            y2 = y4;
            ...
            length2 -= 64;
            if (length2 == 0) {
                long jHash128to64 = hash128to64(v[0], w2[0]) + (shiftMix(y2) * K1) + z2;
                long j = v[1];
                long x7 = w2[1];
                return hash128to64(jHash128to64, hash128to64(j, x7) + x3);
            }
            bArr = bytes;
            w = w2;
            offset2 = offset4;
