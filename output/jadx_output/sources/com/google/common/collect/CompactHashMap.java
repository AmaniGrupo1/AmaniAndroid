...
class CompactHashMap<K, V> extends AbstractMap<K, V> implements Serializable {
    ...
    public V put(@ParametricNullness K k, @ParametricNullness V v) {
        ...
        if (iTableGet == 0) {
            ...
            while (true) {
                int i5 = iTableGet - 1;
                Map<K, V> map = mapDelegateOrNull;
                int i6 = iArrRequireEntries[i5];
                int[] iArr = iArrRequireEntries;
                if (CompactHashing.getHashPrefix(i6, iHashTableMask) == hashPrefix && Objects.equal(k2, objArrRequireKeys[i5])) {
                    V v3 = (V) objArrRequireValues[i5];
                    objArrRequireValues[i5] = v2;
                    accessEntry(i5);
                    return v3;
                }
                iTableGet = CompactHashing.getNext(i6, iHashTableMask);
                i4++;
                if (iTableGet != 0) {
                    k2 = k;
                    v2 = v;
                    mapDelegateOrNull = map;
                    iArrRequireEntries = iArr;
                } else {
                    if (i4 >= 9) {
                    ...
                    if (i2 > iHashTableMask) {
                        ...
                        iArr[i5] = CompactHashing.maskCombine(i6, i + 1, iHashTableMask);
                    }
                }
            }
        }
        resizeMeMaybe(i2);
        insertEntry(i, k2, v2, iSmearedHash, iHashTableMask);
        this.size = i2;
        incrementModCount();
        return null;
    }
