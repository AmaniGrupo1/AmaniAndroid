...
public final class HashBiMap<K, V> extends AbstractMap<K, V> implements BiMap<K, V>, Serializable {
    ...
    private void deleteFromTableKToV(int entry, int keyHash) {
        Preconditions.checkArgument(entry != -1);
        int keyBucket = bucket(keyHash);
        int i = this.hashTableKToV[keyBucket];
        int[] iArr = this.hashTableKToV;
        if (i == entry) {
            iArr[keyBucket] = this.nextInBucketKToV[entry];
        ...
        while (entryInBucket != -1) {
            ...
            if (entryInBucket == entry) {
                iArr2[prevInBucket] = this.nextInBucketKToV[entry];
                this.nextInBucketKToV[entry] = -1;
                return;
            } else {
                prevInBucket = entryInBucket;
                entryInBucket = iArr2[entryInBucket];
            }
        }
    ...
    private void deleteFromTableVToK(int entry, int valueHash) {
        Preconditions.checkArgument(entry != -1);
        int valueBucket = bucket(valueHash);
        int i = this.hashTableVToK[valueBucket];
        int[] iArr = this.hashTableVToK;
        if (i == entry) {
            iArr[valueBucket] = this.nextInBucketVToK[entry];
        ...
        while (entryInBucket != -1) {
            ...
            if (entryInBucket == entry) {
                iArr2[prevInBucket] = this.nextInBucketVToK[entry];
                this.nextInBucketVToK[entry] = -1;
                return;
            } else {
                prevInBucket = entryInBucket;
    ...
    private void moveEntryToIndex(int src, int dest) {
        ...
        if (i == src) {
            iArr3[keyBucket] = dest;
        } else {
            int prevInBucket = iArr3[keyBucket];
            int entryInBucket = this.nextInBucketKToV[prevInBucket];
            while (true) {
                ...
                prevInBucket = entryInBucket;
                entryInBucket = iArr[entryInBucket];
            }
            iArr[prevInBucket] = dest;
        }
        this.nextInBucketKToV[dest] = this.nextInBucketKToV[src];
        this.nextInBucketKToV[src] = -1;
        int valueHash = Hashing.smearedHash(value);
        int valueBucket = bucket(valueHash);
        int i2 = this.hashTableVToK[valueBucket];
        int[] iArr4 = this.hashTableVToK;
        if (i2 == src) {
            iArr4[valueBucket] = dest;
        } else {
            int prevInBucket2 = iArr4[valueBucket];
            ...
            while (true) {
                ...
                entryInBucket2 = iArr2[entryInBucket2];
            }
            iArr2[prevInBucket2] = dest;
        }
