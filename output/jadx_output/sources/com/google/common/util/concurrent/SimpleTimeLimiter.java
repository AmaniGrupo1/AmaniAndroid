...
public final class SimpleTimeLimiter implements TimeLimiter {
    ...
    public <T> T callWithTimeout(Callable<T> callable, long j, TimeUnit timeUnit, boolean z) throws Exception {
        ...
        boolean z2 = (T) (true ? 1 : 0);
        try {
            if (z) {
                z2 = futureSubmit.get(j, timeUnit);
            } else {
                z2 = (T) Uninterruptibles.getUninterruptibly(futureSubmit, j, timeUnit);
            }
            return (T) z2;
        } catch (InterruptedException e) {
            futureSubmit.cancel(z2);
            throw e;
        } catch (ExecutionException e2) {
            throw throwCause(e2, z2);
