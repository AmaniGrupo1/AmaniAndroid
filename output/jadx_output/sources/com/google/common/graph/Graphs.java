...
public final class Graphs extends GraphsBridgeMethods {
        ...
        while (!stack.isEmpty()) {
            ...
            if (top.remainingSuccessors == null) {
                ...
                if (state == NodeVisitState.COMPLETE) {
                    ...
                    top.remainingSuccessors = new ArrayDeque(graph.successors((Object) node));
                }
            }
    ...
    public static <N> ImmutableGraph<N> transitiveClosure(Graph<N> graph) {
        ...
        if (graph.isDirected()) {
            for (N node : graph.nodes()) {
                UnmodifiableIterator it = reachableNodes((Graph) graph, (Object) node).iterator();
                while (it.hasNext()) {
                    builderImmutable.putEdge(node, it.next());
                }
            }
        } else {
            ...
            for (N node2 : graph.nodes()) {
                if (!visitedNodes.contains(node2)) {
                    Set<N> reachableNodes = reachableNodes((Graph) graph, (Object) node2);
                    visitedNodes.addAll(reachableNodes);
                    int pairwiseMatch = 1;
                    for (N nodeU : reachableNodes) {
                        int pairwiseMatch2 = pairwiseMatch + 1;
                        Iterator it2 = Iterables.limit(reachableNodes, pairwiseMatch).iterator();
                        while (it2.hasNext()) {
                            builderImmutable.putEdge(nodeU, it2.next());
                        }
    ...
    public static <N, E> MutableNetwork<N, E> inducedSubgraph(Network<N, E> network, Iterable<? extends N> iterable) {
        ...
        for (?? r2 : mutableNetwork.nodes()) {
            for (E e : network.outEdges(r2)) {
                Object objAdjacentNode = network.incidentNodes(e).adjacentNode(r2);
                if (mutableNetwork.nodes().contains(objAdjacentNode)) {
                    mutableNetwork.addEdge(r2, objAdjacentNode, e);
                }
            }
        }
