...
final class RequestBuilder {
    ...
    private static void canonicalizeForPath(Buffer out, String input, int pos, int limit, boolean alreadyEncoded) throws EOFException {
        ...
        while (i < limit) {
            ...
            if (!alreadyEncoded || (codePoint != 9 && codePoint != 10 && codePoint != 12 && codePoint != 13)) {
                if (codePoint < 32 || codePoint >= 127 || PATH_SEGMENT_ALWAYS_ENCODE_SET.indexOf(codePoint) != -1 || (!alreadyEncoded && (codePoint == 47 || codePoint == 37))) {
                    ...
                    long size = utf8Buffer.size();
                    for (long j = 0; j < size; j++) {
                        int b = utf8Buffer.getByte(j) & 255;
                        out.writeByte(37);
    ...
    Request.Builder get() {
        ...
        if (contentType != null) {
            if (body != null) {
                ...
                this.headersBuilder.add(HttpHeaders.CONTENT_TYPE, contentType.getMediaType());
            }
        }
