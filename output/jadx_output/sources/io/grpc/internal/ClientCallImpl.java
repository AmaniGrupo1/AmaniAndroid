...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private void startInternal(final ClientCall.Listener<RespT> observer, Metadata headers) {
        ...
        if (!deadlineExceeded) {
            ...
            String description = String.format("ClientCall started after %s deadline was exceeded %.9f seconds ago. Name resolution delay %.9f seconds.", deadlineName, Double.valueOf(effectiveDeadline.timeRemaining(TimeUnit.NANOSECONDS) / NANO_TO_SECS), Double.valueOf(nameResolutionDelay == null ? 0.0d : nameResolutionDelay.longValue() / NANO_TO_SECS));
    ...
    private void applyMethodConfig() {
        ...
        if (info.waitForReady != null) {
            boolean zBooleanValue = info.waitForReady.booleanValue();
            CallOptions callOptions = this.callOptions;
            this.callOptions = zBooleanValue ? callOptions.withWaitForReady() : callOptions.withoutWaitForReady();
        }
        if (info.maxInboundMessageSize != null) {
            ...
            if (existingLimit != null) {
                this.callOptions = callOptions2.withMaxInboundMessageSize(Math.min(existingLimit.intValue(), info.maxInboundMessageSize.intValue()));
            } else {
                this.callOptions = callOptions2.withMaxInboundMessageSize(info.maxInboundMessageSize.intValue());
            }
        }
        if (info.maxOutboundMessageSize != null) {
            ...
            if (existingLimit2 != null) {
                this.callOptions = callOptions3.withMaxOutboundMessageSize(Math.min(existingLimit2.intValue(), info.maxOutboundMessageSize.intValue()));
            } else {
                this.callOptions = callOptions3.withMaxOutboundMessageSize(info.maxOutboundMessageSize.intValue());
            }
        }
    }
    ...
    private class DeadlineTimer implements Runnable {
        ...
        public void run() {
            ...
            buf.append(String.format(Locale.US, "Name resolution delay %.9f seconds. ", Double.valueOf(nsDelay == null ? 0.0d : nsDelay.longValue() / ClientCallImpl.NANO_TO_SECS)));
            buf.append(insight);
    ...
    private void sendMessageInternal(ReqT message) {
        ...
        try {
            boolean z = this.stream instanceof RetriableStream;
            ClientStream clientStream = this.stream;
            if (z) {
                RetriableStream<ReqT> retriableStream = (RetriableStream) clientStream;
        ...
        public void headersRead(final Metadata headers) {
            ...
            try {
                ...
                ClientCallImpl.this.callExecutor.execute(new ContextRunnable() { // from class: io.grpc.internal.ClientCallImpl.ClientStreamListenerImpl.1HeadersRead
                    ...
                    @Override // io.grpc.internal.ContextRunnable
                    public void runInContext() {
                        TaskCloseable ignore2 = PerfMark.traceTask("ClientCall$Listener.headersRead");
                        try {
                            PerfMark.attachTag(ClientCallImpl.this.tag);
        ...
        public void messagesAvailable(final StreamListener.MessageProducer producer) {
            ...
            try {
                ...
                ClientCallImpl.this.callExecutor.execute(new ContextRunnable() { // from class: io.grpc.internal.ClientCallImpl.ClientStreamListenerImpl.1MessagesAvailable
                    ...
                    @Override // io.grpc.internal.ContextRunnable
                    public void runInContext() {
                        TaskCloseable ignore2 = PerfMark.traceTask("ClientCall$Listener.messagesAvailable");
                        try {
                            PerfMark.attachTag(ClientCallImpl.this.tag);
                            PerfMark.linkIn(link);
                            runInternal();
        ...
        public void onReady() {
            ...
            try {
                ...
                ClientCallImpl.this.callExecutor.execute(new ContextRunnable() { // from class: io.grpc.internal.ClientCallImpl.ClientStreamListenerImpl.1StreamOnReady
                    ...
                    @Override // io.grpc.internal.ContextRunnable
                    public void runInContext() {
                        TaskCloseable ignore2 = PerfMark.traceTask("ClientCall$Listener.onReady");
                        try {
                            PerfMark.attachTag(ClientCallImpl.this.tag);
                            PerfMark.linkIn(link);
                            runInternal();
