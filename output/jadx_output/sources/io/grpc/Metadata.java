...
public final class Metadata {
    ...
    public <T> void put(Key<T> key, T value) {
        ...
        boolean zSerializesToStreams = key.serializesToStreams();
        int i = this.size;
        if (zSerializesToStreams) {
            value(i, LazyValue.create(key, value));
        } else {
            value(i, key.toBytes(value));
        }
        this.size++;
    }
    ...
    public <T> Iterable<T> removeAll(Key<T> key) {
        ...
        for (int readIdx = 0; readIdx < this.size; readIdx++) {
            if (bytesEqual(key.asciiName(), name(readIdx))) {
                ...
                arrayList2.add(valueAsT(readIdx, key));
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.size; i++) {
            ...
            if (headerName.endsWith(BINARY_HEADER_SUFFIX)) {
                ...
                String headerValue = new String(valueAsBytes(i), Charsets.US_ASCII);
    ...
    public static abstract class Key<T> {
        ...
        private static String validateName(String n, boolean pseudo) {
            Preconditions.checkNotNull(n, HintConstants.AUTOFILL_HINT_NAME);
        ...
        private Key(String name, boolean pseudo, Object marshaller) {
            this.originalName = (String) Preconditions.checkNotNull(name, HintConstants.AUTOFILL_HINT_NAME);
    ...
    private static class AsciiKey<T> extends Key<T> {
        ...
        @Override // io.grpc.Metadata.Key
        byte[] toBytes(T value) {
            String encoded = (String) Preconditions.checkNotNull(this.marshaller.toAsciiString(value), "null marshaller.toAsciiString()");
