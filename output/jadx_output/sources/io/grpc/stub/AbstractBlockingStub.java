...
public abstract class AbstractBlockingStub<S extends AbstractBlockingStub<S>> extends AbstractStub<S> {
    ...
    public static <T extends AbstractStub<T>> T newStub(AbstractStub.StubFactory<T> stubFactory, Channel channel, CallOptions callOptions) {
        T t = (T) stubFactory.newStub(channel, callOptions.withOption(ClientCalls.STUB_TYPE_OPTION, ClientCalls.StubType.BLOCKING));
        if (!(t instanceof AbstractBlockingStub)) {
            throw new AssertionError(String.format("Expected AbstractBlockingStub, but got %s.", t.getClass()));
        }
        return t;
    }
}
