...
public final class Http2 implements Variant {
    ...
    static final class Reader implements FrameReader {
        ...
        private void readData(FrameReader.Handler handler, int paddedLength, byte flags, int streamId) throws IOException {
            ...
            short padding = (flags & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
            int length = Http2.lengthWithoutPadding(paddedLength, flags, padding);
            handler.data(inFinished, streamId, this.source, length, paddedLength);
        ...
        private void readWindowUpdate(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            long increment = ((long) this.source.readInt()) & SieveCacheKt.NodeLinkMask;
            if (increment == 0) {
    ...
    static final class FrameLogger {
        ...
        static String formatHeader(boolean inbound, int streamId, int length, byte type, byte flags) {
            ...
            return String.format(Locale.US, "%s 0x%08x %5d %-13s %s", inbound ? "<<" : ">>", Integer.valueOf(streamId), Integer.valueOf(length), formattedType, formattedFlags);
        }
