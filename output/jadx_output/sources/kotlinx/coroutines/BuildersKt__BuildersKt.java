    ...
    */
    public static final <T> T runBlocking(CoroutineContext coroutineContext, Function2<? super CoroutineScope, ? super Continuation<? super T>, ? extends Object> function2) throws InterruptedException {
        ...
        if (continuationInterceptor == null) {
            ...
            coroutineContextNewCoroutineContext = CoroutineContextKt.newCoroutineContext(GlobalScope.INSTANCE, coroutineContext.plus(eventLoop$kotlinx_coroutines_core));
        } else {
            EventLoop eventLoop = continuationInterceptor instanceof EventLoop ? (EventLoop) continuationInterceptor : null;
            if (eventLoop != null) {
                eventLoopCurrentOrNull$kotlinx_coroutines_core = eventLoop.shouldBeProcessedFromContext() ? eventLoop : null;
                if (eventLoopCurrentOrNull$kotlinx_coroutines_core == null) {
                }
                eventLoop$kotlinx_coroutines_core = eventLoopCurrentOrNull$kotlinx_coroutines_core;
        ...
        BlockingCoroutine blockingCoroutine = new BlockingCoroutine(coroutineContextNewCoroutineContext, threadCurrentThread, eventLoop$kotlinx_coroutines_core);
