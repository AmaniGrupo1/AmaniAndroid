...
public class SemaphoreAndMutexImpl {
    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m10118getSegmentimpl(s$iv);
        int i = (int) (enqIdx % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (!Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(segment.getAcquirers(), i, null, waiter)) {
            Object expected$iv = SemaphoreKt.PERMIT;
            Object value$iv = SemaphoreKt.TAKEN;
            if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(segment.getAcquirers(), i, expected$iv, value$iv)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m10120isClosedimpl(objFindSegmentInternal)) {
                Segment segmentM10118getSegmentimpl = SegmentOrClosed.m10118getSegmentimpl(objFindSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= segmentM10118getSegmentimpl.id) {
                        z3 = true;
                        break;
                    }
                    if (!segmentM10118getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z3 = false;
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, segmentM10118getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        z3 = true;
                    } else if (segmentM10118getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM10118getSegmentimpl.remove();
                    }
                }
                if (z3) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m10118getSegmentimpl(objFindSegmentInternal);
