...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m10007plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m9932toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m10006overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m9896divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m9932toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m10007plusAssignLRDsOJo(half);
                m10007plusAssignLRDsOJo(Duration.m9921minusLRDsOJo(duration, half));
                return;
    ...
    private final void m10006overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m9933toStringimpl(duration)) + '.');
    }
}
