.class public final Landroidx/compose/ui/layout/ScaleFactorKt;
.super Ljava/lang/Object;
.source "ScaleFactor.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nScaleFactor.kt\nKotlin\n*S Kotlin\n*F\n+ 1 ScaleFactor.kt\nandroidx/compose/ui/layout/ScaleFactorKt\n+ 2 InlineClassHelper.kt\nandroidx/compose/ui/util/InlineClassHelperKt\n+ 3 Size.kt\nandroidx/compose/ui/geometry/Size\n+ 4 InlineClassHelper.jvm.kt\nandroidx/compose/ui/util/InlineClassHelper_jvmKt\n+ 5 ScaleFactor.kt\nandroidx/compose/ui/layout/ScaleFactor\n+ 6 Size.kt\nandroidx/compose/ui/geometry/SizeKt\n*L\n1#1,150:1\n87#1:154\n31#1:183\n53#2,3:151\n60#2:156\n70#2:160\n53#2,3:163\n60#2:167\n70#2:171\n53#2,3:174\n60#2:178\n70#2:181\n53#2,3:184\n57#3:155\n61#3:159\n57#3:166\n61#3:170\n22#4:157\n22#4:168\n22#4:179\n22#4:182\n41#5:158\n46#5:161\n41#5:169\n46#5:172\n41#5:177\n46#5:180\n33#6:162\n33#6:173\n*S KotlinDebug\n*F\n+ 1 ScaleFactor.kt\nandroidx/compose/ui/layout/ScaleFactorKt\n*L\n99#1:154\n145#1:183\n31#1:151,3\n109#1:156\n109#1:160\n109#1:163,3\n128#1:167\n128#1:171\n128#1:174,3\n146#1:178\n147#1:181\n145#1:184,3\n109#1:155\n109#1:159\n128#1:166\n128#1:170\n109#1:157\n128#1:168\n146#1:179\n147#1:182\n109#1:158\n109#1:161\n128#1:169\n128#1:172\n146#1:177\n147#1:180\n109#1:162\n128#1:173\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000(\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0007\n\u0002\u0008\u0003\n\u0002\u0010\u000b\n\u0002\u0008\t\n\u0002\u0018\u0002\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0002\u0008\u000e\u001a\u001e\u0010\u0000\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u00032\u0006\u0010\u0004\u001a\u00020\u0003H\u0087\u0008\u00a2\u0006\u0002\u0010\u0005\u001a\"\u0010\u000f\u001a\u00020\u0001*\u00020\u00012\u000c\u0010\u0010\u001a\u0008\u0012\u0004\u0012\u00020\u00010\u0011H\u0086\u0008\u00a2\u0006\u0004\u0008\u0012\u0010\u0013\u001a\u001c\u0010\u0014\u001a\u00020\u0015*\u00020\u00152\u0006\u0010\u0016\u001a\u00020\u0001H\u0087\u0002\u00a2\u0006\u0004\u0008\u0017\u0010\u0018\u001a\u001c\u0010\u0014\u001a\u00020\u0015*\u00020\u00012\u0006\u0010\u0019\u001a\u00020\u0015H\u0087\u0002\u00a2\u0006\u0004\u0008\u001a\u0010\u0018\u001a\u001c\u0010\u001b\u001a\u00020\u0015*\u00020\u00152\u0006\u0010\u0016\u001a\u00020\u0001H\u0087\u0002\u00a2\u0006\u0004\u0008\u001c\u0010\u0018\u001a\'\u0010\u001d\u001a\u00020\u00012\u0006\u0010\u001e\u001a\u00020\u00012\u0006\u0010\u001f\u001a\u00020\u00012\u0006\u0010 \u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008!\u0010\"\"\u001f\u0010\u0006\u001a\u00020\u0007*\u00020\u00018\u00c6\u0002X\u0087\u0004\u00a2\u0006\u000c\u0012\u0004\u0008\u0008\u0010\t\u001a\u0004\u0008\n\u0010\u000b\"\u001f\u0010\u000c\u001a\u00020\u0007*\u00020\u00018\u00c6\u0002X\u0087\u0004\u00a2\u0006\u000c\u0012\u0004\u0008\r\u0010\t\u001a\u0004\u0008\u000e\u0010\u000b\u00a8\u0006#"
    }
    d2 = {
        "ScaleFactor",
        "Landroidx/compose/ui/layout/ScaleFactor;",
        "scaleX",
        "",
        "scaleY",
        "(FF)J",
        "isSpecified",
        "",
        "isSpecified-FK8aYYs$annotations",
        "(J)V",
        "isSpecified-FK8aYYs",
        "(J)Z",
        "isUnspecified",
        "isUnspecified-FK8aYYs$annotations",
        "isUnspecified-FK8aYYs",
        "takeOrElse",
        "block",
        "Lkotlin/Function0;",
        "takeOrElse-oyDd2qo",
        "(JLkotlin/jvm/functions/Function0;)J",
        "times",
        "Landroidx/compose/ui/geometry/Size;",
        "scaleFactor",
        "times-UQTWf7w",
        "(JJ)J",
        "size",
        "times-m-w2e94",
        "div",
        "div-UQTWf7w",
        "lerp",
        "start",
        "stop",
        "fraction",
        "lerp--bDIf60",
        "(JJF)J",
        "ui"
    }
    k = 0x2
    mv = {
        0x2,
        0x0,
        0x0
    }
    xi = 0x30
.end annotation


# direct methods
.method public static final ScaleFactor(FF)J
    .locals 12
    .param p0, "scaleX"    # F
    .param p1, "scaleY"    # F

    const/4 v0, 0x0

    .line 31
    .local v0, "$i$f$ScaleFactor":I
    move v1, p1

    .local v1, "val2$iv":F
    move v2, p0

    .local v2, "val1$iv":F
    const/4 v3, 0x0

    .line 151
    .local v3, "$i$f$packFloats":I
    invoke-static {v2}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v4

    int-to-long v4, v4

    .line 152
    .local v4, "v1$iv":J
    invoke-static {v1}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v6

    int-to-long v6, v6

    .line 153
    .local v6, "v2$iv":J
    const/16 v8, 0x20

    shl-long v8, v4, v8

    const-wide v10, 0xffffffffL

    and-long/2addr v10, v6

    or-long v1, v8, v10

    .line 31
    .end local v1    # "val2$iv":F
    .end local v2    # "val1$iv":F
    .end local v3    # "$i$f$packFloats":I
    .end local v4    # "v1$iv":J
    .end local v6    # "v2$iv":J
    invoke-static {v1, v2}, Landroidx/compose/ui/layout/ScaleFactor;->constructor-impl(J)J

    move-result-wide v1

    return-wide v1
.end method

.method public static final div-UQTWf7w(JJ)J
    .locals 15
    .param p0, "$v$c$androidx-compose-ui-geometry-Size$-$this$div$0"    # J
    .param p2, "$v$c$androidx-compose-ui-layout-ScaleFactor$-scaleFactor$0"    # J

    .line 128
    move-wide v0, p0

    .local v0, "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    const/4 v2, 0x0

    .line 166
    .local v2, "$i$f$getWidth-impl":I
    move-wide v3, v0

    .local v3, "value$iv$iv":J
    const/4 v5, 0x0

    .line 167
    .local v5, "$i$f$unpackFloat1":I
    const/16 v6, 0x20

    shr-long v7, v3, v6

    long-to-int v7, v7

    .local v7, "bits$iv$iv$iv":I
    const/4 v8, 0x0

    .line 168
    .local v8, "$i$f$floatFromBits":I
    invoke-static {v7}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v7

    .line 167
    .end local v7    # "bits$iv$iv$iv":I
    .end local v8    # "$i$f$floatFromBits":I
    nop

    .line 166
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat1":I
    nop

    .line 128
    .end local v0    # "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    .end local v2    # "$i$f$getWidth-impl":I
    move-wide/from16 v0, p2

    .local v0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v2, 0x0

    .line 169
    .local v2, "$i$f$getScaleX-impl":I
    move-wide v3, v0

    .restart local v3    # "value$iv$iv":J
    const/4 v5, 0x0

    .line 167
    .restart local v5    # "$i$f$unpackFloat1":I
    shr-long v8, v3, v6

    long-to-int v8, v8

    .local v8, "bits$iv$iv$iv":I
    const/4 v9, 0x0

    .line 168
    .local v9, "$i$f$floatFromBits":I
    invoke-static {v8}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v8

    .line 167
    .end local v8    # "bits$iv$iv$iv":I
    .end local v9    # "$i$f$floatFromBits":I
    nop

    .line 169
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat1":I
    nop

    .line 128
    .end local v0    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v2    # "$i$f$getScaleX-impl":I
    div-float/2addr v7, v8

    move-wide v0, p0

    .local v0, "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    const/4 v2, 0x0

    .line 170
    .local v2, "$i$f$getHeight-impl":I
    move-wide v3, v0

    .restart local v3    # "value$iv$iv":J
    const/4 v5, 0x0

    .line 171
    .local v5, "$i$f$unpackFloat2":I
    const-wide v8, 0xffffffffL

    and-long v10, v3, v8

    long-to-int v10, v10

    .local v10, "bits$iv$iv$iv":I
    const/4 v11, 0x0

    .line 168
    .local v11, "$i$f$floatFromBits":I
    invoke-static {v10}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v10

    .line 171
    .end local v10    # "bits$iv$iv$iv":I
    .end local v11    # "$i$f$floatFromBits":I
    nop

    .line 170
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat2":I
    nop

    .line 128
    .end local v0    # "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    .end local v2    # "$i$f$getHeight-impl":I
    move-wide/from16 v0, p2

    .local v0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v2, 0x0

    .line 172
    .local v2, "$i$f$getScaleY-impl":I
    move-wide v3, v0

    .restart local v3    # "value$iv$iv":J
    const/4 v5, 0x0

    .line 171
    .restart local v5    # "$i$f$unpackFloat2":I
    and-long v11, v3, v8

    long-to-int v11, v11

    .local v11, "bits$iv$iv$iv":I
    const/4 v12, 0x0

    .line 168
    .local v12, "$i$f$floatFromBits":I
    invoke-static {v11}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v11

    .line 171
    .end local v11    # "bits$iv$iv$iv":I
    .end local v12    # "$i$f$floatFromBits":I
    nop

    .line 172
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat2":I
    nop

    .line 128
    .end local v0    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v2    # "$i$f$getScaleY-impl":I
    div-float/2addr v10, v11

    .local v7, "width$iv":F
    .local v10, "height$iv":F
    const/4 v0, 0x0

    .line 173
    .local v0, "$i$f$Size":I
    move v1, v10

    .local v1, "val2$iv$iv":F
    move v2, v7

    .local v2, "val1$iv$iv":F
    const/4 v3, 0x0

    .line 174
    .local v3, "$i$f$packFloats":I
    invoke-static {v2}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v4

    int-to-long v4, v4

    .line 175
    .local v4, "v1$iv$iv":J
    invoke-static {v1}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v11

    int-to-long v11, v11

    .line 176
    .local v11, "v2$iv$iv":J
    shl-long v13, v4, v6

    and-long/2addr v8, v11

    or-long v1, v13, v8

    .line 173
    .end local v1    # "val2$iv$iv":F
    .end local v2    # "val1$iv$iv":F
    .end local v3    # "$i$f$packFloats":I
    .end local v4    # "v1$iv$iv":J
    .end local v11    # "v2$iv$iv":J
    invoke-static {v1, v2}, Landroidx/compose/ui/geometry/Size;->constructor-impl(J)J

    move-result-wide v0

    .line 128
    .end local v0    # "$i$f$Size":I
    .end local v7    # "width$iv":F
    .end local v10    # "height$iv":F
    return-wide v0
.end method

.method public static final isSpecified-FK8aYYs(J)Z
    .locals 3
    .param p0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-$this$isSpecified$0"    # J

    const/4 v0, 0x0

    .line 87
    .local v0, "$i$f$isSpecified-FK8aYYs":I
    sget-object v1, Landroidx/compose/ui/layout/ScaleFactor;->Companion:Landroidx/compose/ui/layout/ScaleFactor$Companion;

    invoke-virtual {v1}, Landroidx/compose/ui/layout/ScaleFactor$Companion;->getUnspecified-_hLwfpc()J

    move-result-wide v1

    cmp-long v1, p0, v1

    if-eqz v1, :cond_0

    const/4 v1, 0x1

    goto :goto_0

    :cond_0
    const/4 v1, 0x0

    :goto_0
    return v1
.end method

.method public static synthetic isSpecified-FK8aYYs$annotations(J)V
    .locals 0

    return-void
.end method

.method public static final isUnspecified-FK8aYYs(J)Z
    .locals 3
    .param p0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-$this$isUnspecified$0"    # J

    const/4 v0, 0x0

    .line 92
    .local v0, "$i$f$isUnspecified-FK8aYYs":I
    sget-object v1, Landroidx/compose/ui/layout/ScaleFactor;->Companion:Landroidx/compose/ui/layout/ScaleFactor$Companion;

    invoke-virtual {v1}, Landroidx/compose/ui/layout/ScaleFactor$Companion;->getUnspecified-_hLwfpc()J

    move-result-wide v1

    cmp-long v1, p0, v1

    if-nez v1, :cond_0

    const/4 v1, 0x1

    goto :goto_0

    :cond_0
    const/4 v1, 0x0

    :goto_0
    return v1
.end method

.method public static synthetic isUnspecified-FK8aYYs$annotations(J)V
    .locals 0

    return-void
.end method

.method public static final lerp--bDIf60(JJF)J
    .locals 15
    .param p0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-start$0"    # J
    .param p2, "$v$c$androidx-compose-ui-layout-ScaleFactor$-stop$0"    # J
    .param p4, "fraction"    # F

    .line 146
    move/from16 v0, p4

    move-wide v1, p0

    .local v1, "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v3, 0x0

    .line 177
    .local v3, "$i$f$getScaleX-impl":I
    move-wide v4, v1

    .local v4, "value$iv$iv":J
    const/4 v6, 0x0

    .line 178
    .local v6, "$i$f$unpackFloat1":I
    const/16 v7, 0x20

    shr-long v8, v4, v7

    long-to-int v8, v8

    .local v8, "bits$iv$iv$iv":I
    const/4 v9, 0x0

    .line 179
    .local v9, "$i$f$floatFromBits":I
    invoke-static {v8}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v8

    .line 178
    .end local v8    # "bits$iv$iv$iv":I
    .end local v9    # "$i$f$floatFromBits":I
    nop

    .line 177
    .end local v4    # "value$iv$iv":J
    .end local v6    # "$i$f$unpackFloat1":I
    nop

    .line 146
    .end local v1    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v3    # "$i$f$getScaleX-impl":I
    move-wide/from16 v1, p2

    .restart local v1    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v3, 0x0

    .line 177
    .restart local v3    # "$i$f$getScaleX-impl":I
    move-wide v4, v1

    .restart local v4    # "value$iv$iv":J
    const/4 v6, 0x0

    .line 178
    .restart local v6    # "$i$f$unpackFloat1":I
    shr-long v9, v4, v7

    long-to-int v9, v9

    .local v9, "bits$iv$iv$iv":I
    const/4 v10, 0x0

    .line 179
    .local v10, "$i$f$floatFromBits":I
    invoke-static {v9}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v9

    .line 178
    .end local v9    # "bits$iv$iv$iv":I
    .end local v10    # "$i$f$floatFromBits":I
    nop

    .line 177
    .end local v4    # "value$iv$iv":J
    .end local v6    # "$i$f$unpackFloat1":I
    nop

    .line 146
    .end local v1    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v3    # "$i$f$getScaleX-impl":I
    invoke-static {v8, v9, v0}, Landroidx/compose/ui/util/MathHelpersKt;->lerp(FFF)F

    move-result v1

    .line 147
    move-wide v2, p0

    .local v2, "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v4, 0x0

    .line 180
    .local v4, "$i$f$getScaleY-impl":I
    move-wide v5, v2

    .local v5, "value$iv$iv":J
    const/4 v8, 0x0

    .line 181
    .local v8, "$i$f$unpackFloat2":I
    const-wide v9, 0xffffffffL

    and-long v11, v5, v9

    long-to-int v11, v11

    .local v11, "bits$iv$iv$iv":I
    const/4 v12, 0x0

    .line 182
    .local v12, "$i$f$floatFromBits":I
    invoke-static {v11}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v11

    .line 181
    .end local v11    # "bits$iv$iv$iv":I
    .end local v12    # "$i$f$floatFromBits":I
    nop

    .line 180
    .end local v5    # "value$iv$iv":J
    .end local v8    # "$i$f$unpackFloat2":I
    nop

    .line 147
    .end local v2    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v4    # "$i$f$getScaleY-impl":I
    move-wide/from16 v2, p2

    .restart local v2    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v4, 0x0

    .line 180
    .restart local v4    # "$i$f$getScaleY-impl":I
    move-wide v5, v2

    .restart local v5    # "value$iv$iv":J
    const/4 v8, 0x0

    .line 181
    .restart local v8    # "$i$f$unpackFloat2":I
    and-long v12, v5, v9

    long-to-int v12, v12

    .local v12, "bits$iv$iv$iv":I
    const/4 v13, 0x0

    .line 182
    .local v13, "$i$f$floatFromBits":I
    invoke-static {v12}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v12

    .line 181
    .end local v12    # "bits$iv$iv$iv":I
    .end local v13    # "$i$f$floatFromBits":I
    nop

    .line 180
    .end local v5    # "value$iv$iv":J
    .end local v8    # "$i$f$unpackFloat2":I
    nop

    .line 147
    .end local v2    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v4    # "$i$f$getScaleY-impl":I
    invoke-static {v11, v12, v0}, Landroidx/compose/ui/util/MathHelpersKt;->lerp(FFF)F

    move-result v2

    .line 145
    nop

    .local v1, "scaleX$iv":F
    .local v2, "scaleY$iv":F
    const/4 v3, 0x0

    .line 183
    .local v3, "$i$f$ScaleFactor":I
    move v4, v2

    .local v4, "val2$iv$iv":F
    move v5, v1

    .local v5, "val1$iv$iv":F
    const/4 v6, 0x0

    .line 184
    .local v6, "$i$f$packFloats":I
    invoke-static {v5}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v8

    int-to-long v11, v8

    .line 185
    .local v11, "v1$iv$iv":J
    invoke-static {v4}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v8

    int-to-long v13, v8

    .line 186
    .local v13, "v2$iv$iv":J
    shl-long v7, v11, v7

    and-long/2addr v9, v13

    or-long v4, v7, v9

    .line 183
    .end local v4    # "val2$iv$iv":F
    .end local v5    # "val1$iv$iv":F
    .end local v6    # "$i$f$packFloats":I
    .end local v11    # "v1$iv$iv":J
    .end local v13    # "v2$iv$iv":J
    invoke-static {v4, v5}, Landroidx/compose/ui/layout/ScaleFactor;->constructor-impl(J)J

    move-result-wide v1

    .line 145
    .end local v1    # "scaleX$iv":F
    .end local v2    # "scaleY$iv":F
    .end local v3    # "$i$f$ScaleFactor":I
    return-wide v1
.end method

.method public static final takeOrElse-oyDd2qo(JLkotlin/jvm/functions/Function0;)J
    .locals 6
    .param p0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-$this$takeOrElse$0"    # J
    .param p2, "block"    # Lkotlin/jvm/functions/Function0;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(J",
            "Lkotlin/jvm/functions/Function0<",
            "Landroidx/compose/ui/layout/ScaleFactor;",
            ">;)J"
        }
    .end annotation

    const/4 v0, 0x0

    .line 99
    .local v0, "$i$f$takeOrElse-oyDd2qo":I
    move-wide v1, p0

    .local v1, "$v$c$androidx-compose-ui-layout-ScaleFactor$-$this$isSpecified$0$iv":J
    const/4 v3, 0x0

    .line 154
    .local v3, "$i$f$isSpecified-FK8aYYs":I
    sget-object v4, Landroidx/compose/ui/layout/ScaleFactor;->Companion:Landroidx/compose/ui/layout/ScaleFactor$Companion;

    invoke-virtual {v4}, Landroidx/compose/ui/layout/ScaleFactor$Companion;->getUnspecified-_hLwfpc()J

    move-result-wide v4

    cmp-long v4, v1, v4

    if-eqz v4, :cond_0

    const/4 v4, 0x1

    goto :goto_0

    :cond_0
    const/4 v4, 0x0

    .line 99
    .end local v1    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-$this$isSpecified$0$iv":J
    .end local v3    # "$i$f$isSpecified-FK8aYYs":I
    :goto_0
    if-eqz v4, :cond_1

    move-wide v1, p0

    goto :goto_1

    :cond_1
    invoke-interface {p2}, Lkotlin/jvm/functions/Function0;->invoke()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Landroidx/compose/ui/layout/ScaleFactor;

    invoke-virtual {v1}, Landroidx/compose/ui/layout/ScaleFactor;->unbox-impl()J

    move-result-wide v1

    :goto_1
    return-wide v1
.end method

.method public static final times-UQTWf7w(JJ)J
    .locals 15
    .param p0, "$v$c$androidx-compose-ui-geometry-Size$-$this$times$0"    # J
    .param p2, "$v$c$androidx-compose-ui-layout-ScaleFactor$-scaleFactor$0"    # J

    .line 109
    move-wide v0, p0

    .local v0, "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    const/4 v2, 0x0

    .line 155
    .local v2, "$i$f$getWidth-impl":I
    move-wide v3, v0

    .local v3, "value$iv$iv":J
    const/4 v5, 0x0

    .line 156
    .local v5, "$i$f$unpackFloat1":I
    const/16 v6, 0x20

    shr-long v7, v3, v6

    long-to-int v7, v7

    .local v7, "bits$iv$iv$iv":I
    const/4 v8, 0x0

    .line 157
    .local v8, "$i$f$floatFromBits":I
    invoke-static {v7}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v7

    .line 156
    .end local v7    # "bits$iv$iv$iv":I
    .end local v8    # "$i$f$floatFromBits":I
    nop

    .line 155
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat1":I
    nop

    .line 109
    .end local v0    # "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    .end local v2    # "$i$f$getWidth-impl":I
    move-wide/from16 v0, p2

    .local v0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v2, 0x0

    .line 158
    .local v2, "$i$f$getScaleX-impl":I
    move-wide v3, v0

    .restart local v3    # "value$iv$iv":J
    const/4 v5, 0x0

    .line 156
    .restart local v5    # "$i$f$unpackFloat1":I
    shr-long v8, v3, v6

    long-to-int v8, v8

    .local v8, "bits$iv$iv$iv":I
    const/4 v9, 0x0

    .line 157
    .local v9, "$i$f$floatFromBits":I
    invoke-static {v8}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v8

    .line 156
    .end local v8    # "bits$iv$iv$iv":I
    .end local v9    # "$i$f$floatFromBits":I
    nop

    .line 158
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat1":I
    nop

    .line 109
    .end local v0    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v2    # "$i$f$getScaleX-impl":I
    mul-float/2addr v7, v8

    move-wide v0, p0

    .local v0, "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    const/4 v2, 0x0

    .line 159
    .local v2, "$i$f$getHeight-impl":I
    move-wide v3, v0

    .restart local v3    # "value$iv$iv":J
    const/4 v5, 0x0

    .line 160
    .local v5, "$i$f$unpackFloat2":I
    const-wide v8, 0xffffffffL

    and-long v10, v3, v8

    long-to-int v10, v10

    .local v10, "bits$iv$iv$iv":I
    const/4 v11, 0x0

    .line 157
    .local v11, "$i$f$floatFromBits":I
    invoke-static {v10}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v10

    .line 160
    .end local v10    # "bits$iv$iv$iv":I
    .end local v11    # "$i$f$floatFromBits":I
    nop

    .line 159
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat2":I
    nop

    .line 109
    .end local v0    # "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    .end local v2    # "$i$f$getHeight-impl":I
    move-wide/from16 v0, p2

    .local v0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    const/4 v2, 0x0

    .line 161
    .local v2, "$i$f$getScaleY-impl":I
    move-wide v3, v0

    .restart local v3    # "value$iv$iv":J
    const/4 v5, 0x0

    .line 160
    .restart local v5    # "$i$f$unpackFloat2":I
    and-long v11, v3, v8

    long-to-int v11, v11

    .local v11, "bits$iv$iv$iv":I
    const/4 v12, 0x0

    .line 157
    .local v12, "$i$f$floatFromBits":I
    invoke-static {v11}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v11

    .line 160
    .end local v11    # "bits$iv$iv$iv":I
    .end local v12    # "$i$f$floatFromBits":I
    nop

    .line 161
    .end local v3    # "value$iv$iv":J
    .end local v5    # "$i$f$unpackFloat2":I
    nop

    .line 109
    .end local v0    # "$v$c$androidx-compose-ui-layout-ScaleFactor$-this$0$iv":J
    .end local v2    # "$i$f$getScaleY-impl":I
    mul-float/2addr v10, v11

    .local v7, "width$iv":F
    .local v10, "height$iv":F
    const/4 v0, 0x0

    .line 162
    .local v0, "$i$f$Size":I
    move v1, v10

    .local v1, "val2$iv$iv":F
    move v2, v7

    .local v2, "val1$iv$iv":F
    const/4 v3, 0x0

    .line 163
    .local v3, "$i$f$packFloats":I
    invoke-static {v2}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v4

    int-to-long v4, v4

    .line 164
    .local v4, "v1$iv$iv":J
    invoke-static {v1}, Ljava/lang/Float;->floatToRawIntBits(F)I

    move-result v11

    int-to-long v11, v11

    .line 165
    .local v11, "v2$iv$iv":J
    shl-long v13, v4, v6

    and-long/2addr v8, v11

    or-long v1, v13, v8

    .line 162
    .end local v1    # "val2$iv$iv":F
    .end local v2    # "val1$iv$iv":F
    .end local v3    # "$i$f$packFloats":I
    .end local v4    # "v1$iv$iv":J
    .end local v11    # "v2$iv$iv":J
    invoke-static {v1, v2}, Landroidx/compose/ui/geometry/Size;->constructor-impl(J)J

    move-result-wide v0

    .line 109
    .end local v0    # "$i$f$Size":I
    .end local v7    # "width$iv":F
    .end local v10    # "height$iv":F
    return-wide v0
.end method

.method public static final times-m-w2e94(JJ)J
    .locals 2
    .param p0, "$v$c$androidx-compose-ui-layout-ScaleFactor$-$this$times$0"    # J
    .param p2, "$v$c$androidx-compose-ui-geometry-Size$-size$0"    # J

    .line 118
    invoke-static {p2, p3, p0, p1}, Landroidx/compose/ui/layout/ScaleFactorKt;->times-UQTWf7w(JJ)J

    move-result-wide v0

    return-wide v0
.end method
