.class public final Landroidx/compose/ui/node/TraversableNodeKt;
.super Ljava/lang/Object;
.source "TraversableNode.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nTraversableNode.kt\nKotlin\n*S Kotlin\n*F\n+ 1 TraversableNode.kt\nandroidx/compose/ui/node/TraversableNodeKt\n+ 2 NodeKind.kt\nandroidx/compose/ui/node/Nodes\n+ 3 DelegatableNode.kt\nandroidx/compose/ui/node/DelegatableNodeKt\n+ 4 InlineClassHelper.kt\nandroidx/compose/ui/internal/InlineClassHelperKt\n+ 5 Modifier.kt\nandroidx/compose/ui/Modifier$Node\n+ 6 DelegatingNode.kt\nandroidx/compose/ui/node/DelegatingNode\n+ 7 MutableVector.kt\nandroidx/compose/runtime/collection/MutableVectorKt\n+ 8 MutableVector.kt\nandroidx/compose/runtime/collection/MutableVector\n*L\n1#1,218:1\n148#2:219\n148#2:288\n148#2:357\n148#2:426\n148#2:495\n148#2:571\n148#2:647\n148#2:724\n252#3,5:220\n95#3:225\n96#3,8:230\n556#3,6:238\n566#3,2:245\n568#3,8:250\n576#3,9:261\n585#3,8:273\n105#3,7:281\n252#3,5:289\n95#3:294\n96#3,8:299\n556#3,6:307\n566#3,2:314\n568#3,8:319\n576#3,9:330\n585#3,8:342\n105#3,7:350\n252#3,5:358\n95#3:363\n96#3,8:368\n556#3,6:376\n566#3,2:383\n568#3,8:388\n576#3,9:399\n585#3,8:411\n105#3,7:419\n252#3,5:427\n95#3:432\n96#3,8:437\n556#3,6:445\n566#3,2:452\n568#3,8:457\n576#3,9:468\n585#3,8:480\n105#3,7:488\n303#3,5:496\n153#3:501\n154#3:506\n155#3,3:510\n158#3:514\n159#3,9:516\n556#3,6:525\n566#3,2:532\n568#3,17:537\n585#3,8:557\n168#3,6:565\n303#3,5:572\n153#3:577\n154#3:582\n155#3,3:586\n158#3:590\n159#3,9:592\n556#3,6:601\n566#3,2:608\n568#3,17:613\n585#3,8:633\n168#3,6:641\n318#3,6:648\n184#3:654\n185#3:659\n186#3,3:663\n189#3,6:667\n324#3:673\n556#3,6:674\n566#3,2:681\n568#3,17:686\n585#3,8:706\n325#3:714\n195#3,8:715\n326#3:723\n318#3,6:725\n184#3:731\n185#3:736\n186#3,3:740\n189#3,6:744\n324#3:750\n556#3,6:751\n566#3,2:758\n568#3,17:763\n585#3,8:783\n325#3:791\n195#3,8:792\n326#3:800\n57#4,4:226\n57#4,4:295\n57#4,4:364\n57#4,4:433\n57#4,4:502\n57#4,4:578\n57#4,4:655\n57#4,4:732\n247#5:244\n247#5:313\n247#5:382\n247#5:451\n247#5:531\n247#5:607\n247#5:680\n247#5:757\n240#6,3:247\n243#6,3:270\n240#6,3:316\n243#6,3:339\n240#6,3:385\n243#6,3:408\n240#6,3:454\n243#6,3:477\n240#6,3:534\n243#6,3:554\n240#6,3:610\n243#6,3:630\n240#6,3:683\n243#6,3:703\n240#6,3:760\n243#6,3:780\n1107#7:258\n1085#7,2:259\n1107#7:327\n1085#7,2:328\n1107#7:396\n1085#7,2:397\n1107#7:465\n1085#7,2:466\n1107#7:507\n1085#7,2:508\n1107#7:583\n1085#7,2:584\n1107#7:660\n1085#7,2:661\n1107#7:737\n1085#7,2:738\n519#8:513\n44#8:515\n519#8:589\n44#8:591\n519#8:666\n519#8:743\n*S KotlinDebug\n*F\n+ 1 TraversableNode.kt\nandroidx/compose/ui/node/TraversableNodeKt\n*L\n54#1:219\n64#1:288\n83#1:357\n103#1:426\n126#1:495\n148#1:571\n175#1:647\n203#1:724\n54#1:220,5\n54#1:225\n54#1:230,8\n54#1:238,6\n54#1:245,2\n54#1:250,8\n54#1:261,9\n54#1:273,8\n54#1:281,7\n64#1:289,5\n64#1:294\n64#1:299,8\n64#1:307,6\n64#1:314,2\n64#1:319,8\n64#1:330,9\n64#1:342,8\n64#1:350,7\n83#1:358,5\n83#1:363\n83#1:368,8\n83#1:376,6\n83#1:383,2\n83#1:388,8\n83#1:399,9\n83#1:411,8\n83#1:419,7\n103#1:427,5\n103#1:432\n103#1:437,8\n103#1:445,6\n103#1:452,2\n103#1:457,8\n103#1:468,9\n103#1:480,8\n103#1:488,7\n126#1:496,5\n126#1:501\n126#1:506\n126#1:510,3\n126#1:514\n126#1:516,9\n126#1:525,6\n126#1:532,2\n126#1:537,17\n126#1:557,8\n126#1:565,6\n148#1:572,5\n148#1:577\n148#1:582\n148#1:586,3\n148#1:590\n148#1:592,9\n148#1:601,6\n148#1:608,2\n148#1:613,17\n148#1:633,8\n148#1:641,6\n175#1:648,6\n175#1:654\n175#1:659\n175#1:663,3\n175#1:667,6\n175#1:673\n175#1:674,6\n175#1:681,2\n175#1:686,17\n175#1:706,8\n175#1:714\n175#1:715,8\n175#1:723\n203#1:725,6\n203#1:731\n203#1:736\n203#1:740,3\n203#1:744,6\n203#1:750\n203#1:751,6\n203#1:758,2\n203#1:763,17\n203#1:783,8\n203#1:791\n203#1:792,8\n203#1:800\n54#1:226,4\n64#1:295,4\n83#1:364,4\n103#1:433,4\n126#1:502,4\n148#1:578,4\n175#1:655,4\n203#1:732,4\n54#1:244\n64#1:313\n83#1:382\n103#1:451\n126#1:531\n148#1:607\n175#1:680\n203#1:757\n54#1:247,3\n54#1:270,3\n64#1:316,3\n64#1:339,3\n83#1:385,3\n83#1:408,3\n103#1:454,3\n103#1:477,3\n126#1:534,3\n126#1:554,3\n148#1:610,3\n148#1:630,3\n175#1:683,3\n175#1:703,3\n203#1:760,3\n203#1:780,3\n54#1:258\n54#1:259,2\n64#1:327\n64#1:328,2\n83#1:396\n83#1:397,2\n103#1:465\n103#1:466,2\n126#1:507\n126#1:508,2\n148#1:583\n148#1:584,2\n175#1:660\n175#1:661,2\n203#1:737\n203#1:738,2\n126#1:513\n126#1:515\n148#1:589\n148#1:591\n175#1:666\n203#1:743\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000,\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0000\n\u0002\u0008\u0003\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u000b\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0000\u001a\u0016\u0010\u0000\u001a\u0004\u0018\u00010\u0001*\u00020\u00022\u0008\u0010\u0003\u001a\u0004\u0018\u00010\u0004\u001a\u001b\u0010\u0000\u001a\u0004\u0018\u0001H\u0005\"\u0008\u0008\u0000\u0010\u0005*\u00020\u0001*\u0002H\u0005\u00a2\u0006\u0002\u0010\u0006\u001a(\u0010\u0007\u001a\u00020\u0008*\u00020\u00022\u0008\u0010\u0003\u001a\u0004\u0018\u00010\u00042\u0012\u0010\t\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u000b0\n\u001a-\u0010\u0007\u001a\u00020\u0008\"\u0008\u0008\u0000\u0010\u0005*\u00020\u0001*\u0002H\u00052\u0012\u0010\t\u001a\u000e\u0012\u0004\u0012\u0002H\u0005\u0012\u0004\u0012\u00020\u000b0\n\u00a2\u0006\u0002\u0010\u000c\u001a(\u0010\r\u001a\u00020\u0008*\u00020\u00022\u0008\u0010\u0003\u001a\u0004\u0018\u00010\u00042\u0012\u0010\t\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u000b0\n\u001a-\u0010\r\u001a\u00020\u0008\"\u0008\u0008\u0000\u0010\u0005*\u00020\u0001*\u0002H\u00052\u0012\u0010\t\u001a\u000e\u0012\u0004\u0012\u0002H\u0005\u0012\u0004\u0012\u00020\u000b0\n\u00a2\u0006\u0002\u0010\u000c\u001a(\u0010\u000e\u001a\u00020\u0008*\u00020\u00022\u0008\u0010\u0003\u001a\u0004\u0018\u00010\u00042\u0012\u0010\t\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u000f0\n\u001a-\u0010\u000e\u001a\u00020\u0008\"\u0008\u0008\u0000\u0010\u0005*\u00020\u0001*\u0002H\u00052\u0012\u0010\t\u001a\u000e\u0012\u0004\u0012\u0002H\u0005\u0012\u0004\u0012\u00020\u000f0\n\u00a2\u0006\u0002\u0010\u000c\u00a8\u0006\u0010"
    }
    d2 = {
        "findNearestAncestor",
        "Landroidx/compose/ui/node/TraversableNode;",
        "Landroidx/compose/ui/node/DelegatableNode;",
        "key",
        "",
        "T",
        "(Landroidx/compose/ui/node/TraversableNode;)Landroidx/compose/ui/node/TraversableNode;",
        "traverseAncestors",
        "",
        "block",
        "Lkotlin/Function1;",
        "",
        "(Landroidx/compose/ui/node/TraversableNode;Lkotlin/jvm/functions/Function1;)V",
        "traverseChildren",
        "traverseDescendants",
        "Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;",
        "ui"
    }
    k = 0x2
    mv = {
        0x2,
        0x0,
        0x0
    }
    xi = 0x30
.end annotation


# direct methods
.method public static final findNearestAncestor(Landroidx/compose/ui/node/DelegatableNode;Ljava/lang/Object;)Landroidx/compose/ui/node/TraversableNode;
    .locals 32
    .param p0, "$this$findNearestAncestor"    # Landroidx/compose/ui/node/DelegatableNode;
    .param p1, "key"    # Ljava/lang/Object;

    .line 54
    const/4 v0, 0x0

    .line 219
    .local v0, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v1, 0x40000

    invoke-static {v1}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v0

    .line 54
    .end local v0    # "$i$f$getTraversable-OLwlOKw":I
    move-object/from16 v1, p0

    .line 220
    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    nop

    .line 222
    const/4 v2, 0x0

    .line 220
    .local v2, "includeSelf$iv":Z
    const/4 v3, 0x0

    .line 224
    .local v3, "$i$f$visitAncestors-Y-YKmho":I
    move v4, v2

    .local v4, "includeSelf$iv$iv":Z
    move-object v5, v1

    .local v5, "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    move v6, v0

    .local v6, "mask$iv$iv":I
    const/4 v7, 0x0

    .line 225
    .local v7, "$i$f$visitAncestors":I
    invoke-interface {v5}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v8

    .local v8, "value$iv$iv$iv":Z
    const/4 v9, 0x0

    .line 226
    .local v9, "$i$f$checkPrecondition":I
    if-nez v8, :cond_0

    .line 227
    const/4 v10, 0x0

    .line 225
    .local v10, "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    nop

    .line 227
    .end local v10    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    const-string/jumbo v10, "visitAncestors called on an unattached node"

    invoke-static {v10}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 229
    :cond_0
    nop

    .line 230
    .end local v8    # "value$iv$iv$iv":Z
    .end local v9    # "$i$f$checkPrecondition":I
    invoke-interface {v5}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    .line 231
    .local v8, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-static {v5}, Landroidx/compose/ui/node/DelegatableNodeKt;->requireLayoutNode(Landroidx/compose/ui/node/DelegatableNode;)Landroidx/compose/ui/node/LayoutNode;

    move-result-object v9

    .line 232
    .local v9, "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    :goto_0
    if-eqz v9, :cond_13

    .line 233
    invoke-virtual {v9}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v11

    invoke-virtual {v11}, Landroidx/compose/ui/node/NodeChain;->getHead$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v11

    .line 234
    .local v11, "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v12

    and-int/2addr v12, v6

    if-eqz v12, :cond_11

    .line 235
    :goto_1
    if-eqz v8, :cond_10

    .line 236
    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v12

    and-int/2addr v12, v6

    if-eqz v12, :cond_f

    .line 237
    move-object v12, v8

    .local v12, "it$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v13, 0x0

    .line 224
    .local v13, "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    move v14, v0

    .local v14, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object v15, v12

    .local v15, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v16, 0x0

    .line 238
    .local v16, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v17, 0x0

    .line 239
    .local v17, "stack$iv$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .local v18, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v18, v15

    move-object/from16 v10, v18

    const/16 v18, 0x0

    .line 240
    .end local v18    # "node$iv$iv":Ljava/lang/Object;
    .local v10, "node$iv$iv":Ljava/lang/Object;
    :goto_2
    if-eqz v10, :cond_e

    .line 241
    move/from16 v19, v0

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v19, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    instance-of v0, v10, Landroidx/compose/ui/node/TraversableNode;

    if-eqz v0, :cond_2

    .line 242
    move-object v0, v10

    check-cast v0, Landroidx/compose/ui/node/TraversableNode;

    .local v0, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v20, 0x0

    .line 55
    .local v20, "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$findNearestAncestor$1":I
    move-object/from16 v21, v0

    .end local v0    # "it":Landroidx/compose/ui/node/TraversableNode;
    .local v21, "it":Landroidx/compose/ui/node/TraversableNode;
    invoke-interface/range {v21 .. v21}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v0

    move-object/from16 v22, v1

    move-object/from16 v1, p1

    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v22, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_1

    .line 56
    return-object v21

    .line 58
    :cond_1
    nop

    .line 242
    .end local v20    # "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$findNearestAncestor$1":I
    .end local v21    # "it":Landroidx/compose/ui/node/TraversableNode;
    move/from16 v30, v2

    move/from16 v31, v3

    goto/16 :goto_9

    .line 243
    .end local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    :cond_2
    move-object/from16 v22, v1

    move-object/from16 v1, p1

    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    move v0, v14

    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v20, v10

    .local v20, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v21, 0x0

    .line 244
    .local v21, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v20 .. v20}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v23

    and-int v23, v23, v0

    move/from16 v24, v0

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .local v24, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    if-eqz v23, :cond_3

    const/16 v20, 0x1

    goto :goto_3

    :cond_3
    const/16 v20, 0x0

    .line 243
    .end local v20    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v21    # "$i$f$isKind-H91voCI$ui":I
    .end local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    :goto_3
    if-eqz v20, :cond_c

    instance-of v0, v10, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v0, :cond_c

    .line 245
    const/4 v0, 0x0

    .line 246
    .local v0, "count$iv$iv":I
    move-object/from16 v21, v10

    check-cast v21, Landroidx/compose/ui/node/DelegatingNode;

    .local v21, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/16 v23, 0x0

    .line 247
    .local v23, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v24

    .line 248
    .local v24, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_4
    if-eqz v24, :cond_b

    .line 249
    move-object/from16 v25, v24

    .local v25, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v26, 0x0

    .line 250
    .local v26, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v27, v14

    .local v27, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v28, v25

    .local v28, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v29, 0x0

    .line 244
    .local v29, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v28 .. v28}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v30

    and-int v30, v30, v27

    if-eqz v30, :cond_4

    const/16 v27, 0x1

    goto :goto_5

    :cond_4
    const/16 v27, 0x0

    .line 250
    .end local v27    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v28    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v29    # "$i$f$isKind-H91voCI$ui":I
    :goto_5
    if-eqz v27, :cond_a

    .line 251
    add-int/lit8 v0, v0, 0x1

    .line 252
    const/4 v1, 0x1

    if-ne v0, v1, :cond_5

    .line 253
    move-object/from16 v10, v25

    move/from16 v30, v2

    move/from16 v31, v3

    move-object/from16 v2, v25

    const/4 v3, 0x0

    goto :goto_8

    .line 257
    :cond_5
    if-nez v17, :cond_6

    const/4 v1, 0x0

    .line 258
    .local v1, "$i$f$mutableVectorOf":I
    nop

    .line 259
    move/from16 v27, v0

    .end local v0    # "count$iv$iv":I
    .local v27, "count$iv$iv":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v28, 0x0

    .line 260
    .local v28, "$i$f$MutableVector":I
    move/from16 v29, v1

    .end local v1    # "$i$f$mutableVectorOf":I
    .local v29, "$i$f$mutableVectorOf":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    move/from16 v30, v2

    .end local v2    # "includeSelf$iv":Z
    .local v30, "includeSelf$iv":Z
    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v31, v3

    const/4 v3, 0x0

    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .local v31, "$i$f$visitAncestors-Y-YKmho":I
    invoke-direct {v1, v2, v3}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 258
    .end local v0    # "capacity$iv$iv$iv$iv":I
    .end local v28    # "$i$f$MutableVector":I
    goto :goto_6

    .line 257
    .end local v27    # "count$iv$iv":I
    .end local v29    # "$i$f$mutableVectorOf":I
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .local v0, "count$iv$iv":I
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_6
    move/from16 v27, v0

    move/from16 v30, v2

    move/from16 v31, v3

    const/4 v3, 0x0

    .end local v0    # "count$iv$iv":I
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v27    # "count$iv$iv":I
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    move-object/from16 v1, v17

    :goto_6
    nop

    .line 261
    .end local v17    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object v0, v10

    .line 262
    .local v0, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v0, :cond_8

    .line 263
    if-eqz v1, :cond_7

    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 264
    :cond_7
    const/4 v2, 0x0

    move-object v10, v2

    .line 266
    :cond_8
    if-eqz v1, :cond_9

    move-object/from16 v2, v25

    .end local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v2, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_7

    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_9
    move-object/from16 v2, v25

    .line 269
    .end local v0    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_7
    move-object/from16 v17, v1

    move/from16 v0, v27

    goto :goto_8

    .line 250
    .end local v1    # "stack$iv$iv":Ljava/lang/Object;
    .end local v27    # "count$iv$iv":I
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .local v0, "count$iv$iv":I
    .local v2, "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v17    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_a
    move/from16 v30, v2

    move/from16 v31, v3

    move-object/from16 v2, v25

    const/4 v3, 0x0

    .line 269
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .end local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v2, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_8
    nop

    .line 249
    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v26    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 270
    invoke-virtual/range {v24 .. v24}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v24

    move-object/from16 v1, p1

    move/from16 v2, v30

    move/from16 v3, v31

    goto :goto_4

    .line 272
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .local v2, "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_b
    move/from16 v30, v2

    move/from16 v31, v3

    .line 273
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .end local v21    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v23    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v24    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    const/4 v1, 0x1

    if-ne v0, v1, :cond_d

    .line 275
    move/from16 v0, v19

    move-object/from16 v1, v22

    move/from16 v2, v30

    move/from16 v3, v31

    goto/16 :goto_2

    .line 243
    .end local v0    # "count$iv$iv":I
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_c
    move/from16 v30, v2

    move/from16 v31, v3

    .line 278
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_d
    :goto_9
    invoke-static/range {v17 .. v17}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    move/from16 v0, v19

    move-object/from16 v1, v22

    move/from16 v2, v30

    move/from16 v3, v31

    goto/16 :goto_2

    .line 280
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_e
    move/from16 v19, v0

    move-object/from16 v22, v1

    move/from16 v30, v2

    move/from16 v31, v3

    .line 224
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .end local v10    # "node$iv$iv":Ljava/lang/Object;
    .end local v14    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v15    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v16    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v17    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    nop

    .line 237
    .end local v12    # "it$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v13    # "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    goto :goto_a

    .line 236
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_f
    move/from16 v19, v0

    move-object/from16 v22, v1

    move/from16 v30, v2

    move/from16 v31, v3

    const/16 v18, 0x0

    .line 281
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_a
    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    move/from16 v0, v19

    move-object/from16 v1, v22

    move/from16 v2, v30

    move/from16 v3, v31

    goto/16 :goto_1

    .line 235
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_10
    move/from16 v19, v0

    move-object/from16 v22, v1

    move/from16 v30, v2

    move/from16 v31, v3

    const/16 v18, 0x0

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    goto :goto_b

    .line 234
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_11
    move/from16 v19, v0

    move-object/from16 v22, v1

    move/from16 v30, v2

    move/from16 v31, v3

    const/16 v18, 0x0

    .line 284
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "includeSelf$iv":Z
    .restart local v31    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_b
    invoke-virtual {v9}, Landroidx/compose/ui/node/LayoutNode;->getParent$ui()Landroidx/compose/ui/node/LayoutNode;

    move-result-object v9

    .line 285
    if-eqz v9, :cond_12

    invoke-virtual {v9}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v0

    if-eqz v0, :cond_12

    invoke-virtual {v0}, Landroidx/compose/ui/node/NodeChain;->getTail$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    goto :goto_c

    :cond_12
    move-object/from16 v10, v18

    :goto_c
    move-object v8, v10

    move/from16 v0, v19

    move-object/from16 v1, v22

    move/from16 v2, v30

    move/from16 v3, v31

    .end local v11    # "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    goto/16 :goto_0

    .line 287
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "includeSelf$iv":Z
    .end local v31    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_13
    const/16 v18, 0x0

    .line 224
    .end local v4    # "includeSelf$iv$iv":Z
    .end local v5    # "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v6    # "mask$iv$iv":I
    .end local v7    # "$i$f$visitAncestors":I
    .end local v8    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v9    # "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    nop

    .line 59
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    return-object v18
.end method

.method public static final findNearestAncestor(Landroidx/compose/ui/node/TraversableNode;)Landroidx/compose/ui/node/TraversableNode;
    .locals 32
    .param p0, "$this$findNearestAncestor"    # Landroidx/compose/ui/node/TraversableNode;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T::",
            "Landroidx/compose/ui/node/TraversableNode;",
            ">(TT;)TT;"
        }
    .end annotation

    .line 64
    move-object/from16 v0, p0

    move-object v1, v0

    check-cast v1, Landroidx/compose/ui/node/DelegatableNode;

    const/4 v2, 0x0

    .line 288
    .local v2, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v3, 0x40000

    invoke-static {v3}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v2

    .line 64
    .end local v2    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .line 289
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    nop

    .line 291
    const/4 v3, 0x0

    .line 289
    .local v3, "includeSelf$iv":Z
    const/4 v4, 0x0

    .line 293
    .local v4, "$i$f$visitAncestors-Y-YKmho":I
    move v5, v3

    .local v5, "includeSelf$iv$iv":Z
    move v6, v2

    .local v6, "mask$iv$iv":I
    move-object v7, v1

    .local v7, "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    const/4 v8, 0x0

    .line 294
    .local v8, "$i$f$visitAncestors":I
    invoke-interface {v7}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v9

    .local v9, "value$iv$iv$iv":Z
    const/4 v10, 0x0

    .line 295
    .local v10, "$i$f$checkPrecondition":I
    if-nez v9, :cond_0

    .line 296
    const/4 v11, 0x0

    .line 294
    .local v11, "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    nop

    .line 296
    .end local v11    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    const-string/jumbo v11, "visitAncestors called on an unattached node"

    invoke-static {v11}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 298
    :cond_0
    nop

    .line 299
    .end local v9    # "value$iv$iv$iv":Z
    .end local v10    # "$i$f$checkPrecondition":I
    invoke-interface {v7}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    .line 300
    .local v9, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-static {v7}, Landroidx/compose/ui/node/DelegatableNodeKt;->requireLayoutNode(Landroidx/compose/ui/node/DelegatableNode;)Landroidx/compose/ui/node/LayoutNode;

    move-result-object v10

    .line 301
    .local v10, "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    :goto_0
    if-eqz v10, :cond_12

    .line 302
    invoke-virtual {v10}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v12

    invoke-virtual {v12}, Landroidx/compose/ui/node/NodeChain;->getHead$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v12

    .line 303
    .local v12, "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v12}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v13

    and-int/2addr v13, v6

    if-eqz v13, :cond_10

    .line 304
    :goto_1
    if-eqz v9, :cond_f

    .line 305
    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v13

    and-int/2addr v13, v6

    if-eqz v13, :cond_e

    .line 306
    move-object v13, v9

    .local v13, "it$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v14, 0x0

    .line 293
    .local v14, "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    move v15, v2

    .local v15, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object/from16 v16, v13

    .local v16, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v17, 0x0

    .line 307
    .local v17, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v18, 0x0

    .line 308
    .local v18, "stack$iv$iv":Ljava/lang/Object;
    const/16 v19, 0x0

    .local v19, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v19, v16

    move-object/from16 v11, v19

    const/16 v19, 0x0

    .line 309
    .end local v19    # "node$iv$iv":Ljava/lang/Object;
    .local v11, "node$iv$iv":Ljava/lang/Object;
    :goto_2
    if-eqz v11, :cond_d

    .line 310
    move-object/from16 v20, v1

    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v20, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    instance-of v1, v11, Landroidx/compose/ui/node/TraversableNode;

    if-eqz v1, :cond_2

    .line 311
    move-object v1, v11

    check-cast v1, Landroidx/compose/ui/node/TraversableNode;

    .local v1, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v21, 0x0

    .line 65
    .local v21, "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$findNearestAncestor$2":I
    move/from16 v22, v2

    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v22, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    invoke-interface {v0}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v2

    move/from16 v23, v3

    .end local v3    # "includeSelf$iv":Z
    .local v23, "includeSelf$iv":Z
    invoke-interface {v1}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v3

    invoke-static {v2, v3}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_1

    invoke-static {v0, v1}, Landroidx/compose/ui/Actual_jvmKt;->areObjectsOfSameType(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_1

    .line 67
    return-object v1

    .line 69
    :cond_1
    nop

    .line 311
    .end local v1    # "it":Landroidx/compose/ui/node/TraversableNode;
    .end local v21    # "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$findNearestAncestor$2":I
    goto/16 :goto_9

    .line 312
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "includeSelf$iv":Z
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    :cond_2
    move/from16 v22, v2

    move/from16 v23, v3

    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "includeSelf$iv":Z
    move v1, v15

    .local v1, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object v2, v11

    .local v2, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v3, 0x0

    .line 313
    .local v3, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual {v2}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v21

    and-int v21, v21, v1

    const/4 v0, 0x1

    if-eqz v21, :cond_3

    move v1, v0

    goto :goto_3

    :cond_3
    const/4 v1, 0x0

    .line 312
    .end local v1    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v3    # "$i$f$isKind-H91voCI$ui":I
    :goto_3
    if-eqz v1, :cond_c

    instance-of v1, v11, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v1, :cond_c

    .line 314
    const/4 v1, 0x0

    .line 315
    .local v1, "count$iv$iv":I
    move-object v2, v11

    check-cast v2, Landroidx/compose/ui/node/DelegatingNode;

    .local v2, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/4 v3, 0x0

    .line 316
    .local v3, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual {v2}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v21

    .line 317
    .local v21, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_4
    if-eqz v21, :cond_b

    .line 318
    move-object/from16 v24, v21

    .local v24, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v25, 0x0

    .line 319
    .local v25, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v26, v15

    .local v26, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v27, v24

    .local v27, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v28, 0x0

    .line 313
    .local v28, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v27 .. v27}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v29

    and-int v29, v29, v26

    if-eqz v29, :cond_4

    move/from16 v26, v0

    goto :goto_5

    :cond_4
    const/16 v26, 0x0

    .line 319
    .end local v26    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v28    # "$i$f$isKind-H91voCI$ui":I
    :goto_5
    if-eqz v26, :cond_a

    .line 320
    add-int/lit8 v1, v1, 0x1

    .line 321
    if-ne v1, v0, :cond_5

    .line 322
    move-object/from16 v11, v24

    move-object/from16 v30, v2

    move-object/from16 v0, v24

    goto :goto_8

    .line 326
    :cond_5
    if-nez v18, :cond_6

    const/16 v26, 0x0

    .line 327
    .local v26, "$i$f$mutableVectorOf":I
    nop

    .line 328
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v28, 0x0

    .line 329
    .local v28, "$i$f$MutableVector":I
    move/from16 v29, v1

    .end local v1    # "count$iv$iv":I
    .local v29, "count$iv$iv":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    move-object/from16 v30, v2

    .end local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .local v30, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v31, v0

    const/4 v0, 0x0

    .end local v0    # "capacity$iv$iv$iv$iv":I
    .local v31, "capacity$iv$iv$iv$iv":I
    invoke-direct {v1, v2, v0}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 327
    .end local v28    # "$i$f$MutableVector":I
    .end local v31    # "capacity$iv$iv$iv$iv":I
    goto :goto_6

    .line 326
    .end local v26    # "$i$f$mutableVectorOf":I
    .end local v29    # "count$iv$iv":I
    .end local v30    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v1    # "count$iv$iv":I
    .restart local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    :cond_6
    move/from16 v29, v1

    move-object/from16 v30, v2

    const/4 v0, 0x0

    .end local v1    # "count$iv$iv":I
    .end local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v29    # "count$iv$iv":I
    .restart local v30    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    move-object/from16 v1, v18

    :goto_6
    nop

    .line 330
    .end local v18    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object v2, v11

    .line 331
    .local v2, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v2, :cond_8

    .line 332
    if-eqz v1, :cond_7

    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 333
    :cond_7
    const/4 v11, 0x0

    .line 335
    :cond_8
    if-eqz v1, :cond_9

    move-object/from16 v0, v24

    .end local v24    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v0, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_7

    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v24    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_9
    move-object/from16 v0, v24

    .line 338
    .end local v2    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v24    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_7
    move-object/from16 v18, v1

    move/from16 v1, v29

    goto :goto_8

    .line 319
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v29    # "count$iv$iv":I
    .end local v30    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .local v1, "count$iv$iv":I
    .local v2, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v18    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v24    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_a
    move-object/from16 v30, v2

    move-object/from16 v0, v24

    .line 338
    .end local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v24    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v30    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    :goto_8
    nop

    .line 318
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v25    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 339
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v21

    move-object/from16 v2, v30

    const/4 v0, 0x1

    goto :goto_4

    .line 341
    .end local v30    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    :cond_b
    move-object/from16 v30, v2

    .line 342
    .end local v2    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v3    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v21    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v0, 0x1

    if-ne v1, v0, :cond_c

    .line 344
    move-object/from16 v0, p0

    move-object/from16 v1, v20

    move/from16 v2, v22

    move/from16 v3, v23

    goto/16 :goto_2

    .line 347
    .end local v1    # "count$iv$iv":I
    :cond_c
    :goto_9
    invoke-static/range {v18 .. v18}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v11

    move-object/from16 v0, p0

    move-object/from16 v1, v20

    move/from16 v2, v22

    move/from16 v3, v23

    goto/16 :goto_2

    .line 349
    .end local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "includeSelf$iv":Z
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v3, "includeSelf$iv":Z
    :cond_d
    move-object/from16 v20, v1

    move/from16 v22, v2

    move/from16 v23, v3

    .line 293
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v11    # "node$iv$iv":Ljava/lang/Object;
    .end local v15    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v16    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v17    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v18    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "includeSelf$iv":Z
    nop

    .line 306
    .end local v13    # "it$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v14    # "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    goto :goto_a

    .line 305
    .end local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "includeSelf$iv":Z
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    :cond_e
    move-object/from16 v20, v1

    move/from16 v22, v2

    move/from16 v23, v3

    const/16 v19, 0x0

    .line 350
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .restart local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "includeSelf$iv":Z
    :goto_a
    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    move-object/from16 v0, p0

    move-object/from16 v1, v20

    move/from16 v2, v22

    move/from16 v3, v23

    goto/16 :goto_1

    .line 304
    .end local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "includeSelf$iv":Z
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    :cond_f
    move-object/from16 v20, v1

    move/from16 v22, v2

    move/from16 v23, v3

    const/16 v19, 0x0

    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .restart local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "includeSelf$iv":Z
    goto :goto_b

    .line 303
    .end local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "includeSelf$iv":Z
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    :cond_10
    move-object/from16 v20, v1

    move/from16 v22, v2

    move/from16 v23, v3

    const/16 v19, 0x0

    .line 353
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .restart local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "includeSelf$iv":Z
    :goto_b
    invoke-virtual {v10}, Landroidx/compose/ui/node/LayoutNode;->getParent$ui()Landroidx/compose/ui/node/LayoutNode;

    move-result-object v10

    .line 354
    if-eqz v10, :cond_11

    invoke-virtual {v10}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v0

    if-eqz v0, :cond_11

    invoke-virtual {v0}, Landroidx/compose/ui/node/NodeChain;->getTail$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v11

    goto :goto_c

    :cond_11
    move-object/from16 v11, v19

    :goto_c
    move-object v9, v11

    move-object/from16 v0, p0

    move-object/from16 v1, v20

    move/from16 v2, v22

    move/from16 v3, v23

    .end local v12    # "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    goto/16 :goto_0

    .line 356
    .end local v20    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "includeSelf$iv":Z
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    :cond_12
    const/16 v19, 0x0

    .line 293
    .end local v5    # "includeSelf$iv$iv":Z
    .end local v6    # "mask$iv$iv":I
    .end local v7    # "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v8    # "$i$f$visitAncestors":I
    .end local v9    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v10    # "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    nop

    .line 70
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    return-object v19
.end method

.method public static final traverseAncestors(Landroidx/compose/ui/node/DelegatableNode;Ljava/lang/Object;Lkotlin/jvm/functions/Function1;)V
    .locals 34
    .param p0, "$this$traverseAncestors"    # Landroidx/compose/ui/node/DelegatableNode;
    .param p1, "key"    # Ljava/lang/Object;
    .param p2, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/ui/node/DelegatableNode;",
            "Ljava/lang/Object;",
            "Lkotlin/jvm/functions/Function1<",
            "-",
            "Landroidx/compose/ui/node/TraversableNode;",
            "Ljava/lang/Boolean;",
            ">;)V"
        }
    .end annotation

    .line 83
    const/4 v0, 0x0

    .line 357
    .local v0, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v1, 0x40000

    invoke-static {v1}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v0

    .line 83
    .end local v0    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    move-object/from16 v1, p0

    .line 358
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    nop

    .line 360
    const/4 v2, 0x0

    .line 358
    .local v2, "includeSelf$iv":Z
    const/4 v3, 0x0

    .line 362
    .local v3, "$i$f$visitAncestors-Y-YKmho":I
    move v4, v2

    .local v4, "includeSelf$iv$iv":Z
    move v5, v0

    .local v5, "mask$iv$iv":I
    move-object v6, v1

    .local v6, "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    const/4 v7, 0x0

    .line 363
    .local v7, "$i$f$visitAncestors":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v8

    .local v8, "value$iv$iv$iv":Z
    const/4 v9, 0x0

    .line 364
    .local v9, "$i$f$checkPrecondition":I
    if-nez v8, :cond_0

    .line 365
    const/4 v10, 0x0

    .line 363
    .local v10, "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    nop

    .line 365
    .end local v10    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    const-string/jumbo v10, "visitAncestors called on an unattached node"

    invoke-static {v10}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 367
    :cond_0
    nop

    .line 368
    .end local v8    # "value$iv$iv$iv":Z
    .end local v9    # "$i$f$checkPrecondition":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    .line 369
    .local v8, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-static {v6}, Landroidx/compose/ui/node/DelegatableNodeKt;->requireLayoutNode(Landroidx/compose/ui/node/DelegatableNode;)Landroidx/compose/ui/node/LayoutNode;

    move-result-object v9

    .line 370
    .local v9, "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    :goto_0
    if-eqz v9, :cond_14

    .line 371
    invoke-virtual {v9}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v10

    invoke-virtual {v10}, Landroidx/compose/ui/node/NodeChain;->getHead$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    .line 372
    .local v10, "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v11

    and-int/2addr v11, v5

    if-eqz v11, :cond_12

    .line 373
    :goto_1
    if-eqz v8, :cond_11

    .line 374
    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v11

    and-int/2addr v11, v5

    if-eqz v11, :cond_10

    .line 375
    move-object v11, v8

    .local v11, "it$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v12, 0x0

    .line 362
    .local v12, "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    move v13, v0

    .local v13, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object v14, v11

    .local v14, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v15, 0x0

    .line 376
    .local v15, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v16, 0x0

    .line 377
    .local v16, "stack$iv$iv":Ljava/lang/Object;
    const/16 v17, 0x0

    .local v17, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v17, v14

    move-object/from16 v33, v17

    move/from16 v17, v0

    move-object/from16 v0, v33

    .line 378
    .local v0, "node$iv$iv":Ljava/lang/Object;
    .local v17, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    :goto_2
    if-eqz v0, :cond_f

    .line 379
    move-object/from16 v18, v1

    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v18, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    instance-of v1, v0, Landroidx/compose/ui/node/TraversableNode;

    move/from16 v19, v1

    if-eqz v19, :cond_3

    .line 380
    move-object v1, v0

    check-cast v1, Landroidx/compose/ui/node/TraversableNode;

    .local v1, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v20, 0x0

    .line 85
    .local v20, "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$traverseAncestors$1":I
    move/from16 v21, v2

    .end local v2    # "includeSelf$iv":Z
    .local v21, "includeSelf$iv":Z
    invoke-interface {v1}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v2

    move/from16 v22, v3

    move-object/from16 v3, p1

    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .local v22, "$i$f$visitAncestors-Y-YKmho":I
    invoke-static {v3, v2}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_1

    .line 86
    move-object/from16 v2, p2

    invoke-interface {v2, v1}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v19

    check-cast v19, Ljava/lang/Boolean;

    invoke-virtual/range {v19 .. v19}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v19

    goto :goto_3

    .line 88
    :cond_1
    move-object/from16 v2, p2

    const/16 v19, 0x1

    .line 85
    :goto_3
    nop

    .line 84
    nop

    .line 90
    .local v19, "continueTraversal":Z
    if-nez v19, :cond_2

    return-void

    .line 91
    :cond_2
    nop

    .line 380
    .end local v1    # "it":Landroidx/compose/ui/node/TraversableNode;
    .end local v19    # "continueTraversal":Z
    .end local v20    # "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$traverseAncestors$1":I
    goto/16 :goto_b

    .line 381
    .end local v21    # "includeSelf$iv":Z
    .end local v22    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_3
    move/from16 v21, v2

    move/from16 v22, v3

    move-object/from16 v3, p1

    move-object/from16 v2, p2

    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v21    # "includeSelf$iv":Z
    .restart local v22    # "$i$f$visitAncestors-Y-YKmho":I
    move v1, v13

    .local v1, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v20, v0

    .local v20, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v23, 0x0

    .line 382
    .local v23, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v20 .. v20}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v24

    and-int v24, v24, v1

    move/from16 v25, v1

    .end local v1    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .local v25, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    if-eqz v24, :cond_4

    const/16 v20, 0x1

    goto :goto_4

    :cond_4
    const/16 v20, 0x0

    .line 381
    .end local v20    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v23    # "$i$f$isKind-H91voCI$ui":I
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    :goto_4
    if-eqz v20, :cond_e

    instance-of v1, v0, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v1, :cond_e

    .line 383
    const/4 v1, 0x0

    .line 384
    .local v1, "count$iv$iv":I
    move-object/from16 v23, v0

    check-cast v23, Landroidx/compose/ui/node/DelegatingNode;

    .local v23, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/16 v24, 0x0

    .line 385
    .local v24, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual/range {v23 .. v23}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v25

    .line 386
    .local v25, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_5
    if-eqz v25, :cond_c

    .line 387
    move-object/from16 v26, v25

    .local v26, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v27, 0x0

    .line 388
    .local v27, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v28, v13

    .local v28, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v29, v26

    .local v29, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v30, 0x0

    .line 382
    .local v30, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v29 .. v29}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v31

    and-int v31, v31, v28

    if-eqz v31, :cond_5

    const/16 v28, 0x1

    goto :goto_6

    :cond_5
    const/16 v28, 0x0

    .line 388
    .end local v28    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v29    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v30    # "$i$f$isKind-H91voCI$ui":I
    :goto_6
    if-eqz v28, :cond_b

    .line 389
    add-int/lit8 v1, v1, 0x1

    .line 390
    move-object/from16 v28, v0

    const/4 v0, 0x1

    .end local v0    # "node$iv$iv":Ljava/lang/Object;
    .local v28, "node$iv$iv":Ljava/lang/Object;
    if-ne v1, v0, :cond_6

    .line 391
    move-object/from16 v0, v26

    move-object/from16 v2, v16

    move-object/from16 v16, v0

    .end local v28    # "node$iv$iv":Ljava/lang/Object;
    .restart local v0    # "node$iv$iv":Ljava/lang/Object;
    goto :goto_a

    .line 395
    .end local v0    # "node$iv$iv":Ljava/lang/Object;
    .restart local v28    # "node$iv$iv":Ljava/lang/Object;
    :cond_6
    if-nez v16, :cond_7

    const/4 v0, 0x0

    .line 396
    .local v0, "$i$f$mutableVectorOf":I
    nop

    .line 397
    move/from16 v29, v0

    .end local v0    # "$i$f$mutableVectorOf":I
    .local v29, "$i$f$mutableVectorOf":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v30, 0x0

    .line 398
    .local v30, "$i$f$MutableVector":I
    move/from16 v31, v1

    .end local v1    # "count$iv$iv":I
    .local v31, "count$iv$iv":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v32, v0

    const/4 v0, 0x0

    .end local v0    # "capacity$iv$iv$iv$iv":I
    .local v32, "capacity$iv$iv$iv$iv":I
    invoke-direct {v1, v2, v0}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 396
    .end local v30    # "$i$f$MutableVector":I
    .end local v32    # "capacity$iv$iv$iv$iv":I
    goto :goto_7

    .line 395
    .end local v29    # "$i$f$mutableVectorOf":I
    .end local v31    # "count$iv$iv":I
    .restart local v1    # "count$iv$iv":I
    :cond_7
    move/from16 v31, v1

    const/4 v0, 0x0

    .end local v1    # "count$iv$iv":I
    .restart local v31    # "count$iv$iv":I
    move-object/from16 v1, v16

    :goto_7
    nop

    .line 399
    .end local v16    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object/from16 v2, v28

    .line 400
    .local v2, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v2, :cond_9

    .line 401
    if-eqz v1, :cond_8

    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 402
    :cond_8
    const/16 v16, 0x0

    .end local v28    # "node$iv$iv":Ljava/lang/Object;
    .local v16, "node$iv$iv":Ljava/lang/Object;
    goto :goto_8

    .line 400
    .end local v16    # "node$iv$iv":Ljava/lang/Object;
    .restart local v28    # "node$iv$iv":Ljava/lang/Object;
    :cond_9
    move-object/from16 v16, v28

    .line 404
    .end local v28    # "node$iv$iv":Ljava/lang/Object;
    .restart local v16    # "node$iv$iv":Ljava/lang/Object;
    :goto_8
    if-eqz v1, :cond_a

    move-object/from16 v0, v26

    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v0, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_9

    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_a
    move-object/from16 v0, v26

    .line 407
    .end local v2    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_9
    move-object v2, v1

    move/from16 v1, v31

    goto :goto_a

    .line 388
    .end local v31    # "count$iv$iv":I
    .local v0, "node$iv$iv":Ljava/lang/Object;
    .local v1, "count$iv$iv":I
    .local v16, "stack$iv$iv":Ljava/lang/Object;
    .restart local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_b
    move-object/from16 v28, v0

    move-object/from16 v0, v26

    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v0, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v28    # "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v2, v16

    move-object/from16 v16, v28

    .line 407
    .end local v28    # "node$iv$iv":Ljava/lang/Object;
    .local v2, "stack$iv$iv":Ljava/lang/Object;
    .local v16, "node$iv$iv":Ljava/lang/Object;
    :goto_a
    nop

    .line 387
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v27    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 408
    invoke-virtual/range {v25 .. v25}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v25

    move-object/from16 v0, v16

    move-object/from16 v16, v2

    move-object/from16 v2, p2

    goto :goto_5

    .line 410
    .end local v2    # "stack$iv$iv":Ljava/lang/Object;
    .local v0, "node$iv$iv":Ljava/lang/Object;
    .local v16, "stack$iv$iv":Ljava/lang/Object;
    :cond_c
    move-object/from16 v28, v0

    .line 411
    .end local v0    # "node$iv$iv":Ljava/lang/Object;
    .end local v23    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v24    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v25    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v28    # "node$iv$iv":Ljava/lang/Object;
    const/4 v0, 0x1

    if-ne v1, v0, :cond_d

    .line 413
    move-object/from16 v1, v18

    move/from16 v2, v21

    move/from16 v3, v22

    move-object/from16 v0, v28

    goto/16 :goto_2

    .line 411
    :cond_d
    move-object/from16 v0, v28

    .line 416
    .end local v1    # "count$iv$iv":I
    .end local v28    # "node$iv$iv":Ljava/lang/Object;
    .restart local v0    # "node$iv$iv":Ljava/lang/Object;
    :cond_e
    :goto_b
    invoke-static/range {v16 .. v16}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v0

    move-object/from16 v1, v18

    move/from16 v2, v21

    move/from16 v3, v22

    goto/16 :goto_2

    .line 418
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v21    # "includeSelf$iv":Z
    .end local v22    # "$i$f$visitAncestors-Y-YKmho":I
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_f
    move-object/from16 v18, v1

    move/from16 v21, v2

    move/from16 v22, v3

    move-object/from16 v3, p1

    .line 362
    .end local v0    # "node$iv$iv":Ljava/lang/Object;
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .end local v13    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v14    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v15    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v16    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v21    # "includeSelf$iv":Z
    .restart local v22    # "$i$f$visitAncestors-Y-YKmho":I
    nop

    .line 375
    .end local v11    # "it$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v12    # "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    goto :goto_c

    .line 374
    .end local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v21    # "includeSelf$iv":Z
    .end local v22    # "$i$f$visitAncestors-Y-YKmho":I
    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_10
    move/from16 v17, v0

    move-object/from16 v18, v1

    move/from16 v21, v2

    move/from16 v22, v3

    move-object/from16 v3, p1

    .line 419
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v21    # "includeSelf$iv":Z
    .restart local v22    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_c
    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    move/from16 v0, v17

    move-object/from16 v1, v18

    move/from16 v2, v21

    move/from16 v3, v22

    goto/16 :goto_1

    .line 373
    .end local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v21    # "includeSelf$iv":Z
    .end local v22    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_11
    move/from16 v17, v0

    move-object/from16 v18, v1

    move/from16 v21, v2

    move/from16 v22, v3

    move-object/from16 v3, p1

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v21    # "includeSelf$iv":Z
    .restart local v22    # "$i$f$visitAncestors-Y-YKmho":I
    goto :goto_d

    .line 372
    .end local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v21    # "includeSelf$iv":Z
    .end local v22    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_12
    move/from16 v17, v0

    move-object/from16 v18, v1

    move/from16 v21, v2

    move/from16 v22, v3

    move-object/from16 v3, p1

    .line 422
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v21    # "includeSelf$iv":Z
    .restart local v22    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_d
    invoke-virtual {v9}, Landroidx/compose/ui/node/LayoutNode;->getParent$ui()Landroidx/compose/ui/node/LayoutNode;

    move-result-object v9

    .line 423
    if-eqz v9, :cond_13

    invoke-virtual {v9}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v0

    if-eqz v0, :cond_13

    invoke-virtual {v0}, Landroidx/compose/ui/node/NodeChain;->getTail$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v0

    goto :goto_e

    :cond_13
    const/4 v0, 0x0

    :goto_e
    move-object v8, v0

    move/from16 v0, v17

    move-object/from16 v1, v18

    move/from16 v2, v21

    move/from16 v3, v22

    .end local v10    # "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    goto/16 :goto_0

    .line 425
    .end local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v21    # "includeSelf$iv":Z
    .end local v22    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "includeSelf$iv":Z
    .restart local v3    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_14
    nop

    .line 362
    .end local v4    # "includeSelf$iv$iv":Z
    .end local v5    # "mask$iv$iv":I
    .end local v6    # "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v7    # "$i$f$visitAncestors":I
    .end local v8    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v9    # "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    nop

    .line 92
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "includeSelf$iv":Z
    .end local v3    # "$i$f$visitAncestors-Y-YKmho":I
    return-void
.end method

.method public static final traverseAncestors(Landroidx/compose/ui/node/TraversableNode;Lkotlin/jvm/functions/Function1;)V
    .locals 34
    .param p0, "$this$traverseAncestors"    # Landroidx/compose/ui/node/TraversableNode;
    .param p1, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T::",
            "Landroidx/compose/ui/node/TraversableNode;",
            ">(TT;",
            "Lkotlin/jvm/functions/Function1<",
            "-TT;",
            "Ljava/lang/Boolean;",
            ">;)V"
        }
    .end annotation

    .line 103
    move-object/from16 v0, p0

    move-object v1, v0

    check-cast v1, Landroidx/compose/ui/node/DelegatableNode;

    const/4 v2, 0x0

    .line 426
    .local v2, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v3, 0x40000

    invoke-static {v3}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v2

    .line 103
    .end local v2    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .line 427
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    nop

    .line 429
    const/4 v3, 0x0

    .line 427
    .local v3, "includeSelf$iv":Z
    const/4 v4, 0x0

    .line 431
    .local v4, "$i$f$visitAncestors-Y-YKmho":I
    move v5, v3

    .local v5, "includeSelf$iv$iv":Z
    move-object v6, v1

    .local v6, "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    move v7, v2

    .local v7, "mask$iv$iv":I
    const/4 v8, 0x0

    .line 432
    .local v8, "$i$f$visitAncestors":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v9

    .local v9, "value$iv$iv$iv":Z
    const/4 v10, 0x0

    .line 433
    .local v10, "$i$f$checkPrecondition":I
    if-nez v9, :cond_0

    .line 434
    const/4 v11, 0x0

    .line 432
    .local v11, "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    nop

    .line 434
    .end local v11    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitAncestors$1$iv$iv":I
    const-string/jumbo v11, "visitAncestors called on an unattached node"

    invoke-static {v11}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 436
    :cond_0
    nop

    .line 437
    .end local v9    # "value$iv$iv$iv":Z
    .end local v10    # "$i$f$checkPrecondition":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    .line 438
    .local v9, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-static {v6}, Landroidx/compose/ui/node/DelegatableNodeKt;->requireLayoutNode(Landroidx/compose/ui/node/DelegatableNode;)Landroidx/compose/ui/node/LayoutNode;

    move-result-object v10

    .line 439
    .local v10, "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    :goto_0
    if-eqz v10, :cond_14

    .line 440
    invoke-virtual {v10}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v11

    invoke-virtual {v11}, Landroidx/compose/ui/node/NodeChain;->getHead$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v11

    .line 441
    .local v11, "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v12

    and-int/2addr v12, v7

    if-eqz v12, :cond_12

    .line 442
    :goto_1
    if-eqz v9, :cond_11

    .line 443
    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v12

    and-int/2addr v12, v7

    if-eqz v12, :cond_10

    .line 444
    move-object v12, v9

    .local v12, "it$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v13, 0x0

    .line 431
    .local v13, "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    move v14, v2

    .local v14, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object v15, v12

    .local v15, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v16, 0x0

    .line 445
    .local v16, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v17, 0x0

    .line 446
    .local v17, "stack$iv$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .local v18, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v18, v15

    move-object/from16 v33, v18

    move-object/from16 v18, v1

    move-object/from16 v1, v33

    .line 447
    .local v1, "node$iv$iv":Ljava/lang/Object;
    .local v18, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    :goto_2
    if-eqz v1, :cond_f

    .line 448
    move/from16 v19, v2

    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v19, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    instance-of v2, v1, Landroidx/compose/ui/node/TraversableNode;

    move/from16 v20, v2

    if-eqz v20, :cond_3

    .line 449
    move-object v2, v1

    check-cast v2, Landroidx/compose/ui/node/TraversableNode;

    .local v2, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v21, 0x0

    .line 105
    .local v21, "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$traverseAncestors$2":I
    move/from16 v22, v3

    .end local v3    # "includeSelf$iv":Z
    .local v22, "includeSelf$iv":Z
    invoke-interface {v0}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v3

    move/from16 v23, v4

    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    .local v23, "$i$f$visitAncestors-Y-YKmho":I
    invoke-interface {v2}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v4

    invoke-static {v3, v4}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v3

    if-eqz v3, :cond_1

    invoke-static {v0, v2}, Landroidx/compose/ui/Actual_jvmKt;->areObjectsOfSameType(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v3

    if-eqz v3, :cond_1

    .line 106
    move-object/from16 v3, p1

    invoke-interface {v3, v2}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v4

    check-cast v4, Ljava/lang/Boolean;

    invoke-virtual {v4}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v4

    move/from16 v20, v4

    goto :goto_3

    .line 105
    :cond_1
    move-object/from16 v3, p1

    .line 108
    const/16 v20, 0x1

    .line 105
    :goto_3
    nop

    .line 104
    nop

    .line 110
    .local v20, "continueTraversal":Z
    if-nez v20, :cond_2

    return-void

    .line 111
    :cond_2
    nop

    .line 449
    .end local v2    # "it":Landroidx/compose/ui/node/TraversableNode;
    .end local v20    # "continueTraversal":Z
    .end local v21    # "$i$a$-visitAncestors-Y-YKmho$default-TraversableNodeKt$traverseAncestors$2":I
    goto/16 :goto_a

    .line 450
    .end local v22    # "includeSelf$iv":Z
    .end local v23    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v3    # "includeSelf$iv":Z
    .restart local v4    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_3
    move/from16 v22, v3

    move/from16 v23, v4

    move-object/from16 v3, p1

    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v22    # "includeSelf$iv":Z
    .restart local v23    # "$i$f$visitAncestors-Y-YKmho":I
    move v2, v14

    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object v4, v1

    .local v4, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v21, 0x0

    .line 451
    .local v21, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual {v4}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v24

    and-int v24, v24, v2

    if-eqz v24, :cond_4

    const/4 v2, 0x1

    goto :goto_4

    :cond_4
    const/4 v2, 0x0

    .line 450
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v4    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v21    # "$i$f$isKind-H91voCI$ui":I
    :goto_4
    if-eqz v2, :cond_e

    instance-of v2, v1, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v2, :cond_e

    .line 452
    const/4 v2, 0x0

    .line 453
    .local v2, "count$iv$iv":I
    move-object v4, v1

    check-cast v4, Landroidx/compose/ui/node/DelegatingNode;

    .local v4, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/16 v21, 0x0

    .line 454
    .local v21, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual {v4}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v24

    .line 455
    .local v24, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_5
    if-eqz v24, :cond_c

    .line 456
    move-object/from16 v25, v24

    .local v25, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v26, 0x0

    .line 457
    .local v26, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v27, v14

    .local v27, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v28, v25

    .local v28, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v29, 0x0

    .line 451
    .local v29, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v28 .. v28}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v30

    and-int v30, v30, v27

    if-eqz v30, :cond_5

    const/16 v27, 0x1

    goto :goto_6

    :cond_5
    const/16 v27, 0x0

    .line 457
    .end local v27    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v28    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v29    # "$i$f$isKind-H91voCI$ui":I
    :goto_6
    if-eqz v27, :cond_b

    .line 458
    add-int/lit8 v2, v2, 0x1

    .line 459
    const/4 v0, 0x1

    if-ne v2, v0, :cond_6

    .line 460
    move-object/from16 v1, v25

    move-object/from16 v0, v25

    goto :goto_9

    .line 464
    :cond_6
    if-nez v17, :cond_7

    const/4 v0, 0x0

    .line 465
    .local v0, "$i$f$mutableVectorOf":I
    nop

    .line 466
    move/from16 v28, v0

    .end local v0    # "$i$f$mutableVectorOf":I
    .local v28, "$i$f$mutableVectorOf":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v29, 0x0

    .line 467
    .local v29, "$i$f$MutableVector":I
    move-object/from16 v30, v1

    .end local v1    # "node$iv$iv":Ljava/lang/Object;
    .local v30, "node$iv$iv":Ljava/lang/Object;
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    move/from16 v31, v2

    .end local v2    # "count$iv$iv":I
    .local v31, "count$iv$iv":I
    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v32, v0

    const/4 v0, 0x0

    .end local v0    # "capacity$iv$iv$iv$iv":I
    .local v32, "capacity$iv$iv$iv$iv":I
    invoke-direct {v1, v2, v0}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 465
    .end local v29    # "$i$f$MutableVector":I
    .end local v32    # "capacity$iv$iv$iv$iv":I
    goto :goto_7

    .line 464
    .end local v28    # "$i$f$mutableVectorOf":I
    .end local v30    # "node$iv$iv":Ljava/lang/Object;
    .end local v31    # "count$iv$iv":I
    .restart local v1    # "node$iv$iv":Ljava/lang/Object;
    .restart local v2    # "count$iv$iv":I
    :cond_7
    move-object/from16 v30, v1

    move/from16 v31, v2

    const/4 v0, 0x0

    .end local v1    # "node$iv$iv":Ljava/lang/Object;
    .end local v2    # "count$iv$iv":I
    .restart local v30    # "node$iv$iv":Ljava/lang/Object;
    .restart local v31    # "count$iv$iv":I
    move-object/from16 v1, v17

    :goto_7
    nop

    .line 468
    .end local v17    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object/from16 v2, v30

    .line 469
    .local v2, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v2, :cond_9

    .line 470
    if-eqz v1, :cond_8

    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 471
    :cond_8
    const/16 v17, 0x0

    move-object/from16 v30, v17

    .line 473
    :cond_9
    if-eqz v1, :cond_a

    move-object/from16 v0, v25

    .end local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v0, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_8

    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_a
    move-object/from16 v0, v25

    .line 476
    .end local v2    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_8
    move-object/from16 v17, v1

    move-object/from16 v1, v30

    move/from16 v2, v31

    goto :goto_9

    .line 457
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v30    # "node$iv$iv":Ljava/lang/Object;
    .end local v31    # "count$iv$iv":I
    .local v1, "node$iv$iv":Ljava/lang/Object;
    .local v2, "count$iv$iv":I
    .restart local v17    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_b
    move-object/from16 v30, v1

    move-object/from16 v0, v25

    .line 476
    .end local v25    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_9
    nop

    .line 456
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v26    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 477
    invoke-virtual/range {v24 .. v24}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v24

    goto :goto_5

    .line 479
    :cond_c
    move-object/from16 v30, v1

    .line 480
    .end local v1    # "node$iv$iv":Ljava/lang/Object;
    .end local v4    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v21    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v24    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v30    # "node$iv$iv":Ljava/lang/Object;
    const/4 v0, 0x1

    if-ne v2, v0, :cond_d

    .line 482
    move-object/from16 v0, p0

    move/from16 v2, v19

    move/from16 v3, v22

    move/from16 v4, v23

    move-object/from16 v1, v30

    goto/16 :goto_2

    .line 480
    :cond_d
    move-object/from16 v1, v30

    .line 485
    .end local v2    # "count$iv$iv":I
    .end local v30    # "node$iv$iv":Ljava/lang/Object;
    .restart local v1    # "node$iv$iv":Ljava/lang/Object;
    :cond_e
    :goto_a
    invoke-static/range {v17 .. v17}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v1

    move-object/from16 v0, p0

    move/from16 v2, v19

    move/from16 v3, v22

    move/from16 v4, v23

    goto/16 :goto_2

    .line 487
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "includeSelf$iv":Z
    .end local v23    # "$i$f$visitAncestors-Y-YKmho":I
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    .local v4, "$i$f$visitAncestors-Y-YKmho":I
    :cond_f
    move/from16 v19, v2

    move/from16 v22, v3

    move/from16 v23, v4

    move-object/from16 v3, p1

    .line 431
    .end local v1    # "node$iv$iv":Ljava/lang/Object;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    .end local v14    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v15    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v16    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v17    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "includeSelf$iv":Z
    .restart local v23    # "$i$f$visitAncestors-Y-YKmho":I
    nop

    .line 444
    .end local v12    # "it$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v13    # "$i$a$-visitAncestors-DelegatableNodeKt$visitAncestors$2$iv":I
    goto :goto_b

    .line 443
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "includeSelf$iv":Z
    .end local v23    # "$i$f$visitAncestors-Y-YKmho":I
    .local v1, "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    .restart local v4    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_10
    move-object/from16 v18, v1

    move/from16 v19, v2

    move/from16 v22, v3

    move/from16 v23, v4

    move-object/from16 v3, p1

    .line 488
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "includeSelf$iv":Z
    .restart local v23    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_b
    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getParent$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    move-object/from16 v0, p0

    move-object/from16 v1, v18

    move/from16 v2, v19

    move/from16 v3, v22

    move/from16 v4, v23

    goto/16 :goto_1

    .line 442
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "includeSelf$iv":Z
    .end local v23    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    .restart local v4    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_11
    move-object/from16 v18, v1

    move/from16 v19, v2

    move/from16 v22, v3

    move/from16 v23, v4

    move-object/from16 v3, p1

    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "includeSelf$iv":Z
    .restart local v23    # "$i$f$visitAncestors-Y-YKmho":I
    goto :goto_c

    .line 441
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "includeSelf$iv":Z
    .end local v23    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    .restart local v4    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_12
    move-object/from16 v18, v1

    move/from16 v19, v2

    move/from16 v22, v3

    move/from16 v23, v4

    move-object/from16 v3, p1

    .line 491
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v22    # "includeSelf$iv":Z
    .restart local v23    # "$i$f$visitAncestors-Y-YKmho":I
    :goto_c
    invoke-virtual {v10}, Landroidx/compose/ui/node/LayoutNode;->getParent$ui()Landroidx/compose/ui/node/LayoutNode;

    move-result-object v10

    .line 492
    if-eqz v10, :cond_13

    invoke-virtual {v10}, Landroidx/compose/ui/node/LayoutNode;->getNodes$ui()Landroidx/compose/ui/node/NodeChain;

    move-result-object v0

    if-eqz v0, :cond_13

    invoke-virtual {v0}, Landroidx/compose/ui/node/NodeChain;->getTail$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v0

    goto :goto_d

    :cond_13
    const/4 v0, 0x0

    :goto_d
    move-object v9, v0

    move-object/from16 v0, p0

    move-object/from16 v1, v18

    move/from16 v2, v19

    move/from16 v3, v22

    move/from16 v4, v23

    .end local v11    # "head$iv$iv":Landroidx/compose/ui/Modifier$Node;
    goto/16 :goto_0

    .line 494
    .end local v18    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v19    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v22    # "includeSelf$iv":Z
    .end local v23    # "$i$f$visitAncestors-Y-YKmho":I
    .restart local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "includeSelf$iv":Z
    .restart local v4    # "$i$f$visitAncestors-Y-YKmho":I
    :cond_14
    nop

    .line 431
    .end local v5    # "includeSelf$iv$iv":Z
    .end local v6    # "$this$visitAncestors$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v7    # "mask$iv$iv":I
    .end local v8    # "$i$f$visitAncestors":I
    .end local v9    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v10    # "layout$iv$iv":Landroidx/compose/ui/node/LayoutNode;
    nop

    .line 112
    .end local v1    # "$this$visitAncestors_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "includeSelf$iv":Z
    .end local v4    # "$i$f$visitAncestors-Y-YKmho":I
    return-void
.end method

.method public static final traverseChildren(Landroidx/compose/ui/node/DelegatableNode;Ljava/lang/Object;Lkotlin/jvm/functions/Function1;)V
    .locals 32
    .param p0, "$this$traverseChildren"    # Landroidx/compose/ui/node/DelegatableNode;
    .param p1, "key"    # Ljava/lang/Object;
    .param p2, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/ui/node/DelegatableNode;",
            "Ljava/lang/Object;",
            "Lkotlin/jvm/functions/Function1<",
            "-",
            "Landroidx/compose/ui/node/TraversableNode;",
            "Ljava/lang/Boolean;",
            ">;)V"
        }
    .end annotation

    .line 126
    const/4 v0, 0x0

    .line 495
    .local v0, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v1, 0x40000

    invoke-static {v1}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v0

    .line 126
    .end local v0    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    move-object/from16 v1, p0

    .line 496
    .local v1, "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    nop

    .line 498
    const/4 v2, 0x0

    .line 496
    .local v2, "zOrder$iv":Z
    const/4 v3, 0x0

    .line 500
    .local v3, "$i$f$visitChildren-Y-YKmho":I
    move v4, v2

    .local v4, "zOrder$iv$iv":Z
    move v5, v0

    .local v5, "mask$iv$iv":I
    move-object v6, v1

    .local v6, "$this$visitChildren$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    const/4 v7, 0x0

    .line 501
    .local v7, "$i$f$visitChildren":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v8

    .local v8, "value$iv$iv$iv":Z
    const/4 v9, 0x0

    .line 502
    .local v9, "$i$f$checkPrecondition":I
    if-nez v8, :cond_0

    .line 503
    const/4 v10, 0x0

    .line 501
    .local v10, "$i$a$-checkPrecondition-DelegatableNodeKt$visitChildren$1$iv$iv":I
    nop

    .line 503
    .end local v10    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitChildren$1$iv$iv":I
    const-string/jumbo v10, "visitChildren called on an unattached node"

    invoke-static {v10}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 505
    :cond_0
    nop

    .line 506
    .end local v8    # "value$iv$iv$iv":Z
    .end local v9    # "$i$f$checkPrecondition":I
    const/4 v8, 0x0

    .line 507
    .local v8, "$i$f$mutableVectorOf":I
    nop

    .line 508
    const/16 v9, 0x10

    .local v9, "capacity$iv$iv$iv$iv":I
    const/4 v10, 0x0

    .line 509
    .local v10, "$i$f$MutableVector":I
    new-instance v11, Landroidx/compose/runtime/collection/MutableVector;

    new-array v12, v9, [Landroidx/compose/ui/Modifier$Node;

    const/4 v13, 0x0

    invoke-direct {v11, v12, v13}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 507
    .end local v9    # "capacity$iv$iv$iv$iv":I
    .end local v10    # "$i$f$MutableVector":I
    nop

    .line 506
    .end local v8    # "$i$f$mutableVectorOf":I
    nop

    .line 510
    .local v11, "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    .line 511
    .local v8, "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-nez v8, :cond_1

    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-static {v11, v9, v4}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    goto :goto_0

    :cond_1
    invoke-virtual {v11, v8}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 512
    :goto_0
    move-object v9, v11

    .local v9, "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v10, 0x0

    .line 513
    .local v10, "$i$f$isNotEmpty":I
    invoke-virtual {v9}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v12

    const/4 v14, 0x1

    if-eqz v12, :cond_2

    move v9, v14

    goto :goto_1

    :cond_2
    move v9, v13

    .end local v9    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v10    # "$i$f$isNotEmpty":I
    :goto_1
    if-eqz v9, :cond_15

    .line 514
    move-object v9, v11

    .restart local v9    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v10, 0x0

    .line 515
    .local v10, "$i$f$getLastIndex":I
    invoke-virtual {v9}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v12

    sub-int/2addr v12, v14

    .line 514
    .end local v9    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v10    # "$i$f$getLastIndex":I
    invoke-virtual {v11, v12}, Landroidx/compose/runtime/collection/MutableVector;->removeAt(I)Ljava/lang/Object;

    move-result-object v9

    check-cast v9, Landroidx/compose/ui/Modifier$Node;

    .line 516
    .local v9, "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v10

    and-int/2addr v10, v5

    if-nez v10, :cond_3

    .line 517
    invoke-static {v11, v9, v4}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    .line 519
    goto :goto_0

    .line 521
    :cond_3
    move-object v10, v9

    .line 522
    .local v10, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_2
    if-eqz v10, :cond_14

    .line 523
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v12

    and-int/2addr v12, v5

    if-eqz v12, :cond_13

    .line 524
    move-object v12, v10

    .local v12, "it$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v15, 0x0

    .line 500
    .local v15, "$i$a$-visitChildren-DelegatableNodeKt$visitChildren$2$iv":I
    move/from16 v16, v0

    .local v16, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object/from16 v17, v12

    .local v17, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v18, 0x0

    .line 525
    .local v18, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v19, 0x0

    .line 526
    .local v19, "stack$iv$iv":Ljava/lang/Object;
    const/16 v20, 0x0

    .local v20, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v20, v17

    move-object/from16 v13, v20

    .line 527
    .end local v20    # "node$iv$iv":Ljava/lang/Object;
    .local v13, "node$iv$iv":Ljava/lang/Object;
    :goto_3
    if-eqz v13, :cond_12

    .line 528
    instance-of v14, v13, Landroidx/compose/ui/node/TraversableNode;

    if-eqz v14, :cond_6

    .line 529
    move-object v14, v13

    check-cast v14, Landroidx/compose/ui/node/TraversableNode;

    .local v14, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v21, 0x0

    .line 128
    .local v21, "$i$a$-visitChildren-Y-YKmho$default-TraversableNodeKt$traverseChildren$1":I
    move/from16 v22, v0

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v22, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    invoke-interface {v14}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v0

    move-object/from16 v23, v1

    move-object/from16 v1, p1

    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v23, "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_4

    .line 129
    move-object/from16 v0, p2

    invoke-interface {v0, v14}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v24

    check-cast v24, Ljava/lang/Boolean;

    invoke-virtual/range {v24 .. v24}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v24

    goto :goto_4

    .line 131
    :cond_4
    move-object/from16 v0, p2

    const/16 v24, 0x1

    .line 128
    :goto_4
    nop

    .line 127
    nop

    .line 133
    .local v24, "continueTraversal":Z
    if-nez v24, :cond_5

    return-void

    .line 134
    :cond_5
    nop

    .line 529
    .end local v14    # "it":Landroidx/compose/ui/node/TraversableNode;
    .end local v21    # "$i$a$-visitChildren-Y-YKmho$default-TraversableNodeKt$traverseChildren$1":I
    .end local v24    # "continueTraversal":Z
    move/from16 v30, v2

    const/4 v0, 0x1

    goto/16 :goto_b

    .line 530
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    :cond_6
    move/from16 v22, v0

    move-object/from16 v23, v1

    move-object/from16 v1, p1

    move-object/from16 v0, p2

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    move/from16 v14, v16

    .local v14, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v21, v13

    .local v21, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v24, 0x0

    .line 531
    .local v24, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v25

    and-int v25, v25, v14

    if-eqz v25, :cond_7

    const/4 v14, 0x1

    goto :goto_5

    :cond_7
    const/4 v14, 0x0

    .line 530
    .end local v14    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v21    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v24    # "$i$f$isKind-H91voCI$ui":I
    :goto_5
    if-eqz v14, :cond_10

    instance-of v14, v13, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v14, :cond_10

    .line 532
    const/4 v14, 0x0

    .line 533
    .local v14, "count$iv$iv":I
    move-object/from16 v21, v13

    check-cast v21, Landroidx/compose/ui/node/DelegatingNode;

    .local v21, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/16 v24, 0x0

    .line 534
    .local v24, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v25

    .line 535
    .local v25, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_6
    if-eqz v25, :cond_f

    .line 536
    move-object/from16 v26, v25

    .local v26, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v27, 0x0

    .line 537
    .local v27, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v28, v16

    .local v28, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v29, v26

    .local v29, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v30, 0x0

    .line 531
    .local v30, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v29 .. v29}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v31

    and-int v31, v31, v28

    if-eqz v31, :cond_8

    const/16 v28, 0x1

    goto :goto_7

    :cond_8
    const/16 v28, 0x0

    .line 537
    .end local v28    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v29    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v30    # "$i$f$isKind-H91voCI$ui":I
    :goto_7
    if-eqz v28, :cond_e

    .line 538
    add-int/lit8 v14, v14, 0x1

    .line 539
    const/4 v0, 0x1

    if-ne v14, v0, :cond_9

    .line 540
    move-object/from16 v13, v26

    move/from16 v30, v2

    move-object/from16 v0, v26

    goto :goto_a

    .line 544
    :cond_9
    if-nez v19, :cond_a

    const/4 v0, 0x0

    .line 507
    .local v0, "$i$f$mutableVectorOf":I
    nop

    .line 508
    move/from16 v28, v0

    .end local v0    # "$i$f$mutableVectorOf":I
    .local v28, "$i$f$mutableVectorOf":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v29, 0x0

    .line 509
    .local v29, "$i$f$MutableVector":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    move/from16 v30, v2

    .end local v2    # "zOrder$iv":Z
    .local v30, "zOrder$iv":Z
    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v31, v0

    const/4 v0, 0x0

    .end local v0    # "capacity$iv$iv$iv$iv":I
    .local v31, "capacity$iv$iv$iv$iv":I
    invoke-direct {v1, v2, v0}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 507
    .end local v29    # "$i$f$MutableVector":I
    .end local v31    # "capacity$iv$iv$iv$iv":I
    goto :goto_8

    .line 544
    .end local v28    # "$i$f$mutableVectorOf":I
    .end local v30    # "zOrder$iv":Z
    .restart local v2    # "zOrder$iv":Z
    :cond_a
    move/from16 v30, v2

    const/4 v0, 0x0

    .end local v2    # "zOrder$iv":Z
    .restart local v30    # "zOrder$iv":Z
    move-object/from16 v1, v19

    :goto_8
    nop

    .line 545
    .end local v19    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object v2, v13

    .line 546
    .local v2, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v2, :cond_c

    .line 547
    if-eqz v1, :cond_b

    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 548
    :cond_b
    const/4 v13, 0x0

    .line 550
    :cond_c
    if-eqz v1, :cond_d

    move-object/from16 v0, v26

    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v0, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_9

    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_d
    move-object/from16 v0, v26

    .line 553
    .end local v2    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_9
    move-object/from16 v19, v1

    goto :goto_a

    .line 537
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v1    # "stack$iv$iv":Ljava/lang/Object;
    .end local v30    # "zOrder$iv":Z
    .local v2, "zOrder$iv":Z
    .restart local v19    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_e
    move/from16 v30, v2

    move-object/from16 v0, v26

    .line 553
    .end local v2    # "zOrder$iv":Z
    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v30    # "zOrder$iv":Z
    :goto_a
    nop

    .line 536
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v27    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 554
    invoke-virtual/range {v25 .. v25}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v25

    move-object/from16 v1, p1

    move-object/from16 v0, p2

    move/from16 v2, v30

    goto :goto_6

    .line 556
    .end local v30    # "zOrder$iv":Z
    .restart local v2    # "zOrder$iv":Z
    :cond_f
    move/from16 v30, v2

    .line 557
    .end local v2    # "zOrder$iv":Z
    .end local v21    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v24    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v25    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v30    # "zOrder$iv":Z
    const/4 v0, 0x1

    if-ne v14, v0, :cond_11

    .line 559
    move v14, v0

    move/from16 v0, v22

    move-object/from16 v1, v23

    move/from16 v2, v30

    goto/16 :goto_3

    .line 530
    .end local v14    # "count$iv$iv":I
    .end local v30    # "zOrder$iv":Z
    .restart local v2    # "zOrder$iv":Z
    :cond_10
    move/from16 v30, v2

    const/4 v0, 0x1

    .line 562
    .end local v2    # "zOrder$iv":Z
    .restart local v30    # "zOrder$iv":Z
    :cond_11
    :goto_b
    invoke-static/range {v19 .. v19}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v13

    move v14, v0

    move/from16 v0, v22

    move-object/from16 v1, v23

    move/from16 v2, v30

    goto/16 :goto_3

    .line 564
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "zOrder$iv":Z
    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v1, "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    :cond_12
    move/from16 v22, v0

    move-object/from16 v23, v1

    move/from16 v30, v2

    .line 500
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v13    # "node$iv$iv":Ljava/lang/Object;
    .end local v16    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v17    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v18    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v19    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "zOrder$iv":Z
    nop

    .line 524
    .end local v12    # "it$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v15    # "$i$a$-visitChildren-DelegatableNodeKt$visitChildren$2$iv":I
    nop

    .line 565
    const/4 v13, 0x0

    goto/16 :goto_0

    .line 567
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "zOrder$iv":Z
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    :cond_13
    move/from16 v22, v0

    move-object/from16 v23, v1

    move/from16 v30, v2

    move v0, v14

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "zOrder$iv":Z
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    move/from16 v0, v22

    const/4 v13, 0x0

    goto/16 :goto_2

    .line 522
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "zOrder$iv":Z
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    :cond_14
    move/from16 v22, v0

    move-object/from16 v23, v1

    move/from16 v30, v2

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v30    # "zOrder$iv":Z
    const/4 v13, 0x0

    goto/16 :goto_0

    .line 570
    .end local v9    # "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v10    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v30    # "zOrder$iv":Z
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    :cond_15
    nop

    .line 500
    .end local v4    # "zOrder$iv$iv":Z
    .end local v5    # "mask$iv$iv":I
    .end local v6    # "$this$visitChildren$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v7    # "$i$f$visitChildren":I
    .end local v8    # "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v11    # "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    nop

    .line 135
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v3    # "$i$f$visitChildren-Y-YKmho":I
    return-void
.end method

.method public static final traverseChildren(Landroidx/compose/ui/node/TraversableNode;Lkotlin/jvm/functions/Function1;)V
    .locals 32
    .param p0, "$this$traverseChildren"    # Landroidx/compose/ui/node/TraversableNode;
    .param p1, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T::",
            "Landroidx/compose/ui/node/TraversableNode;",
            ">(TT;",
            "Lkotlin/jvm/functions/Function1<",
            "-TT;",
            "Ljava/lang/Boolean;",
            ">;)V"
        }
    .end annotation

    .line 148
    move-object/from16 v0, p0

    move-object v1, v0

    check-cast v1, Landroidx/compose/ui/node/DelegatableNode;

    const/4 v2, 0x0

    .line 571
    .local v2, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v3, 0x40000

    invoke-static {v3}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v2

    .line 148
    .end local v2    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .line 572
    .local v1, "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    nop

    .line 574
    const/4 v3, 0x0

    .line 572
    .local v3, "zOrder$iv":Z
    const/4 v4, 0x0

    .line 576
    .local v4, "$i$f$visitChildren-Y-YKmho":I
    move v5, v3

    .local v5, "zOrder$iv$iv":Z
    move-object v6, v1

    .local v6, "$this$visitChildren$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    move v7, v2

    .local v7, "mask$iv$iv":I
    const/4 v8, 0x0

    .line 577
    .local v8, "$i$f$visitChildren":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v9

    .local v9, "value$iv$iv$iv":Z
    const/4 v10, 0x0

    .line 578
    .local v10, "$i$f$checkPrecondition":I
    if-nez v9, :cond_0

    .line 579
    const/4 v11, 0x0

    .line 577
    .local v11, "$i$a$-checkPrecondition-DelegatableNodeKt$visitChildren$1$iv$iv":I
    nop

    .line 579
    .end local v11    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitChildren$1$iv$iv":I
    const-string/jumbo v11, "visitChildren called on an unattached node"

    invoke-static {v11}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 581
    :cond_0
    nop

    .line 582
    .end local v9    # "value$iv$iv$iv":Z
    .end local v10    # "$i$f$checkPrecondition":I
    const/4 v9, 0x0

    .line 583
    .local v9, "$i$f$mutableVectorOf":I
    nop

    .line 584
    const/16 v10, 0x10

    .local v10, "capacity$iv$iv$iv$iv":I
    const/4 v11, 0x0

    .line 585
    .local v11, "$i$f$MutableVector":I
    new-instance v12, Landroidx/compose/runtime/collection/MutableVector;

    new-array v13, v10, [Landroidx/compose/ui/Modifier$Node;

    const/4 v14, 0x0

    invoke-direct {v12, v13, v14}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 583
    .end local v10    # "capacity$iv$iv$iv$iv":I
    .end local v11    # "$i$f$MutableVector":I
    nop

    .line 582
    .end local v9    # "$i$f$mutableVectorOf":I
    nop

    .line 586
    .local v12, "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    .line 587
    .local v9, "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-nez v9, :cond_1

    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    invoke-static {v12, v10, v5}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    goto :goto_0

    :cond_1
    invoke-virtual {v12, v9}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 588
    :goto_0
    move-object v10, v12

    .local v10, "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v11, 0x0

    .line 589
    .local v11, "$i$f$isNotEmpty":I
    invoke-virtual {v10}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v13

    const/4 v15, 0x1

    if-eqz v13, :cond_2

    move v10, v15

    goto :goto_1

    :cond_2
    move v10, v14

    .end local v10    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v11    # "$i$f$isNotEmpty":I
    :goto_1
    if-eqz v10, :cond_15

    .line 590
    move-object v10, v12

    .restart local v10    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v11, 0x0

    .line 591
    .local v11, "$i$f$getLastIndex":I
    invoke-virtual {v10}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v13

    sub-int/2addr v13, v15

    .line 590
    .end local v10    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v11    # "$i$f$getLastIndex":I
    invoke-virtual {v12, v13}, Landroidx/compose/runtime/collection/MutableVector;->removeAt(I)Ljava/lang/Object;

    move-result-object v10

    check-cast v10, Landroidx/compose/ui/Modifier$Node;

    .line 592
    .local v10, "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v11

    and-int/2addr v11, v7

    if-nez v11, :cond_3

    .line 593
    invoke-static {v12, v10, v5}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    .line 595
    goto :goto_0

    .line 597
    :cond_3
    move-object v11, v10

    .line 598
    .local v11, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_2
    if-eqz v11, :cond_14

    .line 599
    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v13

    and-int/2addr v13, v7

    if-eqz v13, :cond_13

    .line 600
    move-object v13, v11

    .local v13, "it$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v16, 0x0

    .line 576
    .local v16, "$i$a$-visitChildren-DelegatableNodeKt$visitChildren$2$iv":I
    move/from16 v17, v2

    .local v17, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object/from16 v18, v13

    .local v18, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v19, 0x0

    .line 601
    .local v19, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v20, 0x0

    .line 602
    .local v20, "stack$iv$iv":Ljava/lang/Object;
    const/16 v21, 0x0

    .local v21, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v21, v18

    move-object/from16 v14, v21

    .line 603
    .end local v21    # "node$iv$iv":Ljava/lang/Object;
    .local v14, "node$iv$iv":Ljava/lang/Object;
    :goto_3
    if-eqz v14, :cond_12

    .line 604
    instance-of v15, v14, Landroidx/compose/ui/node/TraversableNode;

    if-eqz v15, :cond_6

    .line 605
    move-object v15, v14

    check-cast v15, Landroidx/compose/ui/node/TraversableNode;

    .local v15, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v22, 0x0

    .line 150
    .local v22, "$i$a$-visitChildren-Y-YKmho$default-TraversableNodeKt$traverseChildren$2":I
    move-object/from16 v23, v1

    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v23, "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    invoke-interface {v0}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v1

    move/from16 v24, v2

    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v24, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    invoke-interface {v15}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v2

    invoke-static {v1, v2}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v1

    if-eqz v1, :cond_4

    invoke-static {v0, v15}, Landroidx/compose/ui/Actual_jvmKt;->areObjectsOfSameType(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v1

    if-eqz v1, :cond_4

    .line 151
    move-object/from16 v1, p1

    invoke-interface {v1, v15}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/Boolean;

    invoke-virtual {v2}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    goto :goto_4

    .line 150
    :cond_4
    move-object/from16 v1, p1

    .line 153
    const/4 v2, 0x1

    .line 150
    :goto_4
    nop

    .line 149
    nop

    .line 155
    .local v2, "continueTraversal":Z
    if-nez v2, :cond_5

    return-void

    .line 156
    :cond_5
    nop

    .line 605
    .end local v2    # "continueTraversal":Z
    .end local v15    # "it":Landroidx/compose/ui/node/TraversableNode;
    .end local v22    # "$i$a$-visitChildren-Y-YKmho$default-TraversableNodeKt$traverseChildren$2":I
    const/4 v0, 0x1

    goto/16 :goto_b

    .line 606
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    :cond_6
    move-object/from16 v23, v1

    move/from16 v24, v2

    move-object/from16 v1, p1

    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    move/from16 v2, v17

    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object v15, v14

    .local v15, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v22, 0x0

    .line 607
    .local v22, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual {v15}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v25

    and-int v25, v25, v2

    if-eqz v25, :cond_7

    const/4 v2, 0x1

    goto :goto_5

    :cond_7
    const/4 v2, 0x0

    .line 606
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v15    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v22    # "$i$f$isKind-H91voCI$ui":I
    :goto_5
    if-eqz v2, :cond_10

    instance-of v2, v14, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v2, :cond_10

    .line 608
    const/4 v2, 0x0

    .line 609
    .local v2, "count$iv$iv":I
    move-object v15, v14

    check-cast v15, Landroidx/compose/ui/node/DelegatingNode;

    .local v15, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/16 v22, 0x0

    .line 610
    .local v22, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual {v15}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v25

    .line 611
    .local v25, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_6
    if-eqz v25, :cond_f

    .line 612
    move-object/from16 v26, v25

    .local v26, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v27, 0x0

    .line 613
    .local v27, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v28, v17

    .local v28, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v29, v26

    .local v29, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v30, 0x0

    .line 607
    .local v30, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v29 .. v29}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v31

    and-int v31, v31, v28

    if-eqz v31, :cond_8

    const/16 v28, 0x1

    goto :goto_7

    :cond_8
    const/16 v28, 0x0

    .line 613
    .end local v28    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v29    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v30    # "$i$f$isKind-H91voCI$ui":I
    :goto_7
    if-eqz v28, :cond_e

    .line 614
    add-int/lit8 v2, v2, 0x1

    .line 615
    const/4 v0, 0x1

    if-ne v2, v0, :cond_9

    .line 616
    move-object/from16 v14, v26

    move-object/from16 v0, v26

    goto :goto_a

    .line 620
    :cond_9
    if-nez v20, :cond_a

    const/4 v0, 0x0

    .line 583
    .local v0, "$i$f$mutableVectorOf":I
    nop

    .line 584
    move/from16 v28, v0

    .end local v0    # "$i$f$mutableVectorOf":I
    .local v28, "$i$f$mutableVectorOf":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v29, 0x0

    .line 585
    .local v29, "$i$f$MutableVector":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    move/from16 v30, v2

    .end local v2    # "count$iv$iv":I
    .local v30, "count$iv$iv":I
    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v31, v0

    const/4 v0, 0x0

    .end local v0    # "capacity$iv$iv$iv$iv":I
    .local v31, "capacity$iv$iv$iv$iv":I
    invoke-direct {v1, v2, v0}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 583
    .end local v29    # "$i$f$MutableVector":I
    .end local v31    # "capacity$iv$iv$iv$iv":I
    goto :goto_8

    .line 620
    .end local v28    # "$i$f$mutableVectorOf":I
    .end local v30    # "count$iv$iv":I
    .restart local v2    # "count$iv$iv":I
    :cond_a
    move/from16 v30, v2

    const/4 v0, 0x0

    .end local v2    # "count$iv$iv":I
    .restart local v30    # "count$iv$iv":I
    move-object/from16 v1, v20

    :goto_8
    nop

    .line 621
    .end local v20    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object v2, v14

    .line 622
    .local v2, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v2, :cond_c

    .line 623
    if-eqz v1, :cond_b

    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 624
    :cond_b
    const/4 v14, 0x0

    .line 626
    :cond_c
    if-eqz v1, :cond_d

    move-object/from16 v0, v26

    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v0, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_9

    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_d
    move-object/from16 v0, v26

    .line 629
    .end local v2    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_9
    move-object/from16 v20, v1

    move/from16 v2, v30

    goto :goto_a

    .line 613
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v1    # "stack$iv$iv":Ljava/lang/Object;
    .end local v30    # "count$iv$iv":I
    .local v2, "count$iv$iv":I
    .restart local v20    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_e
    move-object/from16 v0, v26

    .line 629
    .end local v26    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_a
    nop

    .line 612
    .end local v0    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v27    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 630
    invoke-virtual/range {v25 .. v25}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v25

    move-object/from16 v0, p0

    move-object/from16 v1, p1

    goto :goto_6

    .line 632
    :cond_f
    nop

    .line 633
    .end local v15    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v22    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v25    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v0, 0x1

    if-ne v2, v0, :cond_11

    .line 635
    move v15, v0

    move-object/from16 v1, v23

    move/from16 v2, v24

    move-object/from16 v0, p0

    goto/16 :goto_3

    .line 606
    .end local v2    # "count$iv$iv":I
    :cond_10
    const/4 v0, 0x1

    .line 638
    :cond_11
    :goto_b
    invoke-static/range {v20 .. v20}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v14

    move v15, v0

    move-object/from16 v1, v23

    move/from16 v2, v24

    move-object/from16 v0, p0

    goto/16 :goto_3

    .line 640
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v1, "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    :cond_12
    move-object/from16 v23, v1

    move/from16 v24, v2

    .line 576
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v14    # "node$iv$iv":Ljava/lang/Object;
    .end local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v18    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v19    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v20    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    nop

    .line 600
    .end local v13    # "it$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v16    # "$i$a$-visitChildren-DelegatableNodeKt$visitChildren$2$iv":I
    nop

    .line 641
    move-object/from16 v0, p0

    const/4 v14, 0x0

    goto/16 :goto_0

    .line 643
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    :cond_13
    move-object/from16 v23, v1

    move/from16 v24, v2

    move v0, v15

    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v11

    const/4 v14, 0x0

    move-object/from16 v0, p0

    goto/16 :goto_2

    .line 598
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    :cond_14
    move-object/from16 v23, v1

    move/from16 v24, v2

    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    move-object/from16 v0, p0

    const/4 v14, 0x0

    goto/16 :goto_0

    .line 646
    .end local v10    # "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v11    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v23    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v24    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    :cond_15
    nop

    .line 576
    .end local v5    # "zOrder$iv$iv":Z
    .end local v6    # "$this$visitChildren$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v7    # "mask$iv$iv":I
    .end local v8    # "$i$f$visitChildren":I
    .end local v9    # "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v12    # "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    nop

    .line 157
    .end local v1    # "$this$visitChildren_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .end local v4    # "$i$f$visitChildren-Y-YKmho":I
    return-void
.end method

.method public static final traverseDescendants(Landroidx/compose/ui/node/DelegatableNode;Ljava/lang/Object;Lkotlin/jvm/functions/Function1;)V
    .locals 32
    .param p0, "$this$traverseDescendants"    # Landroidx/compose/ui/node/DelegatableNode;
    .param p1, "key"    # Ljava/lang/Object;
    .param p2, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/ui/node/DelegatableNode;",
            "Ljava/lang/Object;",
            "Lkotlin/jvm/functions/Function1<",
            "-",
            "Landroidx/compose/ui/node/TraversableNode;",
            "+",
            "Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;",
            ">;)V"
        }
    .end annotation

    .line 175
    const/4 v0, 0x0

    .line 647
    .local v0, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v1, 0x40000

    invoke-static {v1}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v0

    .line 175
    .end local v0    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    move-object/from16 v1, p0

    .line 648
    .local v1, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    nop

    .line 650
    const/4 v2, 0x0

    .line 648
    .local v2, "zOrder$iv":Z
    const/4 v3, 0x0

    .line 653
    .local v3, "$i$f$visitSubtreeIf-Y-YKmho":I
    move v4, v2

    .local v4, "zOrder$iv$iv":Z
    move v5, v0

    .local v5, "mask$iv$iv":I
    move-object v6, v1

    .local v6, "$this$visitSubtreeIf$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    const/4 v7, 0x0

    .line 654
    .local v7, "$i$f$visitSubtreeIf":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v8

    .local v8, "value$iv$iv$iv":Z
    const/4 v9, 0x0

    .line 655
    .local v9, "$i$f$checkPrecondition":I
    if-nez v8, :cond_0

    .line 656
    const/4 v10, 0x0

    .line 654
    .local v10, "$i$a$-checkPrecondition-DelegatableNodeKt$visitSubtreeIf$1$iv$iv":I
    nop

    .line 656
    .end local v10    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitSubtreeIf$1$iv$iv":I
    const-string/jumbo v10, "visitSubtreeIf called on an unattached node"

    invoke-static {v10}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 658
    :cond_0
    nop

    .line 659
    .end local v8    # "value$iv$iv$iv":Z
    .end local v9    # "$i$f$checkPrecondition":I
    const/4 v8, 0x0

    .line 660
    .local v8, "$i$f$mutableVectorOf":I
    nop

    .line 661
    const/16 v9, 0x10

    .local v9, "capacity$iv$iv$iv$iv":I
    const/4 v10, 0x0

    .line 662
    .local v10, "$i$f$MutableVector":I
    new-instance v11, Landroidx/compose/runtime/collection/MutableVector;

    new-array v12, v9, [Landroidx/compose/ui/Modifier$Node;

    const/4 v13, 0x0

    invoke-direct {v11, v12, v13}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 660
    .end local v9    # "capacity$iv$iv$iv$iv":I
    .end local v10    # "$i$f$MutableVector":I
    nop

    .line 659
    .end local v8    # "$i$f$mutableVectorOf":I
    nop

    .line 663
    .local v11, "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    invoke-virtual {v8}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v8

    .line 664
    .local v8, "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-nez v8, :cond_1

    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-static {v11, v9, v4}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    goto :goto_0

    :cond_1
    invoke-virtual {v11, v8}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 665
    :goto_0
    move-object v9, v11

    .local v9, "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v10, 0x0

    .line 666
    .local v10, "$i$f$isNotEmpty":I
    invoke-virtual {v9}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v12

    const/4 v14, 0x1

    if-eqz v12, :cond_2

    move v9, v14

    goto :goto_1

    :cond_2
    move v9, v13

    .end local v9    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v10    # "$i$f$isNotEmpty":I
    :goto_1
    if-eqz v9, :cond_18

    .line 667
    invoke-virtual {v11}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v9

    sub-int/2addr v9, v14

    invoke-virtual {v11, v9}, Landroidx/compose/runtime/collection/MutableVector;->removeAt(I)Ljava/lang/Object;

    move-result-object v9

    check-cast v9, Landroidx/compose/ui/Modifier$Node;

    .line 668
    .local v9, "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v10

    and-int/2addr v10, v5

    if-eqz v10, :cond_17

    .line 669
    move-object v10, v9

    .line 670
    .local v10, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_2
    if-eqz v10, :cond_16

    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v12

    if-eqz v12, :cond_16

    .line 671
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v12

    and-int/2addr v12, v5

    if-eqz v12, :cond_15

    .line 672
    move-object v12, v10

    .local v12, "node$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v15, 0x0

    .line 673
    .local v15, "$i$a$-visitSubtreeIf-DelegatableNodeKt$visitSubtreeIf$2$iv":I
    move/from16 v16, v0

    .local v16, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object/from16 v17, v12

    .local v17, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v18, 0x0

    .line 674
    .local v18, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v19, 0x0

    .line 675
    .local v19, "stack$iv$iv":Ljava/lang/Object;
    const/16 v20, 0x0

    .local v20, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v20, v17

    move-object/from16 v13, v20

    .line 676
    .end local v20    # "node$iv$iv":Ljava/lang/Object;
    .local v13, "node$iv$iv":Ljava/lang/Object;
    :goto_3
    if-eqz v13, :cond_13

    .line 677
    instance-of v14, v13, Landroidx/compose/ui/node/TraversableNode;

    if-eqz v14, :cond_7

    .line 678
    move-object v14, v13

    .local v14, "it$iv":Ljava/lang/Object;
    const/16 v21, 0x0

    .line 673
    .local v21, "$i$a$-dispatchForKind-6rFNWt0-DelegatableNodeKt$visitSubtreeIf$2$1$iv":I
    move/from16 v22, v0

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v22, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    move-object v0, v14

    check-cast v0, Landroidx/compose/ui/node/TraversableNode;

    .local v0, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v23, 0x0

    .line 177
    .local v23, "$i$a$-visitSubtreeIf-Y-YKmho$default-TraversableNodeKt$traverseDescendants$1":I
    move-object/from16 v24, v1

    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v24, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    invoke-interface {v0}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v1

    move/from16 v25, v2

    move-object/from16 v2, p1

    .end local v2    # "zOrder$iv":Z
    .local v25, "zOrder$iv":Z
    invoke-static {v2, v1}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v1

    if-eqz v1, :cond_3

    .line 178
    move-object/from16 v1, p2

    invoke-interface {v1, v0}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v26

    check-cast v26, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    goto :goto_4

    .line 180
    :cond_3
    move-object/from16 v1, p2

    sget-object v26, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;->ContinueTraversal:Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    .line 177
    :goto_4
    nop

    .line 176
    move-object/from16 v27, v26

    .line 182
    .local v27, "action":Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;
    move-object/from16 v26, v0

    .end local v0    # "it":Landroidx/compose/ui/node/TraversableNode;
    .local v26, "it":Landroidx/compose/ui/node/TraversableNode;
    sget-object v0, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;->CancelTraversal:Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    move-object/from16 v1, v27

    .end local v27    # "action":Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;
    .local v1, "action":Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;
    if-ne v1, v0, :cond_4

    return-void

    .line 187
    :cond_4
    sget-object v0, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;->SkipSubtreeAndContinueTraversal:Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    if-eq v1, v0, :cond_5

    const/4 v0, 0x1

    goto :goto_5

    :cond_5
    const/4 v0, 0x0

    .line 673
    .end local v1    # "action":Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;
    .end local v23    # "$i$a$-visitSubtreeIf-Y-YKmho$default-TraversableNodeKt$traverseDescendants$1":I
    .end local v26    # "it":Landroidx/compose/ui/node/TraversableNode;
    :goto_5
    if-nez v0, :cond_6

    move/from16 v31, v3

    const/4 v1, 0x1

    const/4 v3, 0x0

    const/16 v20, 0x0

    goto/16 :goto_d

    .line 678
    .end local v14    # "it$iv":Ljava/lang/Object;
    .end local v21    # "$i$a$-dispatchForKind-6rFNWt0-DelegatableNodeKt$visitSubtreeIf$2$1$iv":I
    :cond_6
    move/from16 v31, v3

    const/4 v1, 0x1

    const/4 v3, 0x0

    goto/16 :goto_c

    .line 679
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "zOrder$iv":Z
    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v1, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    :cond_7
    move/from16 v22, v0

    move-object/from16 v24, v1

    move/from16 v25, v2

    move-object/from16 v2, p1

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "zOrder$iv":Z
    move/from16 v0, v16

    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object v1, v13

    .local v1, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v14, 0x0

    .line 680
    .local v14, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual {v1}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v21

    and-int v21, v21, v0

    if-eqz v21, :cond_8

    const/4 v0, 0x1

    goto :goto_6

    :cond_8
    const/4 v0, 0x0

    .line 679
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v14    # "$i$f$isKind-H91voCI$ui":I
    :goto_6
    if-eqz v0, :cond_11

    instance-of v0, v13, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v0, :cond_11

    .line 681
    const/4 v0, 0x0

    .line 682
    .local v0, "count$iv$iv":I
    move-object v1, v13

    check-cast v1, Landroidx/compose/ui/node/DelegatingNode;

    .local v1, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/4 v14, 0x0

    .line 683
    .local v14, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual {v1}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v21

    .line 684
    .local v21, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_7
    if-eqz v21, :cond_10

    .line 685
    move-object/from16 v23, v21

    .local v23, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v26, 0x0

    .line 686
    .local v26, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v27, v16

    .local v27, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v28, v23

    .local v28, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v29, 0x0

    .line 680
    .local v29, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v28 .. v28}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v30

    and-int v30, v30, v27

    if-eqz v30, :cond_9

    const/16 v27, 0x1

    goto :goto_8

    :cond_9
    const/16 v27, 0x0

    .line 686
    .end local v27    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v28    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v29    # "$i$f$isKind-H91voCI$ui":I
    :goto_8
    if-eqz v27, :cond_f

    .line 687
    add-int/lit8 v0, v0, 0x1

    .line 688
    move-object/from16 v27, v1

    const/4 v1, 0x1

    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .local v27, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    if-ne v0, v1, :cond_a

    .line 689
    move-object/from16 v13, v23

    move/from16 v31, v3

    move-object/from16 v2, v23

    const/4 v3, 0x0

    goto :goto_b

    .line 693
    :cond_a
    if-nez v19, :cond_b

    const/4 v1, 0x0

    .line 660
    .local v1, "$i$f$mutableVectorOf":I
    nop

    .line 661
    move/from16 v28, v0

    .end local v0    # "count$iv$iv":I
    .local v28, "count$iv$iv":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v29, 0x0

    .line 662
    .local v29, "$i$f$MutableVector":I
    move/from16 v30, v1

    .end local v1    # "$i$f$mutableVectorOf":I
    .local v30, "$i$f$mutableVectorOf":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v31, v3

    const/4 v3, 0x0

    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .local v31, "$i$f$visitSubtreeIf-Y-YKmho":I
    invoke-direct {v1, v2, v3}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 660
    .end local v0    # "capacity$iv$iv$iv$iv":I
    .end local v29    # "$i$f$MutableVector":I
    goto :goto_9

    .line 693
    .end local v28    # "count$iv$iv":I
    .end local v30    # "$i$f$mutableVectorOf":I
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .local v0, "count$iv$iv":I
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_b
    move/from16 v28, v0

    move/from16 v31, v3

    const/4 v3, 0x0

    .end local v0    # "count$iv$iv":I
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v28    # "count$iv$iv":I
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    move-object/from16 v1, v19

    :goto_9
    nop

    .line 694
    .end local v19    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object v0, v13

    .line 695
    .local v0, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v0, :cond_d

    .line 696
    if-eqz v1, :cond_c

    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 697
    :cond_c
    const/4 v2, 0x0

    move-object v13, v2

    .line 699
    :cond_d
    if-eqz v1, :cond_e

    move-object/from16 v2, v23

    .end local v23    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v2, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_a

    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v23    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_e
    move-object/from16 v2, v23

    .line 702
    .end local v0    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v23    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_a
    move-object/from16 v19, v1

    move/from16 v0, v28

    goto :goto_b

    .line 686
    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v28    # "count$iv$iv":I
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .local v0, "count$iv$iv":I
    .local v1, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v19    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v23    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_f
    move-object/from16 v27, v1

    move/from16 v31, v3

    move-object/from16 v2, v23

    const/4 v3, 0x0

    .line 702
    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .end local v23    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :goto_b
    nop

    .line 685
    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v26    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 703
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v21

    move-object/from16 v2, p1

    move-object/from16 v1, v27

    move/from16 v3, v31

    goto :goto_7

    .line 705
    .end local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_10
    move-object/from16 v27, v1

    move/from16 v31, v3

    const/4 v3, 0x0

    .line 706
    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .end local v14    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v21    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    const/4 v1, 0x1

    if-ne v0, v1, :cond_12

    .line 708
    move v14, v1

    move/from16 v0, v22

    move-object/from16 v1, v24

    move/from16 v2, v25

    move/from16 v3, v31

    goto/16 :goto_3

    .line 679
    .end local v0    # "count$iv$iv":I
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_11
    move/from16 v31, v3

    const/4 v1, 0x1

    const/4 v3, 0x0

    .line 711
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_12
    :goto_c
    invoke-static/range {v19 .. v19}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v13

    move v14, v1

    move/from16 v0, v22

    move-object/from16 v1, v24

    move/from16 v2, v25

    move/from16 v3, v31

    goto/16 :goto_3

    .line 713
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "zOrder$iv":Z
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v1, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "zOrder$iv":Z
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_13
    move/from16 v22, v0

    move-object/from16 v24, v1

    move/from16 v25, v2

    move/from16 v31, v3

    move v1, v14

    const/4 v3, 0x0

    .line 714
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .end local v13    # "node$iv$iv":Ljava/lang/Object;
    .end local v16    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v17    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v18    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v19    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "zOrder$iv":Z
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    move/from16 v20, v1

    .line 672
    .end local v12    # "node$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v15    # "$i$a$-visitSubtreeIf-DelegatableNodeKt$visitSubtreeIf$2$iv":I
    :goto_d
    nop

    .line 715
    .local v20, "diveDeeper$iv$iv":Z
    if-eqz v20, :cond_14

    goto :goto_e

    :cond_14
    move v13, v3

    move/from16 v0, v22

    move-object/from16 v1, v24

    move/from16 v2, v25

    move/from16 v3, v31

    goto/16 :goto_0

    .line 671
    .end local v20    # "diveDeeper$iv$iv":Z
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "zOrder$iv":Z
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_15
    move/from16 v22, v0

    move-object/from16 v24, v1

    move/from16 v25, v2

    move/from16 v31, v3

    move v3, v13

    move v1, v14

    .line 717
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "zOrder$iv":Z
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :goto_e
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    move v14, v1

    move v13, v3

    move/from16 v0, v22

    move-object/from16 v1, v24

    move/from16 v2, v25

    move/from16 v3, v31

    goto/16 :goto_2

    .line 670
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "zOrder$iv":Z
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_16
    move/from16 v22, v0

    move-object/from16 v24, v1

    move/from16 v25, v2

    move/from16 v31, v3

    move v3, v13

    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "zOrder$iv":Z
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    goto :goto_f

    .line 668
    .end local v10    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "zOrder$iv":Z
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_17
    move/from16 v22, v0

    move-object/from16 v24, v1

    move/from16 v25, v2

    move/from16 v31, v3

    move v3, v13

    .line 720
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "zOrder$iv":Z
    .restart local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :goto_f
    invoke-static {v11, v9, v4}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    move v13, v3

    move/from16 v0, v22

    move-object/from16 v1, v24

    move/from16 v2, v25

    move/from16 v3, v31

    .end local v9    # "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    goto/16 :goto_0

    .line 722
    .end local v22    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v24    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "zOrder$iv":Z
    .end local v31    # "$i$f$visitSubtreeIf-Y-YKmho":I
    .restart local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "zOrder$iv":Z
    .restart local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    :cond_18
    nop

    .line 723
    .end local v4    # "zOrder$iv$iv":Z
    .end local v5    # "mask$iv$iv":I
    .end local v6    # "$this$visitSubtreeIf$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v7    # "$i$f$visitSubtreeIf":I
    .end local v8    # "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v11    # "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    nop

    .line 189
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "zOrder$iv":Z
    .end local v3    # "$i$f$visitSubtreeIf-Y-YKmho":I
    return-void
.end method

.method public static final traverseDescendants(Landroidx/compose/ui/node/TraversableNode;Lkotlin/jvm/functions/Function1;)V
    .locals 32
    .param p0, "$this$traverseDescendants"    # Landroidx/compose/ui/node/TraversableNode;
    .param p1, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T::",
            "Landroidx/compose/ui/node/TraversableNode;",
            ">(TT;",
            "Lkotlin/jvm/functions/Function1<",
            "-TT;+",
            "Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;",
            ">;)V"
        }
    .end annotation

    .line 203
    move-object/from16 v0, p0

    move-object v1, v0

    check-cast v1, Landroidx/compose/ui/node/DelegatableNode;

    const/4 v2, 0x0

    .line 724
    .local v2, "$i$f$getTraversable-OLwlOKw":I
    const/high16 v3, 0x40000

    invoke-static {v3}, Landroidx/compose/ui/node/NodeKind;->constructor-impl(I)I

    move-result v2

    .line 203
    .end local v2    # "$i$f$getTraversable-OLwlOKw":I
    nop

    .line 725
    .local v1, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    nop

    .line 727
    const/4 v3, 0x0

    .line 725
    .local v3, "zOrder$iv":Z
    const/4 v4, 0x0

    .line 730
    .local v4, "$i$f$visitSubtreeIf-Y-YKmho":I
    move v5, v3

    .local v5, "zOrder$iv$iv":Z
    move-object v6, v1

    .local v6, "$this$visitSubtreeIf$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    move v7, v2

    .local v7, "mask$iv$iv":I
    const/4 v8, 0x0

    .line 731
    .local v8, "$i$f$visitSubtreeIf":I
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v9

    .local v9, "value$iv$iv$iv":Z
    const/4 v10, 0x0

    .line 732
    .local v10, "$i$f$checkPrecondition":I
    if-nez v9, :cond_0

    .line 733
    const/4 v11, 0x0

    .line 731
    .local v11, "$i$a$-checkPrecondition-DelegatableNodeKt$visitSubtreeIf$1$iv$iv":I
    nop

    .line 733
    .end local v11    # "$i$a$-checkPrecondition-DelegatableNodeKt$visitSubtreeIf$1$iv$iv":I
    const-string/jumbo v11, "visitSubtreeIf called on an unattached node"

    invoke-static {v11}, Landroidx/compose/ui/internal/InlineClassHelperKt;->throwIllegalStateException(Ljava/lang/String;)V

    .line 735
    :cond_0
    nop

    .line 736
    .end local v9    # "value$iv$iv$iv":Z
    .end local v10    # "$i$f$checkPrecondition":I
    const/4 v9, 0x0

    .line 737
    .local v9, "$i$f$mutableVectorOf":I
    nop

    .line 738
    const/16 v10, 0x10

    .local v10, "capacity$iv$iv$iv$iv":I
    const/4 v11, 0x0

    .line 739
    .local v11, "$i$f$MutableVector":I
    new-instance v12, Landroidx/compose/runtime/collection/MutableVector;

    new-array v13, v10, [Landroidx/compose/ui/Modifier$Node;

    const/4 v14, 0x0

    invoke-direct {v12, v13, v14}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 737
    .end local v10    # "capacity$iv$iv$iv$iv":I
    .end local v11    # "$i$f$MutableVector":I
    nop

    .line 736
    .end local v9    # "$i$f$mutableVectorOf":I
    nop

    .line 740
    .local v12, "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    invoke-virtual {v9}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v9

    .line 741
    .local v9, "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-nez v9, :cond_1

    invoke-interface {v6}, Landroidx/compose/ui/node/DelegatableNode;->getNode()Landroidx/compose/ui/Modifier$Node;

    move-result-object v10

    invoke-static {v12, v10, v5}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    goto :goto_0

    :cond_1
    invoke-virtual {v12, v9}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 742
    :goto_0
    move-object v10, v12

    .local v10, "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v11, 0x0

    .line 743
    .local v11, "$i$f$isNotEmpty":I
    invoke-virtual {v10}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v13

    const/4 v15, 0x1

    if-eqz v13, :cond_2

    move v10, v15

    goto :goto_1

    :cond_2
    move v10, v14

    .end local v10    # "this_$iv$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v11    # "$i$f$isNotEmpty":I
    :goto_1
    if-eqz v10, :cond_18

    .line 744
    invoke-virtual {v12}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v10

    sub-int/2addr v10, v15

    invoke-virtual {v12, v10}, Landroidx/compose/runtime/collection/MutableVector;->removeAt(I)Ljava/lang/Object;

    move-result-object v10

    check-cast v10, Landroidx/compose/ui/Modifier$Node;

    .line 745
    .local v10, "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v10}, Landroidx/compose/ui/Modifier$Node;->getAggregateChildKindSet$ui()I

    move-result v11

    and-int/2addr v11, v7

    if-eqz v11, :cond_17

    .line 746
    move-object v11, v10

    .line 747
    .local v11, "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_2
    if-eqz v11, :cond_16

    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->isAttached()Z

    move-result v13

    if-eqz v13, :cond_16

    .line 748
    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v13

    and-int/2addr v13, v7

    if-eqz v13, :cond_15

    .line 749
    move-object v13, v11

    .local v13, "node$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v16, 0x0

    .line 750
    .local v16, "$i$a$-visitSubtreeIf-DelegatableNodeKt$visitSubtreeIf$2$iv":I
    move/from16 v17, v2

    .local v17, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    move-object/from16 v18, v13

    .local v18, "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v19, 0x0

    .line 751
    .local v19, "$i$f$dispatchForKind-6rFNWt0":I
    const/16 v20, 0x0

    .line 752
    .local v20, "stack$iv$iv":Ljava/lang/Object;
    const/16 v21, 0x0

    .local v21, "node$iv$iv":Ljava/lang/Object;
    move-object/from16 v21, v18

    move-object/from16 v14, v21

    .line 753
    .end local v21    # "node$iv$iv":Ljava/lang/Object;
    .local v14, "node$iv$iv":Ljava/lang/Object;
    :goto_3
    if-eqz v14, :cond_13

    .line 754
    instance-of v15, v14, Landroidx/compose/ui/node/TraversableNode;

    if-eqz v15, :cond_7

    .line 755
    move-object v15, v14

    .local v15, "it$iv":Ljava/lang/Object;
    const/16 v22, 0x0

    .line 750
    .local v22, "$i$a$-dispatchForKind-6rFNWt0-DelegatableNodeKt$visitSubtreeIf$2$1$iv":I
    move-object/from16 v23, v1

    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v23, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    move-object v1, v15

    check-cast v1, Landroidx/compose/ui/node/TraversableNode;

    .local v1, "it":Landroidx/compose/ui/node/TraversableNode;
    const/16 v24, 0x0

    .line 205
    .local v24, "$i$a$-visitSubtreeIf-Y-YKmho$default-TraversableNodeKt$traverseDescendants$2":I
    move/from16 v25, v2

    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v25, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    invoke-interface {v0}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v2

    move/from16 v26, v3

    .end local v3    # "zOrder$iv":Z
    .local v26, "zOrder$iv":Z
    invoke-interface {v1}, Landroidx/compose/ui/node/TraversableNode;->getTraverseKey()Ljava/lang/Object;

    move-result-object v3

    invoke-static {v2, v3}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_3

    invoke-static {v0, v1}, Landroidx/compose/ui/Actual_jvmKt;->areObjectsOfSameType(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_3

    .line 206
    move-object/from16 v2, p1

    invoke-interface {v2, v1}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    goto :goto_4

    .line 205
    :cond_3
    move-object/from16 v2, p1

    .line 208
    sget-object v3, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;->ContinueTraversal:Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    .line 205
    :goto_4
    nop

    .line 204
    nop

    .line 210
    .local v3, "action":Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;
    sget-object v0, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;->CancelTraversal:Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    if-ne v3, v0, :cond_4

    return-void

    .line 215
    :cond_4
    sget-object v0, Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;->SkipSubtreeAndContinueTraversal:Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;

    if-eq v3, v0, :cond_5

    const/4 v0, 0x1

    goto :goto_5

    :cond_5
    const/4 v0, 0x0

    .line 750
    .end local v1    # "it":Landroidx/compose/ui/node/TraversableNode;
    .end local v3    # "action":Landroidx/compose/ui/node/TraversableNode$Companion$TraverseDescendantsAction;
    .end local v24    # "$i$a$-visitSubtreeIf-Y-YKmho$default-TraversableNodeKt$traverseDescendants$2":I
    :goto_5
    if-nez v0, :cond_6

    const/4 v1, 0x1

    const/4 v3, 0x0

    const/16 v21, 0x0

    goto/16 :goto_d

    .line 755
    .end local v15    # "it$iv":Ljava/lang/Object;
    .end local v22    # "$i$a$-dispatchForKind-6rFNWt0-DelegatableNodeKt$visitSubtreeIf$2$1$iv":I
    :cond_6
    const/4 v1, 0x1

    const/4 v3, 0x0

    goto/16 :goto_c

    .line 756
    .end local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v26    # "zOrder$iv":Z
    .local v1, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v3, "zOrder$iv":Z
    :cond_7
    move-object/from16 v23, v1

    move/from16 v25, v2

    move/from16 v26, v3

    move-object/from16 v2, p1

    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .restart local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v26    # "zOrder$iv":Z
    move/from16 v0, v17

    .local v0, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object v1, v14

    .local v1, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v3, 0x0

    .line 757
    .local v3, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual {v1}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v15

    and-int/2addr v15, v0

    if-eqz v15, :cond_8

    const/4 v0, 0x1

    goto :goto_6

    :cond_8
    const/4 v0, 0x0

    .line 756
    .end local v0    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v3    # "$i$f$isKind-H91voCI$ui":I
    :goto_6
    if-eqz v0, :cond_11

    instance-of v0, v14, Landroidx/compose/ui/node/DelegatingNode;

    if-eqz v0, :cond_11

    .line 758
    const/4 v0, 0x0

    .line 759
    .local v0, "count$iv$iv":I
    move-object v1, v14

    check-cast v1, Landroidx/compose/ui/node/DelegatingNode;

    .local v1, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    const/4 v3, 0x0

    .line 760
    .local v3, "$i$f$forEachImmediateDelegate$ui":I
    invoke-virtual {v1}, Landroidx/compose/ui/node/DelegatingNode;->getDelegate$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v15

    .line 761
    .local v15, "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_7
    if-eqz v15, :cond_10

    .line 762
    move-object/from16 v22, v15

    .local v22, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v24, 0x0

    .line 763
    .local v24, "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    move/from16 v27, v17

    .local v27, "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    move-object/from16 v28, v22

    .local v28, "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/16 v29, 0x0

    .line 757
    .local v29, "$i$f$isKind-H91voCI$ui":I
    invoke-virtual/range {v28 .. v28}, Landroidx/compose/ui/Modifier$Node;->getKindSet$ui()I

    move-result v30

    and-int v30, v30, v27

    if-eqz v30, :cond_9

    const/16 v27, 0x1

    goto :goto_8

    :cond_9
    const/16 v27, 0x0

    .line 763
    .end local v27    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv$iv":I
    .end local v28    # "this_$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v29    # "$i$f$isKind-H91voCI$ui":I
    :goto_8
    if-eqz v27, :cond_f

    .line 764
    add-int/lit8 v0, v0, 0x1

    .line 765
    move-object/from16 v27, v1

    const/4 v1, 0x1

    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .local v27, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    if-ne v0, v1, :cond_a

    .line 766
    move-object/from16 v14, v22

    move/from16 v31, v3

    move-object/from16 v2, v22

    const/4 v3, 0x0

    goto :goto_b

    .line 770
    :cond_a
    if-nez v20, :cond_b

    const/4 v1, 0x0

    .line 737
    .local v1, "$i$f$mutableVectorOf":I
    nop

    .line 738
    move/from16 v28, v0

    .end local v0    # "count$iv$iv":I
    .local v28, "count$iv$iv":I
    const/16 v0, 0x10

    .local v0, "capacity$iv$iv$iv$iv":I
    const/16 v29, 0x0

    .line 739
    .local v29, "$i$f$MutableVector":I
    move/from16 v30, v1

    .end local v1    # "$i$f$mutableVectorOf":I
    .local v30, "$i$f$mutableVectorOf":I
    new-instance v1, Landroidx/compose/runtime/collection/MutableVector;

    new-array v2, v0, [Landroidx/compose/ui/Modifier$Node;

    move/from16 v31, v3

    const/4 v3, 0x0

    .end local v3    # "$i$f$forEachImmediateDelegate$ui":I
    .local v31, "$i$f$forEachImmediateDelegate$ui":I
    invoke-direct {v1, v2, v3}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 737
    .end local v0    # "capacity$iv$iv$iv$iv":I
    .end local v29    # "$i$f$MutableVector":I
    goto :goto_9

    .line 770
    .end local v28    # "count$iv$iv":I
    .end local v30    # "$i$f$mutableVectorOf":I
    .end local v31    # "$i$f$forEachImmediateDelegate$ui":I
    .local v0, "count$iv$iv":I
    .restart local v3    # "$i$f$forEachImmediateDelegate$ui":I
    :cond_b
    move/from16 v28, v0

    move/from16 v31, v3

    const/4 v3, 0x0

    .end local v0    # "count$iv$iv":I
    .end local v3    # "$i$f$forEachImmediateDelegate$ui":I
    .restart local v28    # "count$iv$iv":I
    .restart local v31    # "$i$f$forEachImmediateDelegate$ui":I
    move-object/from16 v1, v20

    :goto_9
    nop

    .line 771
    .end local v20    # "stack$iv$iv":Ljava/lang/Object;
    .local v1, "stack$iv$iv":Ljava/lang/Object;
    move-object v0, v14

    .line 772
    .local v0, "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    if-eqz v0, :cond_d

    .line 773
    if-eqz v1, :cond_c

    invoke-virtual {v1, v0}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 774
    :cond_c
    const/4 v2, 0x0

    move-object v14, v2

    .line 776
    :cond_d
    if-eqz v1, :cond_e

    move-object/from16 v2, v22

    .end local v22    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .local v2, "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    goto :goto_a

    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v22    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_e
    move-object/from16 v2, v22

    .line 779
    .end local v0    # "theNode$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v22    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :goto_a
    move-object/from16 v20, v1

    move/from16 v0, v28

    goto :goto_b

    .line 763
    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v28    # "count$iv$iv":I
    .end local v31    # "$i$f$forEachImmediateDelegate$ui":I
    .local v0, "count$iv$iv":I
    .local v1, "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v3    # "$i$f$forEachImmediateDelegate$ui":I
    .restart local v20    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v22    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    :cond_f
    move-object/from16 v27, v1

    move/from16 v31, v3

    move-object/from16 v2, v22

    const/4 v3, 0x0

    .line 779
    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v3    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v22    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .restart local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v31    # "$i$f$forEachImmediateDelegate$ui":I
    :goto_b
    nop

    .line 762
    .end local v2    # "next$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v24    # "$i$a$-forEachImmediateDelegate$ui-DelegatableNodeKt$dispatchForKind$1$iv$iv":I
    nop

    .line 780
    invoke-virtual {v15}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v15

    move-object/from16 v2, p1

    move-object/from16 v1, v27

    move/from16 v3, v31

    goto :goto_7

    .line 782
    .end local v27    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v31    # "$i$f$forEachImmediateDelegate$ui":I
    .restart local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .restart local v3    # "$i$f$forEachImmediateDelegate$ui":I
    :cond_10
    move-object/from16 v27, v1

    move/from16 v31, v3

    const/4 v3, 0x0

    .line 783
    .end local v1    # "this_$iv$iv$iv":Landroidx/compose/ui/node/DelegatingNode;
    .end local v3    # "$i$f$forEachImmediateDelegate$ui":I
    .end local v15    # "node$iv$iv$iv":Landroidx/compose/ui/Modifier$Node;
    const/4 v1, 0x1

    if-ne v0, v1, :cond_12

    .line 785
    move-object/from16 v0, p0

    move v15, v1

    move-object/from16 v1, v23

    move/from16 v2, v25

    move/from16 v3, v26

    goto/16 :goto_3

    .line 756
    .end local v0    # "count$iv$iv":I
    :cond_11
    const/4 v1, 0x1

    const/4 v3, 0x0

    .line 788
    :cond_12
    :goto_c
    invoke-static/range {v20 .. v20}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$pop(Landroidx/compose/runtime/collection/MutableVector;)Landroidx/compose/ui/Modifier$Node;

    move-result-object v14

    move-object/from16 v0, p0

    move v15, v1

    move-object/from16 v1, v23

    move/from16 v2, v25

    move/from16 v3, v26

    goto/16 :goto_3

    .line 790
    .end local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v26    # "zOrder$iv":Z
    .local v1, "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .local v2, "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .local v3, "zOrder$iv":Z
    :cond_13
    move-object/from16 v23, v1

    move/from16 v25, v2

    move/from16 v26, v3

    move v1, v15

    const/4 v3, 0x0

    .line 791
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .end local v14    # "node$iv$iv":Ljava/lang/Object;
    .end local v17    # "$v$c$androidx-compose-ui-node-NodeKind$-kind$0$iv$iv":I
    .end local v18    # "$this$dispatchForKind_u2d6rFNWt0$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v19    # "$i$f$dispatchForKind-6rFNWt0":I
    .end local v20    # "stack$iv$iv":Ljava/lang/Object;
    .restart local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v26    # "zOrder$iv":Z
    move/from16 v21, v1

    .line 749
    .end local v13    # "node$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v16    # "$i$a$-visitSubtreeIf-DelegatableNodeKt$visitSubtreeIf$2$iv":I
    :goto_d
    nop

    .line 792
    .local v21, "diveDeeper$iv$iv":Z
    if-eqz v21, :cond_14

    goto :goto_e

    :cond_14
    move-object/from16 v0, p0

    move v14, v3

    move-object/from16 v1, v23

    move/from16 v2, v25

    move/from16 v3, v26

    goto/16 :goto_0

    .line 748
    .end local v21    # "diveDeeper$iv$iv":Z
    .end local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v26    # "zOrder$iv":Z
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "zOrder$iv":Z
    :cond_15
    move-object/from16 v23, v1

    move/from16 v25, v2

    move/from16 v26, v3

    move v3, v14

    move v1, v15

    .line 794
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .restart local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v26    # "zOrder$iv":Z
    :goto_e
    invoke-virtual {v11}, Landroidx/compose/ui/Modifier$Node;->getChild$ui()Landroidx/compose/ui/Modifier$Node;

    move-result-object v11

    move-object/from16 v0, p0

    move v15, v1

    move v14, v3

    move-object/from16 v1, v23

    move/from16 v2, v25

    move/from16 v3, v26

    goto/16 :goto_2

    .line 747
    .end local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v26    # "zOrder$iv":Z
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "zOrder$iv":Z
    :cond_16
    move-object/from16 v23, v1

    move/from16 v25, v2

    move/from16 v26, v3

    move v3, v14

    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .restart local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v26    # "zOrder$iv":Z
    goto :goto_f

    .line 745
    .end local v11    # "node$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v26    # "zOrder$iv":Z
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "zOrder$iv":Z
    :cond_17
    move-object/from16 v23, v1

    move/from16 v25, v2

    move/from16 v26, v3

    move v3, v14

    .line 797
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .restart local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v26    # "zOrder$iv":Z
    :goto_f
    invoke-static {v12, v10, v5}, Landroidx/compose/ui/node/DelegatableNodeKt;->access$addLayoutNodeChildren(Landroidx/compose/runtime/collection/MutableVector;Landroidx/compose/ui/Modifier$Node;Z)V

    move-object/from16 v0, p0

    move v14, v3

    move-object/from16 v1, v23

    move/from16 v2, v25

    move/from16 v3, v26

    .end local v10    # "branch$iv$iv":Landroidx/compose/ui/Modifier$Node;
    goto/16 :goto_0

    .line 799
    .end local v23    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v25    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v26    # "zOrder$iv":Z
    .restart local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .restart local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .restart local v3    # "zOrder$iv":Z
    :cond_18
    nop

    .line 800
    .end local v5    # "zOrder$iv$iv":Z
    .end local v6    # "$this$visitSubtreeIf$iv$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v7    # "mask$iv$iv":I
    .end local v8    # "$i$f$visitSubtreeIf":I
    .end local v9    # "child$iv$iv":Landroidx/compose/ui/Modifier$Node;
    .end local v12    # "branches$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    nop

    .line 217
    .end local v1    # "$this$visitSubtreeIf_u2dY_u2dYKmho_u24default$iv":Landroidx/compose/ui/node/DelegatableNode;
    .end local v2    # "$v$c$androidx-compose-ui-node-NodeKind$-type$0$iv":I
    .end local v3    # "zOrder$iv":Z
    .end local v4    # "$i$f$visitSubtreeIf-Y-YKmho":I
    return-void
.end method
