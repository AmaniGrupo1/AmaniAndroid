.class final Landroidx/compose/foundation/BorderCache;
.super Ljava/lang/Object;
.source "Border.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nBorder.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Border.kt\nandroidx/compose/foundation/BorderCache\n+ 2 Size.kt\nandroidx/compose/ui/geometry/Size\n+ 3 InlineClassHelper.kt\nandroidx/compose/ui/util/InlineClassHelperKt\n+ 4 InlineClassHelper.jvm.kt\nandroidx/compose/ui/util/InlineClassHelper_jvmKt\n+ 5 IntSize.kt\nandroidx/compose/ui/unit/IntSize\n+ 6 fake.kt\nkotlin/jvm/internal/FakeKt\n+ 7 CanvasDrawScope.kt\nandroidx/compose/ui/graphics/drawscope/CanvasDrawScope\n*L\n1#1,480:1\n57#2:481\n61#2:484\n60#3:482\n70#3:485\n85#3:488\n90#3:490\n22#4:483\n22#4:486\n54#5:487\n59#5:489\n1#6:491\n536#7,17:492\n*S KotlinDebug\n*F\n+ 1 Border.kt\nandroidx/compose/foundation/BorderCache\n*L\n387#1:481\n388#1:484\n387#1:482\n388#1:485\n392#1:488\n392#1:490\n387#1:483\n388#1:486\n392#1:487\n392#1:489\n401#1:492,17\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000^\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0002\u0018\u0002\n\u0002\u0008\t\n\u0002\u0010\u000b\n\u0002\u0008\u0002\n\u0002\u0010\u0008\n\u0000\n\u0002\u0010\u000e\n\u0000\u0008\u0082\u0008\u0018\u00002\u00020\u0001B7\u0012\n\u0008\u0002\u0010\u0002\u001a\u0004\u0018\u00010\u0003\u0012\n\u0008\u0002\u0010\u0004\u001a\u0004\u0018\u00010\u0005\u0012\n\u0008\u0002\u0010\u0006\u001a\u0004\u0018\u00010\u0007\u0012\n\u0008\u0002\u0010\u0008\u001a\u0004\u0018\u00010\t\u00a2\u0006\u0004\u0008\n\u0010\u000bJ=\u0010\u000c\u001a\u00020\u0003*\u00020\r2\u0006\u0010\u000e\u001a\u00020\u000f2\u0006\u0010\u0010\u001a\u00020\u00112\u0017\u0010\u0012\u001a\u0013\u0012\u0004\u0012\u00020\u0014\u0012\u0004\u0012\u00020\u00150\u0013\u00a2\u0006\u0002\u0008\u0016H\u0086\u0008\u00a2\u0006\u0004\u0008\u0017\u0010\u0018J\u0006\u0010\u0019\u001a\u00020\tJ\u000b\u0010\u001a\u001a\u0004\u0018\u00010\u0003H\u00c2\u0003J\u000b\u0010\u001b\u001a\u0004\u0018\u00010\u0005H\u00c2\u0003J\u000b\u0010\u001c\u001a\u0004\u0018\u00010\u0007H\u00c2\u0003J\u000b\u0010\u001d\u001a\u0004\u0018\u00010\tH\u00c2\u0003J9\u0010\u001e\u001a\u00020\u00002\n\u0008\u0002\u0010\u0002\u001a\u0004\u0018\u00010\u00032\n\u0008\u0002\u0010\u0004\u001a\u0004\u0018\u00010\u00052\n\u0008\u0002\u0010\u0006\u001a\u0004\u0018\u00010\u00072\n\u0008\u0002\u0010\u0008\u001a\u0004\u0018\u00010\tH\u00c6\u0001J\u0013\u0010\u001f\u001a\u00020 2\u0008\u0010!\u001a\u0004\u0018\u00010\u0001H\u00d6\u0003J\t\u0010\"\u001a\u00020#H\u00d6\u0001J\t\u0010$\u001a\u00020%H\u00d6\u0001R\u0010\u0010\u0002\u001a\u0004\u0018\u00010\u0003X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u0004\u001a\u0004\u0018\u00010\u0005X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u0006\u001a\u0004\u0018\u00010\u0007X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u0008\u001a\u0004\u0018\u00010\tX\u0082\u000e\u00a2\u0006\u0002\n\u0000\u00a8\u0006&"
    }
    d2 = {
        "Landroidx/compose/foundation/BorderCache;",
        "",
        "imageBitmap",
        "Landroidx/compose/ui/graphics/ImageBitmap;",
        "canvas",
        "Landroidx/compose/ui/graphics/Canvas;",
        "canvasDrawScope",
        "Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;",
        "borderPath",
        "Landroidx/compose/ui/graphics/Path;",
        "<init>",
        "(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;)V",
        "drawBorderCache",
        "Landroidx/compose/ui/draw/CacheDrawScope;",
        "borderSize",
        "Landroidx/compose/ui/unit/IntSize;",
        "config",
        "Landroidx/compose/ui/graphics/ImageBitmapConfig;",
        "block",
        "Lkotlin/Function1;",
        "Landroidx/compose/ui/graphics/drawscope/DrawScope;",
        "",
        "Lkotlin/ExtensionFunctionType;",
        "drawBorderCache-EMwLDEs",
        "(Landroidx/compose/ui/draw/CacheDrawScope;JILkotlin/jvm/functions/Function1;)Landroidx/compose/ui/graphics/ImageBitmap;",
        "obtainPath",
        "component1",
        "component2",
        "component3",
        "component4",
        "copy",
        "equals",
        "",
        "other",
        "hashCode",
        "",
        "toString",
        "",
        "foundation"
    }
    k = 0x1
    mv = {
        0x2,
        0x0,
        0x0
    }
    xi = 0x30
.end annotation


# instance fields
.field private borderPath:Landroidx/compose/ui/graphics/Path;

.field private canvas:Landroidx/compose/ui/graphics/Canvas;

.field private canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

.field private imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;


# direct methods
.method public constructor <init>()V
    .locals 7

    const/16 v5, 0xf

    const/4 v6, 0x0

    const/4 v1, 0x0

    const/4 v2, 0x0

    const/4 v3, 0x0

    const/4 v4, 0x0

    move-object v0, p0

    invoke-direct/range {v0 .. v6}, Landroidx/compose/foundation/BorderCache;-><init>(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;ILkotlin/jvm/internal/DefaultConstructorMarker;)V

    return-void
.end method

.method public constructor <init>(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;)V
    .locals 0
    .param p1, "imageBitmap"    # Landroidx/compose/ui/graphics/ImageBitmap;
    .param p2, "canvas"    # Landroidx/compose/ui/graphics/Canvas;
    .param p3, "canvasDrawScope"    # Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    .param p4, "borderPath"    # Landroidx/compose/ui/graphics/Path;

    .line 365
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 366
    iput-object p1, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    .line 367
    iput-object p2, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    .line 368
    iput-object p3, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    .line 369
    iput-object p4, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    .line 365
    return-void
.end method

.method public synthetic constructor <init>(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;ILkotlin/jvm/internal/DefaultConstructorMarker;)V
    .locals 1

    .line 365
    and-int/lit8 p6, p5, 0x1

    const/4 v0, 0x0

    if-eqz p6, :cond_0

    .line 366
    move-object p1, v0

    .line 365
    :cond_0
    and-int/lit8 p6, p5, 0x2

    if-eqz p6, :cond_1

    .line 367
    move-object p2, v0

    .line 365
    :cond_1
    and-int/lit8 p6, p5, 0x4

    if-eqz p6, :cond_2

    .line 368
    move-object p3, v0

    .line 365
    :cond_2
    and-int/lit8 p5, p5, 0x8

    if-eqz p5, :cond_3

    .line 369
    move-object p4, v0

    .line 365
    :cond_3
    invoke-direct {p0, p1, p2, p3, p4}, Landroidx/compose/foundation/BorderCache;-><init>(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;)V

    .line 370
    return-void
.end method

.method public static final synthetic access$getCanvas$p(Landroidx/compose/foundation/BorderCache;)Landroidx/compose/ui/graphics/Canvas;
    .locals 1
    .param p0, "$this"    # Landroidx/compose/foundation/BorderCache;

    .line 365
    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    return-object v0
.end method

.method public static final synthetic access$getCanvasDrawScope$p(Landroidx/compose/foundation/BorderCache;)Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    .locals 1
    .param p0, "$this"    # Landroidx/compose/foundation/BorderCache;

    .line 365
    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    return-object v0
.end method

.method public static final synthetic access$getImageBitmap$p(Landroidx/compose/foundation/BorderCache;)Landroidx/compose/ui/graphics/ImageBitmap;
    .locals 1
    .param p0, "$this"    # Landroidx/compose/foundation/BorderCache;

    .line 365
    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    return-object v0
.end method

.method public static final synthetic access$setCanvas$p(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/Canvas;)V
    .locals 0
    .param p0, "$this"    # Landroidx/compose/foundation/BorderCache;
    .param p1, "<set-?>"    # Landroidx/compose/ui/graphics/Canvas;

    .line 365
    iput-object p1, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    return-void
.end method

.method public static final synthetic access$setCanvasDrawScope$p(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;)V
    .locals 0
    .param p0, "$this"    # Landroidx/compose/foundation/BorderCache;
    .param p1, "<set-?>"    # Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    .line 365
    iput-object p1, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    return-void
.end method

.method public static final synthetic access$setImageBitmap$p(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/ImageBitmap;)V
    .locals 0
    .param p0, "$this"    # Landroidx/compose/foundation/BorderCache;
    .param p1, "<set-?>"    # Landroidx/compose/ui/graphics/ImageBitmap;

    .line 365
    iput-object p1, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    return-void
.end method

.method private final component1()Landroidx/compose/ui/graphics/ImageBitmap;
    .locals 1

    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    return-object v0
.end method

.method private final component2()Landroidx/compose/ui/graphics/Canvas;
    .locals 1

    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    return-object v0
.end method

.method private final component3()Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    .locals 1

    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    return-object v0
.end method

.method private final component4()Landroidx/compose/ui/graphics/Path;
    .locals 1

    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    return-object v0
.end method

.method public static synthetic copy$default(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;ILjava/lang/Object;)Landroidx/compose/foundation/BorderCache;
    .locals 0

    and-int/lit8 p6, p5, 0x1

    if-eqz p6, :cond_0

    iget-object p1, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    :cond_0
    and-int/lit8 p6, p5, 0x2

    if-eqz p6, :cond_1

    iget-object p2, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    :cond_1
    and-int/lit8 p6, p5, 0x4

    if-eqz p6, :cond_2

    iget-object p3, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    :cond_2
    and-int/lit8 p5, p5, 0x8

    if-eqz p5, :cond_3

    iget-object p4, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    :cond_3
    invoke-virtual {p0, p1, p2, p3, p4}, Landroidx/compose/foundation/BorderCache;->copy(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;)Landroidx/compose/foundation/BorderCache;

    move-result-object p0

    return-object p0
.end method


# virtual methods
.method public final copy(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;)Landroidx/compose/foundation/BorderCache;
    .locals 1

    new-instance v0, Landroidx/compose/foundation/BorderCache;

    invoke-direct {v0, p1, p2, p3, p4}, Landroidx/compose/foundation/BorderCache;-><init>(Landroidx/compose/ui/graphics/ImageBitmap;Landroidx/compose/ui/graphics/Canvas;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;Landroidx/compose/ui/graphics/Path;)V

    return-object v0
.end method

.method public final drawBorderCache-EMwLDEs(Landroidx/compose/ui/draw/CacheDrawScope;JILkotlin/jvm/functions/Function1;)Landroidx/compose/ui/graphics/ImageBitmap;
    .locals 35
    .param p1, "$this$drawBorderCache_u2dEMwLDEs"    # Landroidx/compose/ui/draw/CacheDrawScope;
    .param p2, "$v$c$androidx-compose-ui-unit-IntSize$-borderSize$0"    # J
    .param p4, "$v$c$androidx-compose-ui-graphics-ImageBitmapConfig$-config$0"    # I
    .param p5, "block"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/ui/draw/CacheDrawScope;",
            "JI",
            "Lkotlin/jvm/functions/Function1<",
            "-",
            "Landroidx/compose/ui/graphics/drawscope/DrawScope;",
            "Lkotlin/Unit;",
            ">;)",
            "Landroidx/compose/ui/graphics/ImageBitmap;"
        }
    .end annotation

    move-object/from16 v0, p0

    const/4 v1, 0x0

    .line 377
    .local v1, "$i$f$drawBorderCache-EMwLDEs":I
    invoke-static {v0}, Landroidx/compose/foundation/BorderCache;->access$getImageBitmap$p(Landroidx/compose/foundation/BorderCache;)Landroidx/compose/ui/graphics/ImageBitmap;

    move-result-object v2

    .line 378
    .local v2, "targetImageBitmap":Landroidx/compose/ui/graphics/ImageBitmap;
    invoke-static {v0}, Landroidx/compose/foundation/BorderCache;->access$getCanvas$p(Landroidx/compose/foundation/BorderCache;)Landroidx/compose/ui/graphics/Canvas;

    move-result-object v3

    .line 382
    .local v3, "targetCanvas":Landroidx/compose/ui/graphics/Canvas;
    const/4 v4, 0x0

    if-eqz v2, :cond_0

    invoke-interface {v2}, Landroidx/compose/ui/graphics/ImageBitmap;->getConfig-_sVssgQ()I

    move-result v5

    invoke-static {v5}, Landroidx/compose/ui/graphics/ImageBitmapConfig;->box-impl(I)Landroidx/compose/ui/graphics/ImageBitmapConfig;

    move-result-object v5

    goto :goto_0

    :cond_0
    move-object v5, v4

    :goto_0
    sget-object v6, Landroidx/compose/ui/graphics/ImageBitmapConfig;->Companion:Landroidx/compose/ui/graphics/ImageBitmapConfig$Companion;

    invoke-virtual {v6}, Landroidx/compose/ui/graphics/ImageBitmapConfig$Companion;->getArgb8888-_sVssgQ()I

    move-result v6

    const/4 v7, 0x0

    if-nez v5, :cond_1

    move v5, v7

    goto :goto_1

    :cond_1
    invoke-virtual {v5}, Landroidx/compose/ui/graphics/ImageBitmapConfig;->unbox-impl()I

    move-result v5

    invoke-static {v5, v6}, Landroidx/compose/ui/graphics/ImageBitmapConfig;->equals-impl0(II)Z

    move-result v5

    :goto_1
    if-nez v5, :cond_3

    .line 383
    if-eqz v2, :cond_2

    invoke-interface {v2}, Landroidx/compose/ui/graphics/ImageBitmap;->getConfig-_sVssgQ()I

    move-result v4

    invoke-static {v4}, Landroidx/compose/ui/graphics/ImageBitmapConfig;->box-impl(I)Landroidx/compose/ui/graphics/ImageBitmapConfig;

    move-result-object v4

    :cond_2
    move/from16 v10, p4

    invoke-static {v10, v4}, Landroidx/compose/ui/graphics/ImageBitmapConfig;->equals-impl(ILjava/lang/Object;)Z

    move-result v4

    if-eqz v4, :cond_4

    goto :goto_2

    .line 382
    :cond_3
    move/from16 v10, p4

    .line 383
    :goto_2
    const/4 v7, 0x1

    .line 381
    :cond_4
    nop

    .line 384
    .local v7, "compatibleConfig":Z
    nop

    .line 385
    const/16 v6, 0x20

    if-eqz v2, :cond_6

    .line 386
    if-eqz v3, :cond_5

    .line 387
    invoke-virtual/range {p1 .. p1}, Landroidx/compose/ui/draw/CacheDrawScope;->getSize-NH-jbRc()J

    move-result-wide v8

    .local v8, "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    const/4 v11, 0x0

    .line 481
    .local v11, "$i$f$getWidth-impl":I
    move-wide v12, v8

    .local v12, "value$iv$iv":J
    const/4 v14, 0x0

    .line 482
    .local v14, "$i$f$unpackFloat1":I
    const-wide v15, 0xffffffffL

    shr-long v4, v12, v6

    long-to-int v4, v4

    .local v4, "bits$iv$iv$iv":I
    const/4 v5, 0x0

    .line 483
    .local v5, "$i$f$floatFromBits":I
    invoke-static {v4}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v4

    .line 482
    .end local v4    # "bits$iv$iv$iv":I
    .end local v5    # "$i$f$floatFromBits":I
    nop

    .line 481
    .end local v12    # "value$iv$iv":J
    .end local v14    # "$i$f$unpackFloat1":I
    nop

    .line 387
    .end local v8    # "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    .end local v11    # "$i$f$getWidth-impl":I
    invoke-interface {v2}, Landroidx/compose/ui/graphics/ImageBitmap;->getWidth()I

    move-result v5

    int-to-float v5, v5

    cmpl-float v4, v4, v5

    if-gtz v4, :cond_7

    .line 388
    invoke-virtual/range {p1 .. p1}, Landroidx/compose/ui/draw/CacheDrawScope;->getSize-NH-jbRc()J

    move-result-wide v4

    .local v4, "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    const/4 v8, 0x0

    .line 484
    .local v8, "$i$f$getHeight-impl":I
    move-wide v11, v4

    .local v11, "value$iv$iv":J
    const/4 v9, 0x0

    .line 485
    .local v9, "$i$f$unpackFloat2":I
    and-long v13, v11, v15

    long-to-int v13, v13

    .local v13, "bits$iv$iv$iv":I
    const/4 v14, 0x0

    .line 486
    .local v14, "$i$f$floatFromBits":I
    invoke-static {v13}, Ljava/lang/Float;->intBitsToFloat(I)F

    move-result v13

    .line 485
    .end local v13    # "bits$iv$iv$iv":I
    .end local v14    # "$i$f$floatFromBits":I
    nop

    .line 484
    .end local v9    # "$i$f$unpackFloat2":I
    .end local v11    # "value$iv$iv":J
    nop

    .line 388
    .end local v4    # "$v$c$androidx-compose-ui-geometry-Size$-this$0$iv":J
    .end local v8    # "$i$f$getHeight-impl":I
    invoke-interface {v2}, Landroidx/compose/ui/graphics/ImageBitmap;->getHeight()I

    move-result v4

    int-to-float v4, v4

    cmpl-float v4, v13, v4

    if-gtz v4, :cond_7

    .line 389
    if-nez v7, :cond_8

    goto :goto_3

    .line 386
    :cond_5
    const-wide v15, 0xffffffffL

    goto :goto_3

    .line 385
    :cond_6
    const-wide v15, 0xffffffffL

    .line 392
    :cond_7
    :goto_3
    move-wide/from16 v4, p2

    .local v4, "$v$c$androidx-compose-ui-unit-IntSize$-this$0$iv":J
    const/4 v8, 0x0

    .line 487
    .local v8, "$i$f$getWidth-impl":I
    move-wide v11, v4

    .restart local v11    # "value$iv$iv":J
    const/4 v9, 0x0

    .line 488
    .local v9, "$i$f$unpackInt1":I
    shr-long v13, v11, v6

    long-to-int v6, v13

    .line 487
    .end local v9    # "$i$f$unpackInt1":I
    .end local v11    # "value$iv$iv":J
    nop

    .line 392
    .end local v4    # "$v$c$androidx-compose-ui-unit-IntSize$-this$0$iv":J
    .end local v8    # "$i$f$getWidth-impl":I
    nop

    .restart local v4    # "$v$c$androidx-compose-ui-unit-IntSize$-this$0$iv":J
    const/4 v8, 0x0

    .line 489
    .local v8, "$i$f$getHeight-impl":I
    nop

    .restart local v11    # "value$iv$iv":J
    const/4 v9, 0x0

    .line 490
    .local v9, "$i$f$unpackInt2":I
    and-long v13, v11, v15

    long-to-int v9, v13

    .line 489
    .end local v9    # "$i$f$unpackInt2":I
    .end local v11    # "value$iv$iv":J
    nop

    .line 392
    .end local v4    # "$v$c$androidx-compose-ui-unit-IntSize$-this$0$iv":J
    .end local v8    # "$i$f$getHeight-impl":I
    const/16 v13, 0x18

    const/4 v14, 0x0

    const/4 v11, 0x0

    const/4 v12, 0x0

    move v8, v6

    invoke-static/range {v8 .. v14}, Landroidx/compose/ui/graphics/ImageBitmapKt;->ImageBitmap-x__-hDU$default(IIIZLandroidx/compose/ui/graphics/colorspace/ColorSpace;ILjava/lang/Object;)Landroidx/compose/ui/graphics/ImageBitmap;

    move-result-object v4

    move-object v5, v4

    .local v5, "it":Landroidx/compose/ui/graphics/ImageBitmap;
    const/4 v6, 0x0

    .line 393
    .local v6, "$i$a$-also-BorderCache$drawBorderCache$1":I
    invoke-static {v0, v5}, Landroidx/compose/foundation/BorderCache;->access$setImageBitmap$p(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/ImageBitmap;)V

    .line 394
    nop

    .line 392
    .end local v5    # "it":Landroidx/compose/ui/graphics/ImageBitmap;
    .end local v6    # "$i$a$-also-BorderCache$drawBorderCache$1":I
    nop

    .line 391
    move-object v2, v4

    .line 396
    invoke-static {v2}, Landroidx/compose/ui/graphics/CanvasKt;->Canvas(Landroidx/compose/ui/graphics/ImageBitmap;)Landroidx/compose/ui/graphics/Canvas;

    move-result-object v4

    move-object v5, v4

    .line 491
    .local v5, "it":Landroidx/compose/ui/graphics/Canvas;
    const/4 v6, 0x0

    .line 396
    .local v6, "$i$a$-also-BorderCache$drawBorderCache$2":I
    invoke-static {v0, v5}, Landroidx/compose/foundation/BorderCache;->access$setCanvas$p(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/Canvas;)V

    .line 395
    .end local v5    # "it":Landroidx/compose/ui/graphics/Canvas;
    .end local v6    # "$i$a$-also-BorderCache$drawBorderCache$2":I
    move-object v3, v4

    .line 399
    :cond_8
    invoke-static {v0}, Landroidx/compose/foundation/BorderCache;->access$getCanvasDrawScope$p(Landroidx/compose/foundation/BorderCache;)Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    move-result-object v4

    if-nez v4, :cond_9

    new-instance v4, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    invoke-direct {v4}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;-><init>()V

    move-object v5, v4

    .line 491
    .local v5, "it":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    const/4 v6, 0x0

    .line 399
    .local v6, "$i$a$-also-BorderCache$drawBorderCache$targetDrawScope$1":I
    invoke-static {v0, v5}, Landroidx/compose/foundation/BorderCache;->access$setCanvasDrawScope$p(Landroidx/compose/foundation/BorderCache;Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;)V

    .line 400
    .end local v5    # "it":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    .end local v6    # "$i$a$-also-BorderCache$drawBorderCache$targetDrawScope$1":I
    .local v4, "targetDrawScope":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    :cond_9
    invoke-static/range {p2 .. p3}, Landroidx/compose/ui/unit/IntSizeKt;->toSize-ozmzZPI(J)J

    move-result-wide v13

    .line 401
    .local v13, "drawSize":J
    move-object/from16 v5, p1

    check-cast v5, Landroidx/compose/ui/unit/Density;

    .local v5, "density$iv":Landroidx/compose/ui/unit/Density;
    invoke-virtual/range {p1 .. p1}, Landroidx/compose/ui/draw/CacheDrawScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v6

    .local v6, "layoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    move-wide v8, v13

    .local v8, "$v$c$androidx-compose-ui-geometry-Size$-size$0$iv":J
    move-object/from16 v21, v4

    .local v21, "this_$iv":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    move-object v10, v3

    .local v10, "canvas$iv":Landroidx/compose/ui/graphics/Canvas;
    const/16 v22, 0x0

    .line 492
    .local v22, "$i$f$draw-yzxVdVo":I
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;->getDrawParams()Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;

    move-result-object v11

    invoke-virtual {v11}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->component1()Landroidx/compose/ui/unit/Density;

    move-result-object v12

    .local v12, "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    invoke-virtual {v11}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->component2()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v15

    move-object/from16 v16, v11

    .local v15, "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    invoke-virtual/range {v16 .. v16}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->component3()Landroidx/compose/ui/graphics/Canvas;

    move-result-object v11

    move-object/from16 v18, v11

    move-object/from16 v17, v12

    .end local v12    # "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    .local v17, "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    .local v18, "prevCanvas$iv":Landroidx/compose/ui/graphics/Canvas;
    invoke-virtual/range {v16 .. v16}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->component4-NH-jbRc()J

    move-result-wide v11

    .line 493
    .local v11, "prevSize$iv":J
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;->getDrawParams()Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;

    move-result-object v0

    .local v0, "$this$draw_yzxVdVo_u24lambda_u240$iv":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;
    const/16 v16, 0x0

    .line 494
    .local v16, "$i$a$-apply-CanvasDrawScope$draw$1$iv":I
    invoke-virtual {v0, v5}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setDensity(Landroidx/compose/ui/unit/Density;)V

    .line 495
    invoke-virtual {v0, v6}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setLayoutDirection(Landroidx/compose/ui/unit/LayoutDirection;)V

    .line 496
    invoke-virtual {v0, v10}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setCanvas(Landroidx/compose/ui/graphics/Canvas;)V

    .line 497
    invoke-virtual {v0, v8, v9}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setSize-uvyYCjk(J)V

    .line 498
    nop

    .line 493
    .end local v0    # "$this$draw_yzxVdVo_u24lambda_u240$iv":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;
    .end local v16    # "$i$a$-apply-CanvasDrawScope$draw$1$iv":I
    nop

    .line 499
    invoke-interface {v10}, Landroidx/compose/ui/graphics/Canvas;->save()V

    .line 500
    move-wide/from16 v19, v8

    .end local v8    # "$v$c$androidx-compose-ui-geometry-Size$-size$0$iv":J
    .local v19, "$v$c$androidx-compose-ui-geometry-Size$-size$0$iv":J
    move-object/from16 v8, v21

    check-cast v8, Landroidx/compose/ui/graphics/drawscope/DrawScope;

    .local v8, "$this$drawBorderCache_EMwLDEs_u24lambda_u243":Landroidx/compose/ui/graphics/drawscope/DrawScope;
    const/4 v0, 0x0

    .line 404
    .local v0, "$i$a$-draw-yzxVdVo-BorderCache$drawBorderCache$3":I
    sget-object v9, Landroidx/compose/ui/graphics/Color;->Companion:Landroidx/compose/ui/graphics/Color$Companion;

    invoke-virtual {v9}, Landroidx/compose/ui/graphics/Color$Companion;->getBlack-0d7_KjU()J

    move-result-wide v23

    sget-object v9, Landroidx/compose/ui/graphics/BlendMode;->Companion:Landroidx/compose/ui/graphics/BlendMode$Companion;

    invoke-virtual {v9}, Landroidx/compose/ui/graphics/BlendMode$Companion;->getClear-0nO6VwU()I

    move-result v9

    move-wide/from16 v25, v19

    .end local v19    # "$v$c$androidx-compose-ui-geometry-Size$-size$0$iv":J
    .local v25, "$v$c$androidx-compose-ui-geometry-Size$-size$0$iv":J
    const/16 v19, 0x3a

    const/16 v20, 0x0

    move-wide/from16 v27, v11

    .end local v11    # "prevSize$iv":J
    .local v27, "prevSize$iv":J
    const-wide/16 v11, 0x0

    move-object/from16 v16, v15

    .end local v15    # "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    .local v16, "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    const/4 v15, 0x0

    move-object/from16 v29, v16

    .end local v16    # "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    .local v29, "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    const/16 v16, 0x0

    move-object/from16 v30, v17

    .end local v17    # "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    .local v30, "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    const/16 v17, 0x0

    move-wide/from16 v31, v23

    move/from16 v24, v0

    move-object/from16 v23, v10

    move-object/from16 v0, v30

    move-object/from16 v30, v4

    move-wide/from16 v33, v27

    move/from16 v27, v1

    move-object/from16 v28, v2

    move-object/from16 v2, v18

    move-object/from16 v1, v29

    move-object/from16 v29, v3

    move/from16 v18, v9

    move-wide/from16 v9, v31

    move-wide/from16 v3, v33

    .end local v4    # "targetDrawScope":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    .end local v10    # "canvas$iv":Landroidx/compose/ui/graphics/Canvas;
    .end local v18    # "prevCanvas$iv":Landroidx/compose/ui/graphics/Canvas;
    .local v0, "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    .local v1, "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    .local v2, "prevCanvas$iv":Landroidx/compose/ui/graphics/Canvas;
    .local v3, "prevSize$iv":J
    .local v23, "canvas$iv":Landroidx/compose/ui/graphics/Canvas;
    .local v24, "$i$a$-draw-yzxVdVo-BorderCache$drawBorderCache$3":I
    .local v27, "$i$f$drawBorderCache-EMwLDEs":I
    .local v28, "targetImageBitmap":Landroidx/compose/ui/graphics/ImageBitmap;
    .local v29, "targetCanvas":Landroidx/compose/ui/graphics/Canvas;
    .local v30, "targetDrawScope":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    invoke-static/range {v8 .. v20}, Landroidx/compose/ui/graphics/drawscope/DrawScope;->drawRect-n-J9OG0$default(Landroidx/compose/ui/graphics/drawscope/DrawScope;JJJFLandroidx/compose/ui/graphics/drawscope/DrawStyle;Landroidx/compose/ui/graphics/ColorFilter;IILjava/lang/Object;)V

    .line 405
    move-object/from16 v9, p5

    invoke-interface {v9, v8}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    .line 406
    nop

    .line 500
    .end local v8    # "$this$drawBorderCache_EMwLDEs_u24lambda_u243":Landroidx/compose/ui/graphics/drawscope/DrawScope;
    .end local v24    # "$i$a$-draw-yzxVdVo-BorderCache$drawBorderCache$3":I
    nop

    .line 501
    invoke-interface/range {v23 .. v23}, Landroidx/compose/ui/graphics/Canvas;->restore()V

    .line 502
    invoke-virtual/range {v21 .. v21}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;->getDrawParams()Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;

    move-result-object v8

    .local v8, "$this$draw_yzxVdVo_u24lambda_u241$iv":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;
    const/4 v10, 0x0

    .line 503
    .local v10, "$i$a$-apply-CanvasDrawScope$draw$2$iv":I
    invoke-virtual {v8, v0}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setDensity(Landroidx/compose/ui/unit/Density;)V

    .line 504
    invoke-virtual {v8, v1}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setLayoutDirection(Landroidx/compose/ui/unit/LayoutDirection;)V

    .line 505
    invoke-virtual {v8, v2}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setCanvas(Landroidx/compose/ui/graphics/Canvas;)V

    .line 506
    invoke-virtual {v8, v3, v4}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;->setSize-uvyYCjk(J)V

    .line 507
    nop

    .line 502
    .end local v8    # "$this$draw_yzxVdVo_u24lambda_u241$iv":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope$DrawParams;
    .end local v10    # "$i$a$-apply-CanvasDrawScope$draw$2$iv":I
    nop

    .line 508
    nop

    .line 407
    .end local v0    # "prevDensity$iv":Landroidx/compose/ui/unit/Density;
    .end local v1    # "prevLayoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    .end local v2    # "prevCanvas$iv":Landroidx/compose/ui/graphics/Canvas;
    .end local v3    # "prevSize$iv":J
    .end local v5    # "density$iv":Landroidx/compose/ui/unit/Density;
    .end local v6    # "layoutDirection$iv":Landroidx/compose/ui/unit/LayoutDirection;
    .end local v21    # "this_$iv":Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;
    .end local v22    # "$i$f$draw-yzxVdVo":I
    .end local v23    # "canvas$iv":Landroidx/compose/ui/graphics/Canvas;
    .end local v25    # "$v$c$androidx-compose-ui-geometry-Size$-size$0$iv":J
    invoke-interface/range {v28 .. v28}, Landroidx/compose/ui/graphics/ImageBitmap;->prepareToDraw()V

    .line 408
    return-object v28
.end method

.method public equals(Ljava/lang/Object;)Z
    .locals 5

    const/4 v0, 0x1

    if-ne p0, p1, :cond_0

    return v0

    :cond_0
    instance-of v1, p1, Landroidx/compose/foundation/BorderCache;

    const/4 v2, 0x0

    if-nez v1, :cond_1

    return v2

    :cond_1
    move-object v1, p1

    check-cast v1, Landroidx/compose/foundation/BorderCache;

    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    iget-object v4, v1, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    invoke-static {v3, v4}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v3

    if-nez v3, :cond_2

    return v2

    :cond_2
    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    iget-object v4, v1, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    invoke-static {v3, v4}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v3

    if-nez v3, :cond_3

    return v2

    :cond_3
    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    iget-object v4, v1, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    invoke-static {v3, v4}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v3

    if-nez v3, :cond_4

    return v2

    :cond_4
    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    iget-object v1, v1, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    invoke-static {v3, v1}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v1

    if-nez v1, :cond_5

    return v2

    :cond_5
    return v0
.end method

.method public hashCode()I
    .locals 4

    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    const/4 v1, 0x0

    if-nez v0, :cond_0

    move v0, v1

    goto :goto_0

    :cond_0
    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    invoke-virtual {v0}, Ljava/lang/Object;->hashCode()I

    move-result v0

    :goto_0
    mul-int/lit8 v2, v0, 0x1f

    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    if-nez v3, :cond_1

    move v3, v1

    goto :goto_1

    :cond_1
    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    invoke-virtual {v3}, Ljava/lang/Object;->hashCode()I

    move-result v3

    :goto_1
    add-int/2addr v2, v3

    mul-int/lit8 v0, v2, 0x1f

    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    if-nez v3, :cond_2

    move v3, v1

    goto :goto_2

    :cond_2
    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    invoke-virtual {v3}, Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;->hashCode()I

    move-result v3

    :goto_2
    add-int/2addr v0, v3

    mul-int/lit8 v2, v0, 0x1f

    iget-object v3, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    if-nez v3, :cond_3

    goto :goto_3

    :cond_3
    iget-object v1, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    invoke-virtual {v1}, Ljava/lang/Object;->hashCode()I

    move-result v1

    :goto_3
    add-int/2addr v2, v1

    return v2
.end method

.method public final obtainPath()Landroidx/compose/ui/graphics/Path;
    .locals 3

    .line 411
    iget-object v0, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    if-nez v0, :cond_0

    invoke-static {}, Landroidx/compose/ui/graphics/AndroidPath_androidKt;->Path()Landroidx/compose/ui/graphics/Path;

    move-result-object v0

    move-object v1, v0

    .line 491
    .local v1, "it":Landroidx/compose/ui/graphics/Path;
    const/4 v2, 0x0

    .line 411
    .local v2, "$i$a$-also-BorderCache$obtainPath$1":I
    iput-object v1, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    .end local v1    # "it":Landroidx/compose/ui/graphics/Path;
    .end local v2    # "$i$a$-also-BorderCache$obtainPath$1":I
    :cond_0
    return-object v0
.end method

.method public toString()Ljava/lang/String;
    .locals 2

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    const-string v1, "BorderCache(imageBitmap="

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    iget-object v1, p0, Landroidx/compose/foundation/BorderCache;->imageBitmap:Landroidx/compose/ui/graphics/ImageBitmap;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v1, ", canvas="

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    iget-object v1, p0, Landroidx/compose/foundation/BorderCache;->canvas:Landroidx/compose/ui/graphics/Canvas;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v1, ", canvasDrawScope="

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    iget-object v1, p0, Landroidx/compose/foundation/BorderCache;->canvasDrawScope:Landroidx/compose/ui/graphics/drawscope/CanvasDrawScope;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v1, ", borderPath="

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    iget-object v1, p0, Landroidx/compose/foundation/BorderCache;->borderPath:Landroidx/compose/ui/graphics/Path;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v0

    const/16 v1, 0x29

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method
