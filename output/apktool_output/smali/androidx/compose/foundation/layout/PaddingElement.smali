.class final Landroidx/compose/foundation/layout/PaddingElement;
.super Landroidx/compose/ui/node/ModifierNodeElement;
.source "Padding.kt"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "Landroidx/compose/ui/node/ModifierNodeElement<",
        "Landroidx/compose/foundation/layout/PaddingNode;",
        ">;"
    }
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nPadding.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Padding.kt\nandroidx/compose/foundation/layout/PaddingElement\n+ 2 Dp.kt\nandroidx/compose/ui/unit/DpKt\n+ 3 InlineClassHelper.kt\nandroidx/compose/foundation/layout/internal/InlineClassHelperKt\n*L\n1#1,477:1\n111#2:478\n111#2:479\n111#2:480\n111#2:481\n122#2:486\n122#2:487\n122#2:488\n122#2:489\n93#3,4:482\n*S KotlinDebug\n*F\n+ 1 Padding.kt\nandroidx/compose/foundation/layout/PaddingElement\n*L\n346#1:478\n347#1:479\n348#1:480\n349#1:481\n336#1:486\n337#1:487\n338#1:488\n339#1:489\n345#1:482,4\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000@\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0004\n\u0002\u0010\u000b\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0017\n\u0002\u0010\u0008\n\u0002\u0008\u0002\n\u0002\u0010\u0000\n\u0002\u0008\u0002\u0008\u0002\u0018\u00002\u0008\u0012\u0004\u0012\u00020\u00020\u0001BP\u0012\u0008\u0008\u0002\u0010\u0003\u001a\u00020\u0004\u0012\u0008\u0008\u0002\u0010\u0005\u001a\u00020\u0004\u0012\u0008\u0008\u0002\u0010\u0006\u001a\u00020\u0004\u0012\u0008\u0008\u0002\u0010\u0007\u001a\u00020\u0004\u0012\u0006\u0010\u0008\u001a\u00020\t\u0012\u0017\u0010\n\u001a\u0013\u0012\u0004\u0012\u00020\u000c\u0012\u0004\u0012\u00020\r0\u000b\u00a2\u0006\u0002\u0008\u000e\u00a2\u0006\u0004\u0008\u000f\u0010\u0010J\u0008\u0010\"\u001a\u00020\u0002H\u0016J\u0010\u0010#\u001a\u00020\r2\u0006\u0010$\u001a\u00020\u0002H\u0016J\u0008\u0010%\u001a\u00020&H\u0016J\u0013\u0010\'\u001a\u00020\t2\u0008\u0010(\u001a\u0004\u0018\u00010)H\u0096\u0002J\u000c\u0010*\u001a\u00020\r*\u00020\u000cH\u0016R\u001c\u0010\u0003\u001a\u00020\u0004X\u0086\u000e\u00a2\u0006\u0010\n\u0002\u0010\u0015\u001a\u0004\u0008\u0011\u0010\u0012\"\u0004\u0008\u0013\u0010\u0014R\u001c\u0010\u0005\u001a\u00020\u0004X\u0086\u000e\u00a2\u0006\u0010\n\u0002\u0010\u0015\u001a\u0004\u0008\u0016\u0010\u0012\"\u0004\u0008\u0017\u0010\u0014R\u001c\u0010\u0006\u001a\u00020\u0004X\u0086\u000e\u00a2\u0006\u0010\n\u0002\u0010\u0015\u001a\u0004\u0008\u0018\u0010\u0012\"\u0004\u0008\u0019\u0010\u0014R\u001c\u0010\u0007\u001a\u00020\u0004X\u0086\u000e\u00a2\u0006\u0010\n\u0002\u0010\u0015\u001a\u0004\u0008\u001a\u0010\u0012\"\u0004\u0008\u001b\u0010\u0014R\u001a\u0010\u0008\u001a\u00020\tX\u0086\u000e\u00a2\u0006\u000e\n\u0000\u001a\u0004\u0008\u001c\u0010\u001d\"\u0004\u0008\u001e\u0010\u001fR\"\u0010\n\u001a\u0013\u0012\u0004\u0012\u00020\u000c\u0012\u0004\u0012\u00020\r0\u000b\u00a2\u0006\u0002\u0008\u000e\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008 \u0010!\u00a8\u0006+"
    }
    d2 = {
        "Landroidx/compose/foundation/layout/PaddingElement;",
        "Landroidx/compose/ui/node/ModifierNodeElement;",
        "Landroidx/compose/foundation/layout/PaddingNode;",
        "start",
        "Landroidx/compose/ui/unit/Dp;",
        "top",
        "end",
        "bottom",
        "rtlAware",
        "",
        "inspectorInfo",
        "Lkotlin/Function1;",
        "Landroidx/compose/ui/platform/InspectorInfo;",
        "",
        "Lkotlin/ExtensionFunctionType;",
        "<init>",
        "(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V",
        "getStart-D9Ej5fM",
        "()F",
        "setStart-0680j_4",
        "(F)V",
        "F",
        "getTop-D9Ej5fM",
        "setTop-0680j_4",
        "getEnd-D9Ej5fM",
        "setEnd-0680j_4",
        "getBottom-D9Ej5fM",
        "setBottom-0680j_4",
        "getRtlAware",
        "()Z",
        "setRtlAware",
        "(Z)V",
        "getInspectorInfo",
        "()Lkotlin/jvm/functions/Function1;",
        "create",
        "update",
        "node",
        "hashCode",
        "",
        "equals",
        "other",
        "",
        "inspectableProperties",
        "foundation-layout"
    }
    k = 0x1
    mv = {
        0x2,
        0x0,
        0x0
    }
    xi = 0x30
.end annotation


# instance fields
.field private bottom:F

.field private end:F

.field private final inspectorInfo:Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function1<",
            "Landroidx/compose/ui/platform/InspectorInfo;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field private rtlAware:Z

.field private start:F

.field private top:F


# direct methods
.method private constructor <init>(FFFFZLkotlin/jvm/functions/Function1;)V
    .locals 7
    .param p1, "start"    # F
    .param p2, "top"    # F
    .param p3, "end"    # F
    .param p4, "bottom"    # F
    .param p5, "rtlAware"    # Z
    .param p6, "inspectorInfo"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(FFFFZ",
            "Lkotlin/jvm/functions/Function1<",
            "-",
            "Landroidx/compose/ui/platform/InspectorInfo;",
            "Lkotlin/Unit;",
            ">;)V"
        }
    .end annotation

    .line 335
    invoke-direct {p0}, Landroidx/compose/ui/node/ModifierNodeElement;-><init>()V

    .line 336
    iput p1, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    .line 337
    iput p2, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    .line 338
    iput p3, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    .line 339
    iput p4, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    .line 340
    iput-boolean p5, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    .line 341
    iput-object p6, p0, Landroidx/compose/foundation/layout/PaddingElement;->inspectorInfo:Lkotlin/jvm/functions/Function1;

    .line 344
    nop

    .line 346
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    const/4 v1, 0x0

    cmpl-float v0, v0, v1

    const/4 v2, 0x0

    const/4 v3, 0x1

    if-gez v0, :cond_1

    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    .local v0, "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    const/4 v4, 0x0

    .line 478
    .local v4, "$i$f$isUnspecified-0680j_4":I
    invoke-static {v0}, Ljava/lang/Float;->isNaN(F)Z

    move-result v5

    .line 346
    .end local v0    # "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    .end local v4    # "$i$f$isUnspecified-0680j_4":I
    if-eqz v5, :cond_0

    goto :goto_0

    :cond_0
    move v0, v2

    goto :goto_1

    :cond_1
    :goto_0
    move v0, v3

    .line 347
    :goto_1
    iget v4, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    cmpl-float v4, v4, v1

    if-gez v4, :cond_3

    iget v4, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    .local v4, "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    const/4 v5, 0x0

    .line 479
    .local v5, "$i$f$isUnspecified-0680j_4":I
    invoke-static {v4}, Ljava/lang/Float;->isNaN(F)Z

    move-result v6

    .line 347
    .end local v4    # "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    .end local v5    # "$i$f$isUnspecified-0680j_4":I
    if-eqz v6, :cond_2

    goto :goto_2

    :cond_2
    move v4, v2

    goto :goto_3

    :cond_3
    :goto_2
    move v4, v3

    .line 346
    :goto_3
    and-int/2addr v0, v4

    .line 348
    iget v4, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    cmpl-float v4, v4, v1

    if-gez v4, :cond_5

    iget v4, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    .restart local v4    # "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    const/4 v5, 0x0

    .line 480
    .restart local v5    # "$i$f$isUnspecified-0680j_4":I
    invoke-static {v4}, Ljava/lang/Float;->isNaN(F)Z

    move-result v6

    .line 348
    .end local v4    # "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    .end local v5    # "$i$f$isUnspecified-0680j_4":I
    if-eqz v6, :cond_4

    goto :goto_4

    :cond_4
    move v4, v2

    goto :goto_5

    :cond_5
    :goto_4
    move v4, v3

    .line 346
    :goto_5
    and-int/2addr v0, v4

    .line 349
    iget v4, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    cmpl-float v1, v4, v1

    if-gez v1, :cond_6

    iget v1, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    const/4 v4, 0x0

    .line 481
    .local v4, "$i$f$isUnspecified-0680j_4":I
    invoke-static {v1}, Ljava/lang/Float;->isNaN(F)Z

    move-result v5

    .line 349
    .end local v1    # "$v$c$androidx-compose-ui-unit-Dp$-$this$isUnspecified$0$iv":F
    .end local v4    # "$i$f$isUnspecified-0680j_4":I
    if-eqz v5, :cond_7

    :cond_6
    move v2, v3

    .line 346
    :cond_7
    and-int/2addr v0, v2

    .line 345
    nop

    .local v0, "value$iv":Z
    const/4 v1, 0x0

    .line 482
    .local v1, "$i$f$requirePrecondition":I
    if-nez v0, :cond_8

    .line 483
    const/4 v2, 0x0

    .line 351
    .local v2, "$i$a$-requirePrecondition-PaddingElement$1":I
    nop

    .line 483
    .end local v2    # "$i$a$-requirePrecondition-PaddingElement$1":I
    const-string/jumbo v2, "Padding must be non-negative"

    invoke-static {v2}, Landroidx/compose/foundation/layout/internal/InlineClassHelperKt;->throwIllegalArgumentException(Ljava/lang/String;)V

    .line 485
    :cond_8
    nop

    .line 353
    .end local v0    # "value$iv":Z
    .end local v1    # "$i$f$requirePrecondition":I
    nop

    .line 335
    return-void
.end method

.method public synthetic constructor <init>(FFFFZLkotlin/jvm/functions/Function1;ILkotlin/jvm/internal/DefaultConstructorMarker;)V
    .locals 8

    .line 335
    and-int/lit8 v0, p7, 0x1

    if-eqz v0, :cond_0

    .line 336
    const/4 p1, 0x0

    .local p1, "$this$dp$iv":I
    const/4 v0, 0x0

    .line 486
    .local v0, "$i$f$getDp":I
    int-to-float v1, p1

    invoke-static {v1}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p1

    move v1, p1

    .end local v0    # "$i$f$getDp":I
    .end local p1    # "$this$dp$iv":I
    goto :goto_0

    .line 335
    :cond_0
    move v1, p1

    :goto_0
    and-int/lit8 p1, p7, 0x2

    if-eqz p1, :cond_1

    .line 337
    const/4 p1, 0x0

    .restart local p1    # "$this$dp$iv":I
    const/4 p2, 0x0

    .line 487
    .local p2, "$i$f$getDp":I
    int-to-float v0, p1

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p2

    move v2, p2

    .end local p1    # "$this$dp$iv":I
    .end local p2    # "$i$f$getDp":I
    goto :goto_1

    .line 335
    :cond_1
    move v2, p2

    :goto_1
    and-int/lit8 p1, p7, 0x4

    if-eqz p1, :cond_2

    .line 338
    const/4 p1, 0x0

    .restart local p1    # "$this$dp$iv":I
    const/4 p2, 0x0

    .line 488
    .restart local p2    # "$i$f$getDp":I
    int-to-float p3, p1

    invoke-static {p3}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p3

    move v3, p3

    .end local p1    # "$this$dp$iv":I
    .end local p2    # "$i$f$getDp":I
    goto :goto_2

    .line 335
    :cond_2
    move v3, p3

    :goto_2
    and-int/lit8 p1, p7, 0x8

    if-eqz p1, :cond_3

    .line 339
    const/4 p1, 0x0

    .restart local p1    # "$this$dp$iv":I
    const/4 p2, 0x0

    .line 489
    .restart local p2    # "$i$f$getDp":I
    int-to-float p3, p1

    invoke-static {p3}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p4

    move v4, p4

    .end local p1    # "$this$dp$iv":I
    .end local p2    # "$i$f$getDp":I
    goto :goto_3

    .line 335
    :cond_3
    move v4, p4

    :goto_3
    const/4 v7, 0x0

    move-object v0, p0

    move v5, p5

    move-object v6, p6

    invoke-direct/range {v0 .. v7}, Landroidx/compose/foundation/layout/PaddingElement;-><init>(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    .line 342
    return-void
.end method

.method public synthetic constructor <init>(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V
    .locals 0

    invoke-direct/range {p0 .. p6}, Landroidx/compose/foundation/layout/PaddingElement;-><init>(FFFFZLkotlin/jvm/functions/Function1;)V

    return-void
.end method


# virtual methods
.method public create()Landroidx/compose/foundation/layout/PaddingNode;
    .locals 7

    .line 356
    new-instance v0, Landroidx/compose/foundation/layout/PaddingNode;

    iget v1, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    iget v3, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    iget v4, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    iget-boolean v5, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    const/4 v6, 0x0

    invoke-direct/range {v0 .. v6}, Landroidx/compose/foundation/layout/PaddingNode;-><init>(FFFFZLkotlin/jvm/internal/DefaultConstructorMarker;)V

    return-object v0
.end method

.method public bridge synthetic create()Landroidx/compose/ui/Modifier$Node;
    .locals 1

    .line 335
    invoke-virtual {p0}, Landroidx/compose/foundation/layout/PaddingElement;->create()Landroidx/compose/foundation/layout/PaddingNode;

    move-result-object v0

    check-cast v0, Landroidx/compose/ui/Modifier$Node;

    return-object v0
.end method

.method public equals(Ljava/lang/Object;)Z
    .locals 4
    .param p1, "other"    # Ljava/lang/Object;

    .line 377
    instance-of v0, p1, Landroidx/compose/foundation/layout/PaddingElement;

    if-eqz v0, :cond_0

    move-object v0, p1

    check-cast v0, Landroidx/compose/foundation/layout/PaddingElement;

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    const/4 v1, 0x0

    if-nez v0, :cond_1

    return v1

    .line 378
    .local v0, "otherModifierElement":Landroidx/compose/foundation/layout/PaddingElement;
    :cond_1
    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    iget v3, v0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    invoke-static {v2, v3}, Landroidx/compose/ui/unit/Dp;->equals-impl0(FF)Z

    move-result v2

    if-eqz v2, :cond_2

    .line 379
    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    iget v3, v0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    invoke-static {v2, v3}, Landroidx/compose/ui/unit/Dp;->equals-impl0(FF)Z

    move-result v2

    if-eqz v2, :cond_2

    .line 380
    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    iget v3, v0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    invoke-static {v2, v3}, Landroidx/compose/ui/unit/Dp;->equals-impl0(FF)Z

    move-result v2

    if-eqz v2, :cond_2

    .line 381
    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    iget v3, v0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    invoke-static {v2, v3}, Landroidx/compose/ui/unit/Dp;->equals-impl0(FF)Z

    move-result v2

    if-eqz v2, :cond_2

    .line 382
    iget-boolean v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    iget-boolean v3, v0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    if-ne v2, v3, :cond_2

    const/4 v1, 0x1

    goto :goto_1

    :cond_2
    nop

    .line 378
    :goto_1
    return v1
.end method

.method public final getBottom-D9Ej5fM()F
    .locals 1

    .line 339
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    return v0
.end method

.method public final getEnd-D9Ej5fM()F
    .locals 1

    .line 338
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    return v0
.end method

.method public final getInspectorInfo()Lkotlin/jvm/functions/Function1;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Lkotlin/jvm/functions/Function1<",
            "Landroidx/compose/ui/platform/InspectorInfo;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation

    .line 341
    iget-object v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->inspectorInfo:Lkotlin/jvm/functions/Function1;

    return-object v0
.end method

.method public final getRtlAware()Z
    .locals 1

    .line 340
    iget-boolean v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    return v0
.end method

.method public final getStart-D9Ej5fM()F
    .locals 1

    .line 336
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    return v0
.end method

.method public final getTop-D9Ej5fM()F
    .locals 1

    .line 337
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    return v0
.end method

.method public hashCode()I
    .locals 3

    .line 368
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->hashCode-impl(F)I

    move-result v0

    .line 369
    .local v0, "result":I
    mul-int/lit8 v1, v0, 0x1f

    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    invoke-static {v2}, Landroidx/compose/ui/unit/Dp;->hashCode-impl(F)I

    move-result v2

    add-int/2addr v1, v2

    .line 370
    .end local v0    # "result":I
    .local v1, "result":I
    mul-int/lit8 v0, v1, 0x1f

    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    invoke-static {v2}, Landroidx/compose/ui/unit/Dp;->hashCode-impl(F)I

    move-result v2

    add-int/2addr v0, v2

    .line 371
    .end local v1    # "result":I
    .restart local v0    # "result":I
    mul-int/lit8 v1, v0, 0x1f

    iget v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    invoke-static {v2}, Landroidx/compose/ui/unit/Dp;->hashCode-impl(F)I

    move-result v2

    add-int/2addr v1, v2

    .line 372
    .end local v0    # "result":I
    .restart local v1    # "result":I
    mul-int/lit8 v0, v1, 0x1f

    iget-boolean v2, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    invoke-static {v2}, Ljava/lang/Boolean;->hashCode(Z)I

    move-result v2

    add-int/2addr v0, v2

    .line 373
    .end local v1    # "result":I
    .restart local v0    # "result":I
    return v0
.end method

.method public inspectableProperties(Landroidx/compose/ui/platform/InspectorInfo;)V
    .locals 1
    .param p1, "$this$inspectableProperties"    # Landroidx/compose/ui/platform/InspectorInfo;

    .line 386
    iget-object v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->inspectorInfo:Lkotlin/jvm/functions/Function1;

    invoke-interface {v0, p1}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    .line 387
    return-void
.end method

.method public final setBottom-0680j_4(F)V
    .locals 0
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-<set-?>$0"    # F

    .line 339
    iput p1, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    return-void
.end method

.method public final setEnd-0680j_4(F)V
    .locals 0
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-<set-?>$0"    # F

    .line 338
    iput p1, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    return-void
.end method

.method public final setRtlAware(Z)V
    .locals 0
    .param p1, "<set-?>"    # Z

    .line 340
    iput-boolean p1, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    return-void
.end method

.method public final setStart-0680j_4(F)V
    .locals 0
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-<set-?>$0"    # F

    .line 336
    iput p1, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    return-void
.end method

.method public final setTop-0680j_4(F)V
    .locals 0
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-<set-?>$0"    # F

    .line 337
    iput p1, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    return-void
.end method

.method public update(Landroidx/compose/foundation/layout/PaddingNode;)V
    .locals 1
    .param p1, "node"    # Landroidx/compose/foundation/layout/PaddingNode;

    .line 360
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->start:F

    invoke-virtual {p1, v0}, Landroidx/compose/foundation/layout/PaddingNode;->setStart-0680j_4(F)V

    .line 361
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->top:F

    invoke-virtual {p1, v0}, Landroidx/compose/foundation/layout/PaddingNode;->setTop-0680j_4(F)V

    .line 362
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->end:F

    invoke-virtual {p1, v0}, Landroidx/compose/foundation/layout/PaddingNode;->setEnd-0680j_4(F)V

    .line 363
    iget v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->bottom:F

    invoke-virtual {p1, v0}, Landroidx/compose/foundation/layout/PaddingNode;->setBottom-0680j_4(F)V

    .line 364
    iget-boolean v0, p0, Landroidx/compose/foundation/layout/PaddingElement;->rtlAware:Z

    invoke-virtual {p1, v0}, Landroidx/compose/foundation/layout/PaddingNode;->setRtlAware(Z)V

    .line 365
    return-void
.end method

.method public bridge synthetic update(Landroidx/compose/ui/Modifier$Node;)V
    .locals 1
    .param p1, "node"    # Landroidx/compose/ui/Modifier$Node;

    .line 335
    move-object v0, p1

    check-cast v0, Landroidx/compose/foundation/layout/PaddingNode;

    invoke-virtual {p0, v0}, Landroidx/compose/foundation/layout/PaddingElement;->update(Landroidx/compose/foundation/layout/PaddingNode;)V

    return-void
.end method
