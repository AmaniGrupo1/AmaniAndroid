.class public final Landroidx/compose/foundation/layout/PaddingKt;
.super Ljava/lang/Object;
.source "Padding.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nPadding.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Padding.kt\nandroidx/compose/foundation/layout/PaddingKt\n+ 2 Dp.kt\nandroidx/compose/ui/unit/DpKt\n*L\n1#1,477:1\n122#2:478\n122#2:479\n122#2:480\n122#2:481\n122#2:482\n122#2:483\n122#2:484\n122#2:485\n*S KotlinDebug\n*F\n+ 1 Padding.kt\nandroidx/compose/foundation/layout/PaddingKt\n*L\n53#1:478\n83#1:479\n162#1:480\n279#1:481\n289#1:482\n290#1:483\n291#1:484\n292#1:485\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000 \n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\r\n\u0002\u0018\u0002\n\u0002\u0008\u0006\n\u0002\u0018\u0002\n\u0002\u0008\n\u001a;\u0010\u0000\u001a\u00020\u0001*\u00020\u00012\u0008\u0008\u0002\u0010\u0002\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0004\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0005\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0006\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008\u0007\u0010\u0008\u001a\'\u0010\u0000\u001a\u00020\u0001*\u00020\u00012\u0008\u0008\u0002\u0010\t\u001a\u00020\u00032\u0008\u0008\u0002\u0010\n\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008\u000b\u0010\u000c\u001a\u001b\u0010\u0000\u001a\u00020\u0001*\u00020\u00012\u0006\u0010\r\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008\u000e\u0010\u000f\u001a\u0014\u0010\u0000\u001a\u00020\u0001*\u00020\u00012\u0006\u0010\u0010\u001a\u00020\u0011H\u0007\u001a;\u0010\u0012\u001a\u00020\u0001*\u00020\u00012\u0008\u0008\u0002\u0010\u0013\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0004\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0014\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0006\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008\u0015\u0010\u0008\u001a\u0019\u0010\u0016\u001a\u00020\u0003*\u00020\u00112\u0006\u0010\u0017\u001a\u00020\u0018H\u0007\u00a2\u0006\u0002\u0010\u0019\u001a\u0019\u0010\u001a\u001a\u00020\u0003*\u00020\u00112\u0006\u0010\u0017\u001a\u00020\u0018H\u0007\u00a2\u0006\u0002\u0010\u0019\u001a\u0017\u0010\u001b\u001a\u00020\u00112\u0006\u0010\r\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008\u001c\u0010\u001d\u001a#\u0010\u001b\u001a\u00020\u00112\u0008\u0008\u0002\u0010\t\u001a\u00020\u00032\u0008\u0008\u0002\u0010\n\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008\u001e\u0010\u001f\u001a7\u0010\u001b\u001a\u00020\u00112\u0008\u0008\u0002\u0010\u0002\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0004\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0005\u001a\u00020\u00032\u0008\u0008\u0002\u0010\u0006\u001a\u00020\u0003H\u0007\u00a2\u0006\u0004\u0008 \u0010!\u00a8\u0006\""
    }
    d2 = {
        "padding",
        "Landroidx/compose/ui/Modifier;",
        "start",
        "Landroidx/compose/ui/unit/Dp;",
        "top",
        "end",
        "bottom",
        "padding-qDBjuR0",
        "(Landroidx/compose/ui/Modifier;FFFF)Landroidx/compose/ui/Modifier;",
        "horizontal",
        "vertical",
        "padding-VpY3zN4",
        "(Landroidx/compose/ui/Modifier;FF)Landroidx/compose/ui/Modifier;",
        "all",
        "padding-3ABfNKs",
        "(Landroidx/compose/ui/Modifier;F)Landroidx/compose/ui/Modifier;",
        "paddingValues",
        "Landroidx/compose/foundation/layout/PaddingValues;",
        "absolutePadding",
        "left",
        "right",
        "absolutePadding-qDBjuR0",
        "calculateStartPadding",
        "layoutDirection",
        "Landroidx/compose/ui/unit/LayoutDirection;",
        "(Landroidx/compose/foundation/layout/PaddingValues;Landroidx/compose/ui/unit/LayoutDirection;)F",
        "calculateEndPadding",
        "PaddingValues",
        "PaddingValues-0680j_4",
        "(F)Landroidx/compose/foundation/layout/PaddingValues;",
        "PaddingValues-YgX7TsA",
        "(FF)Landroidx/compose/foundation/layout/PaddingValues;",
        "PaddingValues-a9UjIt4",
        "(FFFF)Landroidx/compose/foundation/layout/PaddingValues;",
        "foundation-layout"
    }
    k = 0x2
    mv = {
        0x2,
        0x0,
        0x0
    }
    xi = 0x30
.end annotation


# direct methods
.method public static final PaddingValues-0680j_4(F)Landroidx/compose/foundation/layout/PaddingValues;
    .locals 6
    .param p0, "$v$c$androidx-compose-ui-unit-Dp$-all$0"    # F

    .line 272
    new-instance v0, Landroidx/compose/foundation/layout/PaddingValuesImpl;

    const/4 v5, 0x0

    move v2, p0

    move v3, p0

    move v4, p0

    move v1, p0

    .end local p0    # "$v$c$androidx-compose-ui-unit-Dp$-all$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-all$0":F
    invoke-direct/range {v0 .. v5}, Landroidx/compose/foundation/layout/PaddingValuesImpl;-><init>(FFFFLkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/foundation/layout/PaddingValues;

    return-object v0
.end method

.method public static final PaddingValues-YgX7TsA(FF)Landroidx/compose/foundation/layout/PaddingValues;
    .locals 6
    .param p0, "$v$c$androidx-compose-ui-unit-Dp$-horizontal$0"    # F
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-vertical$0"    # F

    .line 280
    new-instance v0, Landroidx/compose/foundation/layout/PaddingValuesImpl;

    const/4 v5, 0x0

    move v3, p0

    move v4, p1

    move v1, p0

    move v2, p1

    .end local p0    # "$v$c$androidx-compose-ui-unit-Dp$-horizontal$0":F
    .end local p1    # "$v$c$androidx-compose-ui-unit-Dp$-vertical$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-horizontal$0":F
    .local v2, "$v$c$androidx-compose-ui-unit-Dp$-vertical$0":F
    invoke-direct/range {v0 .. v5}, Landroidx/compose/foundation/layout/PaddingValuesImpl;-><init>(FFFFLkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/foundation/layout/PaddingValues;

    return-object v0
.end method

.method public static synthetic PaddingValues-YgX7TsA$default(FFILjava/lang/Object;)Landroidx/compose/foundation/layout/PaddingValues;
    .locals 1

    .line 279
    and-int/lit8 p3, p2, 0x1

    if-eqz p3, :cond_0

    const/4 p0, 0x0

    .local p0, "$this$dp$iv":I
    const/4 p3, 0x0

    .line 481
    .local p3, "$i$f$getDp":I
    int-to-float v0, p0

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p0

    .line 279
    .end local p0    # "$this$dp$iv":I
    .end local p3    # "$i$f$getDp":I
    :cond_0
    and-int/lit8 p2, p2, 0x2

    if-eqz p2, :cond_1

    const/4 p1, 0x0

    .local p1, "$this$dp$iv":I
    const/4 p2, 0x0

    .line 481
    .local p2, "$i$f$getDp":I
    int-to-float p3, p1

    invoke-static {p3}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p1

    .line 279
    .end local p1    # "$this$dp$iv":I
    .end local p2    # "$i$f$getDp":I
    :cond_1
    invoke-static {p0, p1}, Landroidx/compose/foundation/layout/PaddingKt;->PaddingValues-YgX7TsA(FF)Landroidx/compose/foundation/layout/PaddingValues;

    move-result-object p0

    return-object p0
.end method

.method public static final PaddingValues-a9UjIt4(FFFF)Landroidx/compose/foundation/layout/PaddingValues;
    .locals 6
    .param p0, "$v$c$androidx-compose-ui-unit-Dp$-start$0"    # F
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-top$0"    # F
    .param p2, "$v$c$androidx-compose-ui-unit-Dp$-end$0"    # F
    .param p3, "$v$c$androidx-compose-ui-unit-Dp$-bottom$0"    # F

    .line 293
    new-instance v0, Landroidx/compose/foundation/layout/PaddingValuesImpl;

    const/4 v5, 0x0

    move v1, p0

    move v2, p1

    move v3, p2

    move v4, p3

    .end local p0    # "$v$c$androidx-compose-ui-unit-Dp$-start$0":F
    .end local p1    # "$v$c$androidx-compose-ui-unit-Dp$-top$0":F
    .end local p2    # "$v$c$androidx-compose-ui-unit-Dp$-end$0":F
    .end local p3    # "$v$c$androidx-compose-ui-unit-Dp$-bottom$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-start$0":F
    .local v2, "$v$c$androidx-compose-ui-unit-Dp$-top$0":F
    .local v3, "$v$c$androidx-compose-ui-unit-Dp$-end$0":F
    .local v4, "$v$c$androidx-compose-ui-unit-Dp$-bottom$0":F
    invoke-direct/range {v0 .. v5}, Landroidx/compose/foundation/layout/PaddingValuesImpl;-><init>(FFFFLkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/foundation/layout/PaddingValues;

    return-object v0
.end method

.method public static synthetic PaddingValues-a9UjIt4$default(FFFFILjava/lang/Object;)Landroidx/compose/foundation/layout/PaddingValues;
    .locals 1

    .line 288
    and-int/lit8 p5, p4, 0x1

    if-eqz p5, :cond_0

    .line 289
    const/4 p0, 0x0

    .local p0, "$this$dp$iv":I
    const/4 p5, 0x0

    .line 482
    .local p5, "$i$f$getDp":I
    int-to-float v0, p0

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p0

    .line 288
    .end local p0    # "$this$dp$iv":I
    .end local p5    # "$i$f$getDp":I
    :cond_0
    and-int/lit8 p5, p4, 0x2

    if-eqz p5, :cond_1

    .line 290
    const/4 p1, 0x0

    .local p1, "$this$dp$iv":I
    const/4 p5, 0x0

    .line 483
    .restart local p5    # "$i$f$getDp":I
    int-to-float v0, p1

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p1

    .line 288
    .end local p1    # "$this$dp$iv":I
    .end local p5    # "$i$f$getDp":I
    :cond_1
    and-int/lit8 p5, p4, 0x4

    if-eqz p5, :cond_2

    .line 291
    const/4 p2, 0x0

    .local p2, "$this$dp$iv":I
    const/4 p5, 0x0

    .line 484
    .restart local p5    # "$i$f$getDp":I
    int-to-float v0, p2

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p2

    .line 288
    .end local p2    # "$this$dp$iv":I
    .end local p5    # "$i$f$getDp":I
    :cond_2
    and-int/lit8 p4, p4, 0x8

    if-eqz p4, :cond_3

    .line 292
    const/4 p3, 0x0

    .local p3, "$this$dp$iv":I
    const/4 p4, 0x0

    .line 485
    .local p4, "$i$f$getDp":I
    int-to-float p5, p3

    invoke-static {p5}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p3

    .line 288
    .end local p3    # "$this$dp$iv":I
    .end local p4    # "$i$f$getDp":I
    :cond_3
    invoke-static {p0, p1, p2, p3}, Landroidx/compose/foundation/layout/PaddingKt;->PaddingValues-a9UjIt4(FFFF)Landroidx/compose/foundation/layout/PaddingValues;

    move-result-object p0

    return-object p0
.end method

.method public static final absolutePadding-qDBjuR0(Landroidx/compose/ui/Modifier;FFFF)Landroidx/compose/ui/Modifier;
    .locals 8
    .param p0, "$this$absolutePadding_u2dqDBjuR0"    # Landroidx/compose/ui/Modifier;
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-left$0"    # F
    .param p2, "$v$c$androidx-compose-ui-unit-Dp$-top$0"    # F
    .param p3, "$v$c$androidx-compose-ui-unit-Dp$-right$0"    # F
    .param p4, "$v$c$androidx-compose-ui-unit-Dp$-bottom$0"    # F

    .line 163
    nop

    .line 164
    new-instance v0, Landroidx/compose/foundation/layout/PaddingElement;

    .line 165
    nop

    .line 166
    nop

    .line 167
    nop

    .line 168
    nop

    .line 169
    nop

    .line 163
    new-instance v6, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda2;

    invoke-direct {v6, p1, p2, p3, p4}, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda2;-><init>(FFFF)V

    .line 164
    const/4 v5, 0x0

    const/4 v7, 0x0

    move v1, p1

    move v2, p2

    move v3, p3

    move v4, p4

    .end local p1    # "$v$c$androidx-compose-ui-unit-Dp$-left$0":F
    .end local p2    # "$v$c$androidx-compose-ui-unit-Dp$-top$0":F
    .end local p3    # "$v$c$androidx-compose-ui-unit-Dp$-right$0":F
    .end local p4    # "$v$c$androidx-compose-ui-unit-Dp$-bottom$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-left$0":F
    .local v2, "$v$c$androidx-compose-ui-unit-Dp$-top$0":F
    .local v3, "$v$c$androidx-compose-ui-unit-Dp$-right$0":F
    .local v4, "$v$c$androidx-compose-ui-unit-Dp$-bottom$0":F
    invoke-direct/range {v0 .. v7}, Landroidx/compose/foundation/layout/PaddingElement;-><init>(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/ui/Modifier;

    .line 163
    invoke-interface {p0, v0}, Landroidx/compose/ui/Modifier;->then(Landroidx/compose/ui/Modifier;)Landroidx/compose/ui/Modifier;

    move-result-object p1

    .line 177
    return-object p1
.end method

.method public static synthetic absolutePadding-qDBjuR0$default(Landroidx/compose/ui/Modifier;FFFFILjava/lang/Object;)Landroidx/compose/ui/Modifier;
    .locals 1

    .line 162
    and-int/lit8 p6, p5, 0x1

    if-eqz p6, :cond_0

    const/4 p1, 0x0

    .local p1, "$this$dp$iv":I
    const/4 p6, 0x0

    .line 480
    .local p6, "$i$f$getDp":I
    int-to-float v0, p1

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p1

    .line 162
    .end local p1    # "$this$dp$iv":I
    .end local p6    # "$i$f$getDp":I
    :cond_0
    and-int/lit8 p6, p5, 0x2

    if-eqz p6, :cond_1

    const/4 p2, 0x0

    .local p2, "$this$dp$iv":I
    const/4 p6, 0x0

    .line 480
    .restart local p6    # "$i$f$getDp":I
    int-to-float v0, p2

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p2

    .line 162
    .end local p2    # "$this$dp$iv":I
    .end local p6    # "$i$f$getDp":I
    :cond_1
    and-int/lit8 p6, p5, 0x4

    if-eqz p6, :cond_2

    const/4 p3, 0x0

    .local p3, "$this$dp$iv":I
    const/4 p6, 0x0

    .line 480
    .restart local p6    # "$i$f$getDp":I
    int-to-float v0, p3

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p3

    .line 162
    .end local p3    # "$this$dp$iv":I
    .end local p6    # "$i$f$getDp":I
    :cond_2
    and-int/lit8 p5, p5, 0x8

    if-eqz p5, :cond_3

    const/4 p4, 0x0

    .local p4, "$this$dp$iv":I
    const/4 p5, 0x0

    .line 480
    .local p5, "$i$f$getDp":I
    int-to-float p6, p4

    invoke-static {p6}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p4

    .line 162
    .end local p4    # "$this$dp$iv":I
    .end local p5    # "$i$f$getDp":I
    :cond_3
    invoke-static {p0, p1, p2, p3, p4}, Landroidx/compose/foundation/layout/PaddingKt;->absolutePadding-qDBjuR0(Landroidx/compose/ui/Modifier;FFFF)Landroidx/compose/ui/Modifier;

    move-result-object p0

    return-object p0
.end method

.method static final absolutePadding_qDBjuR0$lambda$0(FFFFLandroidx/compose/ui/platform/InspectorInfo;)Lkotlin/Unit;
    .locals 3
    .param p0, "$$v$c$androidx-compose-ui-unit-Dp$-left$0"    # F
    .param p1, "$$v$c$androidx-compose-ui-unit-Dp$-top$0"    # F
    .param p2, "$$v$c$androidx-compose-ui-unit-Dp$-right$0"    # F
    .param p3, "$$v$c$androidx-compose-ui-unit-Dp$-bottom$0"    # F
    .param p4, "$this$PaddingElement"    # Landroidx/compose/ui/platform/InspectorInfo;

    .line 171
    const-string/jumbo v0, "absolutePadding"

    invoke-virtual {p4, v0}, Landroidx/compose/ui/platform/InspectorInfo;->setName(Ljava/lang/String;)V

    .line 172
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "left"

    invoke-static {p0}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 173
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "top"

    invoke-static {p1}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 174
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "right"

    invoke-static {p2}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 175
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "bottom"

    invoke-static {p3}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 176
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    return-object v0
.end method

.method public static final calculateEndPadding(Landroidx/compose/foundation/layout/PaddingValues;Landroidx/compose/ui/unit/LayoutDirection;)F
    .locals 1
    .param p0, "$this$calculateEndPadding"    # Landroidx/compose/foundation/layout/PaddingValues;
    .param p1, "layoutDirection"    # Landroidx/compose/ui/unit/LayoutDirection;

    .line 265
    sget-object v0, Landroidx/compose/ui/unit/LayoutDirection;->Ltr:Landroidx/compose/ui/unit/LayoutDirection;

    if-ne p1, v0, :cond_0

    .line 266
    invoke-interface {p0, p1}, Landroidx/compose/foundation/layout/PaddingValues;->calculateRightPadding-u2uoSUM(Landroidx/compose/ui/unit/LayoutDirection;)F

    move-result v0

    goto :goto_0

    .line 268
    :cond_0
    invoke-interface {p0, p1}, Landroidx/compose/foundation/layout/PaddingValues;->calculateLeftPadding-u2uoSUM(Landroidx/compose/ui/unit/LayoutDirection;)F

    move-result v0

    .line 269
    :goto_0
    return v0
.end method

.method public static final calculateStartPadding(Landroidx/compose/foundation/layout/PaddingValues;Landroidx/compose/ui/unit/LayoutDirection;)F
    .locals 1
    .param p0, "$this$calculateStartPadding"    # Landroidx/compose/foundation/layout/PaddingValues;
    .param p1, "layoutDirection"    # Landroidx/compose/ui/unit/LayoutDirection;

    .line 253
    sget-object v0, Landroidx/compose/ui/unit/LayoutDirection;->Ltr:Landroidx/compose/ui/unit/LayoutDirection;

    if-ne p1, v0, :cond_0

    .line 254
    invoke-interface {p0, p1}, Landroidx/compose/foundation/layout/PaddingValues;->calculateLeftPadding-u2uoSUM(Landroidx/compose/ui/unit/LayoutDirection;)F

    move-result v0

    goto :goto_0

    .line 256
    :cond_0
    invoke-interface {p0, p1}, Landroidx/compose/foundation/layout/PaddingValues;->calculateRightPadding-u2uoSUM(Landroidx/compose/ui/unit/LayoutDirection;)F

    move-result v0

    .line 257
    :goto_0
    return v0
.end method

.method public static final padding(Landroidx/compose/ui/Modifier;Landroidx/compose/foundation/layout/PaddingValues;)Landroidx/compose/ui/Modifier;
    .locals 2
    .param p0, "$this$padding"    # Landroidx/compose/ui/Modifier;
    .param p1, "paddingValues"    # Landroidx/compose/foundation/layout/PaddingValues;

    .line 139
    nop

    .line 140
    new-instance v0, Landroidx/compose/foundation/layout/PaddingValuesElement;

    .line 141
    nop

    .line 139
    new-instance v1, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda1;

    invoke-direct {v1, p1}, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda1;-><init>(Landroidx/compose/foundation/layout/PaddingValues;)V

    .line 140
    invoke-direct {v0, p1, v1}, Landroidx/compose/foundation/layout/PaddingValuesElement;-><init>(Landroidx/compose/foundation/layout/PaddingValues;Lkotlin/jvm/functions/Function1;)V

    check-cast v0, Landroidx/compose/ui/Modifier;

    .line 139
    invoke-interface {p0, v0}, Landroidx/compose/ui/Modifier;->then(Landroidx/compose/ui/Modifier;)Landroidx/compose/ui/Modifier;

    move-result-object v0

    .line 146
    return-object v0
.end method

.method static final padding$lambda$0(Landroidx/compose/foundation/layout/PaddingValues;Landroidx/compose/ui/platform/InspectorInfo;)Lkotlin/Unit;
    .locals 2
    .param p0, "$paddingValues"    # Landroidx/compose/foundation/layout/PaddingValues;
    .param p1, "$this$PaddingValuesElement"    # Landroidx/compose/ui/platform/InspectorInfo;

    .line 143
    const-string/jumbo v0, "padding"

    invoke-virtual {p1, v0}, Landroidx/compose/ui/platform/InspectorInfo;->setName(Ljava/lang/String;)V

    .line 144
    invoke-virtual {p1}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "paddingValues"

    invoke-virtual {v0, v1, p0}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 145
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    return-object v0
.end method

.method public static final padding-3ABfNKs(Landroidx/compose/ui/Modifier;F)Landroidx/compose/ui/Modifier;
    .locals 8
    .param p0, "$this$padding_u2d3ABfNKs"    # Landroidx/compose/ui/Modifier;
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-all$0"    # F

    .line 112
    nop

    .line 113
    new-instance v0, Landroidx/compose/foundation/layout/PaddingElement;

    .line 114
    nop

    .line 115
    nop

    .line 116
    nop

    .line 117
    nop

    .line 118
    nop

    .line 112
    new-instance v6, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda4;

    invoke-direct {v6, p1}, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda4;-><init>(F)V

    .line 113
    const/4 v5, 0x1

    const/4 v7, 0x0

    move v2, p1

    move v3, p1

    move v4, p1

    move v1, p1

    .end local p1    # "$v$c$androidx-compose-ui-unit-Dp$-all$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-all$0":F
    invoke-direct/range {v0 .. v7}, Landroidx/compose/foundation/layout/PaddingElement;-><init>(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/ui/Modifier;

    .line 112
    invoke-interface {p0, v0}, Landroidx/compose/ui/Modifier;->then(Landroidx/compose/ui/Modifier;)Landroidx/compose/ui/Modifier;

    move-result-object p1

    .line 123
    return-object p1
.end method

.method public static final padding-VpY3zN4(Landroidx/compose/ui/Modifier;FF)Landroidx/compose/ui/Modifier;
    .locals 8
    .param p0, "$this$padding_u2dVpY3zN4"    # Landroidx/compose/ui/Modifier;
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-horizontal$0"    # F
    .param p2, "$v$c$androidx-compose-ui-unit-Dp$-vertical$0"    # F

    .line 84
    nop

    .line 85
    new-instance v0, Landroidx/compose/foundation/layout/PaddingElement;

    .line 86
    nop

    .line 87
    nop

    .line 88
    nop

    .line 89
    nop

    .line 90
    nop

    .line 84
    new-instance v6, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda3;

    invoke-direct {v6, p1, p2}, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda3;-><init>(FF)V

    .line 85
    const/4 v5, 0x1

    const/4 v7, 0x0

    move v3, p1

    move v4, p2

    move v1, p1

    move v2, p2

    .end local p1    # "$v$c$androidx-compose-ui-unit-Dp$-horizontal$0":F
    .end local p2    # "$v$c$androidx-compose-ui-unit-Dp$-vertical$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-horizontal$0":F
    .local v2, "$v$c$androidx-compose-ui-unit-Dp$-vertical$0":F
    invoke-direct/range {v0 .. v7}, Landroidx/compose/foundation/layout/PaddingElement;-><init>(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/ui/Modifier;

    .line 84
    invoke-interface {p0, v0}, Landroidx/compose/ui/Modifier;->then(Landroidx/compose/ui/Modifier;)Landroidx/compose/ui/Modifier;

    move-result-object p1

    .line 96
    return-object p1
.end method

.method public static synthetic padding-VpY3zN4$default(Landroidx/compose/ui/Modifier;FFILjava/lang/Object;)Landroidx/compose/ui/Modifier;
    .locals 1

    .line 83
    and-int/lit8 p4, p3, 0x1

    if-eqz p4, :cond_0

    const/4 p1, 0x0

    .local p1, "$this$dp$iv":I
    const/4 p4, 0x0

    .line 479
    .local p4, "$i$f$getDp":I
    int-to-float v0, p1

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p1

    .line 83
    .end local p1    # "$this$dp$iv":I
    .end local p4    # "$i$f$getDp":I
    :cond_0
    and-int/lit8 p3, p3, 0x2

    if-eqz p3, :cond_1

    const/4 p2, 0x0

    .local p2, "$this$dp$iv":I
    const/4 p3, 0x0

    .line 479
    .local p3, "$i$f$getDp":I
    int-to-float p4, p2

    invoke-static {p4}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p2

    .line 83
    .end local p2    # "$this$dp$iv":I
    .end local p3    # "$i$f$getDp":I
    :cond_1
    invoke-static {p0, p1, p2}, Landroidx/compose/foundation/layout/PaddingKt;->padding-VpY3zN4(Landroidx/compose/ui/Modifier;FF)Landroidx/compose/ui/Modifier;

    move-result-object p0

    return-object p0
.end method

.method public static final padding-qDBjuR0(Landroidx/compose/ui/Modifier;FFFF)Landroidx/compose/ui/Modifier;
    .locals 8
    .param p0, "$this$padding_u2dqDBjuR0"    # Landroidx/compose/ui/Modifier;
    .param p1, "$v$c$androidx-compose-ui-unit-Dp$-start$0"    # F
    .param p2, "$v$c$androidx-compose-ui-unit-Dp$-top$0"    # F
    .param p3, "$v$c$androidx-compose-ui-unit-Dp$-end$0"    # F
    .param p4, "$v$c$androidx-compose-ui-unit-Dp$-bottom$0"    # F

    .line 54
    nop

    .line 55
    new-instance v0, Landroidx/compose/foundation/layout/PaddingElement;

    .line 56
    nop

    .line 57
    nop

    .line 58
    nop

    .line 59
    nop

    .line 60
    nop

    .line 54
    new-instance v6, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda0;

    invoke-direct {v6, p1, p2, p3, p4}, Landroidx/compose/foundation/layout/PaddingKt$$ExternalSyntheticLambda0;-><init>(FFFF)V

    .line 55
    const/4 v5, 0x1

    const/4 v7, 0x0

    move v1, p1

    move v2, p2

    move v3, p3

    move v4, p4

    .end local p1    # "$v$c$androidx-compose-ui-unit-Dp$-start$0":F
    .end local p2    # "$v$c$androidx-compose-ui-unit-Dp$-top$0":F
    .end local p3    # "$v$c$androidx-compose-ui-unit-Dp$-end$0":F
    .end local p4    # "$v$c$androidx-compose-ui-unit-Dp$-bottom$0":F
    .local v1, "$v$c$androidx-compose-ui-unit-Dp$-start$0":F
    .local v2, "$v$c$androidx-compose-ui-unit-Dp$-top$0":F
    .local v3, "$v$c$androidx-compose-ui-unit-Dp$-end$0":F
    .local v4, "$v$c$androidx-compose-ui-unit-Dp$-bottom$0":F
    invoke-direct/range {v0 .. v7}, Landroidx/compose/foundation/layout/PaddingElement;-><init>(FFFFZLkotlin/jvm/functions/Function1;Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    check-cast v0, Landroidx/compose/ui/Modifier;

    .line 54
    invoke-interface {p0, v0}, Landroidx/compose/ui/Modifier;->then(Landroidx/compose/ui/Modifier;)Landroidx/compose/ui/Modifier;

    move-result-object p1

    .line 68
    return-object p1
.end method

.method public static synthetic padding-qDBjuR0$default(Landroidx/compose/ui/Modifier;FFFFILjava/lang/Object;)Landroidx/compose/ui/Modifier;
    .locals 1

    .line 53
    and-int/lit8 p6, p5, 0x1

    if-eqz p6, :cond_0

    const/4 p1, 0x0

    .local p1, "$this$dp$iv":I
    const/4 p6, 0x0

    .line 478
    .local p6, "$i$f$getDp":I
    int-to-float v0, p1

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p1

    .line 53
    .end local p1    # "$this$dp$iv":I
    .end local p6    # "$i$f$getDp":I
    :cond_0
    and-int/lit8 p6, p5, 0x2

    if-eqz p6, :cond_1

    const/4 p2, 0x0

    .local p2, "$this$dp$iv":I
    const/4 p6, 0x0

    .line 478
    .restart local p6    # "$i$f$getDp":I
    int-to-float v0, p2

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p2

    .line 53
    .end local p2    # "$this$dp$iv":I
    .end local p6    # "$i$f$getDp":I
    :cond_1
    and-int/lit8 p6, p5, 0x4

    if-eqz p6, :cond_2

    const/4 p3, 0x0

    .local p3, "$this$dp$iv":I
    const/4 p6, 0x0

    .line 478
    .restart local p6    # "$i$f$getDp":I
    int-to-float v0, p3

    invoke-static {v0}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p3

    .line 53
    .end local p3    # "$this$dp$iv":I
    .end local p6    # "$i$f$getDp":I
    :cond_2
    and-int/lit8 p5, p5, 0x8

    if-eqz p5, :cond_3

    const/4 p4, 0x0

    .local p4, "$this$dp$iv":I
    const/4 p5, 0x0

    .line 478
    .local p5, "$i$f$getDp":I
    int-to-float p6, p4

    invoke-static {p6}, Landroidx/compose/ui/unit/Dp;->constructor-impl(F)F

    move-result p4

    .line 53
    .end local p4    # "$this$dp$iv":I
    .end local p5    # "$i$f$getDp":I
    :cond_3
    invoke-static {p0, p1, p2, p3, p4}, Landroidx/compose/foundation/layout/PaddingKt;->padding-qDBjuR0(Landroidx/compose/ui/Modifier;FFFF)Landroidx/compose/ui/Modifier;

    move-result-object p0

    return-object p0
.end method

.method static final padding_3ABfNKs$lambda$0(FLandroidx/compose/ui/platform/InspectorInfo;)Lkotlin/Unit;
    .locals 1
    .param p0, "$$v$c$androidx-compose-ui-unit-Dp$-all$0"    # F
    .param p1, "$this$PaddingElement"    # Landroidx/compose/ui/platform/InspectorInfo;

    .line 120
    const-string/jumbo v0, "padding"

    invoke-virtual {p1, v0}, Landroidx/compose/ui/platform/InspectorInfo;->setName(Ljava/lang/String;)V

    .line 121
    invoke-static {p0}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v0

    invoke-virtual {p1, v0}, Landroidx/compose/ui/platform/InspectorInfo;->setValue(Ljava/lang/Object;)V

    .line 122
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    return-object v0
.end method

.method static final padding_VpY3zN4$lambda$0(FFLandroidx/compose/ui/platform/InspectorInfo;)Lkotlin/Unit;
    .locals 3
    .param p0, "$$v$c$androidx-compose-ui-unit-Dp$-horizontal$0"    # F
    .param p1, "$$v$c$androidx-compose-ui-unit-Dp$-vertical$0"    # F
    .param p2, "$this$PaddingElement"    # Landroidx/compose/ui/platform/InspectorInfo;

    .line 92
    const-string/jumbo v0, "padding"

    invoke-virtual {p2, v0}, Landroidx/compose/ui/platform/InspectorInfo;->setName(Ljava/lang/String;)V

    .line 93
    invoke-virtual {p2}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "horizontal"

    invoke-static {p0}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 94
    invoke-virtual {p2}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "vertical"

    invoke-static {p1}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 95
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    return-object v0
.end method

.method static final padding_qDBjuR0$lambda$0(FFFFLandroidx/compose/ui/platform/InspectorInfo;)Lkotlin/Unit;
    .locals 3
    .param p0, "$$v$c$androidx-compose-ui-unit-Dp$-start$0"    # F
    .param p1, "$$v$c$androidx-compose-ui-unit-Dp$-top$0"    # F
    .param p2, "$$v$c$androidx-compose-ui-unit-Dp$-end$0"    # F
    .param p3, "$$v$c$androidx-compose-ui-unit-Dp$-bottom$0"    # F
    .param p4, "$this$PaddingElement"    # Landroidx/compose/ui/platform/InspectorInfo;

    .line 62
    const-string/jumbo v0, "padding"

    invoke-virtual {p4, v0}, Landroidx/compose/ui/platform/InspectorInfo;->setName(Ljava/lang/String;)V

    .line 63
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "start"

    invoke-static {p0}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 64
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "top"

    invoke-static {p1}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 65
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "end"

    invoke-static {p2}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 66
    invoke-virtual {p4}, Landroidx/compose/ui/platform/InspectorInfo;->getProperties()Landroidx/compose/ui/platform/ValueElementSequence;

    move-result-object v0

    const-string/jumbo v1, "bottom"

    invoke-static {p3}, Landroidx/compose/ui/unit/Dp;->box-impl(F)Landroidx/compose/ui/unit/Dp;

    move-result-object v2

    invoke-virtual {v0, v1, v2}, Landroidx/compose/ui/platform/ValueElementSequence;->set(Ljava/lang/String;Ljava/lang/Object;)V

    .line 67
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    return-object v0
.end method
