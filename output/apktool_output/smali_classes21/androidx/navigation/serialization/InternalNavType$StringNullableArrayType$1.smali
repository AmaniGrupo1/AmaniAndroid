.class public final Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;
.super Landroidx/navigation/CollectionNavType;
.source "NavTypeConverter.kt"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Landroidx/navigation/serialization/InternalNavType;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x19
    name = null
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "Landroidx/navigation/CollectionNavType<",
        "[",
        "Ljava/lang/String;",
        ">;"
    }
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nNavTypeConverter.kt\nKotlin\n*S Kotlin\n*F\n+ 1 NavTypeConverter.kt\nandroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1\n+ 2 SavedState.kt\nandroidx/savedstate/SavedStateKt__SavedStateKt\n+ 3 _Arrays.kt\nkotlin/collections/ArraysKt___ArraysKt\n+ 4 ArraysJVM.kt\nkotlin/collections/ArraysKt__ArraysJVMKt\n+ 5 fake.kt\nkotlin/jvm/internal/FakeKt\n*L\n1#1,454:1\n106#2:455\n90#2:462\n11165#3:456\n11500#3,3:457\n11165#3:463\n11500#3,3:464\n11165#3:469\n11500#3,2:470\n11502#3:473\n37#4,2:460\n37#4,2:467\n1#5:472\n*S KotlinDebug\n*F\n+ 1 NavTypeConverter.kt\nandroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1\n*L\n319#1:455\n327#1:462\n321#1:456\n321#1:457,3\n329#1:463\n329#1:464,3\n346#1:469\n346#1:470,2\n346#1:473\n321#1:460,2\n329#1:467,2\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u00007\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u0011\n\u0002\u0010\u000e\n\u0002\u0008\u0004\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0008\n\n\u0002\u0010\u000b\n\u0002\u0008\u0003\n\u0002\u0010 \n\u0002\u0008\u0004*\u0001\u0000\u0008\n\u0018\u00002\u0012\u0012\u000e\u0012\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u00020\u0001J3\u0010\u0007\u001a\u00020\u00082\n\u0010\t\u001a\u00060\nj\u0002`\u000b2\u0006\u0010\u000c\u001a\u00020\u00032\u0010\u0010\r\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u0002H\u0016\u00a2\u0006\u0002\u0010\u000eJ,\u0010\u000f\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u00022\n\u0010\t\u001a\u00060\nj\u0002`\u000b2\u0006\u0010\u000c\u001a\u00020\u0003H\u0096\u0002\u00a2\u0006\u0002\u0010\u0010J\u001d\u0010\u0011\u001a\n\u0012\u0006\u0012\u0004\u0018\u00010\u00030\u00022\u0006\u0010\r\u001a\u00020\u0003H\u0016\u00a2\u0006\u0002\u0010\u0012J1\u0010\u0011\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u00022\u0006\u0010\r\u001a\u00020\u00032\u0010\u0010\u0013\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u0002H\u0016\u00a2\u0006\u0002\u0010\u0014J1\u0010\u0015\u001a\u00020\u00162\u0010\u0010\r\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u00022\u0010\u0010\u0017\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u0002H\u0016\u00a2\u0006\u0002\u0010\u0018J%\u0010\u0019\u001a\u0008\u0012\u0004\u0012\u00020\u00030\u001a2\u0010\u0010\r\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u0002H\u0016\u00a2\u0006\u0002\u0010\u001bJ\u0017\u0010\u001c\u001a\u000c\u0012\u0006\u0012\u0004\u0018\u00010\u0003\u0018\u00010\u0002H\u0016\u00a2\u0006\u0002\u0010\u001dR\u0014\u0010\u0004\u001a\u00020\u00038VX\u0096\u0004\u00a2\u0006\u0006\u001a\u0004\u0008\u0005\u0010\u0006\u00a8\u0006\u001e"
    }
    d2 = {
        "androidx/navigation/serialization/InternalNavType$StringNullableArrayType$1",
        "Landroidx/navigation/CollectionNavType;",
        "",
        "",
        "name",
        "getName",
        "()Ljava/lang/String;",
        "put",
        "",
        "bundle",
        "Landroid/os/Bundle;",
        "Landroidx/savedstate/SavedState;",
        "key",
        "value",
        "(Landroid/os/Bundle;Ljava/lang/String;[Ljava/lang/String;)V",
        "get",
        "(Landroid/os/Bundle;Ljava/lang/String;)[Ljava/lang/String;",
        "parseValue",
        "(Ljava/lang/String;)[Ljava/lang/String;",
        "previousValue",
        "(Ljava/lang/String;[Ljava/lang/String;)[Ljava/lang/String;",
        "valueEquals",
        "",
        "other",
        "([Ljava/lang/String;[Ljava/lang/String;)Z",
        "serializeAsValues",
        "",
        "([Ljava/lang/String;)Ljava/util/List;",
        "emptyCollection",
        "()[Ljava/lang/String;",
        "navigation-common_release"
    }
    k = 0x1
    mv = {
        0x2,
        0x0,
        0x0
    }
    xi = 0x30
.end annotation


# direct methods
.method constructor <init>()V
    .locals 1

    .line 314
    const/4 v0, 0x1

    invoke-direct {p0, v0}, Landroidx/navigation/CollectionNavType;-><init>(Z)V

    return-void
.end method


# virtual methods
.method public bridge synthetic emptyCollection()Ljava/lang/Object;
    .locals 1

    .line 314
    invoke-virtual {p0}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->emptyCollection()[Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public emptyCollection()[Ljava/lang/String;
    .locals 1

    const/4 v0, 0x0

    new-array v0, v0, [Ljava/lang/String;

    .line 348
    return-object v0
.end method

.method public bridge synthetic get(Landroid/os/Bundle;Ljava/lang/String;)Ljava/lang/Object;
    .locals 1
    .param p1, "bundle"    # Landroid/os/Bundle;
    .param p2, "key"    # Ljava/lang/String;

    .line 314
    invoke-virtual {p0, p1, p2}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->get(Landroid/os/Bundle;Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public get(Landroid/os/Bundle;Ljava/lang/String;)[Ljava/lang/String;
    .locals 17
    .param p1, "bundle"    # Landroid/os/Bundle;
    .param p2, "key"    # Ljava/lang/String;

    move-object/from16 v0, p2

    const-string v1, "bundle"

    move-object/from16 v2, p1

    invoke-static {v2, v1}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v1, "key"

    invoke-static {v0, v1}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 327
    move-object/from16 v1, p1

    .local v1, "$this$read$iv":Landroid/os/Bundle;
    const/4 v3, 0x0

    .line 462
    .local v3, "$i$f$read":I
    invoke-static {v1}, Landroidx/savedstate/SavedStateReader;->constructor-impl(Landroid/os/Bundle;)Landroid/os/Bundle;

    move-result-object v4

    .local v4, "$this$get_u24lambda_u243":Landroid/os/Bundle;
    const/4 v5, 0x0

    .line 328
    .local v5, "$i$a$-read-InternalNavType$StringNullableArrayType$1$get$1":I
    invoke-static {v4, v0}, Landroidx/savedstate/SavedStateReader;->contains-impl(Landroid/os/Bundle;Ljava/lang/String;)Z

    move-result v6

    if-eqz v6, :cond_1

    invoke-static {v4, v0}, Landroidx/savedstate/SavedStateReader;->isNull-impl(Landroid/os/Bundle;Ljava/lang/String;)Z

    move-result v6

    if-nez v6, :cond_1

    .line 329
    invoke-static {v4, v0}, Landroidx/savedstate/SavedStateReader;->getStringArray-impl(Landroid/os/Bundle;Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v6

    .local v6, "$this$map$iv":[Ljava/lang/Object;
    const/4 v7, 0x0

    .line 463
    .local v7, "$i$f$map":I
    new-instance v8, Ljava/util/ArrayList;

    array-length v9, v6

    invoke-direct {v8, v9}, Ljava/util/ArrayList;-><init>(I)V

    check-cast v8, Ljava/util/Collection;

    .local v8, "destination$iv$iv":Ljava/util/Collection;
    move-object v9, v6

    .local v9, "$this$mapTo$iv$iv":[Ljava/lang/Object;
    const/4 v10, 0x0

    .line 464
    .local v10, "$i$f$mapTo":I
    array-length v11, v9

    const/4 v13, 0x0

    :goto_0
    if-ge v13, v11, :cond_0

    aget-object v14, v9, v13

    .line 465
    .local v14, "item$iv$iv":Ljava/lang/Object;
    move-object v15, v14

    .local v15, "it":Ljava/lang/String;
    const/16 v16, 0x0

    .line 329
    .local v16, "$i$a$-map-InternalNavType$StringNullableArrayType$1$get$1$1":I
    sget-object v12, Landroidx/navigation/NavType;->StringType:Landroidx/navigation/NavType;

    invoke-virtual {v12, v15}, Landroidx/navigation/NavType;->parseValue(Ljava/lang/String;)Ljava/lang/Object;

    move-result-object v12

    check-cast v12, Ljava/lang/String;

    .line 465
    .end local v15    # "it":Ljava/lang/String;
    .end local v16    # "$i$a$-map-InternalNavType$StringNullableArrayType$1$get$1$1":I
    invoke-interface {v8, v12}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 464
    .end local v14    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v13, v13, 0x1

    goto :goto_0

    .line 466
    :cond_0
    nop

    .end local v8    # "destination$iv$iv":Ljava/util/Collection;
    .end local v9    # "$this$mapTo$iv$iv":[Ljava/lang/Object;
    .end local v10    # "$i$f$mapTo":I
    check-cast v8, Ljava/util/List;

    .line 463
    nop

    .end local v6    # "$this$map$iv":[Ljava/lang/Object;
    .end local v7    # "$i$f$map":I
    check-cast v8, Ljava/util/Collection;

    .line 329
    nop

    .local v8, "$this$toTypedArray$iv":Ljava/util/Collection;
    const/4 v6, 0x0

    .line 467
    .local v6, "$i$f$toTypedArray":I
    move-object v7, v8

    .line 468
    .local v7, "thisCollection$iv":Ljava/util/Collection;
    const/4 v9, 0x0

    new-array v9, v9, [Ljava/lang/String;

    invoke-interface {v7, v9}, Ljava/util/Collection;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v6

    .end local v6    # "$i$f$toTypedArray":I
    .end local v7    # "thisCollection$iv":Ljava/util/Collection;
    .end local v8    # "$this$toTypedArray$iv":Ljava/util/Collection;
    check-cast v6, [Ljava/lang/String;

    goto :goto_1

    .line 330
    :cond_1
    const/4 v6, 0x0

    .line 462
    .end local v4    # "$this$get_u24lambda_u243":Landroid/os/Bundle;
    .end local v5    # "$i$a$-read-InternalNavType$StringNullableArrayType$1$get$1":I
    :goto_1
    nop

    .line 331
    .end local v1    # "$this$read$iv":Landroid/os/Bundle;
    .end local v3    # "$i$f$read":I
    return-object v6
.end method

.method public getName()Ljava/lang/String;
    .locals 1

    .line 316
    const-string/jumbo v0, "string_nullable[]"

    return-object v0
.end method

.method public bridge synthetic parseValue(Ljava/lang/String;)Ljava/lang/Object;
    .locals 1
    .param p1, "value"    # Ljava/lang/String;

    .line 314
    invoke-virtual {p0, p1}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->parseValue(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public bridge synthetic parseValue(Ljava/lang/String;Ljava/lang/Object;)Ljava/lang/Object;
    .locals 1
    .param p1, "value"    # Ljava/lang/String;
    .param p2, "previousValue"    # Ljava/lang/Object;

    .line 314
    move-object v0, p2

    check-cast v0, [Ljava/lang/String;

    invoke-virtual {p0, p1, v0}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->parseValue(Ljava/lang/String;[Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public parseValue(Ljava/lang/String;)[Ljava/lang/String;
    .locals 3
    .param p1, "value"    # Ljava/lang/String;

    const-string/jumbo v0, "value"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 335
    const/4 v0, 0x1

    new-array v0, v0, [Ljava/lang/String;

    sget-object v1, Landroidx/navigation/NavType;->StringType:Landroidx/navigation/NavType;

    invoke-virtual {v1, p1}, Landroidx/navigation/NavType;->parseValue(Ljava/lang/String;)Ljava/lang/Object;

    move-result-object v1

    const/4 v2, 0x0

    aput-object v1, v0, v2

    return-object v0
.end method

.method public parseValue(Ljava/lang/String;[Ljava/lang/String;)[Ljava/lang/String;
    .locals 1
    .param p1, "value"    # Ljava/lang/String;
    .param p2, "previousValue"    # [Ljava/lang/String;

    const-string/jumbo v0, "value"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 340
    if-eqz p2, :cond_0

    invoke-virtual {p0, p1}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->parseValue(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v0

    invoke-static {p2, v0}, Lkotlin/collections/ArraysKt;->plus([Ljava/lang/Object;[Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v0

    check-cast v0, [Ljava/lang/String;

    if-nez v0, :cond_1

    :cond_0
    invoke-virtual {p0, p1}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->parseValue(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v0

    :cond_1
    return-object v0
.end method

.method public bridge synthetic put(Landroid/os/Bundle;Ljava/lang/String;Ljava/lang/Object;)V
    .locals 1
    .param p1, "bundle"    # Landroid/os/Bundle;
    .param p2, "key"    # Ljava/lang/String;
    .param p3, "value"    # Ljava/lang/Object;

    .line 314
    move-object v0, p3

    check-cast v0, [Ljava/lang/String;

    invoke-virtual {p0, p1, p2, v0}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->put(Landroid/os/Bundle;Ljava/lang/String;[Ljava/lang/String;)V

    return-void
.end method

.method public put(Landroid/os/Bundle;Ljava/lang/String;[Ljava/lang/String;)V
    .locals 18
    .param p1, "bundle"    # Landroid/os/Bundle;
    .param p2, "key"    # Ljava/lang/String;
    .param p3, "value"    # [Ljava/lang/String;

    move-object/from16 v0, p2

    const-string v1, "bundle"

    move-object/from16 v2, p1

    invoke-static {v2, v1}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v1, "key"

    invoke-static {v0, v1}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 319
    move-object/from16 v1, p1

    .local v1, "$this$write$iv":Landroid/os/Bundle;
    const/4 v3, 0x0

    .line 455
    .local v3, "$i$f$write":I
    invoke-static {v1}, Landroidx/savedstate/SavedStateWriter;->constructor-impl(Landroid/os/Bundle;)Landroid/os/Bundle;

    move-result-object v4

    .local v4, "$this$put_u24lambda_u241":Landroid/os/Bundle;
    const/4 v5, 0x0

    .line 320
    .local v5, "$i$a$-write-InternalNavType$StringNullableArrayType$1$put$1":I
    if-nez p3, :cond_0

    invoke-static {v4, v0}, Landroidx/savedstate/SavedStateWriter;->putNull-impl(Landroid/os/Bundle;Ljava/lang/String;)V

    goto :goto_1

    .line 321
    :cond_0
    move-object/from16 v6, p3

    .local v6, "$this$map$iv":[Ljava/lang/Object;
    const/4 v7, 0x0

    .line 456
    .local v7, "$i$f$map":I
    new-instance v8, Ljava/util/ArrayList;

    array-length v9, v6

    invoke-direct {v8, v9}, Ljava/util/ArrayList;-><init>(I)V

    check-cast v8, Ljava/util/Collection;

    .local v8, "destination$iv$iv":Ljava/util/Collection;
    move-object v9, v6

    .local v9, "$this$mapTo$iv$iv":[Ljava/lang/Object;
    const/4 v10, 0x0

    .line 457
    .local v10, "$i$f$mapTo":I
    array-length v11, v9

    const/4 v12, 0x0

    move v13, v12

    :goto_0
    if-ge v13, v11, :cond_2

    aget-object v14, v9, v13

    .line 458
    .local v14, "item$iv$iv":Ljava/lang/Object;
    move-object v15, v14

    .local v15, "it":Ljava/lang/String;
    const/16 v16, 0x0

    .line 321
    .local v16, "$i$a$-map-InternalNavType$StringNullableArrayType$1$put$1$1":I
    if-nez v15, :cond_1

    const-string/jumbo v17, "null"

    move-object/from16 v15, v17

    .line 458
    .end local v15    # "it":Ljava/lang/String;
    .end local v16    # "$i$a$-map-InternalNavType$StringNullableArrayType$1$put$1$1":I
    :cond_1
    invoke-interface {v8, v15}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 457
    .end local v14    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v13, v13, 0x1

    goto :goto_0

    .line 459
    :cond_2
    nop

    .end local v8    # "destination$iv$iv":Ljava/util/Collection;
    .end local v9    # "$this$mapTo$iv$iv":[Ljava/lang/Object;
    .end local v10    # "$i$f$mapTo":I
    check-cast v8, Ljava/util/List;

    .line 456
    nop

    .end local v6    # "$this$map$iv":[Ljava/lang/Object;
    .end local v7    # "$i$f$map":I
    check-cast v8, Ljava/util/Collection;

    .line 321
    nop

    .local v8, "$this$toTypedArray$iv":Ljava/util/Collection;
    const/4 v6, 0x0

    .line 460
    .local v6, "$i$f$toTypedArray":I
    move-object v7, v8

    .line 461
    .local v7, "thisCollection$iv":Ljava/util/Collection;
    new-array v9, v12, [Ljava/lang/String;

    invoke-interface {v7, v9}, Ljava/util/Collection;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v6

    .end local v6    # "$i$f$toTypedArray":I
    .end local v7    # "thisCollection$iv":Ljava/util/Collection;
    .end local v8    # "$this$toTypedArray$iv":Ljava/util/Collection;
    check-cast v6, [Ljava/lang/String;

    .line 321
    invoke-static {v4, v0, v6}, Landroidx/savedstate/SavedStateWriter;->putStringArray-impl(Landroid/os/Bundle;Ljava/lang/String;[Ljava/lang/String;)V

    .line 322
    :goto_1
    nop

    .line 455
    .end local v4    # "$this$put_u24lambda_u241":Landroid/os/Bundle;
    .end local v5    # "$i$a$-write-InternalNavType$StringNullableArrayType$1$put$1":I
    nop

    .line 323
    .end local v1    # "$this$write$iv":Landroid/os/Bundle;
    .end local v3    # "$i$f$write":I
    return-void
.end method

.method public bridge synthetic serializeAsValues(Ljava/lang/Object;)Ljava/util/List;
    .locals 1
    .param p1, "value"    # Ljava/lang/Object;

    .line 314
    move-object v0, p1

    check-cast v0, [Ljava/lang/String;

    invoke-virtual {p0, v0}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->serializeAsValues([Ljava/lang/String;)Ljava/util/List;

    move-result-object v0

    return-object v0
.end method

.method public serializeAsValues([Ljava/lang/String;)Ljava/util/List;
    .locals 15
    .param p1, "value"    # [Ljava/lang/String;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "([",
            "Ljava/lang/String;",
            ")",
            "Ljava/util/List<",
            "Ljava/lang/String;",
            ">;"
        }
    .end annotation

    .line 346
    if-eqz p1, :cond_3

    move-object/from16 v0, p1

    .local v0, "$this$map$iv":[Ljava/lang/Object;
    const/4 v1, 0x0

    .line 469
    .local v1, "$i$f$map":I
    new-instance v2, Ljava/util/ArrayList;

    array-length v3, v0

    invoke-direct {v2, v3}, Ljava/util/ArrayList;-><init>(I)V

    check-cast v2, Ljava/util/Collection;

    .local v2, "destination$iv$iv":Ljava/util/Collection;
    move-object v3, v0

    .local v3, "$this$mapTo$iv$iv":[Ljava/lang/Object;
    const/4 v4, 0x0

    .line 470
    .local v4, "$i$f$mapTo":I
    array-length v5, v3

    const/4 v6, 0x0

    :goto_0
    if-ge v6, v5, :cond_2

    aget-object v7, v3, v6

    .line 471
    .local v7, "item$iv$iv":Ljava/lang/Object;
    move-object v8, v7

    .local v8, "it":Ljava/lang/String;
    const/4 v9, 0x0

    .line 346
    .local v9, "$i$a$-map-InternalNavType$StringNullableArrayType$1$serializeAsValues$1":I
    if-eqz v8, :cond_0

    move-object v10, v8

    .line 472
    .local v10, "it":Ljava/lang/String;
    const/4 v11, 0x0

    .line 346
    .local v11, "$i$a$-let-InternalNavType$StringNullableArrayType$1$serializeAsValues$1$1":I
    sget-object v12, Landroidx/navigation/NavUriUtils;->INSTANCE:Landroidx/navigation/NavUriUtils;

    const/4 v13, 0x2

    const/4 v14, 0x0

    invoke-static {v12, v10, v14, v13, v14}, Landroidx/navigation/NavUriUtils;->encode$default(Landroidx/navigation/NavUriUtils;Ljava/lang/String;Ljava/lang/String;ILjava/lang/Object;)Ljava/lang/String;

    move-result-object v10

    .end local v10    # "it":Ljava/lang/String;
    .end local v11    # "$i$a$-let-InternalNavType$StringNullableArrayType$1$serializeAsValues$1$1":I
    if-nez v10, :cond_1

    :cond_0
    const-string/jumbo v10, "null"

    .line 471
    .end local v8    # "it":Ljava/lang/String;
    .end local v9    # "$i$a$-map-InternalNavType$StringNullableArrayType$1$serializeAsValues$1":I
    :cond_1
    invoke-interface {v2, v10}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 470
    .end local v7    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v6, v6, 0x1

    goto :goto_0

    .line 473
    :cond_2
    nop

    .end local v2    # "destination$iv$iv":Ljava/util/Collection;
    .end local v3    # "$this$mapTo$iv$iv":[Ljava/lang/Object;
    .end local v4    # "$i$f$mapTo":I
    check-cast v2, Ljava/util/List;

    .line 469
    nop

    .line 346
    .end local v0    # "$this$map$iv":[Ljava/lang/Object;
    .end local v1    # "$i$f$map":I
    goto :goto_1

    :cond_3
    invoke-static {}, Lkotlin/collections/CollectionsKt;->emptyList()Ljava/util/List;

    move-result-object v2

    :goto_1
    return-object v2
.end method

.method public bridge synthetic valueEquals(Ljava/lang/Object;Ljava/lang/Object;)Z
    .locals 2
    .param p1, "value"    # Ljava/lang/Object;
    .param p2, "other"    # Ljava/lang/Object;

    .line 314
    move-object v0, p1

    check-cast v0, [Ljava/lang/String;

    move-object v1, p2

    check-cast v1, [Ljava/lang/String;

    invoke-virtual {p0, v0, v1}, Landroidx/navigation/serialization/InternalNavType$StringNullableArrayType$1;->valueEquals([Ljava/lang/String;[Ljava/lang/String;)Z

    move-result v0

    return v0
.end method

.method public valueEquals([Ljava/lang/String;[Ljava/lang/String;)Z
    .locals 1
    .param p1, "value"    # [Ljava/lang/String;
    .param p2, "other"    # [Ljava/lang/String;

    .line 343
    invoke-static {p1, p2}, Lkotlin/collections/ArraysKt;->contentDeepEquals([Ljava/lang/Object;[Ljava/lang/Object;)Z

    move-result v0

    return v0
.end method
