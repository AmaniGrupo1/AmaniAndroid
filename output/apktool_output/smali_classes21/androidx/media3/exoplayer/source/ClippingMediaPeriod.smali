.class public final Landroidx/media3/exoplayer/source/ClippingMediaPeriod;
.super Ljava/lang/Object;
.source "ClippingMediaPeriod.java"

# interfaces
.implements Landroidx/media3/exoplayer/source/MediaPeriod;
.implements Landroidx/media3/exoplayer/source/MediaPeriod$Callback;


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;
    }
.end annotation


# instance fields
.field private callback:Landroidx/media3/exoplayer/source/MediaPeriod$Callback;

.field private clippingError:Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;

.field private final enableClippingInMediaPeriod:Z

.field endUs:J

.field private isPeriodClippingEndPosition:Z

.field private lastReportedDiscontinuityUs:J

.field public final mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

.field private pendingInitialDiscontinuityPositionUs:J

.field private sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

.field startUs:J


# direct methods
.method public constructor <init>(Landroidx/media3/exoplayer/source/MediaPeriod;ZJJ)V
    .locals 8
    .param p1, "mediaPeriod"    # Landroidx/media3/exoplayer/source/MediaPeriod;
    .param p2, "enableInitialDiscontinuity"    # Z
    .param p3, "startUs"    # J
    .param p5, "endUs"    # J

    .line 78
    const/4 v7, 0x0

    move-object v0, p0

    move-object v1, p1

    move v2, p2

    move-wide v3, p3

    move-wide v5, p5

    .end local p1    # "mediaPeriod":Landroidx/media3/exoplayer/source/MediaPeriod;
    .end local p2    # "enableInitialDiscontinuity":Z
    .end local p3    # "startUs":J
    .end local p5    # "endUs":J
    .local v1, "mediaPeriod":Landroidx/media3/exoplayer/source/MediaPeriod;
    .local v2, "enableInitialDiscontinuity":Z
    .local v3, "startUs":J
    .local v5, "endUs":J
    invoke-direct/range {v0 .. v7}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;-><init>(Landroidx/media3/exoplayer/source/MediaPeriod;ZJJZ)V

    .line 84
    return-void
.end method

.method public constructor <init>(Landroidx/media3/exoplayer/source/MediaPeriod;ZJJZ)V
    .locals 4
    .param p1, "mediaPeriod"    # Landroidx/media3/exoplayer/source/MediaPeriod;
    .param p2, "enableInitialDiscontinuity"    # Z
    .param p3, "startUs"    # J
    .param p5, "endUs"    # J
    .param p7, "enableClippingInMediaPeriod"    # Z

    .line 108
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 109
    iput-object p1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    .line 110
    const/4 v0, 0x0

    new-array v0, v0, [Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    iput-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    .line 111
    const-wide v0, -0x7fffffffffffffffL    # -4.9E-324

    if-eqz p2, :cond_0

    move-wide v2, p3

    goto :goto_0

    :cond_0
    move-wide v2, v0

    :goto_0
    iput-wide v2, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->pendingInitialDiscontinuityPositionUs:J

    .line 112
    iput-wide v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->lastReportedDiscontinuityUs:J

    .line 113
    iput-boolean p7, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->enableClippingInMediaPeriod:Z

    .line 114
    invoke-virtual {p0, p3, p4, p5, p6}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->updateClipping(JJ)V

    .line 115
    return-void
.end method

.method static synthetic access$000(Landroidx/media3/exoplayer/source/ClippingMediaPeriod;)Z
    .locals 1
    .param p0, "x0"    # Landroidx/media3/exoplayer/source/ClippingMediaPeriod;

    .line 46
    iget-boolean v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->isPeriodClippingEndPosition:Z

    return v0
.end method

.method static synthetic access$100(Landroidx/media3/exoplayer/FormatHolder;JJ)V
    .locals 0
    .param p0, "x0"    # Landroidx/media3/exoplayer/FormatHolder;
    .param p1, "x1"    # J
    .param p3, "x2"    # J

    .line 46
    invoke-static {p0, p1, p2, p3, p4}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->updateDecoderDelayPaddingForClipping(Landroidx/media3/exoplayer/FormatHolder;JJ)V

    return-void
.end method

.method static synthetic access$200(Landroidx/media3/exoplayer/source/ClippingMediaPeriod;)J
    .locals 2
    .param p0, "x0"    # Landroidx/media3/exoplayer/source/ClippingMediaPeriod;

    .line 46
    iget-wide v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->lastReportedDiscontinuityUs:J

    return-wide v0
.end method

.method static synthetic access$202(Landroidx/media3/exoplayer/source/ClippingMediaPeriod;J)J
    .locals 0
    .param p0, "x0"    # Landroidx/media3/exoplayer/source/ClippingMediaPeriod;
    .param p1, "x1"    # J

    .line 46
    iput-wide p1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->lastReportedDiscontinuityUs:J

    return-wide p1
.end method

.method private clipSeekParameters(JLandroidx/media3/exoplayer/SeekParameters;)Landroidx/media3/exoplayer/SeekParameters;
    .locals 8
    .param p1, "positionUs"    # J
    .param p3, "seekParameters"    # Landroidx/media3/exoplayer/SeekParameters;

    .line 320
    iget-wide v0, p3, Landroidx/media3/exoplayer/SeekParameters;->toleranceBeforeUs:J

    iget-wide v2, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->startUs:J

    sub-long v4, p1, v2

    .line 321
    const-wide/16 v2, 0x0

    invoke-static/range {v0 .. v5}, Landroidx/media3/common/util/Util;->constrainValue(JJJ)J

    move-result-wide v0

    .line 323
    .local v0, "toleranceBeforeUs":J
    iget-wide v2, p3, Landroidx/media3/exoplayer/SeekParameters;->toleranceAfterUs:J

    .line 327
    iget-wide v4, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    const-wide/high16 v6, -0x8000000000000000L

    cmp-long v4, v4, v6

    if-nez v4, :cond_0

    const-wide v4, 0x7fffffffffffffffL

    goto :goto_0

    :cond_0
    iget-wide v4, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    sub-long/2addr v4, p1

    :goto_0
    move-wide v6, v4

    .line 324
    const-wide/16 v4, 0x0

    invoke-static/range {v2 .. v7}, Landroidx/media3/common/util/Util;->constrainValue(JJJ)J

    move-result-wide v2

    .line 328
    .local v2, "toleranceAfterUs":J
    iget-wide v4, p3, Landroidx/media3/exoplayer/SeekParameters;->toleranceBeforeUs:J

    cmp-long v4, v0, v4

    if-nez v4, :cond_1

    iget-wide v4, p3, Landroidx/media3/exoplayer/SeekParameters;->toleranceAfterUs:J

    cmp-long v4, v2, v4

    if-nez v4, :cond_1

    .line 330
    return-object p3

    .line 332
    :cond_1
    new-instance v4, Landroidx/media3/exoplayer/SeekParameters;

    invoke-direct {v4, v0, v1, v2, v3}, Landroidx/media3/exoplayer/SeekParameters;-><init>(JJ)V

    return-object v4
.end method

.method private static enforceClippingRange(JJJ)J
    .locals 2
    .param p0, "positionUs"    # J
    .param p2, "minPositionUs"    # J
    .param p4, "maxPositionUs"    # J

    .line 369
    invoke-static {p0, p1, p2, p3}, Ljava/lang/Math;->max(JJ)J

    move-result-wide p0

    .line 370
    const-wide/high16 v0, -0x8000000000000000L

    cmp-long v0, p4, v0

    if-eqz v0, :cond_0

    .line 371
    invoke-static {p0, p1, p4, p5}, Ljava/lang/Math;->min(JJ)J

    move-result-wide p0

    .line 373
    :cond_0
    return-wide p0
.end method

.method private static shouldKeepInitialDiscontinuity(JJ[Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;)Z
    .locals 8
    .param p0, "startUs"    # J
    .param p2, "requestedPositionUs"    # J
    .param p4, "selections"    # [Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;

    .line 341
    cmp-long v0, p0, p2

    const/4 v1, 0x1

    if-gez v0, :cond_0

    .line 342
    return v1

    .line 353
    :cond_0
    const-wide/16 v2, 0x0

    cmp-long v0, p0, v2

    const/4 v2, 0x0

    if-eqz v0, :cond_2

    .line 354
    array-length v0, p4

    move v3, v2

    :goto_0
    if-ge v3, v0, :cond_2

    aget-object v4, p4, v3

    .line 355
    .local v4, "trackSelection":Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;
    if-eqz v4, :cond_1

    .line 356
    invoke-interface {v4}, Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;->getSelectedFormat()Landroidx/media3/common/Format;

    move-result-object v5

    .line 357
    .local v5, "selectedFormat":Landroidx/media3/common/Format;
    iget-object v6, v5, Landroidx/media3/common/Format;->sampleMimeType:Ljava/lang/String;

    iget-object v7, v5, Landroidx/media3/common/Format;->codecs:Ljava/lang/String;

    invoke-static {v6, v7}, Landroidx/media3/common/MimeTypes;->allSamplesAreSyncSamples(Ljava/lang/String;Ljava/lang/String;)Z

    move-result v6

    if-nez v6, :cond_1

    .line 359
    return v1

    .line 354
    .end local v4    # "trackSelection":Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;
    .end local v5    # "selectedFormat":Landroidx/media3/common/Format;
    :cond_1
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 364
    :cond_2
    return v2
.end method

.method private static updateDecoderDelayPaddingForClipping(Landroidx/media3/exoplayer/FormatHolder;JJ)V
    .locals 5
    .param p0, "formatHolder"    # Landroidx/media3/exoplayer/FormatHolder;
    .param p1, "startUs"    # J
    .param p3, "endUs"    # J

    .line 454
    iget-object v0, p0, Landroidx/media3/exoplayer/FormatHolder;->format:Landroidx/media3/common/Format;

    invoke-static {v0}, Lcom/google/common/base/Preconditions;->checkNotNull(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Landroidx/media3/common/Format;

    .line 455
    .local v0, "format":Landroidx/media3/common/Format;
    iget v1, v0, Landroidx/media3/common/Format;->encoderDelay:I

    if-nez v1, :cond_0

    iget v1, v0, Landroidx/media3/common/Format;->encoderPadding:I

    if-eqz v1, :cond_3

    .line 457
    :cond_0
    const-wide/16 v1, 0x0

    cmp-long v1, p1, v1

    const/4 v2, 0x0

    if-eqz v1, :cond_1

    move v1, v2

    goto :goto_0

    :cond_1
    iget v1, v0, Landroidx/media3/common/Format;->encoderDelay:I

    .line 458
    .local v1, "encoderDelay":I
    :goto_0
    const-wide/high16 v3, -0x8000000000000000L

    cmp-long v3, p3, v3

    if-eqz v3, :cond_2

    goto :goto_1

    :cond_2
    iget v2, v0, Landroidx/media3/common/Format;->encoderPadding:I

    .line 459
    .local v2, "encoderPadding":I
    :goto_1
    nop

    .line 461
    invoke-virtual {v0}, Landroidx/media3/common/Format;->buildUpon()Landroidx/media3/common/Format$Builder;

    move-result-object v3

    .line 462
    invoke-virtual {v3, v1}, Landroidx/media3/common/Format$Builder;->setEncoderDelay(I)Landroidx/media3/common/Format$Builder;

    move-result-object v3

    .line 463
    invoke-virtual {v3, v2}, Landroidx/media3/common/Format$Builder;->setEncoderPadding(I)Landroidx/media3/common/Format$Builder;

    move-result-object v3

    .line 464
    invoke-virtual {v3}, Landroidx/media3/common/Format$Builder;->build()Landroidx/media3/common/Format;

    move-result-object v3

    iput-object v3, p0, Landroidx/media3/exoplayer/FormatHolder;->format:Landroidx/media3/common/Format;

    .line 466
    .end local v1    # "encoderDelay":I
    .end local v2    # "encoderPadding":I
    :cond_3
    return-void
.end method


# virtual methods
.method public continueLoading(Landroidx/media3/exoplayer/LoadingInfo;)Z
    .locals 1
    .param p1, "loadingInfo"    # Landroidx/media3/exoplayer/LoadingInfo;

    .line 292
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p1}, Landroidx/media3/exoplayer/source/MediaPeriod;->continueLoading(Landroidx/media3/exoplayer/LoadingInfo;)Z

    move-result v0

    return v0
.end method

.method public discardBuffer(JZ)V
    .locals 1
    .param p1, "positionUs"    # J
    .param p3, "toKeyframe"    # Z

    .line 208
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p1, p2, p3}, Landroidx/media3/exoplayer/source/MediaPeriod;->discardBuffer(JZ)V

    .line 209
    return-void
.end method

.method public getAdjustedSeekPositionUs(JLandroidx/media3/exoplayer/SeekParameters;)J
    .locals 3
    .param p1, "positionUs"    # J
    .param p3, "seekParameters"    # Landroidx/media3/exoplayer/SeekParameters;

    .line 267
    iget-wide v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->startUs:J

    cmp-long v0, p1, v0

    if-nez v0, :cond_0

    .line 269
    iget-wide v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->startUs:J

    return-wide v0

    .line 271
    :cond_0
    invoke-direct {p0, p1, p2, p3}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->clipSeekParameters(JLandroidx/media3/exoplayer/SeekParameters;)Landroidx/media3/exoplayer/SeekParameters;

    move-result-object v0

    .line 272
    .local v0, "clippedSeekParameters":Landroidx/media3/exoplayer/SeekParameters;
    iget-object v1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v1, p1, p2, v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->getAdjustedSeekPositionUs(JLandroidx/media3/exoplayer/SeekParameters;)J

    move-result-wide v1

    return-wide v1
.end method

.method public getBufferedPositionUs()J
    .locals 7

    .line 241
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->getBufferedPositionUs()J

    move-result-wide v0

    .line 242
    .local v0, "bufferedPositionUs":J
    iget-boolean v2, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->isPeriodClippingEndPosition:Z

    const-wide/high16 v3, -0x8000000000000000L

    if-eqz v2, :cond_1

    .line 243
    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    cmp-long v2, v5, v3

    if-eqz v2, :cond_0

    cmp-long v2, v0, v3

    if-eqz v2, :cond_0

    .line 244
    iget-wide v2, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    invoke-static {v2, v3, v0, v1}, Ljava/lang/Math;->min(JJ)J

    move-result-wide v2

    goto :goto_0

    .line 245
    :cond_0
    move-wide v2, v0

    .line 243
    :goto_0
    return-wide v2

    .line 247
    :cond_1
    cmp-long v2, v0, v3

    if-eqz v2, :cond_3

    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    cmp-long v2, v5, v3

    if-eqz v2, :cond_2

    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    cmp-long v2, v0, v5

    if-ltz v2, :cond_2

    goto :goto_1

    .line 251
    :cond_2
    return-wide v0

    .line 249
    :cond_3
    :goto_1
    return-wide v3
.end method

.method public getNextLoadPositionUs()J
    .locals 7

    .line 277
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->getNextLoadPositionUs()J

    move-result-wide v0

    .line 278
    .local v0, "nextLoadPositionUs":J
    iget-boolean v2, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->isPeriodClippingEndPosition:Z

    const-wide/high16 v3, -0x8000000000000000L

    if-eqz v2, :cond_1

    .line 279
    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    cmp-long v2, v5, v3

    if-eqz v2, :cond_0

    cmp-long v2, v0, v3

    if-eqz v2, :cond_0

    .line 280
    iget-wide v2, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    invoke-static {v2, v3, v0, v1}, Ljava/lang/Math;->min(JJ)J

    move-result-wide v2

    goto :goto_0

    .line 281
    :cond_0
    move-wide v2, v0

    .line 279
    :goto_0
    return-wide v2

    .line 283
    :cond_1
    cmp-long v2, v0, v3

    if-eqz v2, :cond_3

    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    cmp-long v2, v5, v3

    if-eqz v2, :cond_2

    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    cmp-long v2, v0, v5

    if-ltz v2, :cond_2

    goto :goto_1

    .line 287
    :cond_2
    return-wide v0

    .line 285
    :cond_3
    :goto_1
    return-wide v3
.end method

.method public getStreamKeys(Ljava/util/List;)Ljava/util/List;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/List<",
            "Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;",
            ">;)",
            "Ljava/util/List<",
            "Landroidx/media3/common/StreamKey;",
            ">;"
        }
    .end annotation

    .line 164
    .local p1, "trackSelections":Ljava/util/List;, "Ljava/util/List<Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;>;"
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p1}, Landroidx/media3/exoplayer/source/MediaPeriod;->getStreamKeys(Ljava/util/List;)Ljava/util/List;

    move-result-object v0

    return-object v0
.end method

.method public getTrackGroups()Landroidx/media3/exoplayer/source/TrackGroupArray;
    .locals 1

    .line 169
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->getTrackGroups()Landroidx/media3/exoplayer/source/TrackGroupArray;

    move-result-object v0

    return-object v0
.end method

.method public isLoading()Z
    .locals 1

    .line 297
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->isLoading()Z

    move-result v0

    return v0
.end method

.method isPendingInitialDiscontinuity()Z
    .locals 4

    .line 316
    iget-wide v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->pendingInitialDiscontinuityPositionUs:J

    const-wide v2, -0x7fffffffffffffffL    # -4.9E-324

    cmp-long v0, v0, v2

    if-eqz v0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method public maybeThrowPrepareError()V
    .locals 1
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .line 156
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->clippingError:Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;

    if-nez v0, :cond_0

    .line 159
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->maybeThrowPrepareError()V

    .line 160
    return-void

    .line 157
    :cond_0
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->clippingError:Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;

    throw v0
.end method

.method public onContinueLoadingRequested(Landroidx/media3/exoplayer/source/MediaPeriod;)V
    .locals 1
    .param p1, "source"    # Landroidx/media3/exoplayer/source/MediaPeriod;

    .line 312
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->callback:Landroidx/media3/exoplayer/source/MediaPeriod$Callback;

    invoke-static {v0}, Lcom/google/common/base/Preconditions;->checkNotNull(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Landroidx/media3/exoplayer/source/MediaPeriod$Callback;

    invoke-interface {v0, p0}, Landroidx/media3/exoplayer/source/MediaPeriod$Callback;->onContinueLoadingRequested(Landroidx/media3/exoplayer/source/SequenceableLoader;)V

    .line 313
    return-void
.end method

.method public bridge synthetic onContinueLoadingRequested(Landroidx/media3/exoplayer/source/SequenceableLoader;)V
    .locals 0
    .annotation system Ldalvik/annotation/MethodParameters;
        accessFlags = {
            0x1000
        }
        names = {
            null
        }
    .end annotation

    .line 45
    check-cast p1, Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-virtual {p0, p1}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->onContinueLoadingRequested(Landroidx/media3/exoplayer/source/MediaPeriod;)V

    return-void
.end method

.method public onPrepared(Landroidx/media3/exoplayer/source/MediaPeriod;)V
    .locals 1
    .param p1, "mediaPeriod"    # Landroidx/media3/exoplayer/source/MediaPeriod;

    .line 304
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->clippingError:Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;

    if-eqz v0, :cond_0

    .line 305
    return-void

    .line 307
    :cond_0
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->callback:Landroidx/media3/exoplayer/source/MediaPeriod$Callback;

    invoke-static {v0}, Lcom/google/common/base/Preconditions;->checkNotNull(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Landroidx/media3/exoplayer/source/MediaPeriod$Callback;

    invoke-interface {v0, p0}, Landroidx/media3/exoplayer/source/MediaPeriod$Callback;->onPrepared(Landroidx/media3/exoplayer/source/MediaPeriod;)V

    .line 308
    return-void
.end method

.method public prepare(Landroidx/media3/exoplayer/source/MediaPeriod$Callback;J)V
    .locals 1
    .param p1, "callback"    # Landroidx/media3/exoplayer/source/MediaPeriod$Callback;
    .param p2, "positionUs"    # J

    .line 150
    iput-object p1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->callback:Landroidx/media3/exoplayer/source/MediaPeriod$Callback;

    .line 151
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p0, p2, p3}, Landroidx/media3/exoplayer/source/MediaPeriod;->prepare(Landroidx/media3/exoplayer/source/MediaPeriod$Callback;J)V

    .line 152
    return-void
.end method

.method public readDiscontinuity()J
    .locals 9

    .line 218
    invoke-virtual {p0}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->isPendingInitialDiscontinuity()Z

    move-result v0

    const-wide v1, -0x7fffffffffffffffL    # -4.9E-324

    if-eqz v0, :cond_1

    .line 219
    iget-wide v3, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->pendingInitialDiscontinuityPositionUs:J

    .line 220
    .local v3, "initialDiscontinuityUs":J
    iput-wide v1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->pendingInitialDiscontinuityPositionUs:J

    .line 221
    iput-wide v3, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->lastReportedDiscontinuityUs:J

    .line 223
    invoke-virtual {p0}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->readDiscontinuity()J

    move-result-wide v5

    .line 224
    .local v5, "childDiscontinuityUs":J
    cmp-long v0, v5, v1

    if-eqz v0, :cond_0

    move-wide v0, v5

    goto :goto_0

    :cond_0
    move-wide v0, v3

    :goto_0
    return-wide v0

    .line 226
    .end local v3    # "initialDiscontinuityUs":J
    .end local v5    # "childDiscontinuityUs":J
    :cond_1
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0}, Landroidx/media3/exoplayer/source/MediaPeriod;->readDiscontinuity()J

    move-result-wide v3

    .line 227
    .local v3, "discontinuityUs":J
    cmp-long v0, v3, v1

    if-nez v0, :cond_2

    .line 228
    return-wide v1

    .line 230
    :cond_2
    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->startUs:J

    iget-wide v7, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    invoke-static/range {v3 .. v8}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->enforceClippingRange(JJJ)J

    move-result-wide v3

    .line 231
    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->lastReportedDiscontinuityUs:J

    cmp-long v0, v3, v5

    if-nez v0, :cond_3

    .line 233
    return-wide v1

    .line 235
    :cond_3
    iput-wide v3, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->lastReportedDiscontinuityUs:J

    .line 236
    return-wide v3
.end method

.method public reevaluateBuffer(J)V
    .locals 1
    .param p1, "positionUs"    # J

    .line 213
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p1, p2}, Landroidx/media3/exoplayer/source/MediaPeriod;->reevaluateBuffer(J)V

    .line 214
    return-void
.end method

.method public seekToUs(J)J
    .locals 7
    .param p1, "positionUs"    # J

    .line 256
    const-wide v0, -0x7fffffffffffffffL    # -4.9E-324

    iput-wide v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->pendingInitialDiscontinuityPositionUs:J

    .line 257
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    array-length v1, v0

    const/4 v2, 0x0

    :goto_0
    if-ge v2, v1, :cond_1

    aget-object v3, v0, v2

    .line 258
    .local v3, "sampleStream":Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;
    if-eqz v3, :cond_0

    .line 259
    invoke-virtual {v3}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;->clearSentEos()V

    .line 257
    .end local v3    # "sampleStream":Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;
    :cond_0
    add-int/lit8 v2, v2, 0x1

    goto :goto_0

    .line 262
    :cond_1
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p1, p2}, Landroidx/media3/exoplayer/source/MediaPeriod;->seekToUs(J)J

    move-result-wide v1

    iget-wide v3, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->startUs:J

    iget-wide v5, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    invoke-static/range {v1 .. v6}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->enforceClippingRange(JJJ)J

    move-result-wide v0

    return-wide v0
.end method

.method public selectTracks([Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;[Z[Landroidx/media3/exoplayer/source/SampleStream;[ZJ)J
    .locals 17
    .param p1, "selections"    # [Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;
    .param p2, "mayRetainStreamFlags"    # [Z
    .param p3, "streams"    # [Landroidx/media3/exoplayer/source/SampleStream;
    .param p4, "streamResetFlags"    # [Z
    .param p5, "positionUs"    # J

    .line 179
    move-object/from16 v0, p0

    move-object/from16 v1, p3

    array-length v2, v1

    new-array v2, v2, [Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    iput-object v2, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    .line 180
    array-length v2, v1

    new-array v6, v2, [Landroidx/media3/exoplayer/source/SampleStream;

    .line 181
    .local v6, "childStreams":[Landroidx/media3/exoplayer/source/SampleStream;
    const/4 v2, 0x0

    .local v2, "i":I
    :goto_0
    array-length v3, v1

    const/4 v10, 0x0

    if-ge v2, v3, :cond_1

    .line 182
    iget-object v3, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aget-object v4, v1, v2

    check-cast v4, Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aput-object v4, v3, v2

    .line 183
    iget-object v3, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aget-object v3, v3, v2

    if-eqz v3, :cond_0

    iget-object v3, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aget-object v3, v3, v2

    iget-object v10, v3, Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;->childStream:Landroidx/media3/exoplayer/source/SampleStream;

    :cond_0
    aput-object v10, v6, v2

    .line 181
    add-int/lit8 v2, v2, 0x1

    goto :goto_0

    .line 185
    .end local v2    # "i":I
    :cond_1
    iget-object v3, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    .line 186
    move-object/from16 v4, p1

    move-object/from16 v5, p2

    move-object/from16 v7, p4

    move-wide/from16 v8, p5

    invoke-interface/range {v3 .. v9}, Landroidx/media3/exoplayer/source/MediaPeriod;->selectTracks([Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;[Z[Landroidx/media3/exoplayer/source/SampleStream;[ZJ)J

    move-result-wide v11

    .line 188
    .local v11, "realEnablePositionUs":J
    iget-wide v2, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    .line 189
    move-wide/from16 v13, p5

    move-wide v15, v2

    invoke-static/range {v11 .. v16}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->enforceClippingRange(JJJ)J

    move-result-wide v2

    .line 190
    .local v2, "correctedEnablePositionUs":J
    nop

    .line 192
    nop

    .line 191
    invoke-virtual {v0}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->isPendingInitialDiscontinuity()Z

    move-result v4

    if-eqz v4, :cond_2

    .line 192
    move-object/from16 v4, p1

    move-wide/from16 v13, p5

    invoke-static {v11, v12, v13, v14, v4}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->shouldKeepInitialDiscontinuity(JJ[Landroidx/media3/exoplayer/trackselection/ExoTrackSelection;)Z

    move-result v5

    if-eqz v5, :cond_3

    .line 193
    move-wide v7, v2

    goto :goto_1

    .line 191
    :cond_2
    move-object/from16 v4, p1

    move-wide/from16 v13, p5

    .line 194
    :cond_3
    const-wide v7, -0x7fffffffffffffffL    # -4.9E-324

    :goto_1
    iput-wide v7, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->pendingInitialDiscontinuityPositionUs:J

    .line 195
    const/4 v5, 0x0

    .local v5, "i":I
    :goto_2
    array-length v7, v1

    if-ge v5, v7, :cond_7

    .line 196
    aget-object v7, v6, v5

    .line 198
    iget-object v8, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    .line 196
    if-nez v7, :cond_4

    .line 197
    aput-object v10, v8, v5

    goto :goto_3

    .line 198
    :cond_4
    aget-object v7, v8, v5

    if-eqz v7, :cond_5

    iget-object v7, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aget-object v7, v7, v5

    iget-object v7, v7, Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;->childStream:Landroidx/media3/exoplayer/source/SampleStream;

    aget-object v8, v6, v5

    if-eq v7, v8, :cond_6

    .line 199
    :cond_5
    iget-object v7, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    new-instance v8, Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aget-object v9, v6, v5

    invoke-direct {v8, v0, v9}, Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;-><init>(Landroidx/media3/exoplayer/source/ClippingMediaPeriod;Landroidx/media3/exoplayer/source/SampleStream;)V

    aput-object v8, v7, v5

    .line 201
    :cond_6
    :goto_3
    iget-object v7, v0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->sampleStreams:[Landroidx/media3/exoplayer/source/ClippingMediaPeriod$ClippingSampleStream;

    aget-object v7, v7, v5

    aput-object v7, v1, v5

    .line 195
    add-int/lit8 v5, v5, 0x1

    goto :goto_2

    .line 203
    .end local v5    # "i":I
    :cond_7
    return-wide v2
.end method

.method public setClippingError(Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;)V
    .locals 0
    .param p1, "clippingError"    # Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;

    .line 145
    iput-object p1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->clippingError:Landroidx/media3/exoplayer/source/ClippingMediaSource$IllegalClippingException;

    .line 146
    return-void
.end method

.method public updateClipping(JJ)V
    .locals 9
    .param p1, "startUs"    # J
    .param p3, "endUs"    # J

    .line 125
    iput-wide p1, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->startUs:J

    .line 126
    iput-wide p3, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->endUs:J

    .line 127
    iget-boolean v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->enableClippingInMediaPeriod:Z

    if-eqz v0, :cond_3

    .line 128
    iget-object v0, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->mediaPeriod:Landroidx/media3/exoplayer/source/MediaPeriod;

    invoke-interface {v0, p3, p4}, Landroidx/media3/exoplayer/source/MediaPeriod;->setEndPositionUs(J)J

    move-result-wide v3

    .line 129
    .local v3, "periodEndPositionUs":J
    const-wide/high16 v0, -0x8000000000000000L

    cmp-long v0, v3, v0

    const/4 v7, 0x0

    const/4 v8, 0x1

    if-eqz v0, :cond_1

    cmp-long v0, v3, p3

    if-nez v0, :cond_0

    goto :goto_0

    :cond_0
    move v1, v7

    goto :goto_1

    :cond_1
    :goto_0
    move v1, v8

    :goto_1
    const-string v2, "Period updating end positions not supported, %s!=%s"

    move-wide v5, p3

    .end local p3    # "endUs":J
    .local v5, "endUs":J
    invoke-static/range {v1 .. v6}, Lcom/google/common/base/Preconditions;->checkState(ZLjava/lang/String;JJ)V

    .line 134
    cmp-long p3, v3, v5

    if-nez p3, :cond_2

    move v7, v8

    :cond_2
    iput-boolean v7, p0, Landroidx/media3/exoplayer/source/ClippingMediaPeriod;->isPeriodClippingEndPosition:Z

    goto :goto_2

    .line 127
    .end local v3    # "periodEndPositionUs":J
    .end local v5    # "endUs":J
    .restart local p3    # "endUs":J
    :cond_3
    move-wide v5, p3

    .line 136
    .end local p3    # "endUs":J
    .restart local v5    # "endUs":J
    :goto_2
    return-void
.end method
